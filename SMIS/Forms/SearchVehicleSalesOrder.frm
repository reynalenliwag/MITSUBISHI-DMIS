...
Private Sub txtIgnitionKey_Change()
    ...
    If txtIgnitionKey = "" Then
        ...
        Set rsSalesInvoice = New ADODB.Recordset
        Set rsSalesInvoice = gconDMIS.Execute("SELECT  IGNKEY_NO AS CSNUMBER, MODELDESCRIPTION, MODEL, COLOR, IGNKEY_NO  ,SALESAE, SOSTATUS , ID,STATUS  FROM SMIS_SalesOrder where ignkey_no is not null " & SAFILTER & " order by CSNUMBER asc")
        If Not (rsSalesInvoice.EOF And rsSalesInvoice.BOF) Then
