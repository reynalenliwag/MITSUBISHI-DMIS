...
Private Sub cmdPrint_Click()
    ...
    Set rsPurchAgree = New ADODB.Recordset
    rsPurchAgree.Open "select * from SMIS_PurchAgree WHERE month(InvoicedDate) >= " & What_month(cboMonth) & " AND month(InvoicedDate) <=" & What_month(cboMonth2), gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not rsPurchAgree.EOF And Not rsPurchAgree.EOF Then
        Screen.MousePointer = 11
        rptGenREP.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
        rptGenREP.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
        rptGenREP.WindowShowGroupTree = False
        PrintSQLReport rptGenREP, SMIS_REPORT_PATH & "GPRREP_INV.rpt", "month({purchagree.InvoicedDate}) >= " & What_month(cboMonth) & " AND month({purchagree.InvoicedDate}) <= " & What_month(cboMonth2) & " AND year({purchagree.InvoicedDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
