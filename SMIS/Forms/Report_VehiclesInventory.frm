...
Sub VEHICLEINVENTORY_ASOFDATE()
    ...
    If picAsofDate.Visible = True Then
        ...
        Set xlWorkbook = xlApplication.Workbooks.Open(SMIS_REPORT_PATH & "VehicleInventory.xlt")
        Set xlWorksheet = xlWorkbook.Worksheets(1)
        
        If COMPANY_CODE = "DGI" Then
             SQL = "SELECT * FROM (" & vbCrLf
            SQL = SQL & "SELECT A.MODEL,A.DESCRIPT,A.COLOR,A.PRODNO PRODUCTNO,A.IGNKEY CSNO,A.ENGINENO,A.SERIALNO,DATEDIFF(d,A.DATERECEIVED, '" & txtDate & "') NODAYS,A.DATERECEIVED INVOICEDATE, " & vbCrLf
            SQL = SQL & "A.PURCHPRICE DNP" & vbCrLf
            SQL = SQL & "--B.INVOICEDDATE,B.DATERELEASED" & vbCrLf
            SQL = SQL & "FROM SMIS_MRRINV_TABLE A LEFT OUTER JOIN" & vbCrLf
            SQL = SQL & "SMIS_SALESORDER B ON A.IGNKEY=B.IGNKEY_NO" & vbCrLf
            SQL = SQL & "WHERE A.[STATUS]='P' AND ISNULL(B.STATUS,'')  IN  ('P','C') AND A.RELEASED = 0 AND CAST(A.DATERECEIVED AS SMALLDATETIME) <= '" & txtDate & "' " & vbCrLf
            ...
            SQL = SQL & "FROM SMIS_MRRINV_TABLE A LEFT OUTER JOIN" & vbCrLf
            SQL = SQL & "SMIS_SALESORDER B ON A.IGNKEY=B.IGNKEY_NO" & vbCrLf
            SQL = SQL & "WHERE A.[STATUS]='P' AND ISNULL(B.STATUS,'')  IN  ('P','C') AND A.RELEASED = 1 AND CAST(A.DATERECEIVED AS SMALLDATETIME) <= '" & txtDate & "' AND CAST(A.DATERELEASED AS SMALLDATETIME) > '" & txtDate & "'" & vbCrLf
            SQL = SQL & ")T" & vbCrLf
        ...
        Else
            ...
            SQL = SQL & "FROM SMIS_MRRINV_TABLE A LEFT OUTER JOIN" & vbCrLf
            SQL = SQL & "SMIS_SALESORDER B ON A.IGNKEY=B.IGNKEY_NO" & vbCrLf
            SQL = SQL & "WHERE A.[STATUS]='P' AND ISNULL(B.STATUS,'') <> 'P' AND A.RELEASED = 0 AND CAST(A.DATERECEIVED AS SMALLDATETIME) <= '" & txtDate & "' " & vbCrLf
            SQL = SQL & "Union" & vbCrLf
            SQL = SQL & "SELECT A.MODEL,A.DESCRIPT,A.COLOR,A.PRODNO PRODUCTNO,A.IGNKEY CSNO,A.ENGINENO,A.SERIALNO,DATEDIFF(d,A.DATERECEIVED,'" & txtDate & "') NODAYS,A.DATERECEIVED INVOICEDATE," & vbCrLf
            SQL = SQL & "A.PURCHPRICE DNP" & vbCrLf
            SQL = SQL & "--B.INVOICEDDATE,B.DATERELEASED" & vbCrLf
            SQL = SQL & "FROM SMIS_MRRINV_TABLE A LEFT OUTER JOIN" & vbCrLf
            SQL = SQL & "SMIS_SALESORDER B ON A.IGNKEY=B.IGNKEY_NO" & vbCrLf
            SQL = SQL & "WHERE A.[STATUS]='P' AND ISNULL(B.STATUS,'') <> 'C' AND A.RELEASED = 1 AND CAST(A.DATERECEIVED AS SMALLDATETIME) <= '" & txtDate & "' AND CAST(A.DATERELEASED AS SMALLDATETIME) > '" & txtDate & "'" & vbCrLf
            SQL = SQL & ")T" & vbCrLf
            SQL = SQL & "ORDER BY INVOICEDATE ASC"
        End If
        
        xlWorksheet.Range("A2").CopyFromRecordset gconDMIS.Execute(SQL)
