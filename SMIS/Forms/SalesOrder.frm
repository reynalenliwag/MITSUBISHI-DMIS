...
Private Function AORVALUE(Principal, AOR, TERM) As Double
   ...
   ' =================Updated by Emanoid June112012================
     ElseIf COMPANY_CODE = "DJM" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "CMC" Then
        xsample = (Principal * (1 + (AOR / 100))) / TERM - Int((Principal * (1 + (AOR / 100))) / TERM)
        If xsample > 0 Then
            AORVALUE = FormatNumber(Int((Principal * (1 + (AOR / 100))) / TERM) + 1)
        Else
            AORVALUE = FormatNumber(Int((Principal * (1 + (AOR / 100))) / TERM))
        End If
   ' ==============================================================
    Else
        AORVALUE = FormatNumber((Principal * AOR / TERM), 2)
...
Sub AddNewSODirect(xCustRs As ADODB.Recordset)
    If Not (xCustRs.EOF Or xCustRs.BOF) Then
        ...
        txtOfficeTelNo = Null2String(xCustRs!TelephoneNo) & "/" & Null2String(xCustRs!Mobile)
...
Sub StoreMemVars()
    If Not rsSO.EOF And Not rsSO.BOF Then
        ...
        'End If
'
        If Null2String(rsSO!TERM) = "F" Or Null2String(rsSO!TERM) = "BPO" Or Null2String(rsSO!TERM) = "CPO" Then
            On Error Resume Next
...
Sub UpdateNetAmount()
    ...
    Dim A, C, D, E, F, G, Z1, z2, Z3, Z4, Z5, Z6, d2
...
'If COMPANY_CODE = "DGI" Or COMPANY_CODE = "DJM" Or COMPANY_CODE = "DSSC" Or COMPANY_CODE = "FMC" Then
    If UCase(cboFinancingTerm) = "FINANCING" Or UCase(cboFinancingTerm = "BANK PO") Or UCase(cboFinancingTerm = "CASH") Or UCase(cboFinancingTerm = "COMPANY PO") Then
...
'''        If Z6 > 0 Then
'            If D > 0 Then
'                txtCL_NetSalesPrice = FormatNumber(NumericVal(Z6) - D)
'            End If
'
...
'                D2 = FormatNumber(NumericVal(G) - D)
'                txtCL_NetSalesPrice = FormatNumber(NumericVal(D2))
'            End If
'        End If
'    Else
'        txtCL_TotalDue = FormatNumber(G + Z1 + z2 + Z3 + Z4 + Z5 + Z6)
'    End If
    ...
    If UCase(cboFinancingTerm) = "FINANCING" Or UCase(cboFinancingTerm = "BANK PO") Or UCase(cboFinancingTerm = "CASH") Or UCase(cboFinancingTerm = "COMPANY PO") Then
        If Z6 > 0 Then
            txtCL_TotalDue = FormatNumber((Z1 + z2 + Z4 + Z5) + (Z6)) - D
        Else
            If NumericVal(txtCL_BalToFinanced) = 0 Then
                txtCL_TotalDue = FormatNumber(txtCL_BalToFinanced)
            Else
                txtCL_TotalDue = FormatNumber(G + Z1 + z2 + Z4 + Z5 + Z6)
            End If
        End If
    Else
        txtCL_TotalDue = FormatNumber(G + Z1 + z2 + Z4 + Z5 + Z6)
...
Sub loadAccesUsers()
    ...
    SQL = "SELECT USER_NAME from ALL_vw_RAMS_PAccess where loglevel= 'ADM'"
    ...
    Do While Not RS.EOF
        CboAuthorized.AddItem Null2String(RS!USER_NAME)
        RS.MoveNext
        AccessFlag = False
    Loop
    Set RS = Nothing
...
Private Sub cmdSave_Click()
    ...
    If AddorEdit = "ADD" Then
        If lng >= 1 Then
            Dim existNo As Long
'            existNo = Format(txt_SONO.Text, "000000")
            'JJE
            existNo = Format(Right(txt_SONO.Text, 6), "000000")
            txt_SONO.Text = existNo + 1
            txt_SONO.Text = Format(txt_SONO.Text, "000000")
'            MessagePop RecSaveWarning, "Duplicate Record", "Vehicle Sales Order Number Already Exist"
            
            If COMPANY_CODE = "DJM" Then
                txt_SONO.Text = "SO" + txt_SONO
            End If
            'JJE
        End If
    Else
...
'        txt_SONO.MaxLength = 8
'        txt_SONO.Enabled = False
'    End If
    'JJE
...
Private Sub txt_SONO_LostFocus()
    ...
    'JJE with Prefix
'    txt_SONO = Format(Right(txt_SONO, 6), "000000")   ** FOR APPROVAL **
    'JJE
End Sub
...
Private Sub txtCL_Discount_Change()
    If AddorEdit = "" Then Exit Sub
    If NumericVal(txtCL_DownPayment.Text) > 0 Then
        UpdateNetAmount
    End If
End Sub
...
'        txtCL_DownpaymentPert = (NumericVal(txtCL_DownPayment) / NumericVal(txtCL_NetSalesPrice)) * 100
    '==============UPDATED BY EMANOID MAY182012================
     If COMPANY_CODE = "DJM" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "CMC" Then
        txtCL_DownpaymentPert = FormatNumber((NumericVal(txtCL_DownPayment) / NumericVal(txtCL_SalesPrice)) * 100)
        Else
        If NumericVal(txtCL_SalesPrice) = 0 Then: Exit Sub
        txtCL_DownpaymentPert = (NumericVal(txtCL_DownPayment) / NumericVal(txtCL_NetSalesPrice)) * 100
        End If
     '==========================================================
    End If
...
Private Sub txtCL_DownPayment_GotFocus()
...
Private Sub txtCL_DownPayment_LostFocus()
    vdown = NumericVal(txtCL_DownPayment)
...
'        txtCL_DownPayment = FormatNumber(NumericVal(txtCL_NetSalesPrice) * (NumericVal(txtCL_DownpaymentPert) / 100))
    '==============UPDATED BY EMANOID MAY182012================
    If COMPANY_CODE = "DJM" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "CMC" Then
       txtCL_DownPayment = FormatNumber(NumericVal(txtCL_SalesPrice) * (NumericVal(txtCL_DownpaymentPert) / 100))
        Else
        If NumericVal(txtCL_SalesPrice) = 0 Then: Exit Sub
        txtCL_DownPayment = FormatNumber(NumericVal(txtCL_NetSalesPrice) * (NumericVal(txtCL_DownpaymentPert) / 100))
    End If
...
If COMPANY_CODE = "DJM" Then
    txtGMI.Text = txtCL_NetMoAmort.Text
    Exit Sub
End If
...
Dim xsample As Double
    ...
    ' =================Updated by Emanoid June112012================
        If COMPANY_CODE = "FMC" Or COMPANY_CODE = "CMC" Then
             xsample = NumericVal(txtCL_NetMoAmort) * 1.03 - Int(NumericVal(txtCL_NetMoAmort) * 1.03)
            If xsample > 0 Then
                txtGMI = Int(NumericVal(txtCL_NetMoAmort) * 1.03) + 1
               Else
                txtGMI = Int(NumericVal(txtCL_NetMoAmort) * 1.03)
            End If
        Else
