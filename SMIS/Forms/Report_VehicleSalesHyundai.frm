...
Begin VB.Form frmSMIS_Report_VehicleSalesHyundai 
   ...
   Caption         =   "Jabez Vehicles Sales Report"
   ClientHeight    =   4500
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4230
   ...
   Begin VB.PictureBox picEnding 
      ...
      Top             =   2520
      Width           =   3500
      Begin MSComCtl2.DTPicker dtDay3 
         Height          =   390
         Left            =   840
         TabIndex        =   28
         Top             =   300
   ...
   Begin VB.PictureBox picRange 
      ...
      Begin MSComCtl2.DTPicker dtDay 
         ...
         CurrentDate     =   42005
      End
      Begin MSComCtl2.DTPicker dtDay2 
         Height          =   390
         Left            =   1920
         TabIndex        =   25
         Top             =   300
...
Sub PRINT_DEALER_ENDING_INVENTORY()
    ...
    If Len(Dir(SMIS_REPORT_PATH & "SMIS_EXCEL\DEALER ENDING INVENTORY.xlt")) = 0 Then
        MsgBox "Excel Directory For Sales Management Information Could Not be Located", vbInformation
        Exit Sub
    End If


'    If gconDMIS.Execute("SELECT COUNT(*) FROM SMIS_MRRINV where released=0 and status <> 'C'").Fields(0).Value = 0 Then
    If gconDMIS.Execute("SELECT COUNT(*) FROM SMIS_MRRINV WHERE DATERECEIVED < '" & (dtDay3.Value) & "'  AND RELEASED=0 AND STATUS<> 'C'").Fields(0).Value = 0 Then
'        MsgBox " NO RECEIVING  RECORD"
        MsgBox " NO ENDING INVENTORY RECORD"
        Exit Sub
    End If
    ...
    Set rsCust = gconDMIS.Execute("SELECT DESCRIPT , IGNKEY,RIGHT(VINO,6) AS VINNO ,COLOR,PULLOUTDATE  FROM SMIS_MRRINV_TABLE WHERE DATERECEIVED < '" & (dtDay3.Value) & "' AND RELEASED=0 AND STATUS <> 'C' ORDER BY DESCRIPT ASC")
    ...
    xlSheet1.Cells(1, "C") = COMPANY_NAME
'    xlSheet1.Cells(3, "C") = cboMonthly_Month & " " & txtMonthly_Year
    xlSheet1.Cells(3, "C") = dtDay3
    Do While Not rsCust.EOF
        xlSheet1.Cells(10 + j, "A") = j + 1
        xlSheet1.Cells(10 + j, "A").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(10 + j, "B") = Null2String(rsCust!DESCRIPT)
        xlSheet1.Cells(10 + j, "B").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(10 + j, "C") = Null2String(rsCust!ignkey)
        xlSheet1.Cells(10 + j, "C").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(10 + j, "D") = Null2String(rsCust!VINNO)
        xlSheet1.Cells(10 + j, "D").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(10 + j, "E") = Null2String(rsCust!Color)
        xlSheet1.Cells(10 + j, "E").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(10 + j, "F") = Null2String(rsCust!PullOutDate)
        xlSheet1.Cells(10 + j, "F").BorderAround ColorIndex:=1, Weight:=xlThin
'        xlSheet1.Cells(10 + j, "G") = DateDiff("d", Null2String(rsCust!PullOutDate), Date)
'        xlSheet1.Cells(10 + j, "G").BorderAround ColorIndex:=1, Weight:=xlThin
'        xlSheet1.Cells(10 + j, "H").BorderAround ColorIndex:=1, Weight:=xlThin
        j = j + 1
        rsCust.MoveNext
    Loop
    ...
    xlSheet1.Cells(11 + j, "B") = "MODEL SUMMARY"
    xlSheet1.Cells(11 + j, "B").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(11 + j, "B").Font.Bold = True
    xlSheet1.Cells(11 + j, "C") = "UNIT"
    xlSheet1.Cells(11 + j, "C").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(11 + j, "C").Font.Bold = True
    Set rsCust = gconDMIS.Execute("SELECT DISTINCT  MODEL , COUNT(*) AS TCOUNT  FROM SMIS_MRRINV WHERE DATERECEIVED < '" & (dtDay3.Value) & "' AND RELEASED=0 AND STATUS <> 'C'  GROUP BY MODEL ")
    Dim TOTALUNIT                                      As Integer
    Do While Not rsCust.EOF
        j = j + 1
        xlSheet1.Cells(11 + j, "B") = Null2String(rsCust!Model)
        xlSheet1.Cells(11 + j, "B").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(11 + j, "C") = Null2String(rsCust!TCOUNT)
        xlSheet1.Cells(11 + j, "C").BorderAround ColorIndex:=1, Weight:=xlThin
        TOTALUNIT = TOTALUNIT + Null2String(rsCust!TCOUNT)
        rsCust.MoveNext
    Loop
    xlSheet1.Cells(12 + j, "B") = "TOTAL"
    xlSheet1.Cells(12 + j, "B").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(12 + j, "B").Font.Bold = True
    xlSheet1.Cells(12 + j, "C") = TOTALUNIT
    xlSheet1.Cells(12 + j, "C").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(12 + j, "C").Font.Bold = True
...
'    If gconDMIS.Execute("SELECT COUNT(*) FROM SMIS_SALESORDER WHERE (DAY(DateReleased)=" & Day(dtDay.Value) & " AND month(DateReleased)= " & Month(dtDay) & " AND year(DateReleased)= " & Year(dtDay) & ")").Fields(0).Value = 0 Then
    If gconDMIS.Execute("SELECT COUNT (*) AS TCOUNT FROM SMIS_MrrInv MRRINV INNER JOIN SMIS_PurchAgree PURCH ON MRRINV.ignkey=PURCH.IGNKEY_NO WHERE MRRINV.Released=1 AND MRRINV.DateReleased BETWEEN'" & (dtDay.Value) & "' AND '" & (dtDay2.Value) & "' ").Fields(0).Value = 0 Then
...
'   Set rsCust = gconDMIS.Execute("SELECT * FROM SMIS_SALESORDER WHERE (DAY(DateReleased)=" & Day(dtDay.Value) & " AND month(DateReleased)= " & Month(dtDay) & " AND year(DateReleased)= " & Year(dtDay) & ") ORDER BY INVOICEDDATE")
    Set rsCust = gconDMIS.Execute("SELECT MRRINV.DateReleased, MRRINV.DESCRIPT, MRRINV.VI_NO, PURCH.VDR_NO, MRRINV.DateReleased, MRRINV.DateReleased, PURCH.Term, PURCH.CustName, PURCH.HomeAddress, PURCH.HomeTelNo, PURCH.SalesAE, * FROM SMIS_MrrInv MRRINV INNER JOIN SMIS_PurchAgree PURCH ON MRRINV.ignkey = PURCH.IGNKEY_NO WHERE MRRINV.Released=1 AND MRRINV.DateReleased BETWEEN '" & (dtDay.Value) & "' AND '" & (dtDay2.Value) & "' ORDER BY MRRINV.INVOICEDDATE")
    Set xlApp = New Excel.Application
    Set xlbook = xlApp.Workbooks.Open(SMIS_REPORT_PATH & "SMIS_EXCEL\DAILY VEHICLE RETAIL SALES.xlt")
    Set xlSheet1 = xlbook.Worksheets(1)

    xlSheet1.Cells(2, "B") = COMPANY_NAME
    xlSheet1.Cells(4, "A") = "From :"
    xlSheet1.Cells(5, "A") = "To :"
    xlSheet1.Cells(4, "B") = dtDay
    xlSheet1.Cells(5, "B") = dtDay2
...
Do While Not rsCust.EOF
    ...
    If Not rsCust.EOF And Not rsCust.BOF Then
        ...
        End If
    End If
    xlSheet1.Cells(10 + j, "A") = j + q
    If vProspectType = "P" Then
        xlSheet1.Cells(10 + j, "B") = "X"
    ElseIf vProspectType = "F" Then
        xlSheet1.Cells(10 + j, "C") = "X"
    Else
        xlSheet1.Cells(10 + j, "d") = "X"
    End If
    ...
    If Apod = "MS" Then
        xlSheet1.Cells(10 + j, "G") = "X"
    ElseIf Apod = "MR" Then
        xlSheet1.Cells(10 + j, "F") = "X"
    Else
        xlSheet1.Cells(10 + j, "F") = ""
    End If
    
    xlSheet1.Cells(10 + j, "I") = Null2String(rsCust!CustName)
    xlSheet1.Cells(10 + j, "J") = Null2String(rsCust!HomeAddress)
    
    If Null2String(rsAllCust!Mobile) <> "" Then
        xlSheet1.Cells(10 + j, "K") = Null2String(rsAllCust!Mobile)
    End If
    If Null2String(rsCust!HomeTelNo) <> "" Then
        If xlSheet1.Cells(10 + j, "K") <> "" Then
            xlSheet1.Cells(10 + j, "K") = xlSheet1.Cells(15 + j, "K") & "/" & Null2String(rsCust!HomeTelNo)
        Else
            xlSheet1.Cells(10 + j, "K") = Null2String(rsCust!HomeTelNo)
        End If
    End If
    
    If Null2String(rsCust!officetelno) <> "" Then
        If xlSheet1.Cells(10 + j, "K") <> "" Then
            xlSheet1.Cells(10 + j, "K") = xlSheet1.Cells(15 + j, "J") & "/" & Null2String(rsCust!officetelno)
        Else
            xlSheet1.Cells(10 + j, "K") = Null2String(rsCust!officetelno)
        End If
    End If
    
    
    xlSheet1.Cells(10 + j, "C") = Null2String(rsCust!modeldescription)
    xlSheet1.Cells(10 + j, "D") = Null2String(rsCust!IGNKEY_NO)
    xlSheet1.Cells(10 + j, "E") = Null2String(rsCust!VI_NO)
    xlSheet1.Cells(10 + j, "B") = Format(Null2String(rsCust!InvoicedDate), "mm/dd/yyyy")
    xlSheet1.Cells(10 + j, "F") = Null2String(rsCust!VDR_NO)
    xlSheet1.Cells(10 + j, "G") = Format(Null2String(rsCust!DateReleased), "mm/dd/yyyy")
    xlSheet1.Cells(10 + j, "R") = Null2String(rsCust!certific8)
    xlSheet1.Cells(10 + j, "H") = Null2String(rsCust!TERM)
    xlSheet1.Cells(10 + j, "L") = Null2String(rsCust!salesae)
    xlSheet1.Cells(10 + j, "M") = vleadsource
    j = j + 1
    
    xlSheet1.Cells.Range("A" & 10 + j, "X" & 10 + j).Insert
    rsCust.MoveNext
Loop
...
'    Set rsCust = gconDMIS.Execute("SELECT DISTINCT  MODEL , COUNT(*) AS TCOUNT  FROM SMIS_SALESORDER WHERE (DAY(DateReleased)=" & Day(dtDay.Value) & " AND month(DateReleased)= " & Month(dtDay) & " AND year(DateReleased)= " & Year(dtDay) & ") GROUP BY MODEL ")
    Set rsCust = gconDMIS.Execute("SELECT DISTINCT PURCH.Model, COUNT (*) AS TCOUNT FROM SMIS_MrrInv MRRINV INNER JOIN SMIS_Purchagree PURCH ON MRRINV.ignkey = PURCH.IGNKEY_NO WHERE MRRINV.Released=1 AND MRRINV.DateReleased BETWEEN '" & (dtDay.Value) & "' AND '" & (dtDay2.Value) & "' GROUP BY PURCH.Model ")
    ...
    Do While Not rsCust.EOF
        ...
        xlSheet1.Cells(20 + j, "N") = Null2String(rsCust!Model)
        xlSheet1.Cells(20 + j, "N").BorderAround ColorIndex:=1, Weight:=xlThin
        xlSheet1.Cells(20 + j, "O") = Null2String(rsCust!TCOUNT)
        xlSheet1.Cells(20 + j, "O").BorderAround ColorIndex:=1, Weight:=xlThin
        TOTALUNIT = TOTALUNIT + Null2String(rsCust!TCOUNT)
        j = j + 1
        rsCust.MoveNext
    Loop
    xlSheet1.Cells(20 + j, "N") = "TOTAL"
    xlSheet1.Cells(20 + j, "N").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(20 + j, "N").Font.Bold = True
    xlSheet1.Cells(20 + j, "O") = TOTALUNIT
    xlSheet1.Cells(20 + j, "O").BorderAround ColorIndex:=1, Weight:=xlThin
    xlSheet1.Cells(20 + j, "O").Font.Bold = True
