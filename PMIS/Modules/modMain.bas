...
Public frmPMISReports_Issuances_M                      As New frmPMISReports_Issuances
Public frmPMISReports_Issuances_A                      As New frmPMISReports_Issuances
...
ErrorCode:
    err.Clear
End Sub
Function COMPUTE_ONHANDASOFDATE2(str_TRANDATE As String, str_Stockno As String, str_type As String, xID As Long) As Long
    Dim SQL                                            As String
    Dim RSTOTAL                                        As ADODB.Recordset
    Dim cmd                                            As ADODB.Command
        On Error GoTo ERROR_QUERY
        ...
        With cmd
            .NamedParameters = True
            .CommandType = adCmdStoredProc
            .CommandText = "SP_PMIS_CHECK_ONHAND_ASOF2"
            .ActiveConnection = gconDMIS
            .CommandTimeout = 1000
            .Parameters.Append .CreateParameter("@STOCKNO", adVarChar, adParamInput, 50, str_Stockno)
            .Parameters.Append .CreateParameter("@TYPE", adVarChar, adParamInput, 10, str_type)
            .Parameters.Append .CreateParameter("@TRANDATE", adDBDate, adParamInput, , str_TRANDATE)
            .Parameters.Append .CreateParameter("@ID", adVarChar, adParamInput, 10, xID)
        End With
        Set RSTOTAL = New ADODB.Recordset
        Set RSTOTAL = cmd.Execute
        If Not RSTOTAL.EOF Or Not RSTOTAL.BOF Then
            COMPUTE_ONHANDASOFDATE2 = N2Str2IntZero(RSTOTAL(0).Value)
        End If
    'UPDATE BY   : MJP 11152010 1116AM
    Set RSTOTAL = Nothing
    Exit Function
...
'updated by: IEBV
'description: to avoid unposting f already receive but not yet posted
Function chkfnotyet_posted(xpono As String, XTYPE As String) As Boolean
If (gconDMIS.Execute("Select count(*) from PMIS_vw_RR_Trans where PONO = '" & xpono & "' and [type] = '" & XTYPE & "' and isnull(status,'N') = 'N'").Fields(0).Value) > 0 Then
    chkfnotyet_posted = True
Else
    chkfnotyet_posted = False
End If
...
'updated BY: IEBV
'description: to get dnp when unposting
Function getlastdnp(Code As String, XTYPE As String, petsa As String, xID As Long) As Double
Dim RSRR As ADODB.Recordset
Set RSRR = New ADODB.Recordset

Set RSRR = gconDMIS.Execute("Select top 1 mac from pmis_alldaytran where trantype in('BEG','RR') and [type] = '" & XTYPE & "' and status = 'P' and stock_ord = " & Code & " and trandate <= '" & petsa & "' and id < '" & xID & "' order by id desc")
If Not (RSRR.EOF And RSRR.BOF) Then
    getlastdnp = N2Str2Zero((RSRR!MAC) * 1.12)
Else
    getlastdnp = "0.00"
End If
Set RSRR = Nothing
...
Function getttlchg(xxcoscde As String, xamout As Double) As Double
Dim rschg                                               As ADODB.Recordset
Dim rsor                                                As ADODB.Recordset
Dim xtotalchgparts                                      As Double
Dim xtotalparts                                         As Double
Set rschg = New ADODB.Recordset
Set rschg = gconDMIS.Execute("select * from (select tranno,netinvamt,case [TYPE] when 'P' then 'PI' when 'A' then 'AI' when 'M' then 'MI' end as [TYPE] from pmis_ord_hd where trantype = 'CHG' and [TYPE] in ('P','M','A') and pay_class = 'C' and status = 'P' and custcode = '" & xxcoscde & "' " & _
                             " UNION ALL " & _
                             "select tranno, netinvamt,case [TYPE] when 'P' then 'PI' when 'A' then 'AI' when 'M' then 'MI' end as [TYPE] from pmis_ord_hist where trantype = 'CHG' and [TYPE] in ('P','M','A') and pay_class = 'C' and status = 'P' and custcode = '" & xxcoscde & "') T order by tranno asc   ")
xtotalchgparts = "0.00"
xtotalparts = "0.00"
If Not (rschg.EOF And rschg.BOF) Then
    rschg.MoveFirst
    Do While Not rschg.EOF
        xtotalchgparts = NumericVal(rschg!NETINVAMT)
        Set rsor = New ADODB.Recordset
        Set rsor = gconDMIS.Execute("Select payment from cmis_off_dt where invoicetype = 'CHG' and cuscde = '" & xxcoscde & "' and trantype = '" & rschg!Type & "' and invoiceno = '" & rschg!TRANNO & "' and paidna = '1'")
        If Not (rsor.EOF And rsor.BOF) Then
            rsor.MoveFirst
            Do While Not rsor.EOF
                xtotalchgparts = NumericVal(xtotalchgparts - rsor!payment)
                rsor.MoveNext
            Loop
            xtotalparts = xtotalparts + xtotalchgparts
        End If
        rschg.MoveNext
    Loop
End If

getttlchg = xtotalparts + xamout

End Function
'updated by: IEBV
'description: to check updated status
Function chkstatus(XTRANNO As String, XTYPE As String, xTRANTYPE As String) As String
Dim RS                                      As ADODB.Recordset
Dim sqlstr                                  As String
If xTRANTYPE = "PO" Then
    sqlstr = "Select isnull(Status,'N') as STATUS from pmis_po_hd where [type] = '" & XTYPE & "' and pono = '" & XTRANNO & "'"
ElseIf xTRANTYPE = "RR" Then
    sqlstr = "Select isnull(Status,'N') as STATUS from pmis_rr_hd where [type] = '" & XTYPE & "' and rrno = '" & XTRANNO & "'"
Else
    sqlstr = "Select isnull(Status,'N') as STATUS from pmis_ord_hd where [type] = '" & XTYPE & "' and tranno = '" & XTRANNO & "' and trantype = '" & xTRANTYPE & "'"
End If

Set RS = gconDMIS.Execute(sqlstr)
If Not (RS.EOF And RS.BOF) Then
    chkstatus = Null2String(RS!Status)
End If
Set RS = Nothing
End Function

Function SETTOZERORATED(XTYPE As String, xTRANTYPE As String, XTRANNO As String, xID As String, xlocal As String) As Boolean

On Error GoTo ivan
Dim RSORD_HD                                        As ADODB.Recordset
Dim RSDAYTRAN                                       As ADODB.Recordset
Dim sql_command                                     As String
Dim ttlinvamt                                       As Double
Dim iscannot                                        As Boolean
Dim partlist                                        As String
Dim TYPES                                           As String
Dim xcnt                                            As Integer
Dim partlist_array()                                As String
Dim CTR                                             As Integer

Set RSORD_HD = New ADODB.Recordset
Set RSORD_HD = gconDMIS.Execute("Select stock_ord,trantype,tranno,[type],ID,stock_ord,tranuprice,tranucost, case [type] when 'A' then 'Accessories' when 'P' then 'Part(s)' when 'M' then 'Material(s)' end as TYPES from PMIS_TDAYTRAN where trantype = '" & xTRANTYPE & "' and [type] = '" & XTYPE & "' and tranno = '" & XTRANNO & "' and tranucost > cast(tranuprice / 1.12 as decimal(18,2)) order by itemno asc ")
If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
    RSORD_HD.MoveFirst
    partlist = ""
    xcnt = RSORD_HD.RecordCount
    ReDim partlist_array(xcnt) As String
    xcnt = 0
    TYPES = Null2String(RSORD_HD!TYPES)
    Do While Not RSORD_HD.EOF
        xcnt = xcnt + 1
        partlist_array(xcnt) = Null2String(RSORD_HD!STOCK_ORD)
        partlist = partlist & Null2String(RSORD_HD!STOCK_ORD) & " from " & Null2String(RSORD_HD!TRANUPRICE) & " to " & Round(Null2String(RSORD_HD!TRANUPRICE) / 1.12, 2) & " ( Cost = " & Null2String(RSORD_HD!TRANUCOST) & " ) " & vbCrLf
        RSORD_HD.MoveNext
    Loop
    If MsgBox("The SRP of this " & TYPES & _
          " will fall below to your unit Cost. " & vbCrLf & _
          "-------------------------------------------------------------------------------" & vbCrLf & _
          "" & partlist & "-------------------------------------------------------------------------------" & vbCrLf & _
          " Proceed to Zero Rated Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbNo Then
        SETTOZERORATED = False
        Exit Function
    Else
        If Module_Access(LOGID, "Allow Issuance Of Srp Less Than Cost", "TRANSACTION") = False Then SETTOZERORATED = False: Exit Function
        For CTR = 1 To xcnt
            Call NEW_LogAudit("MP", xlocal, "SRP below unit cost continue to zero rated transaction?", xID, XTYPE, "TRAN NO: " & XTRANNO & " " & " PART NO: " & partlist_array(CTR) & " SRP below unit cost continue to zero rated transaction?", xTRANTYPE, "")
        Next CTR
        MsgBox "User Action has been Log to Audit Trail", vbInformation, "Audit Trail Information"
    End If
End If
    sql_command = "UPDATE PMIS_ORD_HD SET ZERORATED = '1'  where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO ZERO RATED", xTRANTYPE, "")
    sql_command = "update pmis_tdaytran set tranuprice = cast(round(tranuprice / 1.12,2)  as decimal(18,2)) where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO ZERO RATED", xTRANTYPE, "")
    sql_command = " UPDATE PMIS_ORD_HD SET TTLINVAMT = DT_TOTAL,DS_AMT1 = (DT_TOTAL * (B.DS1 / 100)), " & _
                      " NETINVAMT = (DT_TOTAL -(DT_TOTAL * (B.DS1 / 100))), ZERORATED = '1' " & _
                      " FROM ( SELECT ROUND(SUM(TRANUPRICE * TRANQTY),2) AS DT_TOTAL,[TYPE],TRANTYPE,TRANNO " & _
                      " FROM PMIS_TDAYTRAN GROUP BY [TYPE],TRANTYPE,TRANNO ) A " & _
                      " INNER JOIN PMIS_ORD_HD B ON A.TRANNO = B.TRANNO AND A.TRANTYPE = B.TRANTYPE AND A.[TYPE] = B.[TYPE] " & _
                      " WHERE  A.TRANTYPE = '" & xTRANTYPE & "' AND A.TRANNO = '" & XTRANNO & "' AND A.[TYPE] = '" & XTYPE & "' "
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO ZERO RATED", xTRANTYPE, "")
    SETTOZERORATED = True
    Exit Function
...
Function SETTONOT_ZERORATED(XTYPE As String, xTRANTYPE As String, XTRANNO As String, xID As String, xlocal As String) As Boolean
Dim sql_command                                     As String

On Error GoTo ivan
    sql_command = "update pmis_ORD_HD set ZERORATED = '0'  where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT-ZERO RATED", xTRANTYPE, "")
    sql_command = "update pmis_tdaytran set tranuprice = cast(round(tranuprice * 1.12,2)  as decimal(18,2)) where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT-ZERO RATED", xTRANTYPE, "")
    sql_command = " UPDATE PMIS_ORD_HD SET TTLINVAMT = DT_TOTAL,DS_AMT1 = (DT_TOTAL * (B.DS1 / 100)), " & _
                      " NETINVAMT = (DT_TOTAL -(DT_TOTAL * (B.DS1 / 100))), ZERORATED = '0' " & _
                      " FROM ( SELECT ROUND(SUM(TRANUPRICE * TRANQTY),2) AS DT_TOTAL,[TYPE],TRANTYPE,TRANNO " & _
                      " FROM PMIS_TDAYTRAN GROUP BY [TYPE],TRANTYPE,TRANNO ) A " & _
                      " INNER JOIN PMIS_ORD_HD B ON A.TRANNO = B.TRANNO AND A.TRANTYPE = B.TRANTYPE AND A.[TYPE] = B.[TYPE] " & _
                      " WHERE  A.TRANTYPE = '" & xTRANTYPE & "' AND A.TRANNO = '" & XTRANNO & "' AND A.[TYPE] = '" & XTYPE & "' "
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT-ZERO RATED", xTRANTYPE, "")
    SETTONOT_ZERORATED = True
    Exit Function
ivan:
    SETTONOT_ZERORATED = False
    
End Function


Public Function checkdupRS(Xprs As String, XTYPE As String, prsno As String) As Boolean
Dim rscheckrs                                   As ADODB.Recordset

Set rscheckrs = gconDMIS.Execute(" Select * from PMIS_ORD_HD WHERE [TYPE] = '" & XTYPE & "' AND TRANTYPE = '" & Xprs & "' AND REFPISNO = '" & prsno & "' " & _
                                 " UNION ALL " & _
                                 " Select * from PMIS_ORD_HIST WHERE [TYPE] = '" & XTYPE & "' AND TRANTYPE = '" & Xprs & "' AND REFPISNO = '" & prsno & "'")
If Not (rscheckrs.EOF And rscheckrs.BOF) Then
    checkdupRS = True
Else
    checkdupRS = False
End If
Set rscheckrs = Nothing
End Function

Sub TAGIFINACTIVE(XPART As String, XTYPE As String)
Dim rsSTOCKMAS                                 As ADODB.Recordset
Set rsSTOCKMAS = gconDMIS.Execute("Select ID,isnull(ACTIVE,'N') as ACTIVE from pmis_stockmas where [TYPE] = '" & XTYPE & "' and stockno = '" & XPART & "'")
If Not (rsSTOCKMAS.EOF And rsSTOCKMAS.BOF) Then
    If Null2String(rsSTOCKMAS!Active) = "N" Then
        SQL_STATEMENT = "Update PMIS_STOCKMAS SET ACTIVE = 'Y' where ID = '" & N2Str2IntZero(rsSTOCKMAS!ID) & "'"
        gconDMIS.Execute (SQL_STATEMENT)
        Call NEW_LogAudit("E", XTYPE, SQL_STATEMENT, N2Str2IntZero(rsSTOCKMAS!ID), "", "TAGGED ACTIVE STOCK CODE: " & XPART, "", "")
    End If
End If
End Sub
'updated BY: IEBV
Function checkdup_DRNO(XTYPE As String, xinvo As String, xsupcode As String) As Boolean
    Dim rsINVNO                                        As ADODB.Recordset
    Dim XINVNO                                         As String

    Set rsINVNO = New ADODB.Recordset
    Dim strsqlcommand                                  As String
    strsqlcommand = "Select invno from pmis_rr_hd where DRNO = '" & Null2String(xinvo) & "' and [TYPE] = '" & XTYPE & "' and isnull(status,'N') <> 'C' and recvd_code = '" & xsupcode & "'"
    strsqlcommand = strsqlcommand + " UNION ALL "
    strsqlcommand = strsqlcommand + "Select invno from pmis_rec_hist where DRNO = '" & Null2String(xinvo) & "' and [TYPE] = '" & XTYPE & "' and isnull(status,'N') <> 'C' and recvd_code = '" & xsupcode & "'"
...
Function SETTOVATEXEMPT(XTYPE As String, xTRANTYPE As String, XTRANNO As String, xID As String, xlocal As String) As Boolean
...
If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
    ...
    If MsgBox("The SRP of this " & TYPES & _
          ...
          "" & partlist & "-------------------------------------------------------------------------------" & vbCrLf & _
          " Proceed to Zero Rated Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbNo Then
        SETTOVATEXEMPT = False
        Exit Function
    Else
        If Module_Access(LOGID, "Allow Issuance Of Srp Less Than Cost", "TRANSACTION") = False Then SETTOVATEXEMPT = False: Exit Function
        For CTR = 1 To xcnt
            Call NEW_LogAudit("MP", xlocal, "SRP below unit cost continue to vat exempt transaction?", xID, XTYPE, "TRAN NO: " & XTRANNO & " " & " PART NO: " & partlist_array(CTR) & " SRP below unit cost continue to zero rated transaction?", xTRANTYPE, "")
        Next CTR
        MsgBox "User Action has been Log to Audit Trail", vbInformation, "Audit Trail Information"
    End If
End If
    sql_command = "UPDATE PMIS_ORD_HD SET VAT_EXEMPT = '1'  where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO VAT EXEMPT", xTRANTYPE, "")
    sql_command = "update pmis_tdaytran set tranuprice = cast(round(tranuprice / 1.12,2)  as decimal(18,2)) where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO VAT EXEMPT", xTRANTYPE, "")
    sql_command = " UPDATE PMIS_ORD_HD SET TTLINVAMT = DT_TOTAL,DS_AMT1 = (DT_TOTAL * (B.DS1 / 100)), " & _
                      " NETINVAMT = (DT_TOTAL -(DT_TOTAL * (B.DS1 / 100))), VAT_EXEMPT = '1' " & _
                      " FROM ( SELECT ROUND(SUM(TRANUPRICE * TRANQTY),2) AS DT_TOTAL,[TYPE],TRANTYPE,TRANNO " & _
                      " FROM PMIS_TDAYTRAN GROUP BY [TYPE],TRANTYPE,TRANNO ) A " & _
                      " INNER JOIN PMIS_ORD_HD B ON A.TRANNO = B.TRANNO AND A.TRANTYPE = B.TRANTYPE AND A.[TYPE] = B.[TYPE] " & _
                      " WHERE  A.TRANTYPE = '" & xTRANTYPE & "' AND A.TRANNO = '" & XTRANNO & "' AND A.[TYPE] = '" & XTYPE & "' "
...
ivan:
    ...
    MsgBox error
End Function
Function SETTONOT_VATEXEMPT(XTYPE As String, xTRANTYPE As String, XTRANNO As String, xID As String, xlocal As String) As Boolean
Dim sql_command                                     As String

On Error GoTo ivan
    sql_command = "update pmis_ORD_HD set VAT_EXEMPT = '0'  where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT VAT EXEMPT", xTRANTYPE, "")
    sql_command = "update pmis_tdaytran set tranuprice = cast(round(tranuprice * 1.12,2)  as decimal(18,2)) where [type] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' and tranno = '" & XTRANNO & "'"
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT VAT EXEMPT", xTRANTYPE, "")
    sql_command = " UPDATE PMIS_ORD_HD SET TTLINVAMT = DT_TOTAL,DS_AMT1 = (DT_TOTAL * (B.DS1 / 100)), " & _
                      " NETINVAMT = (DT_TOTAL -(DT_TOTAL * (B.DS1 / 100))), VAT_EXEMPT = '0' " & _
                      " FROM ( SELECT ROUND(SUM(TRANUPRICE * TRANQTY),2) AS DT_TOTAL,[TYPE],TRANTYPE,TRANNO " & _
                      " FROM PMIS_TDAYTRAN GROUP BY [TYPE],TRANTYPE,TRANNO ) A " & _
                      " INNER JOIN PMIS_ORD_HD B ON A.TRANNO = B.TRANNO AND A.TRANTYPE = B.TRANTYPE AND A.[TYPE] = B.[TYPE] " & _
                      " WHERE  A.TRANTYPE = '" & xTRANTYPE & "' AND A.TRANNO = '" & XTRANNO & "' AND A.[TYPE] = '" & XTYPE & "' "
    gconDMIS.Execute (sql_command)
    Call NEW_LogAudit("EE", xlocal, sql_command, xID, XTYPE, "TRAN NO: " & XTRANNO & " - SET TO NOT VAT EXEMPT", xTRANTYPE, "")
    SETTONOT_VATEXEMPT = True
    Exit Function
ivan:
    SETTONOT_VATEXEMPT = False
    
End Function

Function ISZERONOTVAT(XTYPE As String, xTRANTYPE As String, XTRANNO As String, ZORV As String, XDATA As String) As Boolean
Dim rschk                                           As ADODB.Recordset
Dim WATCOL                                          As String
Dim sql_command                                     As String
...
sql_command = "Select * from PMIS_ORD_HD WHERE [type] = '" & XTYPE & "' AND TRANNO = '" & XTRANNO & "' AND trantype = '" & xTRANTYPE & "' AND isnull(" & WATCOL & ",0) = '" & XDATA & "'"
Set rschk = gconDMIS.Execute(sql_command)
...
Dim SQL                                            As String
SQL = ""
SQL = "IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='PMIS_ORD_HD' AND COLUMN_NAME='VAT_EXEMPT')" & vbCrLf
SQL = SQL & "ALTER TABLE PMIS_ORD_HD" & vbCrLf
SQL = SQL & "ADD VAT_EXEMPT CHAR(1) default '0'" & vbCrLf
gconDMIS.Execute (SQL)

SQL = ""
SQL = "IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='PMIS_ORD_HIST' AND COLUMN_NAME='VAT_EXEMPT')" & vbCrLf
SQL = SQL & "ALTER TABLE PMIS_ORD_HIST" & vbCrLf
SQL = SQL & "ADD VAT_EXEMPT CHAR(1) " & vbCrLf
gconDMIS.Execute (SQL)


SQL = ""
SQL = "IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='PMIS_ORD_HD' AND COLUMN_NAME='ZERORATED')" & vbCrLf
SQL = SQL & "ALTER TABLE PMIS_ORD_HD" & vbCrLf
SQL = SQL & "ADD ZERORATED CHAR(1) default '0'" & vbCrLf
gconDMIS.Execute (SQL)

SQL = ""
SQL = "IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='PMIS_ORD_HIST' AND COLUMN_NAME='ZERORATED')" & vbCrLf
SQL = SQL & "ALTER TABLE PMIS_ORD_HIST" & vbCrLf
SQL = SQL & "ADD ZERORATED CHAR(1) " & vbCrLf
gconDMIS.Execute (SQL)

SQL = ""
SQL = "IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='CSMS_REPOR' AND COLUMN_NAME='VAT_EXEMPT1')" & vbCrLf
SQL = SQL & "ALTER TABLE CSMS_REPOR" & vbCrLf
SQL = SQL & "ADD VAT_EXEMPT1 CHAR(1) " & vbCrLf
gconDMIS.Execute (SQL)
...
Function CHKFZERO_ORVATEX(xREPOR As String) As Boolean
Dim rsCheck                                        As ADODB.Recordset
Set rsCheck = gconDMIS.Execute("Select VAT_EXEMPT,VAT_EXEMPT1 from csms_repor where rep_or = '" & xREPOR & "' and transtype = 'R'")
If Not (rsCheck.EOF And rsCheck.BOF) Then
    If N2Str2Zero(rsCheck!VAT_EXEMPT) = 0 And (Null2String(rsCheck!VAT_EXEMPT1) = "" Or Null2Bit(rsCheck!VAT_EXEMPT1) = 0) Then
        CHKFZERO_ORVATEX = False
    ElseIf N2Str2Zero(rsCheck!VAT_EXEMPT) = 1 And (Null2String(rsCheck!VAT_EXEMPT1) = "" Or Null2Bit(rsCheck!VAT_EXEMPT1) = 0) Then
        CHKFZERO_ORVATEX = True
    ElseIf N2Str2Zero(rsCheck!VAT_EXEMPT) = 0 And (Null2String(rsCheck!VAT_EXEMPT1) <> "" Or Null2Bit(rsCheck!VAT_EXEMPT1) <> 0) Then
        CHKFZERO_ORVATEX = True
    End If
Else
    CHKFZERO_ORVATEX = False
...
Function ISISSUED(XTYPE As String, Xprs As String) As Boolean
Dim rsreq                                           As New ADODB.Recordset

Set rsreq = gconDMIS.Execute("Select REFPRSNO from PMIS_ORD_HD where [TYPE] = '" & XTYPE & "' and REFPRSNO = '" & Xprs & "' " & _
                             " UNION ALL " & _
                             "Select REFPRSNO from PMIS_ORD_HIST where [TYPE] = '" & XTYPE & "' and REFPRSNO = '" & Xprs & "' ")
If Not (rsreq.EOF And rsreq.BOF) Then
    ISISSUED = True
    MessagePop InfoVoid, "Action void", "Requisition already used in issuance"
Else
    ISISSUED = False
End If
Set rsreq = Nothing
