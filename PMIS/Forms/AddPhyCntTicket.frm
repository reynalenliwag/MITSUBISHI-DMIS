...
Begin VB.Form frmPMIS_Physical_AddPhyCntTicket 
   ...
   Begin VB.PictureBox picPhyCnt2 
      ...
      Begin VB.CommandButton cmdCancel 
         ...
         Height          =   795
         Left            =   6120
         MouseIcon       =   "AddPhyCntTicket.frx":298A
         MousePointer    =   99  'Custom
         Picture         =   "AddPhyCntTicket.frx":2ADC
         Style           =   1  'Graphical
         TabIndex        =   24
      ...
      Begin VB.CommandButton cmdSave 
         ...
         Height          =   795
         Left            =   5400
         MouseIcon       =   "AddPhyCntTicket.frx":2E1A
         MousePointer    =   99  'Custom
         Picture         =   "AddPhyCntTicket.frx":2F6C
         Style           =   1  'Graphical
         TabIndex        =   25
...
Dim rsPHYCNTDUP                                        As New ADODB.Recordset
Dim rsTAGS                                             As New ADODB.Recordset
Dim rsPHYCNTDUP2                                       As New ADODB.Recordset
Dim rsPartsCount                                       As New ADODB.Recordset
Dim rsPartsCount2                                      As New ADODB.Recordset

Sub rsRefresh()
    Set rsPHYCNT = New ADODB.Recordset
    If UCase(Trim(Cbo_top.Text)) = "ALL" Then
        rsPHYCNT.Open "Select * from PHYCNT  order by tagno asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
    Else
        rsPHYCNT.Open "Select TOP " & N2Str2IntZero(Trim(Cbo_top.Text)) & " * from PHYCNT  order by tagno asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
    End If
    ...
    If Not rsPHYCNT.EOF And Not rsPHYCNT.BOF Then
        ...
        Do While Not rsPHYCNT.EOF
            ...
            grdPhyCnt.AddItem Format(Null2String(rsPHYCNT!tagno), "0000000000") & Chr(9) & _
                              ...
                              Null2String(rsPHYCNT!Group_No) & Chr(9) & _
...
Private Sub Cbo_top_KeyPress(KeyAscii As Integer)
'MsgBox KeyAscii
    If Len(UCase(Cbo_top.Text)) > 5 And KeyAscii <> 8 Then
        KeyAscii = 0
    Else
...
Private Sub Cbo_top_LostFocus()
    If UCase(Trim(Cbo_top.Text)) = "ALL" And IsNumeric(UCase(Trim(Cbo_top.Text))) = False Then
        Cbo_top.Text = "All"
    ElseIf (N2Str2IntZero(Trim(Cbo_top.Text)) = 0 And UCase(Trim(Cbo_top.Text)) <> "ALL") Or N2Str2IntZero(Trim(Cbo_top.Text)) < 0 Then
        Cbo_top.Text = 1
    Else
        Cbo_top.Text = N2Str2IntZero(Trim(Cbo_top.Text))
    End If
End Sub
...
Private Sub cboPartNo_Click()
    ...
    rsCUTOFF.Open "Select onhand,PARTNO,PARTDESC,mac,location from CUTOFF where PARTNO=" & N2Str2Null(LTrim(RTrim(cboPartNo.Text))), gconINVENTORY
...
Private Sub cboPartNo_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then On Error Resume Next: txtTagNo.SetFocus
End Sub
...
Private Sub cmdSave_Click()
    ...
    Set rsPHYCNTDUP2 = Nothing
    Set rsPartsCount = Nothing
    Set rsPartsCount2 = Nothing
    Set rsPHYCNTDUP = Nothing
   ...
   ' On Error GoTo ErrorCode
    ...
    'rsPHYCNTDUP2.Open "Select tagno,PARTNO from PHYCNT where tagno = " & NumericVal(txtTagNo.Text) & " and PartNo = " & N2Str2Null(Trim(cboPartNo.Text)) & ", gconINVENTORY, adOpenForwardOnly, adLockReadOnly"
     'rsPHYCNTDUP2.Open "Select PARTNO from PHYCNT where TAGNO = " & txtTagNo & " , gconINVENTORY"
    rsPartsCount.Open ("Select tagno,PARTNO from phycnt where tagno = " & NumericVal(txtTagNo.Text) & " "), gconINVENTORY, adOpenForwardOnly, adLockReadOnly
    rsPartsCount2.Open ("Select tagno,PARTNO from phycnt where PARTNO = '" & RTrim(LTrim(cboPartNo.Text)) & "'"), gconINVENTORY, adOpenForwardOnly, adLockReadOnly
    rsPHYCNTDUP2.Open "select TAGNO from PHYCNT where TAGNO = " & NumericVal(txtTagNo.Text), gconINVENTORY
    
    rsPHYCNTDUP.Open "select TAG AS TAG from TAGS where tag = " & NumericVal(txtTagNo.Text), gconINVENTORY
    ...
    If AddorEdit = "ADD" Then                         'this validation is for adding data
        ...
        If Not rsPartsCount.EOF And Not rsPartsCount.BOF Then
            MsgSpeechBox "Error: Tag Number " & txtTagNo.Text & " was already used" & vbCrLf & _
                         "by Part number " & Null2String(rsPartsCount!PARTNO)
            On Error Resume Next
            txtTagNo.SetFocus
            Exit Sub
        End If
'        If Not (rsPartsCount2.EOF And rsPartsCount2.BOF) Then
'            MsgSpeechBox "Error: Part Number " & cboPartNo.Text & " was already used" & vbCrLf & _
'                         "by tag number " & Null2String(rsPartsCount2!tagno)
'            On Error Resume Next
'            cboPartNo.SetFocus
'            Exit Sub
'        End If
...
'        If Not (rsPHYCNTDUP2.BOF And rsPHYCNTDUP2.EOF) Then
'            If Null2String(Trim(rsPHYCNTDUP2!tagno)) = Null2String(Trim(txtTagNo)) Then
'                MsgSpeechBox ("Tag Number " & txtTagNo.Text & " was already used")
'                txtTagNo.SetFocus
'                Exit Sub
'            End If
'        End If
        If rsPHYCNTDUP.BOF And rsPHYCNTDUP.EOF Then
                MsgSpeechBox ("Tag Number " & txtTagNo.Text & " Does not exist pls add tag number in Data Entry of Tag Numbers")
                txtTagNo.SetFocus
                Exit Sub
        End If
'
...
'        End If
    Else
        ...
        'in every tag number there just only one part number
        'updated by: IEBV
'        If Null2String(rsPHYCNT!PARTNO) <> Trim(LTrim(cboPartNo.Text)) And NumericVal(rsPHYCNT!tagno) <> NumericVal(txtTagNo.Text) Then
'            If Not rsPartsCount.EOF And Not rsPartsCount.BOF Then
'                MsgSpeechBox "Error: Part Number " & cboPartNo.Text & " was already used" & vbCrLf & _
...
'        If Null2String(rsPHYCNT!PARTNO) <> Trim(LTrim(cboPartNo.Text)) And NumericVal(rsPHYCNT!tagno) = NumericVal(txtTagNo.Text) Then
'            If Not rsPartsCount2.EOF And Not rsPartsCount2.BOF Then
'                MsgSpeechBox "Error: Part Number " & cboPartNo.Text & " was already used" & vbCrLf & _
'                             "by tag number " & NumericVal(rsPartsCount2!tagno)
'                On Error Resume Next
'                cboPartNo.SetFocus
'                Exit Sub
'            End If
'        End If
        'Validation: (To prevent the duplication of tag number
        'There is a Dupliction when you add with the same tag number and part number
        If NumericVal(rsPHYCNT!tagno) <> NumericVal(txtTagNo.Text) Then
        ...
        End If
        If NumericVal(rsPHYCNT!tagno) <> NumericVal(txtTagNo.Text) Then
            If rsPHYCNTDUP.EOF And rsPHYCNTDUP.BOF Then
    ...
    If save = False Then
        ...
        gconINVENTORY.RollbackTrans
        Screen.MousePointer = 0
    Else
        gconINVENTORY.CommitTrans
...
ErrorCode:
    ...
    Exit Sub
'    MsgBox "Tag Number " & txtTagNo.Text & " is not part of Tag num", vbInformation, "Please Change your  Tag Number"
'    Exit Sub

End Sub
Function save() As Boolean
On Error GoTo errodaa
...
Private Sub Form_Load()
    Screen.MousePointer = 11
    CenterMe frmMain, Me, 1: cleargrid grdPhyCnt:  InitCbo: rsRefresh: initMemvars: InitGrid: FillGrid
...
Cbo_top.AddItem "300"
Cbo_top.AddItem "500"
Cbo_top.AddItem "1000"
Cbo_top.AddItem "2000"
    ...
    If KeyAscii = 13 Then
        ...
        If UCase(Trim(Cbo_top.Text)) = "ALL" Then
            If Option2.Value = True Then
                rsPHYCNT.Open "Select * from PHYCNT where PARTNO like '" & Repleys(txtSearchPartNo) & "%' order by PARTNO asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            ElseIf Option1.Value = True Then
                rsPHYCNT.Open "Select * from PHYCNT where TagNo like '" & Repleys(txtSearchPartNo) & "%' order by tagno asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            Else
                rsPHYCNT.Open "Select * from PHYCNT where Location like '" & Repleys(txtSearchPartNo) & "%' order by Location asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            End If
        Else
            If Option2.Value = True Then
                rsPHYCNT.Open "Select TOP " & N2Str2IntZero(Trim(Cbo_top.Text)) & " * from PHYCNT where PARTNO like '" & Repleys(txtSearchPartNo) & "%' order by PARTNO asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            ElseIf Option1.Value = True Then
                rsPHYCNT.Open "Select TOP " & N2Str2IntZero(Trim(Cbo_top.Text)) & " * from PHYCNT where TagNo like '" & Repleys(txtSearchPartNo) & "%' order by tagno asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            Else
                rsPHYCNT.Open "Select TOP " & N2Str2IntZero(Trim(Cbo_top.Text)) & " * from PHYCNT where Location like '" & Repleys(txtSearchPartNo) & "%' order by Location asc", gconINVENTORY, adOpenForwardOnly, adLockReadOnly
            End If
        End If
        Dim KCNT                                       As Integer
        KCNT = 0
