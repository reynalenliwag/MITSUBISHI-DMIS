...
Begin VB.Form frmPMISMAT_MRISForms 
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin VB.CommandButton cmdTranDelete 
         ...
         Height          =   795
         Left            =   2880
         MouseIcon       =   "MAT_MRISForm.frx":78A3
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":79F5
         Style           =   1  'Graphical
         TabIndex        =   68
      ...
      Begin VB.CommandButton cmdTranSave 
         ...
         Height          =   795
         Left            =   1440
         MouseIcon       =   "MAT_MRISForm.frx":7D20
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":7E72
         Style           =   1  'Graphical
         TabIndex        =   67
      ...
      Begin VB.CommandButton cmdTranCancel 
         ...
         Height          =   795
         Left            =   2160
         MouseIcon       =   "MAT_MRISForm.frx":81C2
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":8314
         Style           =   1  'Graphical
         TabIndex        =   66
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   1125
         MouseIcon       =   "MAT_MRISForm.frx":8652
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":87A4
         Style           =   1  'Graphical
         TabIndex        =   78
         Top             =   1575
         Width           =   855
...
Sub FillGrid()
    ...
    Set RSORD_HD = New ADODB.Recordset
    Set RSORD_HD = gconDMIS.Execute("select top 20 tranno,tranno x from PMIS_vw_PRS where trantype = '" & Repleys(txtTranType.Text) & "' order by Tranno asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set RSORD_HD = gconDMIS.Execute("select top 20 tranno,tranno x from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(txtTranType.Text) & "' and tranno like '" & Repleys(XXX) & "%'")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Enabled = False
    Set RSORD_HD = New ADODB.Recordset
    Set RSORD_HD = gconDMIS.Execute("select top 20 rono,tranno from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(txtTranType.Text) & "' and rono is not null order by tranno asc")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set RSORD_HD = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HD = gconDMIS.Execute("select top 20 Rono, tranno from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(txtTranType.Text) & "' and rono like '" & Repleys(XXX) & "%' order by tranno asc")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Private Sub cmdCancelCO_Click()
    ...
    If LOGLEVEL <> "ADM" Then
        ...
        Exit Sub
    End If
    If chkstatus(txtTranNo.Text, "M", "MRS") = "P" Then
        MessagePop InfoVoid, "Action Void", "Transaction already Posted!"
        Exit Sub
    ElseIf chkstatus(txtTranNo.Text, "M", "MRS") = "C" Then
    ...
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "C", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
            End If
...
On Error GoTo errordaa
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                SQL_STATEMENT = "update PMIS_vw_PRS_Tran set" & _
                              ...
                              " where id = " & RSTDAYTRANDUP!ID
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "PP", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", "TRAN NO: " & txtTranNo, txtTranType.Text, ""
    ...
    SQL_STATEMENT = "update PMIS_vw_PRS set" & _
                  ...
                  " where id = " & labID.Caption
    gconDMIS.Execute SQL_STATEMENT
    NEW_LogAudit "P", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", "TRAN NO: " & txtTranNo, txtTranType.Text, ""
...
Private Sub cmdTranDelete_Click()
    ...
    If MsgQuestionBox("Delete This Materials, Are you Sure?", "Delete Materials Entry") = True Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "XX", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", "MAT CODE: " & cboTranPartNo, txtTranType.Text, labDetID
        ShowDeletedMsg
    End If
    ...
    Set RSTDAYTRANDUP = New ADODB.Recordset
    RSTDAYTRANDUP.Open "select id,itemno from PMIS_vw_PRS_Tran where [TYPE] = 'M' and trantype = " & N2Str2Null(txtTranType.Text) & " and tranno = " & N2Str2Null(RSORD_HD!TRANNO) & " order by itemno asc", gconDMIS
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        RSTDAYTRANDUP.MoveFirst
        CNT = 0
        Do While Not RSTDAYTRANDUP.EOF
...
Private Sub cmdTranSave_Click()
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "AA", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", "MAT NO: " & cboTranPartNo, txtTranType.Text, ""
    ...
    Else
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "EE", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", "MAT NO: " & cboTranPartNo, txtTranType.Text, labDetID
...
Private Sub cmdSave_Click()
    ...
    Else
        If AddorEdit = "ADD" Then
            If checkdupRS("MRS", "M", Trim(txtReferencePIS.Text)) = True Then
                MsgBox "Reference MRS Number Already Exist"
                Exit Sub
            End If
        Else
    ...
    Else
        If AddorEdit = "ADD" Then
            If checkdup("M", txtTranType.Text, txtTranNo.Text) = True Then
                If MsgBox("Transaction No. already exist!, Do you want to generate new MRS number?", vbQuestion + vbYesNo) = vbYes Then
                    txtTranNo.Text = getnextISSPORR("M", "MRS")
                    txtRONO.Enabled = True
                    txtTerms.Enabled = False
                Else
                    On Error Resume Next
...
On Error GoTo errordaa
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "A", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, FindTransactionID(txtTranNo, "tranno", "PMIS_vw_PRS", "DETAILS", N2Str2Null("M"), "TYPE"), "Materials", txtTranNo, txtTranType.Text, ""
    Else
        ...
        SQL_STATEMENT = "update PMIS_vw_PRS set" & _
                      ...
                      " where id = " & labID.Caption
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "E", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
        ...
        SQL_STATEMENT = "update PMIS_vw_PRS_Tran set" & _
                      ...
                      " trandate = " & VTXTTRANDATE & "," & _
                      " tranno = " & VTXTTRANNO & _
                      " where trantype = '" & PREVORDTYPE & "' and tranno = '" & PREVORDNO & "' AND [TYPE] = 'M'"
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "E", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
...
If Module_Access(LOGID, "INPUT DISCOUNT", "SYSTEM") = False Or Mid(txtReferencePIS.Text, 3, 1) = "S" Then Exit Sub
On Error Resume Next
txtDS1.Locked = False
txtDS1.SetFocus
End Sub
...
If Mid(txtReferencePIS, 5, 1) = "I" And COMPANY_CODE = "DGI" Then Exit Sub
    ...
    Select Case KeyCode
        ...
        Case vbKeyF4
            If FILD <> "" And FILD <> "No Entry" Then
                If Picture1.Visible = True Then
                    If chkstatus(txtTranNo.Text, "M", "MRS") <> "P" And chkstatus(txtTranNo.Text, "M", "MRS") <> "C" And chkstatus(txtTranNo.Text, "M", "MRS") <> "B" Then
                        grdDetails_DblClick
                        Picture1.Enabled = False
                        fraDetails.Enabled = False
                    End If
        ...
        Case vbKeyF12
            If Picture1.Visible = True Then
                If chkstatus(txtTranNo.Text, "M", "MRS") = "P" Then
                    ...
                    If ISISSUED("M", Trim(txtReferencePIS.Text)) = True Then Exit Sub
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                SQL_STATEMENT = "update PMIS_vw_PRS_Tran set" & _
                              ...
                              " where id = " & RSTDAYTRANDUP!ID
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "U", "MATERIALS REQUISITION SLIP", SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
