...
Begin VB.Form frmPMISMAT_CreateINVDATA 
   ...
   ClientWidth     =   4365
   ForeColor       =   &H00DEDFDE&
   Icon            =   "MAT_CreateINVData.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "MAT_CreateINVData.frx":1472
   ScaleHeight     =   780
   ScaleWidth      =   4365
   ...
   End
End
Attribute VB_Name = "frmPMISMAT_CreateINVDATA"
...
Sub Create_Part_MasterFile_Table()
    ...
    Set FLDCUTOFF(1) = tdCUTOFF.CreateField("PARTNO", dbText, 13)
    FLDCUTOFF(1).Required = True
    FLDCUTOFF(1).AllowZeroLength = False
    Set FLDCUTOFF(2) = tdCUTOFF.CreateField("PARTDESC", dbText, 50)
...
Sub Create_Adjustment_MasterFile_Table()
    ...
    Set FLDADJUST(1) = tdADJUST.CreateField("PARTNO", dbText, 13)
    FLDADJUST(1).Required = True
    FLDADJUST(1).AllowZeroLength = False
    Set FLDADJUST(2) = tdADJUST.CreateField("PARTDESC", dbText, 50)
...
Sub Create_ConPhy_MasterFile_Table()
    ...
    Set FLDCONPHY(1) = tdCONPHY.CreateField("PARTNO", dbText, 13)
    FLDCONPHY(1).Required = True
    FLDCONPHY(1).AllowZeroLength = False
    Set FLDCONPHY(2) = tdCONPHY.CreateField("PARTDESC", dbText, 50)
...
Sub Create_PhyCnt_MasterFile_Table()
    ...
    Set FLDPHYCNT(1) = tdPHYCNT.CreateField("PARTNO", dbText, 13)
    FLDPHYCNT(1).Required = True
    FLDPHYCNT(1).AllowZeroLength = False
    Set FLDPHYCNT(2) = tdPHYCNT.CreateField("PARTDESC", dbText, 50)
...
Sub Create_TagNumber_MasterFile_Table()
    ...
    Set FLDTAGS(2) = tdTAGS.CreateField("PARTNO", dbText, 13)
...
Sub Create_Ledger_MasterFile_Table()
    ...
    Set FLDLEDGER(1) = tdLEDGER.CreateField("PARTNO", dbText, 13)
    FLDLEDGER(1).Required = True
    FLDLEDGER(1).AllowZeroLength = False
    Set FLDLEDGER(2) = tdLEDGER.CreateField("PARTDESC", dbText, 50)
...
Private Sub cmdCreateData_Click()
    ...
    If Function_Access(LOGID, "Acess_Process", "MATERIALS CUTOFF") = False Then Exit Sub
    ...
    Create_Database
    LogAudit "G", "CREATE INVENTORY DATABASE"
    If Err = 32755 Then Exit Sub
End Sub
