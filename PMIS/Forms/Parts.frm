...
Begin VB.Form frmPMISMaster_Parts 
   ...
   Begin VB.PictureBox PIC_UNPOSTED 
      ...
      Begin VB.Label Label2 
         BackStyle       =   0  'Transparent
         Caption         =   "Double click to go to transaction."
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Label labPrev 
      ...
      Width           =   285
   End
   Begin VB.Menu capmenu 
      Caption         =   "menu"
      Visible         =   0   'False
      Begin VB.Menu menutransac 
         Caption         =   "See transaction History..."
      End
      Begin VB.Menu menuPO 
         Caption         =   "Add to P.O...."
      End
      Begin VB.Menu menurr 
         Caption         =   "Add to R.R...."
      End
      Begin VB.Menu menuiss 
         Caption         =   "Add to Issuance...."
      End
      Begin VB.Menu menuadj 
         Caption         =   "Add to Adjustment..."
      End
   End
End
...
Sub StoreMemVars()
    Set RSPARTS_DETAIL = New ADODB.Recordset
...
'updated by:    IEBV_02232011_0215pm
'description:
'--------------------------------------------------------------------------------
        If Null2String(RSPARTS_DETAIL!NON_HARI) = "Y" Then
            'chkNonHARI.Value = 1
            cboClass.ListIndex = 1
        ElseIf Null2String(RSPARTS_DETAIL!NON_HARI) = "N" Then
            'chkNonHARI.Value = 0
...
Sub FillGrid()
    ...
    If optPartNo.Value = True Then
        If OPT_ACTIVE.Value = True Then
            Set RSPARTMAS = gconDMIS.Execute("SELECT STOCKNO, ID FROM PMIS_STOCKMAS WHERE ACTIVE='Y' AND TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        ElseIf OPT_INACTIVE.Value = True Then
            Set RSPARTMAS = gconDMIS.Execute("SELECT  STOCKNO, ID FROM PMIS_STOCKMAS WHERE ISNULL(ACTIVE,'N')='N' AND TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        Else
            Set RSPARTMAS = gconDMIS.Execute("SELECT STOCKNO, ID FROM PMIS_STOCKMAS WHERE TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        End If
    Else
        If OPT_ACTIVE.Value = True Then
            Set RSPARTMAS = gconDMIS.Execute("SELECT STOCKDESC, ID FROM PMIS_STOCKMAS WHERE ACTIVE='Y' AND TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        ElseIf OPT_INACTIVE.Value = True Then
            Set RSPARTMAS = gconDMIS.Execute("SELECT  STOCKDESC, ID FROM PMIS_STOCKMAS WHERE ISNULL(ACTIVE,'N')='N' AND TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        Else
            Set RSPARTMAS = gconDMIS.Execute("SELECT STOCKDESC, ID FROM PMIS_STOCKMAS WHERE TYPE='" & LOCAL_STOCKTYPE & "'" & SEARCH_STRING)
        End If
    End If
    
    Listview_Loadval Me.lstParts.ListItems, RSPARTMAS
...
Private Sub cmdSave_Click()
    ...
    Else
        ...
        End If
    End If
'updated By:    IEBV 02282011_0240pm
'description:
'------------------------------------------------------------------------------------
    If AddorEdit = "EDIT" Then
        If Null2String(RSPARTS_DETAIL!NON_HARI) = "" Then
            'do nothing
        ElseIf Null2String(RSPARTS_DETAIL!NON_HARI) = "Y" Then
            If cboClass.ListIndex = 0 Then
                MsgBox "This is a Non-Hari parts, cannot change to MMPC parts.", vbInformation + vbOKOnly
                cboClass.ListIndex = 1
                Exit Sub
            ElseIf cboClass.ListIndex = 2 Then
                MsgBox "This is a Non-Hari parts, cannot change to Other Brands.", vbInformation + vbOKOnly
                cboClass.ListIndex = 1
                Exit Sub
            End If
        ElseIf Null2String(RSPARTS_DETAIL!NON_HARI) = "N" Then
            If cboClass.ListIndex = 1 Then
                MsgBox "This is a Hari parts, cannot change to Non-MMPC parts.", vbInformation + vbOKOnly
                cboClass.ListIndex = 0
                Exit Sub
            ElseIf cboClass.ListIndex = 2 Then
                MsgBox "This is a Hari parts, cannot change to Other Brands.", vbInformation + vbOKOnly
                cboClass.ListIndex = 0
                Exit Sub
            End If
        ElseIf Null2String(RSPARTS_DETAIL!NON_HARI) = "0" Then
            If cboClass.ListIndex = 0 Then
                MsgBox "This is an Other Brand parts, cannot change to MMPC parts.", vbInformation + vbOKOnly
                cboClass.ListIndex = 2
                Exit Sub
            ElseIf cboClass.ListIndex = 1 Then
                MsgBox "This is an Other Brand parts, cannot change to Non-MMPC.", vbInformation + vbOKOnly
                cboClass.ListIndex = 2
                Exit Sub
            End If
        End If
        If LTrim(RTrim(Null2String(RSPARTS_DETAIL!StockType))) = "GJ" Then
            If CBOtype.ListIndex = 1 Then
                MsgBox "This is an GJ stock type, cannot change to BP.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 0
                Exit Sub
            ElseIf CBOtype.ListIndex = 2 Then
                MsgBox "This is an GJ stock type, cannot change to Others.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 0
                Exit Sub
            End If
        ElseIf LTrim(RTrim(Null2String(RSPARTS_DETAIL!StockType))) = "BP" Then
            If CBOtype.ListIndex = 0 Then
                MsgBox "This is an BP stock type, cannot change to GJ.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 1
                Exit Sub
            ElseIf CBOtype.ListIndex = 2 Then
                MsgBox "This is an BP stock type, cannot change to Others.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 1
                Exit Sub
            End If
        ElseIf LTrim(RTrim(Null2String(RSPARTS_DETAIL!StockType))) = "Other" Then
            If CBOtype.ListIndex = 0 Then
                MsgBox "This is an Other stock type, cannot change to GJ.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 2
                Exit Sub
            ElseIf CBOtype.ListIndex = 1 Then
                MsgBox "This is an Other stock type, cannot change to BP.", vbInformation + vbOKOnly
                CBOtype.ListIndex = 2
                Exit Sub
            End If
        End If
    ...
    If AddorEdit = "ADD" Then
        'If LOCAL_STOCKTYPE = "P" Then
            SQL_STATEMENT = "INSERT INTO PMIS_STOCKMAS " & _
                          " (TYPE,LASTM_MAC,STOCKNO,STOCKDESC,VEHTYPE,MODELCODE,LOCATION,MAC,OLDNO,WFP,SRP,TISSQTY,TPOQTY,PHYCOUNT,ADJPHYCNT,RECEIPTS,ISSUANCES,SSTOCK,RESSERVICE,LASTUPDATE,USERCODE,ACTIVE,NON_HARI,STOCKTYPE)" & _
                          ...
                          " " & VTXTMODELCODE & ", " & VTXTLocation & ", " & vtxtMAC & _
                            ...
                            ", " & VTXTPHYCOUNT & ", " & VTXTADJPHYCNT & ", " & VTXTRECEIPTS & ", " & VTXTISSUANCES & ", " & VTXTSSTOCK & ", " & VTXTRESSERVICE & ", '" & LOGDATE & "', " & N2Str2Null(LOGCODE) & ",'N'," & NON_HARI_STR & ", '" & VTXTstocktype & "')"
'        Else
'            SQL_STATEMENT = "INSERT INTO PMIS_STOCKMAS " & _
...
Sub initcr()
    With LST_UNPOSTED
        ...
        .Columns.DeleteAll
        .Columns.Add 0, "ROW #", 0, True:               .Columns(0).Resizable = False:                      .Columns(0).Visible = False
        .Columns.Add 1, "Date", 50, True:               .Columns(1).Alignment = xtpAlignmentCenter
        .Columns.Add 2, "Transaction #", 50, True:              .Columns(2).Alignment = xtpAlignmentCenter
        .Columns.Add 3, "Trantype", 30, True:           .Columns(3).Alignment = xtpAlignmentCenter
        .Columns.Add 4, "Type", 20, True:               .Columns(4).Alignment = xtpAlignmentCenter
    End With
End Sub
...
Sub InitCbo()
    'If LOCAL_STOCKTYPE = "P" Then
    cboClass.Clear
    cboClass.AddItem "MMPC"
    cboClass.AddItem "Non-MMPC"
...
If Row.Record(0).Value Mod 2 = 0 Then
    Metrics.BackColor = &HC0FFC0
Else
    Metrics.BackColor = &HFFFFFF
End If
End Sub
...
If chkstatus(Row.Record(2).Value, LOCAL_STOCKTYPE, Row.Record(3).Value) = "P" Or chkstatus(Row.Record(2).Value, LOCAL_STOCKTYPE, Row.Record(3).Value) = "B" Then MsgBox "Transaction Already Posted", vbInformation + vbOKOnly, "Action Void": Exit Sub
    If LOCAL_STOCKTYPE = "P" Then
        If Row.Record(3).Value = "RR" Then
                ...
                frmPMISTrans_Receiving2.textSearch.Text = Row.Record(2).Value
                frmPMISTrans_Receiving2.click
                frmPMISTrans_Receiving2.cmdAddTran.Value = True
                frmPMISTrans_Receiving2.cboTranPartNo.Text = txtPartNo.Text
                
        ElseIf Row.Record(3).Value = "PO" Then
            If Module_Access(LOGID, "PURCHASE ORDER", "TRANSACTION") = False Then Exit Sub
            Unload frmPMISTrans_Purchase
            FormExistsShow frmPMISTrans_Purchase
            frmPMISTrans_Purchase.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_Purchase.click
            frmPMISTrans_Purchase.cmdAddTran.Value = True
            frmPMISTrans_Purchase.cboTranPartNo.Text = txtPartNo.Text
        Else
            If Row.Record(3).Value = "CSH" Then
                If Module_Access(LOGID, "PARTS ISSUANCE COUNTER CASH", "TRANSACTION") = False Then Exit Sub
            ElseIf Row.Record(3).Value = "CHG" Then
                If Module_Access(LOGID, "PARTS ISSUANCE COUNTER CHARGE", "TRANSACTION") = False Then Exit Sub
            ElseIf Row.Record(3).Value = "RIV" Then
                If Module_Access(LOGID, "PARTS ISSUANCE SERVICE ISSUANCE", "TRANSACTION") = False Then Exit Sub
            ElseIf Row.Record(3).Value = "DR" Then
                If Module_Access(LOGID, "PARTS DR OUT ISSUANCE", "TRANSACTION") = False Then Exit Sub
            End If
            Unload frmPMISTrans_CustomerOrder
            COUNTERTYPE = Row.Record(3).Value
            frmPMISTrans_CustomerOrder.txtTranType.Text = Row.Record(3).Value
            FormExistsShow frmPMISTrans_CustomerOrder
            frmPMISTrans_CustomerOrder.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_CustomerOrder.click
            frmPMISTrans_CustomerOrder.cmdAddTran.Value = True
            frmPMISTrans_CustomerOrder.cboTranPartNo.Text = txtPartNo.Text
        End If
    ElseIf LOCAL_STOCKTYPE = "A" Then
        If Row.Record(3).Value = "RR" Then
            ...
            frmPMISTrans_Receiving2_AC.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_Receiving2_AC.click
            frmPMISTrans_Receiving2_AC.cmdAddTran.Value = True
            frmPMISTrans_Receiving2_AC.cboTranPartNo.Text = txtPartNo.Text
        ElseIf Row.Record(3).Value = "PO" Then
            If Module_Access(LOGID, "ACCESSORIES PURCHASE ORDER", "TRANSACTION") = False Then Exit Sub
            Unload frmPMISAC_Purchase
            FormExistsShow frmPMISAC_Purchase
            frmPMISAC_Purchase.textSearch.Text = Row.Record(2).Value
            frmPMISAC_Purchase.click
            frmPMISAC_Purchase.cmdAddTran.Value = True
            frmPMISAC_Purchase.cboTranPartNo.Text = txtPartNo.Text
        Else
            If Row.Record(3).Value = "CSH" Then
            ...
            COUNTERTYPE = Row.Record(3).Value
            frmPMISTrans_CustomerOrder_AC.txtTranType.Text = Row.Record(3).Value
            FormExistsShow frmPMISTrans_CustomerOrder_AC
            frmPMISTrans_CustomerOrder_AC.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_CustomerOrder_AC.click
            frmPMISTrans_CustomerOrder_AC.cmdAddTran.Value = True
            frmPMISTrans_CustomerOrder_AC.cboTranPartNo = txtPartNo.Text
        End If
    Else
        If Row.Record(3).Value = "RR" Then
            ...
            frmPMISTrans_Receiving2_MAT.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_Receiving2_MAT.click
            frmPMISTrans_Receiving2_MAT.cmdAddTran.Value = True
            frmPMISTrans_Receiving2_MAT.cboTranPartNo.Text = txtPartNo.Text
        ElseIf Row.Record(3).Value = "PO" Then
            If Module_Access(LOGID, "MATERIALS PURCHASE ORDER", "TRANSACTION") = False Then Exit Sub
            Unload frmPMISMAT_Purchase
            FormExistsShow frmPMISMAT_Purchase
            frmPMISMAT_Purchase.textSearch.Text = Row.Record(2).Value
            frmPMISMAT_Purchase.click
            frmPMISMAT_Purchase.cmdAddTran.Value = True
            frmPMISMAT_Purchase.cboTranPartNo.Text = txtPartNo.Text
        Else
            If Row.Record(3).Value = "CSH" Then
            ...
            COUNTERTYPE = Row.Record(3).Value
            frmPMISTrans_CustomerOrder_MAT.txtTranType.Text = Row.Record(3).Value
            FormExistsShow frmPMISTrans_CustomerOrder_MAT
            frmPMISTrans_CustomerOrder_MAT.textSearch.Text = Row.Record(2).Value
            frmPMISTrans_CustomerOrder_MAT.click
            frmPMISTrans_CustomerOrder_MAT.cmdAddTran.Value = True
            frmPMISTrans_CustomerOrder_MAT.cboTranPartNo.Text = txtPartNo.Text
        End If
    End If
...
Private Sub lstParts_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Len(lstParts.SelectedItem.Text) < 0 Then Exit Sub
    
    If Button = vbRightButton Then
        'Me.capmenu.Visible = True
        menutransac.Visible = True
        menutransac.Enabled = True
        
        PopupMenu capmenu
    End If
End Sub

Private Sub menuadj_Click()
    If TXT_ACTIVE.Text <> "ACTIVE" Then MsgBox "Part number not active", vbInformation: Exit Sub
    If LOCAL_STOCKTYPE = "P" Then
        ...
        frmPMISTrans_InventoryAdjustment_Parts.cmdAdd.Value = True
        frmPMISTrans_InventoryAdjustment_Parts.cboPartNo.Text = lstParts.SelectedItem.Text
    ElseIf LOCAL_STOCKTYPE = "A" Then
        ...
        frmPMISTrans_InventoryAdjustment_Accessories.cmdAdd.Value = True
        frmPMISTrans_InventoryAdjustment_Accessories.cboPartNo.Text = lstParts.SelectedItem.Text
    Else
        ...
        frmPMISTrans_InventoryAdjustment_Materials.cmdAdd.Value = True
        frmPMISTrans_InventoryAdjustment_Materials.cboPartNo.Text = lstParts.SelectedItem.Text
    End If
End Sub

Private Sub menuiss_Click()
    If NumericVal(txtOnHand.Text) = "0" Then MsgBox "Zero Onhand", vbInformation + vbOKOnly:  Exit Sub
    addtoo ("ISS")
End Sub

Private Sub menuPO_Click()
    If TXT_ACTIVE.Text <> "ACTIVE" Then MsgBox "Part number not active", vbInformation: Exit Sub
    addtoo ("PO")
End Sub

Private Sub menurr_Click()
    If TXT_ACTIVE.Text <> "ACTIVE" Then MsgBox "Part number not active", vbInformation: Exit Sub
    addtoo ("RR")
End Sub

Private Sub menutransac_Click()
    ...
    frmPMISInquiry_Query.txt_Ledger_Search.Text = (lstParts.SelectedItem.Text)
    frmPMISInquiry_Query.frommaster_SHOWLEDGER (lstParts.SelectedItem.Text)
...
Private Sub OPT_ACTIVE_Click()
    ...
    If optPartNo.Value = True Then
        RSPARTMAS.Open "select stockno,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' AND ACTIVE='Y' order by ID DESC", gconDMIS, adOpenDynamic
    Else
        RSPARTMAS.Open "select STOCKDESC,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' AND ACTIVE='Y' order by ID DESC", gconDMIS, adOpenDynamic
    End If
    Listview_Loadval Me.lstParts.ListItems, RSPARTMAS
    lstParts.Sorted = True
    rsRefresh
...
Private Sub OPT_ALL_Click()
    ...
    lstParts.Sorted = False
    If optPartNo.Value = True Then
        RSPARTMAS.Open "select stockno,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' order by ID DESC", gconDMIS, adOpenDynamic
    Else
        RSPARTMAS.Open "select STOCKDESC,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' order by ID DESC", gconDMIS, adOpenDynamic
    End If
    Listview_Loadval Me.lstParts.ListItems, RSPARTMAS
    lstParts.Sorted = True
    rsRefresh
...
Private Sub OPT_INACTIVE_Click()
    ...
    If optPartNo.Value = True Then
        RSPARTMAS.Open "select stockno,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' AND ISNULL(ACTIVE,'N')='N' order by ID DESC", gconDMIS, adOpenDynamic
    Else
        RSPARTMAS.Open "select STOCKDESC,ID from PMIS_STOCKMAS where TYPE = '" & LOCAL_STOCKTYPE & "' AND ISNULL(ACTIVE,'N')='N' order by ID DESC", gconDMIS, adOpenDynamic
    End If
    Listview_Loadval Me.lstParts.ListItems, RSPARTMAS
    lstParts.Sorted = True
    rsRefresh
...
Private Sub PIC_UNPOSTED_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Label2(5).FontBold = False
    Label2(5).Font.Size = 10
End Sub
...
Sub InitializeRC()
    ...
    End With
End Sub
Function addtoo(XXX As String)
Dim SQLX                                    As String
Dim ix As Integer
Set RS = New ADODB.Recordset
    If XXX = "ISS" Then
        SQLX = "SELECT  ROW_NUMBER() OVER (ORDER BY TRANNO) AS Row,TRANDATE,TRANNO,TRANTYPE,TYPE FROM PMIS_ORD_HD WHERE ISNULL(STATUS2,'')<>'R' AND TRANTYPE IN ('RIV','CHG','CSH','DR') AND (ISNULL(STATUS,'')='' OR  ISNULL(STATUS,'')='U' OR ISNULL(STATUS,'')='N') and [type] = '" & LOCAL_STOCKTYPE & "' "
    ElseIf XXX = "RR" Then
        SQLX = " SELECT  ROW_NUMBER() OVER (ORDER BY RRNO) AS Row,RRDATE,RRNO,'RR',TYPE FROM PMIS_RR_HD WHERE  (ISNULL(STATUS,'')='' OR  ISNULL(STATUS,'')='U' OR ISNULL(STATUS,'')='N') and [type] = '" & LOCAL_STOCKTYPE & "' "
    ElseIf XXX = "PO" Then
        SQLX = " SELECT  ROW_NUMBER() OVER (ORDER BY PONO) AS Row,PODATE,PONO,'PO',TYPE FROM PMIS_PO_HD WHERE  (ISNULL(STATUS,'')='' OR  ISNULL(STATUS,'')='N') and [type] = '" & LOCAL_STOCKTYPE & "'"
    Else
        
    End If
Set RS = gconDMIS.Execute(SQLX)
If Not (RS.EOF And RS.BOF) Then
    ix = RS.RecordCount
    flex_FillReportView RS, LST_UNPOSTED
    PIC_UNPOSTED.Visible = True
    PIC_UNPOSTED.ZOrder 0
    Picture1.Enabled = False
    fraDetails.Enabled = False
    Call InitializeRC
    Lblstock.Caption = "Stock #: " & txtPartNo.Text
Else
    ShowNoRecord
End If
End Function
