...
'description:
    ...
    For ictx = 1 To rssheetscount
        Set tdDNPP = dbDNPP.CreateTableDef("ANNEX " & ictx)
...
'updated by IEBV:
    If Exists(txtExcel) Then
        Set xlsApp = CreateObject("Excel.Application")
        Set xlsWB1 = xlsApp.Workbooks.Open(txtExcel)
        rssheetscount = xlsWB1.Sheets.Count
    Else
        MessagePop InfoVoid, "Action Void", "File doesn't exist."
        Exit Sub
    End If
'-----------------------------------------------------------------
    dlg.FileName = ""
...
Private Sub cmdOk_Click()
    ...
    For ictx = 1 To rssheetscount
        Set rsDNPP = New ADODB.Recordset
        Call rsDNPP.Open("select * from [annex " & ictx & "]", oConExcelToAcess, adOpenDynamic, adLockOptimistic)
        For int_wsheet = 1 To rssheetscount
            Set xlSheet = xlsWB1.Worksheets(int_wsheet)
    ...
    For int_wsheet = 1 To rssheetscount
        Set rsDNPP = New ADODB.Recordset
        Call rsDNPP.Open("select * from [annex " & int_wsheet & "]", oConExcelToAcess, adOpenDynamic, adLockOptimistic)
        ...
        If i = 0 Then
            ...
            End If
        End If
        Prg1.Max = xlSheet.Range("B" & Rows.Count).End(xlUp).Row
