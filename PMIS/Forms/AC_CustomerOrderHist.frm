...
Sub DRPRINTING()
    Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "DR_HIst.RPT", "{ord_hd.TRANTYPE} = 'DR' and {ord_hd.TYPE} = 'A' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
    Screen.MousePointer = 0
End Sub
Sub CHGPRINTING()
    If NumericVal(txtDS1.Text) = 0 Then
        Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CHGACC_Hist.RPT", "{ord_hd.TRANTYPE} = 'CHG' and {ord_hd.TYPE} = 'A' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        Screen.MousePointer = 0
    Else
        Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CHGACCDisc_Hist.rpt", "{ord_hd.TRANTYPE} = 'CHG' and {ord_hd.TYPE} = 'A' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        Screen.MousePointer = 0
    End If
End Sub
...
Sub CSHPRINTING()
    If NumericVal(txtDS1.Text) = 0 Then
        Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CSHACC_Hist.RPT", "{ord_hd.TYPE} = 'A' AND {ord_hd.TRANTYPE} = 'CSH' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        Screen.MousePointer = 0
    Else
        Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CSHACCDisc_Hist.RPT", "{ord_hd.TYPE} = 'A' AND {ord_hd.TRANTYPE} = 'CSH' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        Screen.MousePointer = 0
    End If
End Sub
...
Sub StoreMemVars()
    If Not RSORD_HIST.EOF And Not RSORD_HIST.BOF Then
        ...
        labORNo = CheckORNum(Null2String(RSORD_HIST!TRANNO), "AI", txtTranType.Text)
...
Sub FillGrid()
    ...
    Set RSORD_HIST = New ADODB.Recordset
    Set RSORD_HIST = gconDMIS.Execute("select Tranno,ID from PMIS_Ord_Hist where type = 'A' and trantype = '" & txtTranType.Text & "' order by Tranno asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set RSORD_HIST = gconDMIS.Execute("select tranno, ID from PMIS_Ord_Hist where type = 'A' and trantype = '" & txtTranType.Text & "' and tranno like '" & XXX & "%'")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Sorted = False: lstOrd_Hd.ListItems.Clear
    Set RSORD_HIST = New ADODB.Recordset
    Set RSORD_HIST = gconDMIS.Execute("select rono,ID from PMIS_Ord_Hist where type = 'A' and trantype = '" & txtTranType.Text & "' and rono is not null order by tranno asc")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set RSORD_HIST = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HIST = gconDMIS.Execute("select Rono, ID from PMIS_Ord_Hist where type = 'A' and trantype = '" & txtTranType.Text & "' and rono like '" & XXX & "%' order by tranno asc")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillGrid3()
    ...
    Set RSORD_HD = gconDMIS.Execute("select Custname,tranno from PMIS_Ord_Hist where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' order by CUSTNAME asc")
...
Sub FillSearchCusTomer(XXX As String)
    ...
    Set RSORD_HD = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HD = gconDMIS.Execute("select custname, tranno from PMIS_Ord_Hist where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' and CUSTNAME  like '" & XXX & "%' order by CUSTNAME")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
    Else
        lstOrd_Hd.Enabled = False
...
Private Sub cmd_seemaster_Click()
    ...
    frmMasterFile_Parts.SETSTOCKTYPE ("A")
    FormExistsShow frmMasterFile_Parts
    frmMasterFile_Parts.textSearch.Text = FILD
    Call frmMasterFile_Parts.SearchStock(FILD, "A")
End Sub
...
Private Sub cmdPrint_Click()
    ...
    If RSORD_HIST!TranType = "DR" Then
        ...
        End If
    End If
    NEW_LogAudit "V", "ACCESSORIES ISSUANCE - HISTORY", "", "", "Accessories", txtTranNo, txtTranType.Text, ""
End Sub
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyF1 And Shift = 1:
            ...
            frmALL_AuditInquiry.Caption = "Audit Inquiry (Accessories Issuance)"
            '====================================================================
            If txtTranType.Text = "CSH" Then
