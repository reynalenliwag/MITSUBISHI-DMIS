...
Private Sub cmdSave_Click()
    ...
    Else
        If AddorEdit = "ADD" Then
            If COMPANY_CODE = "DGI" Then
                ...
                If N2Str2Null(UCase(txtModule.Text)) = "CSH" Or N2Str2Null(UCase(txtModule.Text)) = "CHG" Then
                    RSFINDDUP.Open "select modul from PMIS_Counter where [TYPE] = '" & LOCAL_STOCKTYPE & "' AND Modul in('CSH','CHG')", gconDMIS, adOpenForwardOnly, adLockReadOnly
                Else
    ...
    If txtModule.Text = "RR" Then
        If checkavailableno = True Then
            MsgBox "Next number already in use!", vbCritical + vbOKOnly
            On Error Resume Next
            txtNextNumber.SetFocus
            Exit Sub
        End If
    ...
    If txtModule.Text = "PO" Then
        If checkavailableno_PO = True Then
            MsgBox "Next number already in use!", vbCritical + vbOKOnly
            On Error Resume Next
            txtNextNumber.SetFocus
            Exit Sub
    ...
    If txtModule.Text = "CSH" Or txtModule.Text = "CHG" Or txtModule.Text = "RIV" Then
        If checkavailable_tranno = True Then
            MsgBox "Next number already in use!", vbCritical + vbOKOnly
            On Error Resume Next
            txtNextNumber.SetFocus
            Exit Sub
    ...
    If txtModule.Text = "PRS" Or txtModule.Text = "ARS" Or txtModule.Text = "MRS" Then
        If checkavailable_req = True Then
            MsgBox "Next number already in use!", vbCritical + vbOKOnly
            On Error Resume Next
            txtNextNumber.SetFocus
            Exit Sub
    ...
    Else
        If COMPANY_CODE = "DGI" Then
            If Trim(UCase(txtModule.Text)) = "CSH" Or Trim(UCase(txtModule.Text)) = "CHG" Then
                SQL_STATEMENT = "UPDATE PMIS_COUNTER SET" & _
                              " NEXTNUMBER = " & VTXTNextNumber & "," & _
                              " LASTUPDATE = " & "'" & LOGDATE & "'" & "," & _
                              " USERCODE = " & "" & N2Str2Null(LOGCODE) & "" & _
                              " WHERE modul in ('CSH','CHG') and [TYPE] = '" & LOCAL_STOCKTYPE & "'"
            Else
                SQL_STATEMENT = "UPDATE PMIS_COUNTER SET" & _
                              " MODUL = " & VTXTModule & "," & _
...
Function checkavailableno() As Boolean
    Dim rsvalidno                               As ADODB.Recordset
    Dim sqlcommand                              As String
    Dim no                                      As String
    
    Set rsvalidno = New ADODB.Recordset
    no = Format(VTXTNextNumber, "000000")
    sqlcommand = "Select * from pmis_rr_hd where [TYPE] = '" & LOCAL_STOCKTYPE & "' and rrno = '" & no & "' "
    sqlcommand = sqlcommand + "UNION ALL "
    sqlcommand = sqlcommand + "Select* from pmis_rec_hist where [TYPE] = '" & LOCAL_STOCKTYPE & "' and rrno = '" & no & "'"
    Set rsvalidno = gconDMIS.Execute(sqlcommand)
    
    If Not rsvalidno.EOF And Not rsvalidno.BOF Then
        checkavailableno = True
    Else
        checkavailableno = False
    End If
End Function

Function checkavailableno_PO() As Boolean
    Dim rsvalidno                               As ADODB.Recordset
    Dim sqlcommand                              As String
    Dim no                                      As String
    ...
    Set rsvalidno = New ADODB.Recordset
    no = Format(VTXTNextNumber, "000000")
    sqlcommand = "Select * from pmis_po_hd where [TYPE] = '" & LOCAL_STOCKTYPE & "' and pono = '" & no & "' "
    sqlcommand = sqlcommand + "UNION ALL "
    sqlcommand = sqlcommand + "Select * from pmis_po_hist where [TYPE] = '" & LOCAL_STOCKTYPE & "' and pono = '" & no & "'"
    Set rsvalidno = gconDMIS.Execute(sqlcommand)
    
    If Not rsvalidno.EOF And Not rsvalidno.BOF Then
        checkavailableno_PO = True
    Else
        checkavailableno_PO = False
    End If
    
End Function

Function checkavailable_tranno() As Boolean
    Dim rsvalidno                               As ADODB.Recordset
    Dim sqlcommand                              As String
    Dim no                                      As String
    ...
    Set rsvalidno = New ADODB.Recordset
    no = Format(VTXTNextNumber, "000000")
    sqlcommand = "Select * from pmis_ORD_HD where [TYPE] = '" & LOCAL_STOCKTYPE & "' and right(tranno,6) = '" & no & "' and trantype = " & VTXTModule & " "
    sqlcommand = sqlcommand + "UNION ALL "
    sqlcommand = sqlcommand + "Select * from pmis_ORD_HIST where [TYPE] = '" & LOCAL_STOCKTYPE & "' and right(tranno,6) = '" & no & "' and trantype = " & VTXTModule & ""
    Set rsvalidno = gconDMIS.Execute(sqlcommand)
    
    If Not rsvalidno.EOF And Not rsvalidno.BOF Then
        checkavailable_tranno = True
    Else
        checkavailable_tranno = False
    End If
    
End Function


Function checkavailable_req() As Boolean
    Dim rsvalidno                               As ADODB.Recordset
    Dim sqlcommand                              As String
    Dim no                                      As String
    ...
    Set rsvalidno = New ADODB.Recordset
    no = Format(VTXTNextNumber, "000000")
    sqlcommand = "Select * from pmis_ord_hd where [TYPE] = '" & LOCAL_STOCKTYPE & "' and tranno = '" & no & "' and trantype = " & VTXTModule & " "
    sqlcommand = sqlcommand + "UNION ALL "
    sqlcommand = sqlcommand + "Select * from pmis_ord_hist where [TYPE] = '" & LOCAL_STOCKTYPE & "' and tranno = '" & no & "' and trantype = " & VTXTModule & ""
    Set rsvalidno = gconDMIS.Execute(sqlcommand)
    
    If Not rsvalidno.EOF And Not rsvalidno.BOF Then
        checkavailable_req = True
    Else
        checkavailable_req = False
    End If
