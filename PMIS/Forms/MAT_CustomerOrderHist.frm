...
Sub DRPRINTING()
    Screen.MousePointer = 11
        PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "DR_HIst.RPT", "{ord_hd.TRANTYPE} = 'DR' and {ord_hd.TYPE} = 'M' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
...
Sub StoreMemVars()
    If Not RSORD_HIST.EOF And Not RSORD_HIST.BOF Then
        ...
        labORNo = CheckORNum(Null2String(RSORD_HIST!TRANNO), "MI", txtTranType.Text)
...
Sub FillGrid()
    ...
    Set RSORD_HIST = New ADODB.Recordset
    Set RSORD_HIST = gconDMIS.Execute("select Tranno,ID from PMIS_Ord_Hist where type = 'M' and trantype = '" & txtTranType.Text & "' order by id desc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set RSORD_HIST = gconDMIS.Execute("select tranno, ID from PMIS_Ord_Hist where type = 'M' and trantype = '" & txtTranType.Text & "' and tranno like '" & XXX & "%' order by id desc")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Sorted = False: lstOrd_Hd.ListItems.Clear
    Set RSORD_HIST = New ADODB.Recordset
    Set RSORD_HIST = gconDMIS.Execute("select rono,ID from PMIS_Ord_Hist where type = 'M' and trantype = '" & txtTranType.Text & "' and rono is not null order by rono desc")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set RSORD_HIST = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HIST = gconDMIS.Execute("select Rono, ID from PMIS_Ord_Hist where type = 'M' and trantype = '" & txtTranType.Text & "' and rono like '" & XXX & "%' order by id desc")
    If Not (RSORD_HIST.EOF And RSORD_HIST.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HIST
        lstOrd_Hd.Refresh
...
Sub FillGrid3()
    ...
    Set RSORD_HD = gconDMIS.Execute("select Custname,tranno from PMIS_Ord_Hist where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' order by CUSTNAME asc")
...
Sub FillSearchCusTomer(XXX As String)
    ...
    Set RSORD_HD = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HD = gconDMIS.Execute("select custname, tranno from PMIS_Ord_Hist where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and CUSTNAME  like '" & XXX & "%' order by CUSTNAME")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
    Else
        lstOrd_Hd.Enabled = False
...
Private Sub cmd_seemaster_Click()
    ...
    frmMasterFile_Parts.SETSTOCKTYPE ("M")
    FormExistsShow frmMasterFile_Parts
    frmMasterFile_Parts.textSearch.Text = FILD
    Call frmMasterFile_Parts.SearchStock(FILD, "M")
End Sub
...
Private Sub cmdPrint_Click()
    ...
    If RSORD_HIST!TranType = "DR" Then
        ...
        End If
    End If
    NEW_LogAudit "V", "MATERIALS ISSUANCE - HISTORY", "", "", "Materials", txtTranNo, txtTranType.Text, ""
End Sub
Sub SendToBack()
'    cmdAddTran.ZOrder 1
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyF1 And Shift = 1:
            ...
            frmALL_AuditInquiry.Caption = "Audit Inquiry (Materials Issuance)"
            '====================================================================
            If txtTranType.Text = "CSH" Then
            ...
            ElseIf txtTranType.Text = "RIV" Then
                Call frmALL_AuditInquiry.DisplayHistory(labID, "MATERIALS ISSUANCE SERVICE ISSUANCE")
            Else
...
Private Sub lstOrd_Hd_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyEscape Then On Error Resume Next: textSearch.SetFocus: StoreMemVars
End Sub
...
End Sub
Sub click()
    RSORD_HIST.Bookmark = rsFind(RSORD_HIST.Clone, "tranno", lstOrd_Hd.SelectedItem).Bookmark
    StoreMemVars
End Sub
