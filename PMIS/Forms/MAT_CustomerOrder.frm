...
Begin VB.Form frmPMISTrans_CustomerOrder_MAT 
   ...
   Begin VB.PictureBox Frame1 
      ...
      Begin VB.PictureBox Picture4 
         ...
         Height          =   405
         Left            =   1740
         Picture         =   "MAT_CustomerOrder.frx":4B67
         ScaleHeight     =   405
         ScaleWidth      =   435
         TabIndex        =   60
         Top             =   -540
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   3060
         MouseIcon       =   "MAT_CustomerOrder.frx":78A3
         MousePointer    =   99  'Custom
         Picture         =   "MAT_CustomerOrder.frx":79F5
         Style           =   1  'Graphical
         TabIndex        =   86
         Top             =   1560
         Width           =   855
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin VB.CommandButton cmdTranDelete 
         ...
         Height          =   795
         Left            =   2850
         MouseIcon       =   "MAT_CustomerOrder.frx":7D5B
         MousePointer    =   99  'Custom
         Picture         =   "MAT_CustomerOrder.frx":7EAD
         Style           =   1  'Graphical
         TabIndex        =   85
      ...
      Begin VB.CommandButton cmdTranCancel 
         ...
         Height          =   795
         Left            =   2130
         MouseIcon       =   "MAT_CustomerOrder.frx":81D8
         MousePointer    =   99  'Custom
         Picture         =   "MAT_CustomerOrder.frx":832A
         Style           =   1  'Graphical
         TabIndex        =   83
      ...
      Begin VB.CommandButton cmdTranSave 
         ...
         Height          =   795
         Left            =   1410
         MouseIcon       =   "MAT_CustomerOrder.frx":8668
         MousePointer    =   99  'Custom
         Picture         =   "MAT_CustomerOrder.frx":87BA
         Style           =   1  'Graphical
         TabIndex        =   84
                    ...
                    Else
                       If txtTranType.Text = "CSH" Or txtTranType.Text = "CHG" Then
                            If (Null2String(RSORD_HD!ZERORATED) = "" Or Null2Bit(RSORD_HD!ZERORATED) = 0) And (Null2String(RSORD_HD!VAT_EXEMPT) = "" Or Null2Bit(RSORD_HD!VAT_EXEMPT) = 0) Then
                                ...
                                txtTranUCost.Text = ToDoubleNumber(N2Str2Zero(RSPARTMAS!MAC))
                            Else
                                txtTranUPrice.Text = ToDoubleNumber(N2Str2Zero(RSPARTMAS!SRP) / ConvertToBIRDecimalFormat(VAT_RATE))
                                txtTranUCost.Text = ToDoubleNumber(N2Str2Zero(RSPARTMAS!MAC))
                            End If
                       Else
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    If COMPANY_CODE = "DGI" Then
        If txtTranType.Text = "CSH" Or txtTranType.Text = "CHG" Then
            Set RSCUNTER = New ADODB.Recordset
            RSCUNTER.Open "select max(nextnumber) as nextnumber from PMIS_Counter where [TYPE] = 'M' AND modul in('CSH','CHG')", gconDMIS, adOpenForwardOnly, adLockReadOnly
            If Not RSCUNTER.EOF And Not RSCUNTER.BOF Then
                txtTranNo.Text = Format(Null2String(RSCUNTER!nextnumber), "000000")
            Else
                txtTranNo.Text = "000001"
            End If
            If FOR_CAS = True Then txtTranNo.Text = "M" + txtTranNo.Text
    ...
    Else
        If txtTranType.Text = "CSH" Then
            ...
            If FOR_CAS = True Or COMPANY_CODE = "DSSC" Then
                txtTranNo.Text = "M" + txtTranNo.Text
'            ElseIf COMPANY_CODE = "DJM" Then           ** FOR APPROVAL **
'                txtTranNo.Text = "MI" + txtTranNo.Text
            End If
            txtRONO.Enabled = False
            txtTerms.Enabled = False
            'JJE
    ...
    If Not RSORD_HD.EOF And Not RSORD_HD.BOF Then
        ...
        Else
            If txtTranType.Text = "CSH" Or txtTranType.Text = "CHG" Then
                ...
                labORNo = CheckORNum(Null2String(RSORD_HD!TRANNO), "MI", txtTranType.Text)
...
'    Set RSORD_HD = New ADODB.Recordset
'    Set RSORD_HD = gconDMIS.Execute("select Custname,tranno from PMIS_Ord_Hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' order by CUSTNAME asc")
...
'    Set RSORD_HD = gconDMIS.Execute("select custname, tranno,ID from PMIS_Ord_Hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and CUSTNAME  like '" & xxx & "%' order by CUSTNAME")
'    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
'        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
'    Else
'        lstOrd_Hd.Enabled = False
...
'    lstOrd_Hd.Enabled = False
'    Set RSORD_HD = New ADODB.Recordset
'    Set RSORD_HD = gconDMIS.Execute("select Tranno,tranno,ID   from PMIS_Ord_Hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' order by Tranno asc")
'    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
'        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
'    Else
'        lstOrd_Hd.Enabled = False
...
'    Set RSORD_HD = New ADODB.Recordset
'    xxx = Repleys(LTrim(RTrim(xxx)))
'    Set RSORD_HD = gconDMIS.Execute("select tranno, tranno ,ID from PMIS_Ord_Hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and tranno like '" & xxx & "%'")
'    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
'        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
'    Else
'        lstOrd_Hd.Enabled = False
...
'    lstOrd_Hd.Enabled = False
'    Set RSORD_HD = New ADODB.Recordset
'    Set RSORD_HD = gconDMIS.Execute("select rono,tranno ,ID from PMIS_Ord_Hd where[TYPE] = 'M' AND  trantype = '" & txtTranType.Text & "' and rono is not null order by tranno asc")
'    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
'        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
'    Else
'        lstOrd_Hd.Enabled = False
...
'    Set RSORD_HD = New ADODB.Recordset
'    xxx = Repleys(LTrim(RTrim(xxx)))
'    Set RSORD_HD = gconDMIS.Execute("select Rono, tranno,ID from PMIS_Ord_Hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and rono like '" & xxx & "%' order by tranno asc")
'    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
'        lstOrd_Hd.Enabled = True: Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD: lstOrd_Hd.Refresh
'    Else
'        lstOrd_Hd.Enabled = False
...
Private Sub chkzerorated_Click()
    If chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "P" Or chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "B" Then
        MessagePop InfoVoid, "Action void", "Transaction already posted!"
        Exit Sub
    ElseIf chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "C" Then
    ...
    If chkzerorated.Caption = "Set to Zero Rated" Then
        ...
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "ZERO", "1") = True Then MsgBox "Transaction Already Set To Zero Rated Transaction": Exit Sub
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "VAT", "1") = True Then MsgBox "Transaction Already Set To Vat Exempted Transaction": Exit Sub
        If MsgBox("Are you sure you want to set this to Zero Rated Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbYes Then
            gconDMIS.BeginTrans
            If SETTOZERORATED("M", txtTranType.Text, txtTranNo.Text, labID.Caption, LOCALACESS) = False Then
    ...
    Else
        ...
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "ZERO", "0") = True Then MsgBox "Transaction Already Set To Zero Rated Transaction": Exit Sub
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "VAT", "0") = False Then MsgBox "Transaction Already Set To Vat Exempted Transaction": Exit Sub
        If MsgBox("Are you sure you want to set this to Not-Zero Rated Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbYes Then
            gconDMIS.BeginTrans
            If SETTONOT_ZERORATED("M", txtTranType.Text, txtTranNo.Text, labID.Caption, LOCALACESS) = False Then
                gconDMIS.RollbackTrans
            Else
                gconDMIS.CommitTrans
    ...
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                NEW_LogAudit "C", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
            End If
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                SQL_STATEMENT = "UPDATE PMIS_TDAYTRAN SET" & _
                              ...
                              " WHERE ID = " & RSTDAYTRANDUP!ID
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "P", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
            End If
            RSTDAYTRANDUP.MoveNext
        Loop
    End If
...
Private Sub cmdPrint_Click()
    ...
    NEW_LogAudit "V", LOCALACESS, "", labID, "Materials", "", txtTranType.Text, ""
...
Sub DR_PARTS_PRINTING()
    If COMPANY_CODE = "HLU" Then
        ...
        rptCustomerOrder.Formulas(2) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
        If NumericVal(txtDS1.Text) = 0 Then
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "DR-MAT.RPT", "{ord_hd.TRANTYPE} = 'DR' and {ord_hd.TYPE} ='M' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        Else
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "DR-MAT.RPT", "{ord_hd.TRANTYPE} = 'DR' and {ord_hd.TYPE} = 'M' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
        End If
    End If
...
Private Sub cmdTranDelete_Click()
    ...
    If MsgQuestionBox("Delete This Material, Are you Sure?", "Delete Material Entry") = True Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "XX", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, labDetID
        ShowDeletedMsg
    End If
    Dim CNT                                            As Integer
    Dim RSTDAYTRANDUP                                  As ADODB.Recordset
    Set RSTDAYTRANDUP = New ADODB.Recordset
    RSTDAYTRANDUP.Open "select id,itemno from PMIS_TdayTran where [TYPE] = 'M' AND trantype = " & N2Str2Null(txtTranType.Text) & " and tranno = " & N2Str2Null(RSORD_HD!TRANNO) & " order by itemno asc", gconDMIS
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        RSTDAYTRANDUP.MoveFirst
        CNT = 0
        Do While Not RSTDAYTRANDUP.EOF
...
Private Sub cmdTranSave_Click()
    ...
    If txtTranType.Text <> "ADB" Then
        Set RSPARTMAS = New ADODB.Recordset
        RSPARTMAS.Open "Select isnull(srp,0) as srp,STOCKNO,onhand,sstock,resservice,TISSQTY,Mac,LASTM_MAC,issuances,dnp from CSMS_MATMAS where STOCKNO = '" & cboTranPartNo.Text & "' AND ACTIVE = 'Y'", gconDMIS
        If Not RSPARTMAS.EOF And Not RSPARTMAS.BOF Then
            CURONHAND = N2Str2IntZero(RSPARTMAS!ONHAND)
            CURSAFESTOCK = N2Str2IntZero(RSPARTMAS!SSTOCK)
            CURTISSQTY = N2Str2IntZero(RSPARTMAS!TISSQTY)
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "AA", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, labDetID
    Else
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "EE", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, labDetID
    End If
    cleargrid grdDetails
    FillDetails
    gconDMIS.Execute "update PMIS_Ord_Hd set" & _
...
Private Sub cmdSave_Click()
    ...
    If FOR_CAS = True Then
        ...
        End If
    Else
        'JJE Prefixes 02/07/2013 4:43PM
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If COMPANY_CODE = "DGI" Then
        ...
        Else
            Set RSFINDDUP = gconDMIS.Execute("SELECT ISNULL(SUM(TQ),0) AS TQ  FROM ( " & vbCrLf & _
                                           " SELECT COUNT(*) TQ FROM PMIS_ORD_HD WHERE [TYPE] = 'M' AND TRANTYPE in ('CHG','CSH') AND TRANNO = '" & txtTranNo & "' " & vbCrLf & _
                                           " UNION " & vbCrLf & _
                                           " SELECT COUNT(*) FROM PMIS_ORD_HIST WHERE [TYPE] = 'M' AND TRANTYPE in ('CHG','CSH') AND TRANNO = '" & txtTranNo & "')T ")
        End If
    Else
        Set RSFINDDUP = gconDMIS.Execute("SELECT ISNULL(SUM(TQ),0) AS TQ  FROM ( " & vbCrLf & _
                                       " SELECT COUNT(*) TQ FROM PMIS_ORD_HD WHERE [TYPE] = 'M' AND TRANTYPE = '" & txtTranType & "' AND TRANNO = '" & txtTranNo & "' " & vbCrLf & _
    ...
    If AddorEdit = "ADD" Then
        If RSFINDDUP.Fields(0).Value > 0 Then
            If MsgBox("Transaction No. already exist!, Do you want to generate new transaction number?", vbQuestion + vbYesNo, "Duplicate Transaction Number") = vbYes Then
                txtTranNo.Text = getnextISSPORR("M", txtTranType.Text)
                txtRONO.Enabled = True
                txtTerms.Enabled = False
...
'    End If
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "A", LOCALACESS, SQL_STATEMENT, FindTransactionID(txtTranNo, "TRANNO", "PMIS_ORD_HD", "DETAILS", N2Str2Null("M"), "TYPE"), "MATERIALS", txtTranNo & " - " & VTXTREFPRSNO, txtTranType.Text, ""
    ...
    Else
        ...
        SQL_STATEMENT = "UPDATE PMIS_ORD_HD SET" & _
                      ...
                      " WHERE ID = " & labID.Caption
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "E", LOCALACESS, SQL_STATEMENT, labID, "MATERIALS", txtTranNo, txtTranType.Text, ""
        SQL_STATEMENT = "UPDATE PMIS_ORD_HD SET" & _
    ...
    If AddorEdit = "ADD" Then
        If Left(txtTranNo.Text, 1) = "M" Then
            If COMPANY_CODE = "DGI" Then
                ...
                Else
                    gconDMIS.Execute "update PMIS_Counter set nextnumber = '" & NEXTCUNTER & "', lastupdate = '" & LOGDATE & "', usercode = '" & "USER" & "' where [TYPE] = 'M' AND modul in ('CSH','CHG')"
                End If
            Else
                gconDMIS.Execute "update PMIS_Counter set nextnumber = '" & NEXTCUNTER & "', lastupdate = '" & LOGDATE & "', usercode = '" & "USER" & "' where [TYPE] = 'M' AND modul = " & VTXTTRANTYPE
            End If
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If AddorEdit = "ADD" Then
        ...
        Set RSTDAYTRANDUP = New ADODB.Recordset
        RSTDAYTRANDUP.Open "select trantype,tranno from PMIS_TdayTran where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and tranno = " & N2Str2Null(RSORD_HD!TRANNO), gconDMIS
            ...
            If Not RSPRS_HD.EOF And Not RSPRS_HD.BOF Then
                ...
                If Not rstdaytranDUp2.EOF And Not rstdaytranDUp2.BOF Then
                    ...
                    Do While Not rstdaytranDUp2.EOF
                        ...
                        Set rsPartMasClone = gconDMIS.Execute("Select STOCKNO,ONHAND,mac,DNP,sstock from PMIS_StockMas where TYPE = 'M' and STOCKNO = " & N2Str2Null(rstdaytranDUp2!STOCK_ORD))
                        If Not rsPartMasClone.EOF And Not rsPartMasClone.BOF Then
                            If N2Str2Zero(rsPartMasClone!ONHAND) > 0 Then
                                ...
                                SQL_STATEMENT = "insert into PMIS_TdayTran " & _
                                              ...
                                              " values ('M'," & VMACSTOCKNO & "," & RRTRANDATE & ", '" & txtTranType.Text & "', " & RRTRANNO & "," & _
                                              " " & RRITEMNO & "," & RRSTOCK_ORD & "," & _
                                              " " & RRSTOCK_SUP & ", " & RRTRANQTY & "," & _
                                              " " & RRTRANUCOST & ", " & RRTRANINVAMT & ", '" & LOGDATE & "', " & N2Str2Null(LOGCODE) & ", " & RRSTATUS & ", " & RRIN_OUT & ")"
                                gconDMIS.Execute SQL_STATEMENT
                                NEW_LogAudit "A", LOCALACESS, SQL_STATEMENT, labID, "Materials", txtTranNo, txtTranType.Text, ""
                            Else
...
rs_ord_hd.Open "select ID,trantype,tranno from PMIS_ord_hd where [TYPE] = 'M' AND trantype = '" & txtTranType.Text & "' and tranno = " & N2Str2Null(RSORD_HD!TRANNO), gconDMIS
If Not (rs_ord_hd.EOF And rs_ord_hd.BOF) Then
    Set rs_tdaytran = New ADODB.Recordset
    rs_tdaytran.Open "select id,itemno,STOCK_ORD,STOCK_SUP,tranqty,traninvamt,tranucost,tranuprice from PMIS_TdayTran where trantype = '" & txtTranType.Text & "' and tranno = " & N2Str2Null(rs_ord_hd!TRANNO) & " and [TYPE] = 'M' order by itemno asc", gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not (rs_tdaytran.EOF And rs_tdaytran.BOF) Then
        ...
        Do While Not rs_tdaytran.EOF
            ...
            Set rs_stockmas = gconDMIS.Execute("Select STOCKNO,ONHAND,NON_HARI,MAC,DNP,srp from PMIS_StockMas where TYPE = 'M' and STOCKNO = " & N2Str2Null(rs_tdaytran!STOCK_ORD))
...
Private Sub CmdVatexmpt_Click()
    ...
    If CmdVatexmpt.Caption = "Vat Exempt" Then
        ...
        If MsgBox("Are you sure you want to set this to Vat Exempt Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbYes Then
            gconDMIS.BeginTrans
            If SETTOVATEXEMPT("M", txtTranType.Text, txtTranNo.Text, labID.Caption, LOCALACESS) = False Then
                gconDMIS.RollbackTrans
            Else
                gconDMIS.CommitTrans
    ...
    Else
        ...
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "VAT", "0") = True Then MsgBox "Transaction Already Set To Not Vat Exempted Transaction": Exit Sub
        If ISZERONOTVAT("M", txtTranType.Text, txtTranNo.Text, "ZERO", "0") = False Then MsgBox "Transaction Already Set To Non-Zero Rated Transaction": Exit Sub
        If MsgBox("Are you sure you want to set this to Not Vat Exempted Transaction?", vbQuestion + vbYesNo + vbDefaultButton1) = vbYes Then
            gconDMIS.BeginTrans
            If SETTONOT_VATEXEMPT("M", txtTranType.Text, txtTranNo.Text, labID.Caption, LOCALACESS) = False Then
                gconDMIS.RollbackTrans
            Else
                gconDMIS.CommitTrans
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        ...
        Case vbKeyF3
            If Picture1.Visible = True Then
                If chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "C" Then
                    MsgSpeechBox "Transactions are Already Cancelled and cannot be Change..."
                ElseIf chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "B" Then
                    MsgSpeechBox "Transactions are Already Billed-Out and cannot be Change..."
                ElseIf chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "P" Then
        ...
        Case vbKeyF4
            If FILD <> "" And FILD <> "No Entry" Then
                If Picture1.Visible = True Then
                    If chkstatus(txtTranNo.Text, "M", txtTranType.Text) <> "P" And chkstatus(txtTranNo.Text, "M", txtTranType.Text) <> "C" And chkstatus(txtTranNo.Text, "M", txtTranType.Text) <> "B" Then
                        grdDetails_DblClick
                        Picture1.Enabled = False
                        fraDetails.Enabled = False
                    End If
        ...
        Case vbKeyF12
            If Picture1.Visible = True And (labSJ = "" And labORNo = "") Then
                If chkstatus(txtTranNo.Text, "M", txtTranType.Text) = "B" Then
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                If txtTranType.Text <> "ADB" Then
                    ...
                    SQL_STATEMENT = "UPDATE PMIS_TDAYTRAN SET" & _
                                  ...
                                  " WHERE ID = " & RSTDAYTRANDUP!ID
                    gconDMIS.Execute SQL_STATEMENT
                    NEW_LogAudit "U", LOCALACESS, SQL_STATEMENT, labID, "MATERIALS", txtTranNo, txtTranType.Text, ""
                End If
            End If
            RSTDAYTRANDUP.MoveNext
        Loop
...
Private Sub Form_Load()
    ...
    If txtTranType.Text = "RIV" Then
        ...
        fraDetails.Height = 6725
        lstOrd_Hd.Height = 5090
...
Private Sub menumaster_Click()
   If Module_Access(LOGID, "PARTS MASTER FILE", "DATA ENTRY") = False Then Exit Sub
    ...
    frmMasterFile_Material.textSearch.Text = FILD
    Call frmMasterFile_Material.SearchStock(FILD, "M")
End Sub

Private Sub optCASH_Click()
    txtTranType.Text = "CSH"
    txtTranType.Text = txtTranType.Text
End Sub
