...
Begin VB.Form frmPMISAC_Location 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "ACCESSORIES BY LOCATION"
   ClientHeight    =   1350
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4440
   ForeColor       =   &H00DEDFDE&
   Icon            =   "AC_Location.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin wizButton.cmd cmdByLocation 
      ...
      _ExtentX        =   7594
      _ExtentY        =   1085
      TX              =   "View Accessories By Location"
      ENAB            =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmPMISAC_Location"
...
Private Sub cmdByLocation_Click()
    If Function_Access(LOGID, "Acess_PRINT", "ACCESSORIES LOCATION") = False Then Exit Sub
    ...
    If cboLocation.Text = "No Location" Then
        ...
        PrintSQLReport rptLocation, PMIS_REPORT_PATH & "bylocation.rpt", "isnull({partmas.location}) = true", DMIS_REPORT_Connection, 1
    ElseIf cboLocation.Text = "All Accessories By Location" Then
        rptLocation.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
        rptLocation.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
    ...
    Call NEW_LogAudit("V", "ACCESSORIES LOCATION", "", "", "", cboLocation, "", "")
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyF1 And Shift = 1:
            ...
            frmALL_AuditInquiry.Caption = "Audit Inquiry (ACCESSORIES LOCATION)"
            Call frmALL_AuditInquiry.DisplayHistory("", "ACCESSORIES LOCATION", "PRINTING")
    End Select
End Sub
...
Private Sub Form_Load()
    ...
    rsLocation.Open "select location from PMIS_vw_Location where [TYPE] = 'A' order by location asc", gconDMIS
    If Not rsLocation.EOF And Not rsLocation.BOF Then
        rsLocation.MoveFirst
        cboLocation.Clear
        cboLocation.AddItem "All Accessories By Location"
