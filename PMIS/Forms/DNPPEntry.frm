...
On Error GoTo ErrorCode
    Dim rsDNPP, RS                                     As ADODB.Recordset
    ...
    Set RS = gconHARI_DNP.OpenSchema(adSchemaTables, Array(Empty, Empty, Empty, "Table"))
    Do While (Not RS.EOF)
        lngCount = lngCount + 1
        RS.MoveNext
    Loop
    ...
    Set RS = Nothing
'updated by IEBV
    For ictr = 1 To lngCount
        Set rsDNPP = New ADODB.Recordset
        rsDNPP.Open "Select * from [ANNEX " & ictr & "] Order by Field2 asc", gconHARI_DNP, adOpenForwardOnly
        ...
        If Not rsDNPP.EOF And Not rsDNPP.BOF Then
            ...
            Do While Not rsDNPP.EOF
                'If N2Str2Zero(rsDNPP![FIELD7]) > 0 Then
                    'PARTNO = newpart(Repleys(Null2String(rsDNPP!FIELD2))) old code
                    PARTNO = Null2String(rsDNPP!FIELD2)
                    ...
                    Set RSCURR_DNPP = gconDMIS.Execute("Select DNPP3,DNPP2,DNPP from PMIS_DNPP where PartNumber = '" & PARTNO & "'")
                    ...
                    Else
                        gconDMIS.Execute ("Insert into PMIS_DNPP (DNPP,DNPP2,DNPP3,SRP,MODEL,PARTNUMBER,DESCRIPTIO) values (" & _
                                          DNP20 & "," & DNP32 & "," & DNP40 & "," & SRP & "," & MODELAPP & ",'" & PARTNO & "','" & PARTDESC & "')")
...
Function newpart(XXX As String) As String
If Len(XXX) <= 6 Then
    newpart = XXX
Else
    If Mid(XXX, 6, 1) <> " " Then
        newpart = Mid(XXX, 1, 5) + " " + Mid(XXX, 6, Len(XXX) - 5)
    Else
        newpart = XXX
    End If
End If
...
Private Sub cmdSave_Click()
    ...
    If AddorEdit = "ADD" Then
        ...
        SQL_STATEMENT = "Insert into PMIS_DNPP" & _
                      " (PARTNUMBER,DESCRIPTIO,ICC,model,newPARTNO,srp,dnpp,dnpp2,dnpp3,lastupdate,usercode)" & _
...
Private Sub cmdRegister_parts_Click()
    ...
    If MsgBox("Select Stock Type" & vbCrLf & "-------------------------------" & vbCrLf & "General Job (GJ) = Yes " & vbCrLf & "Body and Paint (BP) = No" & vbCrLf & "-------------------------------", vbQuestion + vbYesNo) = vbYes Then
...
'    Else
        gconDMIS.Execute ("INSERT INTO PMIS_STOCKMAS (TYPE,STOCKNO, STOCKDESC, DNP,SRP,MODELCODE,ACTIVE,GENUINE,NON_HARI,DATE_ENTERED,STOCKTYPE) SELECT 'P', PARTNUMBER,DESCRIPTIO,  DNPP2  ,SRP, MODEL,'Y','Y','N',GETDATE(),'GJ'   FROM PMIS_DNPP WHERE PARTNUMBER=" & N2Str2Null(txtPartNo))
'    End If
    Else
        gconDMIS.Execute ("INSERT INTO PMIS_STOCKMAS (TYPE,STOCKNO, STOCKDESC, DNP,SRP,MODELCODE,ACTIVE,GENUINE,NON_HARI,DATE_ENTERED,STOCKTYPE) SELECT 'P', PARTNUMBER,DESCRIPTIO,  DNPP2  ,SRP, MODEL,'Y','Y','N',GETDATE(),'BP'   FROM PMIS_DNPP WHERE PARTNUMBER=" & N2Str2Null(txtPartNo))
    End If
...
Private Sub txtPartNo_KeyPress(KeyAscii As Integer)
    If KeyAscii = 45 Then KeyAscii = 32
