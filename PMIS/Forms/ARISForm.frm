...
Begin VB.Form frmPMISAC_ARISForms 
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin VB.CommandButton cmdTranDelete 
         ...
         Height          =   795
         Left            =   2880
         MouseIcon       =   "ARISForm.frx":78A3
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":79F5
         Style           =   1  'Graphical
         TabIndex        =   86
      ...
      Begin VB.CommandButton cmdTranCancel 
         ...
         Height          =   795
         Left            =   2160
         MouseIcon       =   "ARISForm.frx":7D20
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":7E72
         Style           =   1  'Graphical
         TabIndex        =   84
      ...
      Begin VB.CommandButton cmdTranSave 
         ...
         Height          =   795
         Left            =   1440
         MouseIcon       =   "ARISForm.frx":81B0
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":8302
         Style           =   1  'Graphical
         TabIndex        =   85
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   3405
         MouseIcon       =   "ARISForm.frx":8652
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":87A4
         Style           =   1  'Graphical
         TabIndex        =   87
         Top             =   1575
         Width           =   855
...
Sub FillGrid()
    ...
    Set RSORD_HD = New ADODB.Recordset
    Set RSORD_HD = gconDMIS.Execute("select top 20 Tranno,tranno x from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' order by Tranno asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set RSORD_HD = gconDMIS.Execute("select top 20 Tranno,tranno x from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' and tranno like '" & XXX & "%'")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Sorted = False: lstOrd_Hd.ListItems.Clear
    Set RSORD_HD = New ADODB.Recordset
    Set RSORD_HD = gconDMIS.Execute("select top 20 rono,tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' and rono is not null order by tranno asc")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set RSORD_HD = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set RSORD_HD = gconDMIS.Execute("select top 20 Rono, tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' and rono like '" & XXX & "%' order by tranno asc")
    If Not (RSORD_HD.EOF And RSORD_HD.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, RSORD_HD
        lstOrd_Hd.Refresh
...
Private Sub cmdCancelCO_Click()
    ...
    If chkstatus(txtTranNo.Text, "A", "ARS") = "P" Then
        MessagePop InfoVoid, "Action Void", "Transaction already Posted!"
        Exit Sub
    ElseIf chkstatus(txtTranNo.Text, "A", "ARS") = "C" Then
    ...
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "C", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, ""
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                SQL_STATEMENT = "update PMIS_vw_PRS_Tran set" & _
                              ...
                              " where id = " & RSTDAYTRANDUP!ID
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "P", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, ""
            End If
            RSTDAYTRANDUP.MoveNext
        Loop
    End If
...
Private Sub cmdTranDelete_Click()
    ...
    If MsgQuestionBox("Delete This Parts, Are you Sure?", "Delete Parts Entry") = True Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "XX", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, labDetID
        ShowDeletedMsg
    End If
    Dim CNT                                            As Integer
    Dim RSTDAYTRANDUP                                  As ADODB.Recordset
    Set RSTDAYTRANDUP = New ADODB.Recordset
    RSTDAYTRANDUP.Open "select id,itemno from PMIS_vw_PRS_Tran where [TYPE] = 'A' AND trantype = " & N2Str2Null(txtTranType.Text) & " and tranno = " & N2Str2Null(RSORD_HD!TRANNO) & " order by itemno asc", gconDMIS
    If Not RSTDAYTRANDUP.EOF And Not RSTDAYTRANDUP.BOF Then
        RSTDAYTRANDUP.MoveFirst
        CNT = 0
        Do While Not RSTDAYTRANDUP.EOF
...
Private Sub cmdTranSave_Click()
    ...
    If AddorEdit = "ADD" Then
        ...
        If Len(StockISSUE) > 0 Then
            If MsgBox("Same Accessory number has been requested for this repair order." & vbCrLf & "Repair Order#  :" & txtRONO & vbCrLf & "Transaction#    :" & StockISSUE & vbCrLf & "Do you want to continue?", vbYesNo + vbInformation, "Accessories Requisition") = vbNo Then
                ...
                Exit Sub
            End If
            'NEW_LogAudit "AA", "ACCESSORIES REQUISITION SLIP", "", labID, "Accessories", txtTranNo, txtTranType.Text, ""
        End If
    End If
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "AA", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, ""
    ...
    Else
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "EE", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, labDetID
...
Private Sub cmdSave_Click()
    ...
    Else
        If AddorEdit = "ADD" Then
            If checkdupRS("ARS", "A", Trim(txtReferencePIS.Text)) = True Then
                MsgBox "Reference ARS Number Already Exist"
                Exit Sub
            End If
        Else
    ...
    Else
        If AddorEdit = "ADD" Then
            If checkdup("A", txtTranType.Text, txtTranNo.Text) = True Then
                If MsgBox("Transaction No. already exist!,Do you want to generate new ARS number?", vbQuestion + vbYesNo) = vbYes Then
                    txtTranNo.Text = getnextISSPORR("A", "ARS")
                    txtRONO.Enabled = True
                    txtTerms.Enabled = False
                Else
                    On Error Resume Next
...
On Error GoTo errordaa
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "A", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, FindTransactionID(txtTranNo, "tranno", "PMIS_vw_PRS", "DETAILS", N2Str2Null("A"), "TYPE"), "Accessories", txtTranNo, txtTranType.Text, ""
    Else
        ...
        SQL_STATEMENT = "update PMIS_vw_PRS set" & _
                      ...
                      " where id = " & labID.Caption
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "E", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, ""
    ...
    Select Case KeyCode
        ...
        Case vbKeyF4
            If FILD <> "" And FILD <> "No Entry" Then
                If Picture1.Visible = True Then
                    If chkstatus(txtTranNo.Text, "A", "ARS") <> "P" And chkstatus(txtTranNo.Text, "A", "ARS") <> "C" And chkstatus(txtTranNo.Text, "A", "ARS") <> "B" Then
                        grdDetails_DblClick
                        Picture1.Enabled = False
                        fraDetails.Enabled = False
                    End If
        ...
        Case vbKeyF12
            If Picture1.Visible = True Then
                ...
                If ISISSUED("A", Trim(txtReferencePIS.Text)) = True Then Exit Sub
                If chkstatus(txtTranNo.Text, "A", "ARS") = "P" Then
        ...
        Do While Not RSTDAYTRANDUP.EOF
            ...
            If Not RSPARTMASDUP.EOF And Not RSPARTMASDUP.BOF Then
                ...
                SQL_STATEMENT = "update PMIS_vw_PRS_Tran set" & _
                              ...
                              " where id = " & RSTDAYTRANDUP!ID
                gconDMIS.Execute SQL_STATEMENT
                NEW_LogAudit "U", "ACCESSORIES REQUISITION SLIP", SQL_STATEMENT, labID, "Accessories", txtTranNo, txtTranType.Text, ""
            End If
            RSTDAYTRANDUP.MoveNext
        Loop
    End If
