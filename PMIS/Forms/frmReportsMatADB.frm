...
Begin VB.Form frmReportsMatADB 
   Caption         =   "MATERIAL ADB"
   ClientHeight    =   2700
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   5445
   ...
   Begin wizProgBar.Prg prgExcelGen 
      ...
      _ExtentX        =   7964
      _ExtentY        =   582
      Picture         =   "frmReportsMatADB.frx":0000
      ForeColor       =   0
      Appearance      =   2
      BorderStyle     =   2
      BarForeColor    =   8454016
      BarPicture      =   "frmReportsMatADB.frx":001C
      ShowText        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmReportsMatADB"
...
Private Function GETHEADER(XDATETO As Date, XDATEFROM As Date) As String
    ...
    sqltxt = sqltxt & "FROM PMIS_ORD_HD WHERE TRANTYPE='ADB' AND TYPE='M' AND ISNULL(STATUS3,'')  <>'F' AND "
    ...
    sqltxt = sqltxt & "FROM PMIS_ORD_HIST WHERE TRANTYPE='ADB' AND TYPE='M' AND "
 ...
 Private Function GETDETAILS(REP_OR As String) As String
    ...
    sqltxt = sqltxt & " WHERE  PMIS_STOCKMAS.TYPE='M' AND "
    sqltxt = sqltxt & "(TRANNO  IN (SELECT TRANNO FROM PMIS_ORD_HD WHERE TRANTYPE='ADB' AND TYPE='M' AND RONO='" & REP_OR & "'   AND (STATUS='P' OR STATUS='B')) "
    sqltxt = sqltxt & "OR TRANNO IN (SELECT TRANNO FROM PMIS_ORD_HIST WHERE TRANTYPE='ADB' AND TYPE='M' AND RONO='" & REP_OR & "'   AND (STATUS='P' OR STATUS='B'))) "
