...
Begin VB.Form frmMACTool_2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "MAC Tool Beta"
   ClientHeight    =   8640
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   14475
   ...
   Begin wizProgBar.Prg Prg 
      ...
      _ExtentX        =   25426
      _ExtentY        =   609
      Picture         =   "frmMACTool_2.frx":0000
      ForeColor       =   0
      BarPicture      =   "frmMACTool_2.frx":001C
      ShowText        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmMACTool_2"
...
'        Call ShowIsRequiredMsg("part number cannot be blank")
'        txtsearch.SetFocus
'        Exit Sub
'    End If
    Screen.MousePointer = 11
    ...
    rptRO.Populate
    gconDMIS.Execute ("Update PMIS_STOCKMAS set VALID_ICC = 'U'")
'    Call cleargrid(grdPartsLedger)
    Call InitGrid
    
'    Set DistinctTrans = New ADODB.Recordset
'    Set DistinctTrans = gconDMIS.Execute("Select DISTINCT STOCK_ORD from PMIS_AllDaytran where STOCK_ORD = " & N2Str2Null(txtsearch) & "")
...
'            Set rsTrans = gconDMIS.Execute("select id,ItemNo,trandate,STOCK_ORD,trantype,tranno,itemno,tranqty,tranucost,mac,status,in_out,TRANUPRICE,usercode from PMIS_AllDaytran where " & _
'                " (IN_OUT = 'I' OR IN_OUT = 'O')  and " & _
'                " (STATUS = 'P' OR STATUS = 'B') " & _
'                " AND STOCK_ORD = '" & DistinctTrans!STOCK_ORD & _
'                "' order by trandate asc, id asc,tranno asc")
    ...
    If Not rsStkMas.EOF And Not rsStkMas.BOF Then
        ...
        Do While Not rsStkMas.EOF
            ...
            If Not rsTrans.EOF And Not rsTrans.BOF Then
                ...
                Do While Not rsTrans.EOF
                    DoEvents
                    Me.Caption = "Part no : " & Null2String(rsStkMas!STOCKNO) & "   Date tran : " & Null2String(rsTrans!trandate) & "   Tran no: " & Null2String(rsTrans!tranno)
                    Value = 0
                    If Null2String(rsTrans!IN_OUT) = "I" Then
                        If BALANSE <= 0 Then
                            COMPUTED_MAC = N2Str2Zero(rsTrans!TRANUCOST)
...
Private Sub Command1_Click()
    ...
    If MsgBox("Update Transactions History and Parts Master File with Correct MAC?", vbQuestion + vbYesNo, "Please Confirm Action...") = vbYes Then
        ...
        For KIM = 0 To rptRO.Rows.Count - 1
            ...
            DoEvents
            Me.Caption = "Part no : " & XSTOCK_ORD & "   Date Tran: " & rptRO.Rows(KIM).Record(1).Value & "    Tran no. : " & rptRO.Rows(KIM).Record(2).Value
...
'    End With
    ...
    With rptRO
      .Columns.DeleteAll
        .Columns.Add 0, "PART NO", 100, False::               .Columns(0).Alignment = xtpAlignmentCenter:    .Columns(0).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 1, "TRANDATE", 110, False:              .Columns(1).Alignment = xtpAlignmentCenter:     .Columns(1).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 2, "TRANNO", 100, False:                .Columns(2).Alignment = xtpAlignmentCenter:     .Columns(2).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 3, "RECEIVED", 60, False:               .Columns(3).Alignment = xtpAlignmentCenter:     .Columns(3).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 4, "ISSUED", 75, False:                 .Columns(4).Alignment = xtpAlignmentCenter:       .Columns(4).AllowRemove = False:  .Columns(0).Sortable = False
        .Columns.Add 5, "BALANCE", 60, False:                .Columns(5).Alignment = xtpAlignmentCenter:     .Columns(5).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 6, "UNIT COST", 70, False:              .Columns(6).Alignment = xtpAlignmentRight:     .Columns(6).AllowRemove = False:     .Columns(0).Sortable = False
        .Columns.Add 7, "MAC", 60, False:                    .Columns(7).Alignment = xtpAlignmentRight:     .Columns(7).AllowRemove = False:     .Columns(0).Sortable = False
        .Columns.Add 8, "COMP. MAC", 80, False:              .Columns(8).Alignment = xtpAlignmentRight:       .Columns(8).AllowRemove = False:   .Columns(0).Sortable = False
        .Columns.Add 9, "EXT. MAC", 80, False:              .Columns(9).Alignment = xtpAlignmentRight:       .Columns(9).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 10, "BALANCE * COMP. MAC", 140, False:  .Columns(10).Alignment = xtpAlignmentRight:      .Columns(10).AllowRemove = False:  .Columns(0).Sortable = False
        .Columns.Add 11, "DIFF. MAC", 70, False:             .Columns(11).Alignment = xtpAlignmentRight:      .Columns(11).AllowRemove = False:  .Columns(0).Sortable = False
        .Columns.Add 12, "DIFF. EXT. MAC", 100, False:       .Columns(12).Alignment = xtpAlignmentRight:      .Columns(12).AllowRemove = False:  .Columns(0).Sortable = False
        .Columns.Add 13, "DIFF. MAC %", 90, False:         .Columns(13).Alignment = xtpAlignmentRight:      .Columns(13).AllowRemove = False:   .Columns(0).Sortable = False
        .Columns.Add 14, "DIFF. EXT %", 90, False:        .Columns(14).Alignment = xtpAlignmentRight:      .Columns(14).AllowRemove = False:    .Columns(0).Sortable = False
        .Columns.Add 15, "ID %", 100, False:                  .Columns(15).Alignment = xtpAlignmentRight:   .Columns(15).AllowRemove = False:    .Columns(0).Sortable = False
