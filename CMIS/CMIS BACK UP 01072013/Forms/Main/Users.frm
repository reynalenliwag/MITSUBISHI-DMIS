...
Private Sub DisplayUsers()
    Set rs = gconDMIS.Execute("Select username, usergroup, userid from CMIS_USERS order by username")
...
Private Sub cboGroups_Click()
    ...
    lblDisplayUser.Caption = gconDMIS.Execute("Select groupname from CMIS_USERGROUPS Where Code='" & ReplaceQuote(cboGroups.Text) & "'").Collect(0)
    Exit Sub
errorlbl:
    Err.Clear
End Sub
...
Private Sub cmdDelete_Click()
    ...
    If MsgBox("Are you sure you want to remove user " & Me.lvwUsers.SelectedItem & "?", vbExclamation + vbYesNo, "Remove User") = vbYes Then
        ...
        Else
            gconDMIS.Execute ("Delete from CMIS_USERS where username = '" & Me.lvwUsers.SelectedItem & "' ")
            gconACCESS.Execute ("Delete from PAccess where username = '" & wizVar.DecryptAccess(Me.lvwUsers.SelectedItem) & "' ")
            Call Combo_Loadval(cboGroups, gconDMIS.Execute("Select code,groupname from CMIS_USERGROUPS"))
            'Call populateCbo("Select code, groupname from CMIS_USERGROUPS", gconDMIS, Me.cboGroups)
            Call DisplayUsers
            ResetEntry
        End If
    End If
...
Private Sub Form_Load()
    ...
    'Call populateCbo("Select code, groupname from CMIS_USERGROUPS", gconDMIS, cboGroups)
...
Private Sub SaveUser()
    ...
    If isNew Then
        If Username_Exists(Trim(Me.txtUser)) Then
            ...
            Exit Sub
        End If
        gconDMIS.Execute "Insert into CMIS_USERS ([username],[password],usergroup)" _
    ...
    Else
        gconDMIS.Execute ("update CMIS_USERS set [username] = " & N2Str2Null(Trim(Me.txtUser)) & ", [password] = " & _
...
Private Sub Display_Edit(UserID As Long)
    Set rs = gconDMIS.Execute("Select * from CMIS_USERS where userid = " & UserID & "")
    If Not (rs.BOF And rs.EOF) Then
...
Private Function Username_Exists(Username As String) As Boolean
    Set rs = gconDMIS.Execute("Select username from CMIS_USERS where username = '" & Username & "' ")
    If Not (rs.BOF And rs.EOF) Then
