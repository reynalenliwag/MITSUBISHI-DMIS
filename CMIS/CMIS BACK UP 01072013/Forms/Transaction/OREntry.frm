...
Begin VB.Form frmCMISOREntry 
   ...
   Begin VB.PictureBox picCreditCard 
      ...
      Begin VB.CommandButton cmdCardCancel 
         ...
         Height          =   795
         Left            =   8070
         MouseIcon       =   "OREntry.frx":68EE
         MousePointer    =   99  'Custom
         Picture         =   "OREntry.frx":6A40
         Style           =   1  'Graphical
         TabIndex        =   143
         Top             =   360
         Width           =   705
      ...
      Begin VB.CommandButton cmdCardSave 
         ...
         Height          =   795
         Left            =   7380
         MouseIcon       =   "OREntry.frx":6D7E
         MousePointer    =   99  'Custom
         Picture         =   "OREntry.frx":6ED0
         Style           =   1  'Graphical
         TabIndex        =   142
         Top             =   360
         Width           =   705
...
Sub StoreMemVars()
    ...
    If Not rsOFF_HD.EOF And Not rsOFF_HD.BOF Then
        ...
        If COMPANY_CODE = "DGI" Then
            If Mid(rsOFF_HD!OR_NUM, 1, 1) = "S" Then
                txtOR_NUM.Text = Format(Left(Null2String(rsOFF_HD!OR_NUM), 7), "0000000")
            Else
                txtOR_NUM.Text = Format(Left(Null2String(rsOFF_HD!OR_NUM), 6), "000000")
            End If
        'JJE 12/05/2012 2:30PM
        ElseIf COMPANY_CODE = "DSSC" Then
            If Left(rsOFF_HD!OR_NUM, 1) = "S" Then
                txtOR_NUM.Text = Format(Left(Null2String(rsOFF_HD!OR_NUM), 7), "0000000")
            Else
                txtOR_NUM.Text = Format(Left(Null2String(rsOFF_HD!OR_NUM), 6), "000000")
            End If
        ...
        'JJE
        If Left(Null2String(rsOFF_HD!OR_NUM), 1) = "S" Then
...
Sub StoreDetails()
    ...
    If Not rsOFF_DT.EOF And Not rsOFF_DT.BOF Then
        ...
        Do While Not rsOFF_DT.EOF
            ...
            If COMPANY_CODE <> "DGI" Then
'                Set rsOFF_Payment = gconDMIS.Execute("SELECT PAYMENT,(SELECT Sum(AMOUNT) FROM CMIS_DEPOSITS WHERE ID_DET = " & N2Str2Null(rsOFF_DT!OR_NUM) & ") AS DEPOSIT FROM CMIS_OFF_DT WHERE OR_NUM IN (SELECT ID_DET FROM CMIS_DEPOSITS WHERE ID_DET = " & N2Str2Null(rsOFF_DT!OR_NUM) & " AND APPLIED = 'Y')")    'JJE Deposit update
                Set rsOFF_Payment = gconDMIS.Execute("SELECT PAYMENT,(SELECT Sum(AMOUNT) FROM CMIS_DEPOSITDT WHERE OR_NUM = " & N2Str2Null(rsOFF_DT!OR_NUM) & ") AS DEPOSIT FROM CMIS_OFF_DT WHERE OR_NUM IN (SELECT OR_NUM FROM CMIS_DEPOSITDT WHERE OR_NUM = " & N2Str2Null(rsOFF_DT!OR_NUM) & " )") 'JJE Deposit Fix bugs
            Else
...
'                               Chr(9) & Null2String(rsOFF_DT!PAIDFOR) & Chr(9) & Null2String(rsOFF_DT!BRANCH) & Chr(9) & Null2String(rsOFF_DT!DISCOUNT) & Chr(9) & Null2String(rsOFF_DT!tax) & Chr(9) & Null2String(rsOFF_DT!Payment) & Chr(9) & rsOFF_DT!Id
            grdDetails.AddItem TRAN_INVOICE_TYPE & Chr(9) & Null2String(rsOFF_DT!INVOICENO) & Chr(9) & Null2String(rsOFF_DT!DESCRIPT) & Chr(9) & ToDoubleNumber(N2Str2Zero(rsOFF_DT!amount)) & Chr(9) & ToDoubleNumber(IIf(N2Str2Zero(rsOFF_DT!balance) = 0, 0, Round(N2Str2Zero(rsOFF_DT!balance) - ((N2Str2Zero(rsOFF_DT!Payment) + (N2Str2Zero(rsOFF_DT!tax)) + N2Str2Zero(vDeposit))), 2))) & _
                               Chr(9) & Null2String(rsOFF_DT!PAIDFOR) & Chr(9) & Null2String(rsOFF_DT!BRANCH) & Chr(9) & Null2String(rsOFF_DT!DISCOUNT) & Chr(9) & Null2String(rsOFF_DT!tax) & Chr(9) & Null2String(rsOFF_DT!Payment) & Chr(9) & rsOFF_DT!Id
...
Sub InitGridMemvars()
    ...
    txtDiscount.Enabled = False: txtTax.Enabled = True
...
Private Sub cboTranType_LostFocus()
    ...
    If SetTranTypeCode(cboTranType.Text) = "OTH" Then
        ...
        If COMPANY_CODE = "DGI1" Then
            Set rsJoy = gconDMIS.Execute("Select * from CMIS_Off_Dt Where VAT = " & VAT_OR & " AND trantype = 'OTH' and Left(REFERENCE,1)<>'M' order by reference desc")
        Else
...
Private Sub Payment_Change()
    'JJE
    If NumericVal(Payment.Text) <= 0 Or Payment.Text = "" Then
        wizDigit1.TextValue = 0
        txtPayment.Text = 0
    Else
        txtPayment = Payment - (NumericVal(txtDiscount) + NumericVal(txtTax))
        txtPayment = ToDoubleNumber(txtPayment.Text)
...
Private Sub Payment_KeyPress(KeyAscii As Integer)
    'JJE
...
Private Sub Payment_LostFocus()
    ...
    Else
        txtPayment = Payment - (NumericVal(txtDiscount) + NumericVal(txtTax))
        txtPayment = ToDoubleNumber(txtPayment.Text)
        wizDigit1.TextValue = txtPayment.Text
    End If
    'JJE
End Sub
...
Private Sub cmdCancelOR_Click()
    ...
    Else
        If MsgBox("Cancel this O.R. Entries, Are you Sure?", vbQuestion + vbYesNo, "Confirm Cancelation") = vbYes Then
            ...
            Set rsStatus = gconDMIS.Execute("select status from CMIS_Off_Hd Where OR_NUM = '" & txtOR_NUM.Text & "'")
            If Not rsStatus.EOF And Not rsStatus.BOF Then
                OrStatus = rsStatus!Status
            End If
...
'updating code:    JAA - 07112007
    ...
    'JJE 12/14/2012 4:12PM
    'Cancelled Applied Deposit will be Unapplied
    Dim rsUndoDeposit As ADODB.Recordset
    Set rsUndoDeposit = New ADODB.Recordset
    rsUndoDeposit.Open "SELECT * FROM CMIS_DEPOSITDT WHERE INVOICENO='" & txtReference.Text & "' AND INVOICETYPE='" & SetTranTypeCode(cboTranType.Text) & "'", gconDMIS, adOpenForwardOnly
    If Not rsUndoDeposit.EOF And Not rsUndoDeposit.BOF Then
...
Private Sub cmdTranDelete_Click()
    ...
    If MsgQuestionBox("Delete This Entry, Are you Sure?", "Delete OR Entry") = True Then
        ...
        Else
            ...
            gconDMIS.Execute ("DELETE FROM CMIS_DEPOSITDT WHERE INVOICENO='" & txtReference.Text & "' AND INVOICETYPE='" & SetTranTypeCode(cboTranType.Text) & "'")
            'JJE 12/14/2012 4:43PM
            gconDMIS.Execute ("DELETE FROM CMIS_TranList WHERE REFERENCE ='" & txtReference.Text & "' AND TRANTYPE ='" & SetTranTypeCode(cboTranType.Text) & "'")
            'JJE
            '=================================================
            If OR_VAT_NONVAT = "VAT" Then
...
Function SaveTransaction() As Boolean
    ...
    If AddorEdit = "EDIT" And SetTranTypeCode(cboTranType.Text) = "VI" And vCustype <> "B" Then
    ...
    If AddorEdit = "ADD" And SetTranTypeCode(cboTranType.Text) = "VI" And vCustype <> "B" Then
    ...
    If AddorEdit = "ADD" Then
      ...
      Else
'            Dim rsttlamt As New ADODB.Recordset
'            Set rsttlamt = New ADODB.Recordset
'            If vtrantype = "'PI'" Then
'               vtrantype = "P"
'            End If
''            rsttlamt.Open "SELECT * FROM pmis_vw_iss_history where type in ('A','M','P') and tranno="
            xTTLINV = xTTLINV
            SQL_STATEMENT = "Insert into CMIS_Off_Dt " & _
                        "(OR_NUM,INVOICETYPE,TRANTYPE,REFERENCE,REFERENCENO,INVOICENO,CUSCDE,DESCRIPT,BALANCE,AMOUNT,DOCDTE,ORDATE,PAYMENT,DISCOUNT,TAX,PAIDFOR,BRANCH,[OVER],ORIGINAL_D,VAT,ENTITY,XTTLINVAMT)" & _
                        " values (" & vOR_NUM & "," & vinvoicetype & "," & vtrantype & "," & vReference & "," & vREFERENCENO & "," & vInvoiceno & "," & vCUSCDE & "," & vDescript & "," & vBalance & "," & vAmount & "," & vDOCDTE & "," & vORDATE & "," & vPAYMENT & "," & vDISCOUNT & "," & vTAX & "," & vPaidFor & "," & vBRANCH & "," & vOVER & "," & vDOCDTE & "," & IS_VAT & "," & N2Str2Null(vENTITY) & ",'" & xTTLINV & "')"
            gconDMIS.Execute SQL_STATEMENT
            xTTLINV = 0
            lvDeposits.ListItems.Clear
...
'        rsUnapplied.Open "SELECT AMOUNT-APPLIEDAMT AS BALANCE FROM (SELECT HD.OR_NUM,HD.STATUS,HD.PAIDNA,DP.ORDATE,ISNULL((SELECT SUM(ISNULL(AMOUNT,0)) AS AMOUNT FROM CMIS_DEPOSITDT WHERE DEPOSIT_ID=DP.ID),0) AS APPLIEDAMT,HD.CUSCDE,DP.APPLIED,DP.ID_DET,DP.ID,DP.PAIDFOR,DP.AMOUNT FROM CMIS_OFF_HD HD INNER JOIN CMIS_DEPOSITS DP ON HD.OR_NUM=DP.OR_NUM)T WHERE AMOUNT-APPLIEDAMT>0 AND CUSCDE ='" & txtCUSCDE.Text & "' AND PAIDNA =1 ", gconDMIS, adOpenKeyset
    ...
    If Not lvDeposits.SelectedItem Is Nothing Then
        ...
        If reply = vbYes Then
            ...
            If picDetails.Visible = False Then      'JJE Deposit Fix bugs COMMENT
'                If SetTranTypeCode(cboTranType.Text) <> "OTH" Then
...
'                End If                             'JJE
            ...
            'JJE 12/17/2012 10:10AM
            'Deposit update. If Deposit is applied to Official Receipt payment is always = 0
            If vDeposits > 0 Then
                If vDeposits < NumericVal(txtAmount.Text) Then
...
Private Sub txtCustomer_Change()
    Dim xList                                          As ListItem
    Dim rsCMIS_OFF_HD                                  As ADODB.Recordset
    Set rsCMIS_OFF_HD = gconDMIS.Execute("SELECT ReferenceNo,CusCde,CusName,Or_Amt,OR_NUM,OR_Date FROM CMIS_OFF_HD Where TOF = '3' and (Paidby is null or paidby = 'N') and CusName like '" & txtCustomer.Text & "%' order by OR_Date")
    If Not (rsCMIS_OFF_HD.EOF And rsCMIS_OFF_HD.BOF) Then
        lvPayments.ListItems.Clear
        lblTotal = "0.00"
        Do While Not rsCMIS_OFF_HD.EOF
...
Private Sub txtDiscount_GotFocus()
    If NumericVal(txtDiscount.Text) = 0 Then
        txtDiscount.Text = ""
    Else
    ...
    If KeyCode = vbKeyReturn And txtReference.Text <> "" Then
        ...
        If SetTranTypeCode(cboTranType.Text) = "PI" Then
            ...
            ElseIf cboInvoiceType = "CHG" Then
                Set rsOrd_Hd = New ADODB.Recordset
                Set rsOrd_Hd = gconDMIS.Execute("Select custcode,trandate,tranno,custname,netinvamt,ttlinvAMT from PMIS_vw_ISS_HISTORY where TYPE = 'P' AND trantype = 'CHG' and tranno = " & N2Str2Null(txtReference.Text) & " AND custcode = " & N2Str2Null(txtCUSCDE.Text))
                If Not rsOrd_Hd.EOF And Not rsOrd_Hd.BOF Then
                    txtDescript.Text = Null2String(rsOrd_Hd!custname)
                    txtAmount.Text = ToDoubleNumber(rsOrd_Hd!NETINVAMT)
                    labDocDate.Caption = Null2Date(rsOrd_Hd!trandate)
...
'                txtReference.SetFocus
        ...
        If SetTranTypeCode(cboTranType.Text) = "UI" Then
            ...
            Else
                cmdTranSave.Enabled = True
            End If
            cboPaidFor.Enabled = False: cboBranch.Enabled = False: txtPayment.Enabled = False
            txtDiscount.Enabled = True: txtTax.Enabled = True: txtPayment.Enabled = True
        End If
        If SetTranTypeCode(cboTranType.Text) = "OTH" Then
            If lvDeposits.ListItems.Count <> 0 Then
       ...
       Set rsBalance = gconDMIS.Execute("select round(sum(PAYMENT),2) as MGA_BAYAD_MO from CMIS_OFF_DT where INVOICENO =" & N2Str2Null(txtReference.Text) & "and CUSCDE = " & N2Str2Null(txtCUSCDE.Text))
           If Not rsBalance.BOF And Not rsBalance.EOF And rsBalance!MGA_BAYAD_MO <> 0 Then
             ...
             Else
               ...
               txtPayment.Text = ToDoubleNumber(NumericVal(rsBankFinance!balancefinance - rsBalance!MGA_BAYAD_MO))
            ...
            Else
               ...
               txtAmount.Text = ToDoubleNumber(NumericVal(rsBankFinance!balancefinance))
               labCUSCODE.Caption = Null2String(VVV)
               txtPayment.Text = ToDoubleNumber(NumericVal(rsBankFinance!balancefinance))
            End If
...
Function PersonalPayment() As Boolean
    ...
    Set rsOFF_Det = gconDMIS.Execute("Select TRANTYPE,REFERENCE,paymenttype,SUM(payment) as MGA_BAYAD from CMIS_Off_Dt Where trantype = 'VI' and Reference = " & N2Str2Null(txtReference.Text) & " GROUP BY REFERENCE,TRANTYPE,PAYMENTTYPE")
...
Function InitializePayment() As Boolean
    ...
    Set rsAllPayment = gconDMIS.Execute("Select Insurance,LTORegFee,Others,Downpayment,CHMOFEE,salesprice,Term from SMIS_SalesOrder where VI_NO ='" & txtReference.Text & "'AND CODE = '" & txtCUSCDE & "'")
...
Private Sub txtTax_GotFocus()
    If NumericVal(txtTax.Text) = 0 Then
        txtTax.Text = ""
    Else
...
'DESCRIPTION: List all Credit Card Payments
    ...
    ElseIf COMPANY_CODE = "DGI" Then
        Set rsCMIS_OFF_HD = gconDMIS.Execute("SELECT ReferenceNo,CusCde,CusName,Or_Amt,OR_NUM,OR_Date FROM CMIS_OFF_HD Where TOF = '3' and (Paidby IS NULL OR PAIDBY = 'N')  order by OR_Date")
    Else
