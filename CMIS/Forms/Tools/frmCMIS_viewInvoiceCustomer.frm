...
Private Sub cboInvoiceType_Click()
     If OR_OPTION = True Then
        If Left(frmCMISOREntry.txtOR_NUM, 2) = "CR" Or Left(frmCMISOREntry.txtOR_NUM, 2) = "PR" Then
            If UCase(cboInvoiceType.Text) = "SERVICE INVOICE-CASH" Or UCase(cboInvoiceType.Text) = "SERVICE INVOICE-CHARGE" Then
        ...
        ElseIf Left(frmCMISOREntry.txtOR_NUM, 2) = "OR" Then
            If COMPANY_CODE = "DJM" Then
                If UCase(cboInvoiceType.Text) <> "SERVICE INVOICE-CASH" And UCase(cboInvoiceType.Text) <> "SERVICE INVOICE-CHARGE" Then
                    MsgBox "Official Receipts only allows Service Invoices and Other Transaction", vbOKOnly, "INVALID INVOICE TYPE"
                    cboInvoiceType.ListIndex = 10
                    cboInvoiceType.Enabled = True
                    Exit Sub
                End If
            Else
...
Private Sub cmdOK_Click()
    ...
    If OR_OPTION = True Then
        If COLLECTION_RECEIPTS_CR = True Then
            If (Left(frmCMISOREntry.txtOR_NUM, 2) = "CR" Or Left(frmCMISOREntry.txtOR_NUM, 2) = "PR") Then
                If UCase(cboInvoiceType.Text) = "SERVICE INVOICE" Then
                    MsgBox "Service Invoice is not allowed in Collection Receipts", vbOKOnly, "INVALID INVOICE TYPE"
                    cboInvoiceType.ListIndex = -1
                    cboInvoiceType.Enabled = True
                    VBComBoBoxDroppedDown cboInvoiceType
        ...
        ElseIf OFFICIAL_RECEIPT_OR = True Then
            If Left(frmCMISOREntry.txtOR_NUM, 2) = "OR" Then
                If COMPANY_CODE = "DJM" Then
                    If UCase(cboInvoiceType.Text) <> "SERVICE INVOICE" And UCase(cboInvoiceType.Text) <> "OTHER TRANSACTION" Then
                        MsgBox "Official Receipts only allows Service Invoices and Other Transaction", vbOKOnly, "INVALID INVOICE TYPE"
                        cboInvoiceType.ListIndex = 10
                        cboInvoiceType.Enabled = True
                        Exit Sub
...
'        End If
        If Not (RSACCOUNTNAME.EOF Or RSACCOUNTNAME.BOF) Then
            frmCMISOREntry.cboInvoiceType = INVTYPE
            ...
            'frmCMISOREntry.cboInvoiceType.Text = TTYPE
...
Private Sub Form_Load()
    With cboInvoiceType
        ...
        .AddItem "SERVICE INVOICE-CASH"
        .AddItem "SERVICE INVOICE-CHARGE"
    End With
    
    If OR_OPTION = True Then
        If Left(frmCMISOREntry.txtOR_NUM, 2) = "CR" Then
            cboInvoiceType.ListIndex = -1
        ElseIf Left(frmCMISOREntry.txtOR_NUM, 2) = "OR" Then
            cboInvoiceType.ListIndex = 10
        End If
    Else
    '...JRE 06/08/2016 Service invoice is only allowed in OR VAT - CMC
'        If COMPANY_CODE = "CMC" Then
'            If OR_VAT_NONVAT = "VAT" Then
'                cboInvoiceType.ListIndex = 10
'            Else
