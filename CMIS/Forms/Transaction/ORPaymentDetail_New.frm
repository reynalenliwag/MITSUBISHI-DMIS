...
Function SetBankName2(XXX As Variant)
    Dim rsSBOOK                                                     As New ADODB.Recordset
    Set rsSBOOK = gconDMIS.Execute("Select ACCTNAME from CMIS_CARDBANK where CUSCDE = '" & XXX & "'")
    If Not rsSBOOK.EOF And Not rsSBOOK.BOF Then
        SetBankName2 = rsSBOOK!AcctName
    End If
End Function
...
Sub StoreMemVars()
    Dim rsOFF_HD                                                    As New ADODB.Recordset
    ...
    If Not rsOFF_HD.EOF And Not rsOFF_HD.BOF Then
        ...
        If COMPANY_CODE = M_COMPANY_CODE Then
            If N2Str2Zero(rsOFF_HD!CHKAMOUNT) > 0 And N2Str2Zero(rsOFF_HD!CashAmount) = 0 And N2Str2Zero(rsOFF_HD!CardAmount) = 0 Then
                txtModeOfPayment.Text = "Check Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) = 0 And N2Str2Zero(rsOFF_HD!CashAmount) > 0 And N2Str2Zero(rsOFF_HD!CardAmount) = 0 Then
                txtModeOfPayment.Text = "Cash Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) = 0 And N2Str2Zero(rsOFF_HD!CashAmount) = 0 And N2Str2Zero(rsOFF_HD!CardAmount) > 0 Then
                txtModeOfPayment.Text = "Card Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) > 0 And N2Str2Zero(rsOFF_HD!CashAmount) > 0 And N2Str2Zero(rsOFF_HD!CardAmount) = 0 Then
                txtModeOfPayment.Text = "Check/Cash Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) > 0 And N2Str2Zero(rsOFF_HD!CashAmount) = 0 And N2Str2Zero(rsOFF_HD!CardAmount) > 0 Then
                txtModeOfPayment.Text = "Check/Card Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) = 0 And N2Str2Zero(rsOFF_HD!CashAmount) > 0 And N2Str2Zero(rsOFF_HD!CardAmount) > 0 Then
                txtModeOfPayment.Text = "Cash/Card Payment"
            ElseIf N2Str2Zero(rsOFF_HD!CHKAMOUNT) > 0 And N2Str2Zero(rsOFF_HD!CashAmount) > 0 And N2Str2Zero(rsOFF_HD!CardAmount) > 0 Then
                txtModeOfPayment.Text = "Cash/Check/Card"
            Else
        ...
        If COMPANY_CODE = M_COMPANY_CODE Then
            txtCARDBNKCODE.Text = SetBankName2(Null2String(rsOFF_HD!cardbnkcde)) & " - " & Null2String(rsOFF_HD!Bankbranch)
...
Private Sub cmdChangeToCash_Click()
    ...
    'NEW LOG AUDIT---------------------------------------------------------
    'EAP:033109 comment the code so it cam make amis.exe just like in manila code this code has been comment
    ...
    Dim rsCashPos                                                   As New ADODB.Recordset
