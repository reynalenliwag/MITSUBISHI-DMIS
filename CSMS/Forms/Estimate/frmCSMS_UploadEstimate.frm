...
Function GenerateNewRONO() As String
    ...
    Dim vRO                                             As String
    If FOR_J = True Then
        Set RSTMP = gconDMIS.Execute("SELECT REP_OR FROM CSMS_REPOR where transtype = 'R' AND left(REP_OR,1) = 'J' ORDER BY REP_OR DESC")
        If Not (RSTMP.BOF And RSTMP.EOF) Then
            RSTMP.MoveFirst
            vRO = Mid(RSTMP!REP_OR, 3, 8) + 1
            GenerateNewRONO = "J-" & Format(vRO, "00000000")
        Else
            GenerateNewRONO = "J-00000001'"
        End If
    Else
        Set RSTMP = gconDMIS.Execute("SELECT REP_OR FROM CSMS_REPOR where transtype = 'R' ORDER BY REP_OR DESC")
        If Not (RSTMP.BOF And RSTMP.EOF) Then
            RSTMP.MoveFirst
            vRO = Mid(RSTMP!REP_OR, 3, 8) + 1
