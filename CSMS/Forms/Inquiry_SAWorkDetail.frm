...
Begin VB.Form frmCSMS_Inquiry_SAWorkDetail 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "SERVICE ADVISOR WORK DETAIL INQUIRY "
   ClientHeight    =   6780
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   12375
   ...
   EndProperty
   ForeColor       =   &H00DEDFDE&
   Icon            =   "Inquiry_SAWorkDetail.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.CommandButton Command1 
         ...
         Height          =   795
         Left            =   11460
         MouseIcon       =   "Inquiry_SAWorkDetail.frx":030A
         MousePointer    =   99  'Custom
         Picture         =   "Inquiry_SAWorkDetail.frx":045C
         Style           =   1  'Graphical
         TabIndex        =   17
   ...
   Begin MSFlexGridLib.MSFlexGrid grdQUERY2 
      ...
      Appearance      =   0
      FormatString    =   $"Inquiry_SAWorkDetail.frx":0797
   ...
   Begin MSFlexGridLib.MSFlexGrid grdQUERY 
      ...
      Appearance      =   0
      MousePointer    =   99
      FormatString    =   $"Inquiry_SAWorkDetail.frx":0826
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Arial"
         Size            =   8.25
         Charset         =   0
         ...
         Strikethrough   =   0   'False
      EndProperty
      MouseIcon       =   "Inquiry_SAWorkDetail.frx":0954
   End
   ...
   End
End
Attribute VB_Name = "frmCSMS_Inquiry_SAWorkDetail"
    ...
    If Not rsREPOR.EOF And Not rsREPOR.BOF Then
        ...
        Do While Not rsREPOR.EOF
            ...
            grdQUERY.AddItem Null2String(rsREPOR!rep_OR) & Chr(9) & _
                             Null2String(rsREPOR!NAYM) & Chr(9) & _
                             ...
                             Null2String(rsREPOR!REPAIRODERSTATUS)
...
Sub VcmdRODetails()
    ...
    RONumber = grdQUERY.TextMatrix(grdQUERY.Row, 0)
    cleargrid grdQUERY2
...
Private Sub Command1_Click()
    ...
    rsREPOR.Open " SELECT CSMS_REPAIRORDER.STATUS AS REPAIRODERSTATUS,CSMS_vw_EMPNO.naym, " & _
               " CSMS_REPOR.* FROM CSMS_REPOR " & _
               " INNER JOIN CSMS_REPAIRORDER ON " & _
               " CSMS_REPAIRORDER.RO_NO = CSMS_REPOR.REP_OR " & _
               " INNER JOIN CSMS_vw_EMPNO ON " & _
               " CSMS_REPOR.recd_by = CSMS_vw_EMPNO.code " & _
               " WHERE CSMS_REPOR.TRANSTYPE='R' " & SearchString1 & "ORDER BY REP_OR ASC ", gconDMIS
    If Not rsREPOR.EOF And Not rsREPOR.BOF Then
        FillReporGrid
    End If
    ...
    'LogAudit "I", "SERVICE ADVISOR WORK DETAIL INQUIRY" & SearchString1
    'NEW LOG AUDIT-----------------------------------------------------
    Call NEW_LogAudit("I", "SERVICE ADVISOR WORK DETAILS", "", "", "", SearchString1, "", "")
    'NEW LOG AUDIT-----------------------------------------------------
End Sub
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyF1 And Shift = 1:
            ...
            frmALL_AuditInquiry.Caption = "Audit Inquiry (SERVICE ADVISOR WORK DETAILS)"
            Call frmALL_AuditInquiry.DisplayHistory("", "SERVICE ADVISOR WORK DETAILS", "PRINTING")
...
Private Sub Form_Load()
    ...
    rsREPOR.Open " SELECT CSMS_REPAIRORDER.STATUS AS REPAIRODERSTATUS,CSMS_vw_EMPNO.naym, " & _
               ...
               " INNER JOIN CSMS_vw_EMPNO ON " & _
               " CSMS_REPOR.recd_by = CSMS_vw_EMPNO.code " & _
               " WHERE CSMS_REPOR.TRANSTYPE='R' ORDER BY NAYM ASC ", gconDMIS
    If Not rsREPOR.EOF And Not rsREPOR.BOF Then
        FillReporGrid
    End If
    If grdQUERY.Cols > 1 Then
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmCSMS_Inquiry_SAWorkDetail = Nothing
End Sub
