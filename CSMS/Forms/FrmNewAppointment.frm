...
'    End If
    ...
    If labType(0).Caption = "Appointment" Then
        ...
        If Not (rstmp.BOF And rstmp.EOF) Then
            MsgBox "Appointment No. already Exist , System will generate new Appointment No.", vbExclamation, "Info."
            Call GetDefaultTransactionType
            txtTranNo.SetFocus
            Exit Sub
        End If
    ElseIf labType(0).Caption = "Repair Order" Then
        ...
        If Not (rstmp.BOF And rstmp.EOF) Then
            MsgBox "Repair Order No. already Exist, System will generate new Repair Order No.", vbExclamation, "Info."
            Call GetDefaultTransactionType
            txtTranNo.SetFocus
            Exit Sub
        End If
    Else
        ...
        If Not (rstmp.BOF And rstmp.EOF) Then
            MsgBox "Estimate No. already Exist, System will generate new Estimate Order No.", vbExclamation, "Info."
            Call GetDefaultTransactionType
            txtTranNo.SetFocus
            Exit Sub
        End If
...
Dim str_MSG                                        As String
    ...
    If Not rsDupRepor.EOF And Not rsDupRepor.BOF Then
        If UCase(labType(0).Caption) = "ESTIMATE" Then
            MsgSpeechBox "Estimate Number Already Exist, System Will Generate New Estimate No.!"
            Call GetDefaultTransactionType
        ElseIf UCase(labType(0).Caption) = "APPOINTMENT" Then
            MsgSpeechBox "Appointment Number Already Exist, System Will Generate New Appointment No.!"
             Call GetDefaultTransactionType
        Else
            MsgSpeechBox "Repair Order Number Already Exist, System Will Generate New Repair Order No.!"
             Call GetDefaultTransactionType
        End If
...
Sub SaveJobs()
    ...
    For X = 1 To lblJob4Service.ListItems.Count
        ...
        'updated by: IEBV 03182012_0535pm
        'description: to less vat if charge to internal
        If lblJob4Service.ListItems(X).SubItems(6) = "C" Or lblJob4Service.ListItems(X).SubItems(6) = "S" Then
            JOBDETPRC = NumericVal(xFLATRATE) * JOBDET_HRS / ConvertToBIRDecimalFormat(VAT_RATE)
        Else
...
Sub GetDefaultTransactionType()
    ...
    If labType(0).Caption = "Repair Order" Then
        ...
        If FOR_J = True Then
            rsAddRepor.Open "select id,rep_or from CSMS_RepOr where TransType = 'R' and len(rep_or) = 10 AND left(rep_or,1) = 'J' order by substring(rep_or,3,8) desc", gconDMIS, adOpenForwardOnly, adLockReadOnly
            If Not rsAddRepor.EOF And Not rsAddRepor.BOF Then
                rsAddRepor.MoveFirst
                txtRep_Or.Text = Format(NumericVal(Mid$(rsAddRepor!REP_OR, 3, 8)) + 1, "J-00000000")
            Else
                txtRep_Or.Text = "J-00000001"
            End If
        Else
            rsAddRepor.Open "select id,rep_or from CSMS_RepOr where TransType = 'R' and len(rep_or) = 10 order by substring(rep_or,3,8) desc", gconDMIS, adOpenForwardOnly, adLockReadOnly
            If Not rsAddRepor.EOF And Not rsAddRepor.BOF Then
                rsAddRepor.MoveFirst
        ...
        If COMPANY_CODE <> "CMC" And COMPANY_CODE <> "DJM" Then txtTranNo.Locked = True
    ElseIf labType(0).Caption = "Estimate" Then       '
        xTransType = "E"
...
Private Sub txtTranNo_LostFocus()
    If xTransType <> "A" Then
        If FOR_J = True And xTransType = "R" Then
            If Len(txtTranNo) >= 3 And IsNumeric(Left(txtTranNo.Text, 2)) = False Then
                txtTranNo.Text = Mid(txtTranNo.Text, 3, Len(txtTranNo.Text) - 2)
            ElseIf IsNumeric(txtTranNo.Text) = True Then
                txtTranNo.Text = Format(Right(txtTranNo.Text, 8), "00000000")
            End If
            txtTranNo.Text = "J-" + Format(Right(NumericVal(Mid(txtTranNo.Text, 1, Len(txtTranNo.Text))), 8), "00000000")
        Else
