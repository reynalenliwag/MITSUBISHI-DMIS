...
Begin VB.Form frmCSMSEditRO 
   ...
   Begin VB.PictureBox picAppointment 
      ...
      Begin VB.ComboBox Cbo_Rotype 
         ...
         ForeColor       =   &H00000000&
         Height          =   330
         ItemData        =   "FrmEditRO.frx":1A2C
         Left            =   4710
         List            =   "FrmEditRO.frx":1A2E
      ...
      Begin VB.ComboBox txtTerm 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "FrmEditRO.frx":1A42
         Left            =   9090
         List            =   "FrmEditRO.frx":1A4C
         TabIndex        =   20
         Top             =   6600
         Width           =   915
      End
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   795
      Left            =   7320
      MouseIcon       =   "FrmEditRO.frx":1A5A
      MousePointer    =   99  'Custom
      Picture         =   "FrmEditRO.frx":1BAC
      Style           =   1  'Graphical
      TabIndex        =   18
      ToolTipText     =   "Cancel"
      Top             =   7350
   ...
   Begin VB.CommandButton cmdSave 
      ...
      Height          =   795
      Left            =   6600
      MouseIcon       =   "FrmEditRO.frx":1EEA
      MousePointer    =   99  'Custom
      Picture         =   "FrmEditRO.frx":203C
      Style           =   1  'Graphical
      TabIndex        =   17
...
Public Sub PassRepairOrderNo(XRONO As String, xID As Long)
    ...
    OLD_RO_Number = XRONO
    labid.Caption = xID
    'updated By: IEBV 05092011_0235pm
    'description:   To disable editing of RO number if there are part'(s) that issued already
    '---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    If COMPANY_CODE = "DJM" Then
        txtRep_Or.Locked = True
    Else
        If (gconDMIS.Execute("select count(*) from csms_ro_det where livil <> '1' and rep_or = '" & xLOCAL_RO & "'").Fields(0).Value) > 0 Then
             txtRep_Or.Locked = True
        Else
            If (gconDMIS.Execute("select count(*) from csms_ro_det where rotype = 'SR' and livil = '1' and rep_or = '" & xLOCAL_RO & "'").Fields(0).Value) > 0 Then
                txtRep_Or.Locked = True
            End If
        End If
    End If
...
Private Sub cmdSave_Click()
    ...
    'NEW LOG AUDIT-----------------------------------------------------
        Call NEW_LogAudit("E", "BILLING SYSTEM", SQL_STATEMENT, FindTransactionID(N2Str2Null(VTXTREP_OR), "REP_OR", "CSMS_REPOR"), "", "RO NO: " & OLD_RO_Number, "", "")
    'NEW LOG AUDIT-----------------------------------------------------
    ...
    'NEW LOG AUDIT-----------------------------------------------------
        Call NEW_LogAudit("EE", "BILLING SYSTEM", SQL_STATEMENT, FindTransactionID(N2Str2Null(VTXTREP_OR), "REP_OR", "CSMS_REPOR"), "", "RO NO: " & OLD_RO_Number, "", "")
    'NEW LOG AUDIT-----------------------------------------------------
...
'Description:   To enable and display the rotype of the HCI
    ...
    Set RSUPLOAD = gconDMIS.Execute("Select PromiseDate,RO_No as REP_OR from [CSMS_RepairOrder] where RO_No = '" & xLOCAL_RO & "'")
    If Not RSUPLOAD.EOF And Not RSUPLOAD.BOF Then
...
Else
    If COMPANY_CODE = "DJM" Then
        txtRep_Or.Text = "R-" & Format(NumericVal(Right(txtRep_Or.Text, Len(txtRep_Or.Text) - 2)), "00000000")
    Else
        If Left(Null2String(RSUPLOAD!REP_OR), 2) = "J-" And FOR_J = True Then
            If Len(txtRep_Or) >= 3 And IsNumeric(Left(txtRep_Or.Text, 2)) = False Then
                txtRep_Or.Text = Mid(txtRep_Or.Text, 3, Len(txtRep_Or.Text) - 2)
            ElseIf IsNumeric(txtRep_Or.Text) = True Then
                txtRep_Or.Text = Format(Right(txtRep_Or.Text, 8), "00000000")
            End If
            txtRep_Or.Text = "J-" + Format(Right(NumericVal(Mid(txtRep_Or.Text, 1, Len(txtRep_Or.Text))), 8), "00000000")
        ElseIf Left(txtRep_Or.Text, 2) = "R-" Then
            txtRep_Or.Text = "R-" & Format(NumericVal(Right(txtRep_Or.Text, Len(txtRep_Or.Text) - 2)), "00000000")
        ElseIf Left(txtRep_Or.Text, 2) = "J-" Then
            txtRep_Or.Text = "J-" & Format(NumericVal(Right(txtRep_Or.Text, Len(txtRep_Or.Text) - 2)), "00000000")
        Else
            txtRep_Or.Text = "R-" & Format(NumericVal(Right(txtRep_Or.Text, Len(txtRep_Or.Text))), "00000000")
        End If
    End If
End If
txtRep_Or.Text = UCase(txtRep_Or.Text)
End Sub
Sub fillCboType()
    Cbo_Rotype.Clear
    Cbo_Rotype.AddItem ""
