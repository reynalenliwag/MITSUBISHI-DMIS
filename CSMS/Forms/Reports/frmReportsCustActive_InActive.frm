...
'    End If
    ...
    If opt_Active = True Then
        ...
        SQLCUST = ("select * from (select datediff(month, max(csms_repor.dte_rel),'" & dateMonth & "') + 1 as cntActive," & _
            " acct_no as acct_no,max(dte_rel) as daterel from csms_repor where csms_repor.acct_no in " & _
            " ( select cuscde from all_customer_table where cuscde = csms_repor.acct_no ) and transtype = 'R' group " & _
            " by acct_no) t where cntActive <= " & LEAD_MONTH & " And cntActive > 0 order by daterel asc ")
    ElseIf opt_Inactive = True Then
        ...
        SQLCUST = ("select * from (select datediff(month , max(csms_repor.dte_rel),'" & dateMonth & "') + 1 as cntActive," & _
            " acct_no as acct_no,max(dte_rel) as daterel from csms_repor where csms_repor.acct_no in " & _
            " ( select cuscde from all_customer_table where cuscde = csms_repor.acct_no ) and transtype = 'R' group " & _
            " by acct_no) t where cntActive > " & LEAD_MONTH & " order by daterel asc ")
    End If
    ...
    If Not (rsCust.BOF And rsCust.EOF) Then
        ...
        dateRecd = rsCust!daterel
        cntActive = rsCust!cntActive
        
        'GET DETAILS
        SQLREPOR = ("select * from csms_repor where acct_no = '" & custAcct & "' and dte_rel = '" & dateRecd & "'")
        ...
        SQLRODET = ("select detdsc  from csms_ro_det where rep_or = '" & REP_OR & "'")
            ...
            If Not (rsREPOR.BOF And rsREPOR.EOF) Then dteService = Null2String(Trim(rsREPOR!DTE_rel))
