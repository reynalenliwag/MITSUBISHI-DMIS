...
Begin VB.Form frmCSMS_SubletJob 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   690
      Width           =   7965
      Begin VB.ComboBox cbosublettype 
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   8.25
            Charset         =   0
         ...
         EndProperty
         Height          =   315
         ItemData        =   "fmrSubletJob.frx":1082
         Left            =   6180
         List            =   "fmrSubletJob.frx":1084
      ...
      Begin VB.ComboBox cboBP_TYPE 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "fmrSubletJob.frx":1086
         Left            =   4890
         List            =   "fmrSubletJob.frx":1090
         Style           =   2  'Dropdown List
         TabIndex        =   29
         Top             =   1560
         Visible         =   0   'False
      ...
      Begin VB.ComboBox cboBPorGJ 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "fmrSubletJob.frx":10A2
         Left            =   6090
         List            =   "fmrSubletJob.frx":10AC
         Style           =   2  'Dropdown List
         TabIndex        =   26
         Top             =   360
         Width           =   1125
      ...
      Begin VB.ComboBox cboSubletCategory 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "fmrSubletJob.frx":10B8
         Left            =   1200
         List            =   "fmrSubletJob.frx":10C5
         Style           =   2  'Dropdown List
         TabIndex        =   2
         Top             =   360
         Width           =   3645
      ...
      Begin VB.CommandButton cmdCancel 
         ...
         Height          =   855
         Left            =   7050
         MouseIcon       =   "fmrSubletJob.frx":10F7
         MousePointer    =   99  'Custom
         Picture         =   "fmrSubletJob.frx":1249
         Style           =   1  'Graphical
         TabIndex        =   10
         ToolTipText     =   "Cancel"
         Top             =   4500
      ...
      Begin VB.ComboBox cboJobChargeTo 
         ...
         ForeColor       =   &H00000000&
         Height          =   330
         ItemData        =   "fmrSubletJob.frx":1587
         Left            =   4890
         List            =   "fmrSubletJob.frx":1594
         Sorted          =   -1  'True
         TabIndex        =   7
         Top             =   1260
         Width           =   765
      ...
      Begin VB.CommandButton cmdOk 
         ...
         Height          =   855
         Left            =   6150
         MouseIcon       =   "fmrSubletJob.frx":15A1
         MousePointer    =   99  'Custom
         Picture         =   "fmrSubletJob.frx":16F3
         Style           =   1  'Graphical
         TabIndex        =   9
...
Function GetJobLineNo(XXX As String)
    ...
    Dim rslineno                                       As ADODB.Recordset
    Dim ictr                                           As Integer
    Dim tempMAX_LINE_NO                                As String
    Dim vFinalResult                                   As String

    'updated by: IEBV   04152013
    'new generate new line no
    Set rslineno = gconDMIS.Execute("select max (line_no) + 1  as nextline from ( select line_no from csms_ro_det WHERE [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & " Union All select line_no from  CSMS_PO_DT WHERE [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & " Union All select line_no from  CSMS_PO_RC_DT WHERE [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & ") Tx")
    
    If Not (rslineno.EOF And rslineno.BOF) Then
        If N2Str2IntZero(rslineno!nextline) = 0 Then GetJobLineNo = "01"
        For ictr = 1 To N2Str2IntZero(rslineno!nextline)
            Set rsGetLN = gconDMIS.Execute("select line_no from csms_ro_det where line_no = '" & Format(ictr, "00") & "' AND [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & " UNION ALL select line_no from CSMS_PO_DT where line_no = '" & Format(ictr, "00") & "' AND [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & " UNION ALL select line_no from CSMS_PO_RC_DT where line_no = '" & Format(ictr, "00") & "' AND [REP_OR] = '" & XXX & "' and LIVIL =" & vLIVIL & " ")
            If Not (rsGetLN.EOF And rsGetLN.BOF) Then
                
            Else
                GetJobLineNo = Format(ictr, "00")
                Exit Function
            End If
        Next
    End If
...
Sub AdditionalJobFromReceiving()
    ...
    If lblAddorEdit.Caption = "RADD" Then
        If MsgBox("Are you sure you want to add this Job", vbYesNo, vbInformation) = vbYes Then
            SQL_STATEMENT = "Insert into CSMS_PO_RC_Dt " & _
            ...
            gconDMIS.Execute SQL_STATEMENT
                ...
                'NEW LOG AUDIT-----------------------------------------------------
                    Call NEW_LogAudit("AA", "SUBLET RECEIVING", SQL_STATEMENT, FindTransactionID(rRC_NO, "RC_NO", "CSMS_PO_RC_HD"), "", "CODE : " & Null2String(rDETCDE), vLIVIL, "")
                'NEW LOG AUDIT-----------------------------------------------------
    ...
    Else
        If MsgBox("Are you sure you want to save this Job", vbYesNo + vbInformation, "INFORMATION") = vbYes Then
            SQL_STATEMENT = "Update CSMS_PO_RC_DT set " & _
                "RC_NO = " & rRC_NO & "," & _
                "Po_No = " & rPo_No & "," & _
                "Rep_or = " & rRep_or & "," & _
                "JOBTYPE = " & rJOBTYPE & "," & _
            ...
            'NEW LOG AUDIT-----------------------------------------------------
                Call NEW_LogAudit("E", "SUBLET RECEIVING", SQL_STATEMENT, FindTransactionID(rRC_NO, "RC_NO", "CSMS_PO_RC_HD"), "", "RC NO: " & Null2String(rRC_NO) & " - EDIT DETAILS", "", "")
            'NEW LOG AUDIT-----------------------------------------------------
...
Private Sub cboBPorGJ_Change()
    If UCase(cboSubletCategory.Text) = "SUBLET LABOR" Then
            ...
            If UCase(cboBPorGJ.Text) = "GJ" Then
                ...
                cbosublettype.Clear
                cbosublettype.AddItem "Undercoating"
                cbosublettype.AddItem "Aircon"
                cbosublettype.AddItem "Detailing"
                cbosublettype.AddItem "Sublet"
                cbosublettype.AddItem "Towing"
               
            ElseIf UCase(cboBPorGJ.Text) = "BP" Then
                cboBP_TYPE.Visible = True
                Label4.Visible = True
                cbosublettype.Clear
                cbosublettype.AddItem "Tinsmith"
                cbosublettype.AddItem "Painting"
            End If
           
    Else
        cbosublettype.Visible = False
...
Private Sub cboSubletCategory_Change()
...
Private Sub cmdOK_Click()
    ...
    Else
        ...
        If COMPANY_CODE = "DJM" Or COMPANY_CODE = "DGI" Or COMPANY_CODE = "MGS" Then
        Else
            If UCase(cboSubletCategory.Text) = "SUBLET LABOR" Then
                If cboBPorGJ.Text = "BP" Then
                    ...
                    End If
                End If
                If cbosublettype.Text = "" Then
                        On Error Resume Next
                        ShowIsRequiredMsg "Type Cannot be Blank"
                        cbosublettype.SetFocus
                        Exit Sub
                 End If
            End If
        End If
        ...
        If COMPANY_CODE = "CMC" Or COMPANY_CODE = "DSSC" Then
                    If cbosublettype.Text = "Tinsmith" Then
                    vSUBLET_TYPE = N2Str2Null("T")
                ElseIf cbosublettype.Text = "Painting" Then
                    vSUBLET_TYPE = N2Str2Null("P")
                ElseIf cbosublettype.Text = "Aircon" Then
                    vSUBLET_TYPE = N2Str2Null("A")
                ElseIf cbosublettype.Text = "Undercoating" Then
                    vSUBLET_TYPE = N2Str2Null("U")
                ElseIf cbosublettype.Text = "Detailing" Then
                    vSUBLET_TYPE = N2Str2Null("D")
                ElseIf cbosublettype.Text = "Sublet" Then
                    vSUBLET_TYPE = N2Str2Null("S")
                ElseIf cbosublettype.Text = "Towing" Then
                    vSUBLET_TYPE = N2Str2Null("O")
                End If
            End If
            
        If cbosublettype.Visible = False Then
            vSUBLET_TYPE = N2Str2Null("")
        End If
...
'Description:       To check if the cotractor in non vat or with vat
        ...
        Else
            ...
            Else
                ...
                vTaxRate = (VAT_RATE / 100)
                vtxtContracAmount = NumericVal(txtContracAmount) / 1.12
            End If
        End If
        ...
        If lblAddorEdit = "ADD" Then
            If MsgBox("Are you sure do you want to add this Job?", vbQuestion + vbYesNo, "Are You Sure") = vbYes Then
                SQL_STATEMENT = "Insert into CSMS_PO_dt" & _
                    "(TRANSTATUS, Po_No,Rep_or,ROTYPE,JOBTYPE,LIVIL,LINE_NO,DETAMT,DETCDE,DETDSC,TECHNICIAN,WCODE,TAXRATE,TAXVAL,DETAIL,DET_AMT,USERCODE,SAVEDATE,TECHCODE,CONTRACTAMOUNT,COMPAMOUNT,DONE,SUBLET_TYPE)" & _
