...
Begin VB.Form frmLOGIN 
   ...
   Begin VB.Label lblS 
      BackStyle       =   0  'Transparent
      Caption         =   "Key In Your Employee No. then press enter. And place your finger in the sensor for validation."
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
...
Private Sub CmdAbsent_Click()
    'frmListAbsences.Show vbModal
End Sub

Private Sub CmdEditCards_Click()
    'frmHRMSEditCards.Show vbModal
End Sub
...
Private Sub CmdSummary_Click()
    'frmAttSummary.Show vbModal
End Sub

Private Sub CmdViewCards_Click()
    'frmViewCards.Show vbModal
End Sub
...
Private Sub Form_Load()
    ...
    Calendar1.Value = Now
    picOT.Visible = False
    Flag = -1: TxtDate.Text = Format(Date, "dddddd"): RefreshCurrentDate: InitializeScanner
End Sub
...
Private Sub ZKFPEngX1_OnCapture(ByVal ActionResult As Boolean, ByVal ATemplate As Variant)
    If TxtEmpNumber.Text <> "" Then
        ...
        If FMatchType = 2 Then '1:N
           Score = 8
           Set rsEMPINFO = New ADODB.Recordset
           Set rsEMPINFO = gconDMIS.Execute("SELECT EMPNO, FPTEMPLATE1, FPTEMPLATE2, FPIMAGE1, FPIMAGE2 FROM HRMS_EMPINFO WHERE EMPNO = '" & TxtEmpNumber & "'")
           If Not rsEMPINFO.EOF Then
                If ZKFPEngX1.VerFingerFromStr(Null2String(rsEMPINFO!fptemplate1), sTemp, False, RegChanged) = True Then
                    Call LOG
                    CmdContinue.Value = True
                    Exit Sub
                'End If
                ElseIf ZKFPEngX1.VerFingerFromStr(Null2String(rsEMPINFO!fptemplate2), sTemp, False, RegChanged) = True Then
                    Call LOG
                    CmdContinue.Value = True
                    Exit Sub
                'End If
                Else
                    MsgBox "Finger print didn't match for this employee number..Please try again"
                    TxtEmpNumber.Text = ""
                ...
                End If
           Else
                MsgBox "Please enter a valid employee number..Thank you"
                TxtEmpNumber = ""
                TxtEmpNumber.SetFocus
           End If
        End If
...
Private Sub TxtEmpNumber_LostFocus()
    'Dim rsTemp As ADODB.Recordset
    'Set rsTemp = New ADODB.Recordset
    'Set rsTemp = gconDMIS.Execute("SELECT LASTNAME FROM HRMS_EMPINFO WHERE EMPNO = '" & TxtEmpNumber & "'")
    'If Not rsTemp.EOF And Not rsTemp.BOF Then
            
    'Else
    '    MsgBox "Employee Number not found..Please key-in an existing employee number.."
    '    TxtEmpNumber.Text = ""
    '    TxtEmpNumber.SetFocus
    'End If
    'Set rsTemp = Nothing
...
Private Sub CmdContinue_Click()
    ...
    If TxtTime.Text = "" Then
        MsgBox "     Either you didn't enter a valid employee number " & vbCrLf & _
               "   or you haven't placed your finger on the sensor yet " & vbCrLf & _
               "Please read and understand the instructions thoroughly. " & vbCrLf & _
               "                           Thank you."
