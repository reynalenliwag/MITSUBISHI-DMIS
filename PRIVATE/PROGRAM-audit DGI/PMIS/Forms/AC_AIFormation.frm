...
Begin VB.Form frmPMISAC_AIFormation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Accessories Issuance Number Formation"
   ClientHeight    =   6375
   ClientLeft      =   1620
   ClientTop       =   5880
   ClientWidth     =   4440
   ...
   Begin VB.Frame Frame4 
      Caption         =   "Tracking code of Accessories"
      BeginProperty Font 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmPMISAC_AIFormation"
...
Sub GetSeries()
    With frmPMISTrans_CustomerOrder_AC
    ...
    RSORD_HD.Open "select is_series from PMIS_Ord_Hd where TYPE = 'A' AND sales_origin = '" & lbl2.Caption & "' and SI_TYPE ='" & lbl3.Caption & "' and  PAY_CLASS = '" & lbl4.Caption & "'  order by is_series desc ", gconDMIS
...
Private Sub cmdOk_Click()
    If lbl2.Caption = "" Or lbl3.Caption = "" Or lbl4.Caption = "" Then
        MsgBox "AIS number not completed!"
        Exit Sub
    End If
    ...
    Dim RSORD_HD                                       As ADODB.Recordset
    Set RSORD_HD = New ADODB.Recordset
    RSORD_HD.Open "select refpisno,TRANNO from PMIS_Ord_Hd where TYPE = 'A' AND refpisno = '" & PISNUMBER & "' and status='P'", gconDMIS
    If Not RSORD_HD.EOF And Not RSORD_HD.BOF Then
        MsgBox "AIS number already exist in Transaction number : " & Null2String(RSORD_HD!TRANNO)
        Exit Sub
    End If
