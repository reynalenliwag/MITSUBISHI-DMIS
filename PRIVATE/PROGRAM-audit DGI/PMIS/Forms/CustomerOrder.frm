...
Begin VB.Form frmPMISTrans_CustomerOrder 
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin wizProgBar.Prg Prgbr 
         ...
         _ExtentX        =   11456
         _ExtentY        =   344
         Picture         =   "CustomerOrder.frx":80CC
         ForeColor       =   255
         BorderStyle     =   2
         BarPicture      =   "CustomerOrder.frx":80E8
         ShowText        =   -1  'True
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Arial"
            Size            =   8.25
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   3000
         MouseIcon       =   "CustomerOrder.frx":8104
         MousePointer    =   99  'Custom
         Picture         =   "CustomerOrder.frx":8256
         Style           =   1  'Graphical
         TabIndex        =   86
         Top             =   1560
         Width           =   855
   ...
   Begin VB.PictureBox PICLoyaltyID 
      ...
      Begin VB.CommandButton CmdloyaltyCancel 
         ...
         Height          =   795
         Left            =   2700
         MouseIcon       =   "CustomerOrder.frx":85BC
         MousePointer    =   99  'Custom
         Picture         =   "CustomerOrder.frx":870E
         Style           =   1  'Graphical
         TabIndex        =   152
         ToolTipText     =   "Cancel"
         Top             =   840
      ...
      Begin VB.CommandButton CmdloyaltySave 
         ...
         Height          =   795
         Left            =   2010
         MouseIcon       =   "CustomerOrder.frx":8A4C
         MousePointer    =   99  'Custom
         Picture         =   "CustomerOrder.frx":8B9E
         Style           =   1  'Graphical
         TabIndex        =   153
         ToolTipText     =   "Save this Record"
         Top             =   840
...
Sub CHGPRINTING()
    ...
    If NumericVal(txtDS1.Text) = 0 Then
          ...
          Else
            ...
            Screen.MousePointer = 11
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CHG.RPT", "{ord_hd.trantype} = " & N2Str2Null(COUNTERTYPE) & "  and  {ord_hd.TYPE} = 'P' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
            'PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CHG.RPT", "{ord_hd.TYPE} = 'P' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text) & " AND {ord_hd.trantype} = " & N2Str2Null(COUNTERTYPE) & " , DMIS_REPORT_Connection, 1"
          End If
    ...
    Else
        ...
        Else
            Screen.MousePointer = 11
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "CHGDisc.RPT", "{ord_hd.trantype} = " & N2Str2Null(COUNTERTYPE) & "  and  {ord_hd.TYPE} = 'P' AND {ord_hd.tranno} = " & N2Str2Null(txtTranNo.Text), DMIS_REPORT_Connection, 1
...
'---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            If Mid((txtReferencePIS.Text), 5, 1) = "C" Then
                ...
                rsBALANCE.Open "SELECT ISNULL(SUM(ISNULL(AMOUNT_TOPAY,0)),0)-ISNULL((SELECT SUM(ISNULL(INVOICEAMOUNT,0)) FROM AMIS_DETAIL WHERE STATUS='P' AND CUSTOMERCODE=AR.CUSTOMERCODE AND INVOICETYPE IN ('PI','SI')),0) AS BALANCE FROM AMIS_AR AR WHERE STATUS='P' AND INVOICETYPE IN ('PI','SI') AND CUSTOMERCODE='" & Me.txtCustCode.Text & "' GROUP BY CUSTOMERCODE", gconDMIS, adOpenForwardOnly
                If Not rsBALANCE.EOF And Not rsBALANCE.BOF Then
                    BALANCE = rsBALANCE!BALANCE
                End If
...
'                        Else
                            If rsTERMS!CREDITDAYS = 0 Then
                                ...
                                Exit Sub
                            Else
'                            If NumericVal(txtNetInvAmt.Text) + BALANCE > NumericVal(rsTERMS!CREDITLIMIT) Then
                                ...
                                If CreditLimitLine(txtCustCode, txtNetInvAmt) = True Then
                                    If MsgBox("You cannot post this transaction! Credit limit exception encountered. Do you want to override the credit limit?", vbYesNo, "Warning") = vbYes Then
                                        ...
                                        If Module_Access(frmCMISCreditLimitInfo.lblId, "CREDIT LIMIT OVERRIDE", "SYSTEM") = False Then
                                            Exit Sub
                                        Else
                                            Call Override_rec(txtCustCode, txtNetInvAmt, txtTranNo, "PI", "PMIS", txtTranDate)
...
Private Sub cmdTranSave_Click()
    ...
    If txtTranType.Text <> "ADB" Then
        ...
        Set RSPARTMAS = New ADODB.Recordset
        RSPARTMAS.Open "Select isnull(srp,0) as srp,STOCKNO,onhand,sstock,resservice,TISSQTY,issuances,MAC,NON_HARI from PMIS_PARTMAS where STOCKNO = '" & cboTranPartNo.Text & "' AND ACTIVE = 'Y'", gconDMIS
        If Not RSPARTMAS.EOF And Not RSPARTMAS.BOF Then
...
Sub click()
    ...
    StoreMemVars
End Sub
'Function CreditLimitLine(xCustCode As String, xbaltofinanced As Double) As Boolean
'    Dim RsCreditLimit1 As New ADODB.Recordset
'    Dim xlimit_amt As Double
'
'    CreditLimitLine = False
'    'credit limit
'    Set RsCreditLimit1 = gconDMIS.Execute("SELECT *,DATEADD(month,(terms_expiry_year * 12)+terms_expiry_month, getdate()) as expiry FROM ALL_CUSTOMER_TERMS WHERE customercode = '" & xCustCode & "'")
'    If Not (RsCreditLimit1.EOF Or RsCreditLimit1.BOF) Then
'        If RsCreditLimit1!CREDITLIMIT < xbaltofinanced Then
'        frmCMISCreditLimitInfo.txtCrExpiry.Text = RsCreditLimit1!expiry
'            CreditLimitLine = True
''            xlimit_amt =
'        End If
'    End If
''    'credit balanse ni itay
'    Set RsCreditLimit1 = gconDMIS.Execute("SELECT SUM(TTLINV)-(SELECT ISNULL(SUM(PAYMENT),0) FROM CMIS_OFF_DT WHERE TRANTYPE IN ('PI','AI','MI','SI') AND PAIDNA = 1 AND CUSCDE = '" & xCustCode & "')  as balanse FROM " & _
'            " ( " & _
'            "    SELECT ISNULL(SUM(CAST(TTLINVAMT AS DECIMAL(18,2))),0) TTLINV FROM PMIS_ORD_HD WHERE TYPE IN ('P','A','M') AND TRANTYPE IN ('CHG') AND STATUS IN ('P') and custcode= '" & xCustCode & "' " & _
'            "        Union " & _
'            "    SELECT ISNULL(SUM(CAST(TTLINVAMT AS DECIMAL(18,2))),0) TTLINV FROM PMIS_ORD_HIST WHERE TYPE IN ('P','A','M') AND TRANTYPE IN ('CHG') AND STATUS IN ('P') and custcode= '" & xCustCode & "'" & _
'            "        Union " & _
'            "    SELECT ISNULL(SUM(CAST(AMOUNT AS DECIMAL(18,2))),0) TTLINV FROM CSMS_REPOR WHERE ACCT_NO = '" & xCustCode & "' AND TRANSTYPE = 'R' AND DTE_COMP IS NOT NULL " & _
'            " ) T")
'
'    If RsCreditLimit1!BALANSE <> 0 Then
'        If ToDoubleNumber(RsCreditLimit1!BALANSE) < ToDoubleNumber(xbaltofinanced) Then
'            CreditLimitLine = True
'        End If
'    End If
''    'past due
'    Set RsCreditLimit1 = gconDMIS.Execute("select ar.customercode,ar.invoicetype,ar.invoiceno,ar.amount_topay,dt.invoiceamount from amis_ar ar inner join amis_detail dt on ar.InvoiceType = dt.InvoiceType " & _
...
'    " and ar.amount_topay>dt.invoiceamount and ar.invoicetype in ('PI','AI','MI','VI') " & _
'    " and ar.customercode = '" & xCustCode & "' " & _
...
'    " and invoiceno not in (select invoiceno from amis_detail where amis_detail.invoicetype = 'VI' and status = 'P'and customercode = '" & xCustCode & "' ) and ar.customercode = '" & xCustCode & "' and DateDiff(Day, ar.invoicedate, getdate()) > tr.creditterm")
'
'    If (RsCreditLimit1.EOF Or RsCreditLimit1.BOF) Then
'        CreditLimitLine = True
'    End If
'    'expiry
'    Set RsCreditLimit1 = gconDMIS.Execute("select DATEADD(month,(terms_expiry_year * 12)+terms_expiry_month, getdate()) as expiry,getdate() present from all_customer_terms where customercode = '" & xCustCode & "'")
'    If Not (RsCreditLimit1.EOF Or RsCreditLimit1.BOF) Then
'        If RsCreditLimit1!expiry < RsCreditLimit1!present Then
'            CreditLimitLine = True
'        End If
'    End If
'    Set RsCreditLimit1 = Nothing
'End Function
'
'Sub Override_rec(xCustCode As String, xbaltofin As Double, XINVNO As String, xinvtype As String, xMODULENAME As String)
'    Dim SQL                                                           As String
'    Dim xcodeuser As String
'    Dim rsxcode As New ADODB.Recordset
'    '==============================================initialise
'    Dim xlimit As Double
'    Dim xavailable As Double
'    Dim xpastdue As String
'    Dim xXpired As String
'    Dim RsCreditLimit2 As New ADODB.Recordset
'    Set RsCreditLimit2 = gconDMIS.Execute("SELECT *,DATEADD(month,(terms_expiry_year * 12)+terms_expiry_month, getdate()) as expiry FROM ALL_CUSTOMER_TERMS WHERE customercode = '" & xCustCode & "'")
'        If Not (RsCreditLimit2.EOF Or RsCreditLimit2.BOF) Then
'            xlimit = ToDoubleNumber(NumericVal(RsCreditLimit2!CREDITLIMIT))
'        End If
'        'credit available
'    Set RsCreditLimit2 = gconDMIS.Execute("SELECT SUM(TTLINV)-(SELECT ISNULL(SUM(PAYMENT),0) FROM CMIS_OFF_DT WHERE TRANTYPE IN ('PI','AI','MI','SI') AND PAIDNA = 1 AND CUSCDE = '" & xCustCode & "')  as balanse FROM " & _
'            " ( " & _
'            "    SELECT ISNULL(SUM(CAST(TTLINVAMT AS DECIMAL(18,2))),0) TTLINV FROM PMIS_ORD_HD WHERE TYPE IN ('P','A','M') AND TRANTYPE IN ('CHG') AND STATUS IN ('P') and custcode= '" & xCustCode & "' " & _
'            "        Union " & _
'            "    SELECT ISNULL(SUM(CAST(TTLINVAMT AS DECIMAL(18,2))),0) TTLINV FROM PMIS_ORD_HIST WHERE TYPE IN ('P','A','M') AND TRANTYPE IN ('CHG') AND STATUS IN ('P') and custcode= '" & xCustCode & "'" & _
...
'            "    SELECT ISNULL(SUM(CAST(AMOUNT AS DECIMAL(18,2))),0) TTLINV FROM CSMS_REPOR WHERE ACCT_NO = '" & xCustCode & "' AND TRANSTYPE = 'R' AND DTE_COMP IS NOT NULL " & _
'            " ) T")
'        If RsCreditLimit2!BALANSE <> 0 Then
'            xavailable = xlimit - ToDoubleNumber(ToDoubleNumber(RsCreditLimit2!BALANSE) + ToDoubleNumber(xbaltofin))
'        End If
'    '    'past due
'        Set RsCreditLimit2 = gconDMIS.Execute("select ar.customercode,ar.invoicetype,ar.invoiceno,ar.amount_topay,dt.invoiceamount from amis_ar ar inner join amis_detail dt on ar.InvoiceType = dt.InvoiceType " & _
'        " And ar.INVOICENO = dt.INVOICENO And ar.CustomerCode = dt.CustomerCode And ar.account_code = dt.acct_code " & _
'        " inner join all_customer_terms tr on ar.customercode=tr.customercode " & _
'        " Where DateDiff(Day, ar.invoicedate, getdate()) > tr.creditterm " & _
'        " and ar.amount_topay>dt.invoiceamount and ar.invoicetype in ('PI','AI','MI','VI') " & _
...
'        End If
'    '    'expiry
'        Set RsCreditLimit2 = gconDMIS.Execute("select DATEADD(month,(terms_expiry_year * 12)+terms_expiry_month, getdate()) as expiry,getdate() present from all_customer_terms where customercode = '" & xCustCode & "'")
'        If Not (RsCreditLimit2.EOF Or RsCreditLimit2.BOF) Then
'            If RsCreditLimit2!expiry = RsCreditLimit2!present Then
'                xXpired = "Y"
'            Else
'                xXpired = "N"
'            End If
...
'    '==============================================initialise
'
'    Set rsxcode = New ADODB.Recordset
'    rsxcode.Open "select top 1 usercode from all_rams_users where userid = '" & LOGID & "'", gconDMIS
'    If Not rsxcode.EOF And Not rsxcode.BOF Then
'        xcodeuser = rsxcode!USERCODE
'    End If
'    Dim rsoverride As New ADODB.Recordset
'    Set rsoverride = New ADODB.Recordset
'    rsoverride.Open "select * from CreditLimit_override where invoicetype = '" & xinvtype & "' and invoiceno = '" & XINVNO & "' and module_name = '" & xMODULENAME & "'", gconDMIS
'    If Not rsoverride.EOF And Not rsoverride.BOF Then
'    Else
'        SQL = " INSERT INTO CreditLimit_override("
'        SQL = SQL & " who_over_ride, invoicetype, invoiceno , invoicedate,creditlimit,available,invoiceamount,pastdue,expiredterm,usercode,module_name)"
'        SQL = SQL & " values ('" & frmCMISCreditLimitInfo.lblUsercode & "','" & xinvtype & "', '" & XINVNO & "', '" & txtTranDate & "','" & ToDoubleNumber(xlimit) & "','" & ToDoubleNumber(xavailable) & "','" & ToDoubleNumber(xbaltofin) & "','" & xpastdue & "','" & xXpired & "','" & xcodeuser & "','" & xMODULENAME & "')"
'        gconDMIS.Execute SQL
'    End If
'End Sub
'Sub delete_override(xinv_no As String)
'    Dim SQL                                                           As String
'    Dim rsoverride As New ADODB.Recordset
'    Set rsoverride = New ADODB.Recordset
'    rsoverride.Open "select * from CreditLimit_override where invoicetype = 'VI' and invoiceno = '" & xinv_no & "'", gconDMIS
'    If Not rsoverride.EOF And Not rsoverride.BOF Then
'        While Not rsoverride.EOF
'            SQL = " delete from CreditLimit_override where id = '" & rsoverride!ID & "'"
'            gconDMIS.Execute SQL
'            rsoverride.MoveNext
'        Wend
...
'    End If
'End Sub
''Sub user_override(username1 As String, userid1 As Long)
''    Overrider = UserName
''    Overrider_id = USERID
''End Sub
