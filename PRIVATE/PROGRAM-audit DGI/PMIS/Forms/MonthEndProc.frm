                    ...
                    Else
                        ...
                        If Number_Of_Years_No_Sale > 0 Then
                            ...
                            If A_SALES >= 360 Then
                                ...
                                vSubClass = "1"
                                GoTo con
                            ElseIf (A_SALES >= 240 And A_SALES <= 359) Then
                                vRankType = RANK_SUPER_FAST_MOVING
                                vSubClass = "2"
                                GoTo con
                            ElseIf (A_SALES >= 120 And A_SALES <= 239) Then
                                vRankType = RANK_SUPER_FAST_MOVING
                                vSubClass = "3"
                                GoTo con
                            ElseIf (A_SALES >= 48 And A_SALES <= 119) Then
                                vRankType = RANK_FAST_MOVING
                                GoTo con
                            ElseIf (A_SALES >= 24 And A_SALES <= 47) Then
                                vRankType = RANK_MEDIUM_MOVING
                                GoTo con
                            ElseIf (A_SALES >= 1 And A_SALES <= 23) Then
                                vRankType = RANK_SLOW_MOVING
                                GoTo con
                            ElseIf (A_SALES >= 0 And A_SALES <= 0) Then
                                vRankType = RANK_NON_MOVING
                                GoTo con
                            ElseIf ((A_SALES + B_SALES) >= 0 And (A_SALES + B_SALES) <= 0) Then
                                vRankType = RANK_NON_MOVING
                                vSubClass = "2"
                                GoTo con
...
Private Sub cmdPost_Click()
    ...
    If MsgQuestionBox("Close All Transactions, Are You Sure?", "Month End Processing") = True Then
        ...
        If FIXONHAND = False Then
            ...
            str_MSG = Replace(str_MSG, "@UTX83912839123", "Ledger Balancing")
        ...
        If FIXMAC = False Then
            ...
            str_MSG = Replace(str_MSG, "@UTX83912839123", "Fixing Mac")
            MsgBox str_MSG, vbCritical, "Month End Error"
            cmdExit.Enabled = True
            gconDMIS.RollbackTrans
            Screen.MousePointer = 0
