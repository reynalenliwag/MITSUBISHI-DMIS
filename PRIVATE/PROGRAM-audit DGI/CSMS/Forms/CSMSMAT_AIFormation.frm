...
Begin VB.Form frmPMISMAT_MIFormation_CSMS 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Materials Issuance Number Formation"
   ClientHeight    =   6375
   ClientLeft      =   1620
   ClientTop       =   5880
   ClientWidth     =   4440
   ...
   Begin VB.Frame Frame4 
      Caption         =   "Tracking code of Materials"
      BeginProperty Font 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmPMISMAT_MIFormation_CSMS"
...
Private Sub cmdOk_Click()
    If lbl2.Caption = "" Or lbl3.Caption = "" Or lbl4.Caption = "" Then
        MsgBox "MIS number not completed!"
        Exit Sub
    End If
    ...
    rsOrd_Hd.Open "select refpisno,TRANNO from PMIS_Ord_Hd where [TYPE] = 'M' AND refpisno = '" & pisnumber & "' and status='P'", gconDMIS
    If Not rsOrd_Hd.EOF And Not rsOrd_Hd.BOF Then
        MsgBox "MIS number already exist in Transaction number : " & Null2String(rsOrd_Hd!Tranno)
        Exit Sub
    End If
...
Sub GetSeries()
    ...
    Dim rsOrd_Hd                                       As ADODB.Recordset
    Set rsOrd_Hd = New ADODB.Recordset
    rsOrd_Hd.Open "select is_series from PMIS_Ord_Hd where [TYPE] = 'M' AND sales_origin = '" & lbl2.Caption & "' and SI_TYPE ='" & lbl3.Caption & "' and  PAY_CLASS = '" & lbl4.Caption & "'  order by is_series desc ", gconDMIS
    If Not rsOrd_Hd.EOF And Not rsOrd_Hd.BOF Then
