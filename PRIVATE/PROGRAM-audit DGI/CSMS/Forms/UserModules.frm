...
Begin VB.Form frmCSMSUserModules 
   ...
   End
End
Attribute VB_Name = "frmCSMSUserModules"
...
Private Sub Form_Load()
    ...
    '== Get Modules list ==
    'Set rs = gconDMIS.Execute("select description,id from CSMS_User_Modules order by description")
    
    Set rs = gconDMIS.Execute("select description,id from CSMS_User_Modules order by id")
    ...
    'Set rs = gconDMIS.Execute("SELECT user_modules.DESCRIPTION, user_modules.ID from CSMS_User_Modules INNER JOIN " _
     '        & "USERACCESS ON user_modules.ID = USERACCESS.MODULE_ID WHERE (USERACCESS.USERID = " & mUserID & ") order by user_modules.DESCRIPTION")
'    Set rs = gconDMIS.Execute("SELECT user_modules.DESCRIPTION, user_modules.ID from CSMS_User_Modules INNER JOIN " _
 '                           & "USERACCESS ON user_modules.ID = USERACCESS.MODULE_ID WHERE (USERACCESS.USERID = " & mUserID & ") order by user_modules.ID")
                            
                            
Set rs = gconDMIS.Execute("SELECT A.[DESCRIPTION], A.[ID] FROM CSMS_USER_MODULES A " & _
                            " Inner Join " & _
                            " CSMS_USERACCESS U ON A.[ID] = U.MODULE_ID " & _
...
Private Sub Save_Modules()
    ...
    gconDMIS.Execute ("Delete from CSMS_USERACCESS where userid = " & mUserID & " ")
    For i = 1 To Me.lvwGranted.ListItems.Count
        gconDMIS.Execute ("Insert into CSMS_USERACCESS(module_id, userid) values(" & _
