...
Sub DisplayWarrantyRO()
    ...
    Call flex_FillReportView(gconDMIS.Execute(" SELECT dbo.CSMS_Repor.REP_OR, dbo.CSMS_Repor.NIYM, dbo.CSMS_Ro_Det.DETDSC, dbo.CSMS_Repor.ID,  dbo.CSMS_Ro_Det.ID AS Expr13, CSMS_Ro_Det.LINE_NO, dbo.CSMS_Ro_Det.DETCDE  FROM dbo.CSMS_Repor " & _
                                            "INNER JOIN  dbo.CSMS_Ro_Det ON dbo.CSMS_Repor.REP_OR = dbo.CSMS_Ro_Det.REP_OR " & _
                                            "WHERE dbo.CSMS_Repor.TRANSTYPE = 'R' AND (dbo.CSMS_Ro_Det.LIVIL = '1') AND (dbo.CSMS_Ro_Det.WCODE = 'W') and dbo.CSMS_Ro_Det.ID  not in (SELECT  dbo.CSMS_Ro_Det.ID  FROM dbo.CSMS_Repor " & _
                                            "INNER JOIN  dbo.CSMS_Ro_Det ON dbo.CSMS_Repor.REP_OR = dbo.CSMS_Ro_Det.REP_OR " & _
                                            "right outer join CSMS_CQIR on dbo.CSMS_Repor.REP_OR = dbo.CSMS_CQIR.Ro_no and CSMS_Ro_Det.LINE_NO = dbo.CSMS_CQIR.LINE_NO and  dbo.CSMS_CQIR.DETCODE = dbo.CSMS_Ro_Det.DETCDE " & _
                                            " WHERE dbo.CSMS_Repor.TRANSTYPE = 'R' AND (dbo.CSMS_Ro_Det.LIVIL = '1') AND (dbo.CSMS_Ro_Det.WCODE = 'W') and (dbo.CSMS_CQIR.STATUS = 'P'))"), rptLIST)
...
Sub FillParts(VNO As String)
    ...
    If Not (rstmp.BOF And rstmp.EOF) Then
        Do While Not rstmp.EOF
            ...
            If Null2String(rstmp!ISSUBLET) = "" Then
                VPART = VPART + (N2Str2Zero(rstmp!cost) * NumericVal(rstmp!QTY))
            Else
