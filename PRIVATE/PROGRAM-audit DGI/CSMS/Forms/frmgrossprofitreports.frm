...
START:
        Do While Not rsREPOR.EOF
            ...
            Set rsrepordetail = gconDMIS.Execute("Select case rotype   when 'SR' then 'SR' when 'SR ' then 'SR' when 'SR  ' then 'SR' when 'SR   ' then 'SR' end as rotype,* from csms_ro_det where rep_or = '" & rsREPOR!REP_OR & "' and livil not in ('4') order by rotype,rep_or,livil,line_no asc")
