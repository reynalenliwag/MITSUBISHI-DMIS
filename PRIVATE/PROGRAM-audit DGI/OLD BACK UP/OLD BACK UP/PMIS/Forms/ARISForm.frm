...
Begin VB.Form frmPMISAC_ARISForms 
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin VB.CommandButton cmdTranDelete 
         ...
         Height          =   795
         Left            =   2880
         MouseIcon       =   "ARISForm.frx":45DD
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":472F
         Style           =   1  'Graphical
         TabIndex        =   88
      ...
      Begin VB.CommandButton cmdTranCancel 
         ...
         Height          =   795
         Left            =   2160
         MouseIcon       =   "ARISForm.frx":4A5A
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":4BAC
         Style           =   1  'Graphical
         TabIndex        =   86
      ...
      Begin VB.CommandButton cmdTranSave 
         ...
         Height          =   795
         Left            =   1440
         MouseIcon       =   "ARISForm.frx":4EEA
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":503C
         Style           =   1  'Graphical
         TabIndex        =   87
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   3405
         MouseIcon       =   "ARISForm.frx":53C4
         MousePointer    =   99  'Custom
         Picture         =   "ARISForm.frx":5516
         Style           =   1  'Graphical
         TabIndex        =   89
         Top             =   1575
         Width           =   855
...
Sub FillGrid()
    ...
    Set rsOrd_Hd = New ADODB.Recordset
    Set rsOrd_Hd = gconDMIS.Execute("select Tranno,tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & WAREHOUSETYPE & "' order by Tranno asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set rsOrd_Hd = gconDMIS.Execute("select tranno, tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & WAREHOUSETYPE & "' and tranno like '" & XXX & "%'")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Sorted = False: lstOrd_Hd.ListItems.Clear
    Set rsOrd_Hd = New ADODB.Recordset
    Set rsOrd_Hd = gconDMIS.Execute("select rono,tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & WAREHOUSETYPE & "' and rono is not null order by tranno asc")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set rsOrd_Hd = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set rsOrd_Hd = gconDMIS.Execute("select Rono, tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & WAREHOUSETYPE & "' and rono like '" & XXX & "%' order by tranno asc")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Private Sub cmdPrint_Click()
    ...
    If MsgQuestionBox("Accessories  Requisition Slip will be Printed. Are you sure?", "Confirm Printing...") = True Then
        ...
        Else
            ...
            rptCustomerOrder.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptCustomerOrder.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "RequisitionSlip_ACCESSORIES.rpt", "({PMIS_vw_PRS.TYPE} = 'A' and {PMIS_vw_PRS.TRANTYPE} = 'ARS' and {PMIS_vw_PRS_Tran.TRANNO} = " & N2Str2Null(txtTranNo.Text) & ")", DMIS_REPORT_Connection, 1
        End If
        Screen.MousePointer = 0
    End If
...
Private Sub cmdSave_Click()
    ...
    Else
        If ADDOREDIT = "ADD" Then
            ...
            RSFINDDUP.Open "select trantype,tranno from PMIS_vw_PRS where [TYPE] = 'A' AND trantype = '" & txtTranType.Text & "' and tranno = '" & txtTranNo.Text & "' order by trantype,tranno", gconDMIS, adOpenForwardOnly, adLockReadOnly
