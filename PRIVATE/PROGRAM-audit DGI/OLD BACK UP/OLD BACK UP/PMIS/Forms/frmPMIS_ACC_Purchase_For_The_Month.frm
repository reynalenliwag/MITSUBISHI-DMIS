...
Begin VB.Form frmPMISReports_Purchase_For_The_Month_AC 
   AutoRedraw      =   -1  'True
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Accessories Monthly Purchase"
   ClientHeight    =   2310
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4365
   ForeColor       =   &H00DEDFDE&
   Icon            =   "frmPMIS_ACC_Purchase_For_The_Month.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   795
      Left            =   2100
      MouseIcon       =   "frmPMIS_ACC_Purchase_For_The_Month.frx":0E42
      MousePointer    =   99  'Custom
      Picture         =   "frmPMIS_ACC_Purchase_For_The_Month.frx":0F94
      Style           =   1  'Graphical
      TabIndex        =   7
   ...
   Begin VB.CommandButton cmdPrint 
      ...
      Height          =   795
      Left            =   1380
      MouseIcon       =   "frmPMIS_ACC_Purchase_For_The_Month.frx":13DF
      MousePointer    =   99  'Custom
      Picture         =   "frmPMIS_ACC_Purchase_For_The_Month.frx":1531
      Style           =   1  'Graphical
      TabIndex        =   8
   ...
   End
End
Attribute VB_Name = "frmPMISReports_Purchase_For_The_Month_AC"
...
Private Sub cmdPrint_Click()
    ...
    RSPO_HIST.Open "select PODATE from PMIS_Po_Hist where TYPE = 'A' AND month(PODATE) = " & What_month(cboMonth) & " AND year(PODATE) =" & cboYear.Text, gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not RSPO_HIST.EOF And Not RSPO_HIST.EOF Then
        Screen.MousePointer = 11
        If cboSupplier.Text = "ALL SUPPLIERS" Then
            rptPurchase.WindowTitle = "Accessories - Purchases for the Month"
            rptPurchase.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptPurchase.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptPurchase, PMIS_REPORT_PATH & "PO_Hist.rpt", "{rr_hd.TYPE} = 'A' AND month({rr_hd.PODATE}) = " & What_month(cboMonth.Text) & " AND year({rr_hd.PODATE}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
        Else
            rptPurchase.WindowTitle = "Accessories - Purchases for the Month"
            rptPurchase.WindowTitle = "Purchases for the Month"
            rptPurchase.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptPurchase.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptPurchase, PMIS_REPORT_PATH & "PO_Hist.rpt", "{rr_hd.TYPE} = 'A AND {rr_hd.SUPCODE} = '" & txtSupCode.Text & "' AND month({rr_hd.PODATE}) = " & What_month(cboMonth.Text) & " AND year({rr_hd.PODATE}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
        End If
