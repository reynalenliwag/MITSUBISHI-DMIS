...
Sub UpdateAdjustment()
    ...
    Do While Not rsAdjust.EOF
        ...
        Set rsPmasMAC = gconDMIS.Execute("SELECT MAC FROM PMIS_STOCKMAS WHERE STOCKNO = " & N2Str2Null(vPARTNO))
        ...
        If vAdd <> 0 Then
            ...
            gconDMIS.Execute "Insert into PMIS_TdayTran " & _
                           ...
                           " values (" & N2Str2Null(rsAdjust!Type) & "," & VMAC & "," & N2Str2Null(rsAdjust!COST) & "," & vTrandate & ", 'ADJ'," & vPARTNO & ", " & vPARTNO & ", 'P'," & vAdd & ",'111111','1111','I'," & N2Str2Null(rsAdjust!USERCODE) & ")"
            ...
            If Not rsPartsAdjust.EOF And Not rsPartsAdjust.BOF Then
                ...
                'gconDMIS.Execute "update PMIS_STOCKMAS set onhand = " & AdjustQty_Add & " where TYPE = " & N2Str2Null(rsAdjust!Type) & " AND STOCKNO = " & N2Str2Null(vPARTNO)
                gconDMIS.Execute "update PMIS_STOCKMAS set onhand = " & AdjustQty_Add & ",trecqty = " & vAdd + N2Str2Zero(rsPartsAdjust!TRECQTY) & ",receipts = " & vAdd + N2Str2Zero(rsPartsAdjust!RECEIPTS) & " where TYPE = " & N2Str2Null(rsAdjust!Type) & " AND STOCKNO = " & N2Str2Null(vPARTNO)
            End If
        ...
        Else
            ...
            gconDMIS.Execute "Insert into PMIS_TdayTran " & _
                             "(TYPE,MAC,TRANUCOST,trandate,trantype,STOCK_ORD,STOCK_SUP,status,tranqty,tranno,itemno,in_out,usercode)" & _
                           " values (" & N2Str2Null(rsAdjust!Type) & "," & VMAC & "," & N2Str2Null(rsAdjust!COST) & "," & vTrandate & ", 'ADJ'," & vPARTNO & ", " & vPARTNO & ", 'P'," & vMinus & ",'000000','0000','O'," & N2Str2Null(rsAdjust!USERCODE) & ")"
...
'                             "tissqty = ISNULL(tissqty,0) + " & vMinus & ", " & _
'                             "issuances = ISNULL(issuances,0) + " & vMinus & _
'                             " where TYPE = " & N2Str2Null(rsAdjust!Type) & " AND STOCKNO = " & N2Str2Null(vPARTNO)
                 gconDMIS.Execute "update PMIS_STOCKMAS set " & _
                             "onhand = " & AdjustQty_Minus & ", " & _
