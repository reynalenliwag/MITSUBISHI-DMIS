...
Begin VB.Form frmPMISMAT_CustomerSearch 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Label Label2 
         ...
         Caption         =   $"MAT_CustomerSearch.frx":030A
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   9
            Charset         =   0
   ...
   Begin MSComctlLib.ListView lstCustomer 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      MouseIcon       =   "MAT_CustomerSearch.frx":0399
      NumItems        =   10
   ...
   Begin VB.CommandButton cmdAdd 
      ...
      Height          =   915
      Left            =   6480
      MouseIcon       =   "MAT_CustomerSearch.frx":04FB
      MousePointer    =   99  'Custom
      Picture         =   "MAT_CustomerSearch.frx":064D
      Style           =   1  'Graphical
      TabIndex        =   9
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   915
      Left            =   5640
      MouseIcon       =   "MAT_CustomerSearch.frx":0960
      MousePointer    =   99  'Custom
      Picture         =   "MAT_CustomerSearch.frx":0AB2
      Style           =   1  'Graphical
      TabIndex        =   10
      ToolTipText     =   "Cancel"
      Top             =   5700
   ...
   Begin VB.CommandButton cmdSelect 
      ...
      Height          =   915
      Left            =   4800
      MouseIcon       =   "MAT_CustomerSearch.frx":0DF0
      MousePointer    =   99  'Custom
      Picture         =   "MAT_CustomerSearch.frx":0F42
      Style           =   1  'Graphical
      TabIndex        =   11
...
Sub FillGrid()
    ...
    If COMPANY_CODE = "HAI" Then
        Set rsCUSTOMER = gconDMIS.Execute("select CusCde,UPPER(LastName) AS LASTNAME,UPPER(FirstName) AS FIRSTNAME,AcctName,Mobile,HomePhone,Fax,CustomerAdd,City,ProvincialAdd from ALL_Customer where cuscde <> '999999' order by lastname asc")
    Else
...
Sub FillSearchGrid(XXX As String)
    ...
    If optLN.Value = True Then
        Set rsCUSTOMER = gconDMIS.Execute("select CusCde,UPPER(LastName) LastName,UPPER(FirstName) FirstName,AcctName,Mobile,HomePhone,Fax,CustomerAdd,City,ProvincialAdd from ALL_Customer where LastName like '" & XXX & "%'" & SEARCHFILTER & " order by lastname asc")
    ElseIf optFN.Value = True Then
        Set rsCUSTOMER = gconDMIS.Execute("select CusCde,UPPER(LastName) LastName,UPPER(FirstName) FirstName,AcctName,Mobile,HomePhone,Fax,CustomerAdd,City,ProvincialAdd from ALL_Customer where FirstName like '" & XXX & "%'" & SEARCHFILTER & " order by firstname asc")
    ElseIf optFullName.Value = True Then
        Set rsCUSTOMER = gconDMIS.Execute("select CusCde,UPPER(LastName)LastName,UPPER(FirstName)FirstName,AcctName,Mobile,HomePhone,Fax,CustomerAdd,City,ProvincialAdd from ALL_Customer where AcctName like '" & XXX & "%'" & SEARCHFILTER & " order by AcctName asc")
    End If
    If Not (rsCUSTOMER.EOF And rsCUSTOMER.BOF) Then
        Listview_Loadval Me.lstCustomer.ListItems, rsCUSTOMER
        lstCustomer.Enabled = True
...
Private Sub cmdSelect_Click()
    ...
    If Trim(txtCode.Text) <> "" Then
        frmPMISTrans_CustomerOrder_MAT.txtCustCode.Text = txtCode.Text
        frmPMISTrans_CustomerOrder_MAT.SetCustomer
        Unload Me
    End If
    Exit Sub
Errorcode:
