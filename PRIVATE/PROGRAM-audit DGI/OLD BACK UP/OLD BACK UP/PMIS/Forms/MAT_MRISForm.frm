...
Begin VB.Form frmPMISMAT_MRISForms 
   ...
   Begin VB.PictureBox fraAddTran 
      ...
      Begin VB.CommandButton cmdTranDelete 
         ...
         Height          =   795
         Left            =   2880
         MouseIcon       =   "MAT_MRISForm.frx":45DD
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":472F
         Style           =   1  'Graphical
         TabIndex        =   70
      ...
      Begin VB.CommandButton cmdTranSave 
         ...
         Height          =   795
         Left            =   1440
         MouseIcon       =   "MAT_MRISForm.frx":4A5A
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":4BAC
         Style           =   1  'Graphical
         TabIndex        =   69
      ...
      Begin VB.CommandButton cmdTranCancel 
         ...
         Height          =   795
         Left            =   2160
         MouseIcon       =   "MAT_MRISForm.frx":4EFC
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":504E
         Style           =   1  'Graphical
         TabIndex        =   68
   ...
   Begin VB.PictureBox fraSignatories 
      ...
      Begin VB.CommandButton cmdPrintRIV 
         ...
         Height          =   645
         Left            =   1125
         MouseIcon       =   "MAT_MRISForm.frx":53A8
         MousePointer    =   99  'Custom
         Picture         =   "MAT_MRISForm.frx":54FA
         Style           =   1  'Graphical
         TabIndex        =   81
         Top             =   1575
         Width           =   855
...
Sub FillGrid()
    ...
    Set rsOrd_Hd = New ADODB.Recordset
    Set rsOrd_Hd = gconDMIS.Execute("select Tranno,tranno from PMIS_vw_PRS where trantype = '" & Repleys(WAREHOUSETYPE) & "' order by Tranno asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set rsOrd_Hd = gconDMIS.Execute("select tranno, tranno from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(WAREHOUSETYPE) & "' and tranno like '" & Repleys(XXX) & "%'")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Sub FillGrid2()
    ...
    lstOrd_Hd.Enabled = False
    Set rsOrd_Hd = New ADODB.Recordset
    Set rsOrd_Hd = gconDMIS.Execute("select rono,tranno from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(WAREHOUSETYPE) & "' and rono is not null order by tranno asc")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Sub FillSearchGrid2(XXX As String)
    ...
    Set rsOrd_Hd = New ADODB.Recordset
    XXX = Repleys(LTrim(RTrim(XXX)))
    Set rsOrd_Hd = gconDMIS.Execute("select Rono, tranno from PMIS_vw_PRS WHERE TYPE = 'M' and trantype = '" & Repleys(WAREHOUSETYPE) & "' and rono like '" & Repleys(XXX) & "%' order by tranno asc")
    If Not (rsOrd_Hd.EOF And rsOrd_Hd.BOF) Then
        lstOrd_Hd.Enabled = True
        Listview_Loadval Me.lstOrd_Hd.ListItems, rsOrd_Hd
        lstOrd_Hd.Refresh
...
Private Sub cmdPrint_Click()
    ...
    If MsgQuestionBox("Materials Requisition Slip will be Printed. Are you sure?", "Confirm Printing...") = True Then
        ...
        If Mid((rsOrd_Hd!refpisno), 5, 1) = "W" Then
            ...
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "RequisitionSlip_Materials_OTC.rpt", "({PMIS_vw_PRS.TYPE} = 'M' and {PMIS_vw_PRS.TRANTYPE} = 'MRS' and {PMIS_vw_PRS_Tran.TYPE} = 'M' and {PMIS_vw_PRS_Tran.TRANTYPE} = 'MRS' and{PMIS_vw_PRS.STATUS} = 'M' and {PMIS_vw_PRS_Tran.TRANNO} = " & N2Str2Null(txtTranNo.Text) & ")", DMIS_REPORT_Connection, 1
        Else
            rptCustomerOrder.WindowTitle = "Parts Requisition Slip"
            rptCustomerOrder.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptCustomerOrder.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptCustomerOrder, PMIS_REPORT_PATH & "RequisitionSlip_Materials.rpt", "({PMIS_vw_PRS.TYPE} = 'M' and {PMIS_vw_PRS.TRANTYPE} = 'MRS' and {PMIS_vw_PRS_Tran.TYPE} = 'M' and {PMIS_vw_PRS_Tran.TRANTYPE} = 'MRS' and{PMIS_vw_PRS.STATUS} = 'M' and {PMIS_vw_PRS_Tran.TRANNO} = " & N2Str2Null(txtTranNo.Text) & ")", DMIS_REPORT_Connection, 1
