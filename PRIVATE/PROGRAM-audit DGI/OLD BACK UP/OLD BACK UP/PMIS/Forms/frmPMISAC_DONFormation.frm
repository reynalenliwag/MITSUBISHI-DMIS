...
Begin VB.Form frmPMISAC_DONFormation 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Accessories Order Number Formation"
   ClientHeight    =   4545
   ClientLeft      =   1620
   ClientTop       =   5880
   ClientWidth     =   4440
   ...
   End
End
Attribute VB_Name = "frmPMISAC_DONFormation"
...
Sub GetSeries()
    With frmPMISTrans_Purchase
        ...
        Else
            ...
            RSPO_HD.Open "Select ORDER_SERIES from PMIS_PO_HD where [TYPE] = 'A' AND ORDERTYPE = '" & lbl2.Caption & "' order by Order_series desc ", gconDMIS
...
Private Sub cmdOk_Click()
    ...
    Dim RSPO_HD                                                       As ADODB.Recordset
    Set RSPO_HD = New ADODB.Recordset
    RSPO_HD.Open "select DON,PONO from PMIS_PO_HD where [TYPE] = 'A' AND DON = '" & DON & "' and status='P'", gconDMIS
    If Not RSPO_HD.EOF And Not RSPO_HD.BOF Then
        MsgBox "Order Number already exist in Transaction number : " & Null2String(RSPO_HD!PONO)
        Exit Sub
    End If
    With frmPMISAC_Purchase
        .txtDON = DON
