...
Private Sub cmdPrint_Click()
    ...
    If Option1.Value = True Then
        ...
        If Not rsJournal_hd.EOF And Not rsJournal_hd.EOF Then
            Set rsJournal_hd = New ADODB.Recordset
            rsJournal_hd.Open "select SUM(DEBIT) AS DEBIT_TOTAL, SUM(CREDIT) AS CREDIT_TOTAL, ACCT_CODE from AMIS_Journal_Det where jtype <> 'CLO' and Status = 'P' AND (jdate >= '" & dtpFrom & "' AND jdate <= '" & CDate(dtpTo) & "') group by ACCT_CODE order by ACCT_CODE asc", gconDMIS, adOpenDynamic
