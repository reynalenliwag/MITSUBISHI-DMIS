...
Private Sub ListVendorPayeeName_DblClick()
    On Error Resume Next
    frmAMISJournalEntry.txtPO_No.Text = (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(4)))
    frmAMISJournalEntry.txtMRR_No.Text = (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(2)))
    frmAMISJournalEntry.txtPVAmount.Text = (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(3)))
...
Private Sub ListVoucherNo_DblClick()
    ...
    frmAMISJournalEntry.txtPO_No.Text = (Trim(Me.ListVoucherNo.SelectedItem.SubItems(4)))
    frmAMISJournalEntry.txtMRR_No.Text = (Trim(Me.ListVoucherNo.SelectedItem))
    frmAMISJournalEntry.txtPVAmount.Text = (Trim(Me.ListVoucherNo.SelectedItem.SubItems(6)))
...
Private Sub txtVoucherNo_Change()
    ...
    If txtVoucherNo = "" Then
        ...
        If chkCurrentVend.Value = True Then
           ...
           Set rsJournal_HD = gconDMIS.Execute("SELECT * FROM (SELECT AMIS_JOURNAL_HD.VOUCHERNO,AMIS_JOURNAL_HD.Jdate,vendorcode,AMIS_JOURNAL_HD.AMOUNTTOPAY,AMIS_JOURNAL_HD.jtype,AMIS_JOURNAL_HD.remarks,AMIS_JOURNAL_HD.AMOUNTTOPAY - ISNULL((SELECT SUM(AMOUNT) FROM AMIS_CV_DETAIL WHERE (AMIS_JOURNAL_HD.JTYPE=AMIS_CV_DETAIL.JTYPE  AND PV_VOUCHERNO=AMIS_JOURNAL_HD.VOUCHERNO)) ,0) AS XBALANCE From AMIS_JOURNAL_HD WHERE AMIS_JOURNAL_HD.JTYPE IN('APJ','VPJ','VDJ')AND AMIS_JOURNAL_HD.STATUS ='P') AS T WHERE VENDORCODE = '" & CURRENT_VENDORCODE & "' and T.XBALANCE>0")
        Else
           'Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_VENDOR.nameofvendor,AMIS_Journal_Hd.Balance,AMIS_Journal_Hd.JType,AMIS_Journal_Hd.remarks from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where (Jtype = 'VPJ' OR Jtype = 'APJ' or Jtype = 'VDJ') and status = 'P' and (PaidStatus = 'N' OR AMIS_Journal_Hd.Balance > 0) order by VoucherNo asc")
           Set rsJournal_HD = gconDMIS.Execute("SELECT * FROM (SELECT AMIS_JOURNAL_HD.VOUCHERNO,AMIS_JOURNAL_HD.Jdate,AMIS_JOURNAL_HD.vendorcode,AMIS_JOURNAL_HD.AMOUNTTOPAY,AMIS_JOURNAL_HD.jtype,AMIS_JOURNAL_HD.remarks,AMIS_JOURNAL_HD.AMOUNTTOPAY - ISNULL((SELECT SUM(AMOUNT) FROM AMIS_CV_DETAIL WHERE (AMIS_JOURNAL_HD.JTYPE=AMIS_CV_DETAIL.JTYPE  AND PV_VOUCHERNO=AMIS_JOURNAL_HD.VOUCHERNO)) ,0) AS XBALANCE From AMIS_JOURNAL_HD WHERE AMIS_JOURNAL_HD.JTYPE IN('APJ','VPJ','VDJ')AND AMIS_JOURNAL_HD.STATUS ='P') AS T WHERE T.XBALANCE>0")
        End If
...
Private Sub txtVendorPayeeName_Change()
    ...
    Else
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkCurrentVend.Value = True Then
           Set rsJournal_HD = gconDMIS.Execute("select All_VENDOR.nameofvendor,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.Balance,AMIS_Journal_Hd.JType from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code Where AMIS_Journal_Hd.vendorcode = '" & CURRENT_VENDORCODE & "' AND (Jtype = 'VPJ' OR Jtype = 'APJ' OR Jtype = 'VDJ') and status = 'P' and (PaidStatus = 'N' OR AMIS_Journal_Hd.Balance > 0)  and ALL_VENDOR.nameofvendor like '" & Trim(Me.txtVendorPayeeName) & "%' order by All_VENDOR.nameofvendor asc")
        Else
           Set rsJournal_HD = gconDMIS.Execute("select All_VENDOR.nameofvendor,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.Balance,AMIS_Journal_Hd.JType from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code Where (Jtype = 'VPJ' OR Jtype = 'APJ' OR Jtype = 'VDJ') and status = 'P' and (PaidStatus = 'N' OR AMIS_Journal_Hd.Balance > 0)  and ALL_VENDOR.nameofvendor like '" & Trim(Me.txtVendorPayeeName) & "%' order by All_VENDOR.nameofvendor asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
