...
'                    gridStatus.TopRow = gridStatus.Rows - 1
    ...
    ElseIf Status = "SLGLCUSTOMER" Then
        ...
        rsSLGLCustomer.Open "SELECT D.ACCT_CODE, D.ACCT_NAME,SUM(H.INVOICEAMT) AS [SLBalance], " & _
                            "(SELECT SUM(ISNULL(DEBIT,0))-SUM(ISNULL(CREDIT,0)) FROM AMIS_JOURNAL_DET WHERE JTYPE='OPB' AND STATUS='P' AND ACCT_CODE=D.ACCT_CODE) [GLBalance] " & _
                            "FROM AMIS_JOURNAL_HD H Inner Join AMIS_JOURNAL_DET D ON H.VOUCHERNO = D.VOUCHERNO AND H.jtype = D.jtype " & _
                            "WHERE H.JTYPE='COB' AND H.STATUS='P' Group By D.ACCT_CODE, D.ACCT_NAME", gconDMIS
        If Not rsSLGLCustomer.EOF And Not rsSLGLCustomer.BOF Then
            ...
            Do While Not rsSLGLCustomer.EOF
                ...
                If rsSLGLCustomer![SLBALANCE] <> rsSLGLCustomer![GLBALANCE] Then
                    xRemarks = "Unbalanced"
                Else
                    xRemarks = "Balanced"
                End If
    ...
    ElseIf Status = "SLGLVENDOR" Then
        ...
        rsSLGLVendor.Open "SELECT D.ACCT_CODE, D.ACCT_NAME,ABS(SUM(H.AMOUNTTOPAY)) AS [SLBalance], " & _
                          "(SELECT ABS(SUM(ISNULL(DEBIT,0))-SUM(ISNULL(CREDIT,0))) FROM AMIS_JOURNAL_DET WHERE JTYPE='OPB' AND STATUS='P' AND ACCT_CODE=D.ACCT_CODE) [GLBalance] " & _
                          "FROM AMIS_JOURNAL_HD H INNER JOIN AMIS_JOURNAL_DET D ON H.VOUCHERNO=D.VOUCHERNO AND H.JTYPE=D.JTYPE " & _
                          "WHERE H.JTYPE='VPJ' AND H.STATUS='P'GROUP BY D.ACCT_CODE, D.ACCT_NAME", gconDMIS
        If Not rsSLGLVendor.EOF And Not rsSLGLVendor.BOF Then
            ...
            Do While Not rsSLGLVendor.EOF
                ...
                If rsSLGLVendor![SLBALANCE] <> rsSLGLVendor![GLBALANCE] Then
                    xRemarks = "Unbalanced"
                Else
                    xRemarks = "Balanced"
                End If
