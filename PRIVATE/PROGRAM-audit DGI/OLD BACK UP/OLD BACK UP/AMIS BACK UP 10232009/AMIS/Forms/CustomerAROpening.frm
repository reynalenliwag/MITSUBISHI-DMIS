...
Sub FillCboAcctName()
    Dim rsCOBAcctName                                                 As ADODB.Recordset
    Set rsCOBAcctName = New ADODB.Recordset
    Set rsCOBAcctName = gconDMIS.Execute("Select * from AMIS_ChartAccount Where (Titles = '1102') Order by AcctCode asc")
    If Not rsCOBAcctName.EOF And Not rsCOBAcctName.BOF Then
...
Private Sub cmdSave_Click()
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute "Insert into AMIS_Journal_HD" & _
                       " (jdate,voucherno,jtype,vendorcode,customercode,CustomerName,invoicedate,invoicetype,invoiceno,invoiceamt,duedate,paytype,refno,refdate,terms,dealer,amounttopay,Balance,AmountPaid,jno,debit,credit,outbalance,status,CheckNo,BankCode,remarks,PaidStatus,ReceiveStatus,USERCODE,LASTUPDATE)" & _
                       " values (" & J_JDATE & ", " & J_VOUCHERNO & ", " & J_JTYPE & ", " & J_VENDORCODE & "," & J_CUSTOMERCODE & "," & N2Str2Null(cboCustName.Text) & ", " & J_INVOICEDATE & "," & J_INVOICETYPE & "," & J_INVOICENO & "," & J_INVOICEAMT & ", " & J_DUEDATE & ", " & J_PAYTYPE & "," & J_REFNO & "," & J_REFDATE & "," & J_TERMS & "," & J_DEALER & ", " & J_AMOUNTTOPAY & "," & J_BALANCE & "," & J_AMOUNTPAID & _
                         ", " & J_JNO & ", " & J_DEBIT & ", " & J_CREDIT & ", " & J_OUTBALANCE & ", " & J_STATUS & ", " & J_CHECKNO & ", " & J_BANKCODE & "," & J_REMARKS & "," & J_PAIDSTATUS & "," & J_RECEIVESTATUS & ",'" & LOGCODE & "','" & LOGDATE & "')"
