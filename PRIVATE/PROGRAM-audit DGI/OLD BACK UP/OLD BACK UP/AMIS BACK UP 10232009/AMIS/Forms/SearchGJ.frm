...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        If JOURNALTYPE = "GJ" Then
            Set rsJournal_hd = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.REMARKS,AMIS_Journal_HD.status from AMIS_Journal_HD where Jtype='GJ' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        Else
            Set rsJournal_hd = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.REMARKS,AMIS_Journal_HD.status from AMIS_Journal_HD where Jtype='OPB' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        End If
    ...
    Else
        ...
        Set rsJournal_hd = New ADODB.Recordset
        If JOURNALTYPE = "GJ" Then
            Set rsJournal_hd = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.REMARKS,AMIS_Journal_HD.status from AMIS_Journal_HD Where Jtype='GJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        Else
            Set rsJournal_hd = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.REMARKS,AMIS_Journal_HD.status from AMIS_Journal_HD Where Jtype='OPB' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        End If
        If Not (rsJournal_hd.EOF And rsJournal_hd.BOF) Then
            ListVoucherNo.Enabled = True
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_hd
