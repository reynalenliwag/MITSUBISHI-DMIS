...
Public Sub DisplayHistory(xTRANID As String, xMODNAME As String, Optional VTYPE As String)
    ...
    If VTYPE = "PRINTING" Then
        DoEvents
        Set RSUPLOAD = gconAudit.Execute("SELECT * FROM ALL_VW_AUDIT WHERE DESCRIPTION  = '" & xMODNAME & "' ORDER BY ACTION_DATE ASC")
    Else
        ...
        DoEvents
        Set RSUPLOAD = gconAudit.Execute("SELECT * FROM ALL_VW_AUDIT WHERE DESCRIPTION  = '" & xMODNAME & "' AND TRANSACTION_ID = " & xTRANID & " ORDER BY ACTION_DATE ASC")
    End If
