...
Private Sub cmdYearEnd_Click()
    ...
    Else
        If CheckCloseBook = True Then
            ...
            rsJournal_HD.Open "select SUM(DEBIT) AS DEBIT_TOTAL, SUM(CREDIT) AS CREDIT_TOTAL, ACCT_CODE from AMIS_Journal_Det where LEFT(ACCT_CODE,1) > 3 AND jtype <> 'CLO' and Status = 'P' AND YEAR(jdate) >= '" & Format(dtFrom.Value, "yyyy") - 1 & "' AND YEAR(jdate) <= '" & Format(dtTo.Value, "yyyy") - 1 & "' group by ACCT_CODE order by ACCT_CODE asc", gconDMIS, adOpenDynamic
            If Not rsJournal_HD.EOF And Not rsJournal_HD.BOF Then
                rsJournal_HD.MoveFirst
                TOTAL_DEBIT_BALANCE = 0: TOTAL_CREDIT_BALANCE = 0
                Screen.MousePointer = 11
                gconDMIS.Execute ("Delete from AMIS_Journal_HD Where Jtype = 'CLO' and YEAR(JDate) = " & NumericVal(Format(dtTo.Value, "yyyy")) + 1)
                gconDMIS.Execute ("Delete from AMIS_Journal_Det Where Jtype = 'CLO' and YEAR(JDate) = " & NumericVal(Format(dtTo.Value, "yyyy")) + 1)
                ...
                J_JDATE = N2Str2Null("1/1/" & NumericVal(Format(dtTo.Value, "yyyy")) + 1)
                ...
                MsgBox "Closing Entries for Accounting Year = " & Format(dtTo.Value, "mm") & "/" & Format(dtTo.Value, "dd") & "/" & Format(dtTo.Value, "yyyy") - 1 & " Successfully Created!", vbInformation, "Done"
                FillGrid
                Screen.MousePointer = 0
            Else
                MsgBox "Check if transactions are posted.", vbInformation, "Message"
            End If
        Else
...
Function CheckCloseBook() As Boolean
    ...
    rsCheckCloseBook.Open "SELECT DISTINCT ACCTYEAR FROM (SELECT YEAR(ACCTMONTH) AS ACCTYEAR,STATUS FROM AMIS_ACCOUNTINGPERIOD WHERE Status=0) X  WHERE AcctYear = '" & Format(LOGDATE, "yyyy") - 1 & "'", gconDMIS, adOpenKeyset
...
Private Sub lvStatus_DblClick()
    Dim rsCheckStatus                             As ADODB.Recordset
    Set rsCheckStatus = New ADODB.Recordset
    rsCheckStatus.Open "SELECT VoucherNo,JType,JDate,Sum(Debit) as Debit,Sum(Credit) as Credit FROM AMIS_Journal_Det where Status <> 'P' and Status <> 'C' and JType <> 'BOB' and JDate <= '" & lastDay(xAccountingMonth) & "' and VoucherNo ='" & lvStatus.SelectedItem.SubItems(1) & "' Group by VoucherNo,JType,JDate", gconDMIS, adOpenForwardOnly
    If Not rsCheckStatus.EOF And Not rsCheckStatus.BOF Then
