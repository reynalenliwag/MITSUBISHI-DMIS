...
Sub LOAD_CUSTOMER()
    ...
    rsLOAD_CUSTOMER.Open "SELECT DISTINCT TOP 22 CUSTOMERCODE,CUSTOMERNAME FROM AMIS_AR ORDER BY CUSTOMERNAME ASC", gconDMIS, adOpenKeyset
...
Private Sub textSearch_Change()
    ...
    If TextSearch.Text <> "" Then
        rssearch.Open "SELECT DISTINCT CUSTOMERCODE,CUSTOMERNAME FROM AMIS_AR WHERE CUSTOMERNAME LIKE '" & Replace(TextSearch.Text, "'", "") & "%' ORDER BY CUSTOMERNAME ASC", gconDMIS, adOpenKeyset
    Else
        rssearch.Open "SELECT DISTINCT TOP 22 CUSTOMERCODE,CUSTOMERNAME FROM AMIS_AR ORDER BY CUSTOMERNAME ASC", gconDMIS, adOpenKeyset
    End If
...
Sub LOAD_AR_ENTITY()
    ...
    If cboAR_ACCT_CODE.Text = "ALL" Then
        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO FROM AMIS_AR WHERE CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "' ORDER BY JDATE,SJVOUCHERNO", gconDMIS, adOpenKeyset
    Else
        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO FROM AMIS_AR WHERE CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "' AND " & _
    ...
    If Not rsLOAD_AR.EOF And Not rsLOAD_AR.BOF Then
        Do While Not rsLOAD_AR.EOF
            ...
            End If
            If xSJVOUCHERNO = "COB" Or xSJVOUCHERNO = "SJ" Or xSJVOUCHERNO = "CRJ" Then
                Set REC = rptLEDGER.Records.Add
...
Sub REFERENCE_INVOICE(xINVOICENO As String, xINVOICETYPE As String, xCUSCODE As String, xAcctCode As String)
    ...
    rsINVOICE.Open "SELECT * FROM AMIS_DETAIL WHERE INVOICENO = '" & xINVOICENO & "' AND INVOICETYPE = '" & xINVOICETYPE & "' AND CUSTOMERCODE = '" & xCUSCODE & "' AND ACCT_CODE = '" & xAcctCode & "' " & _
...
'            End If
            ...
            rsINVOICE.MoveNext
        Loop
        Call ADJUSTMENT_BYVOUHCHERNO_PRINTING(xINVOICENO, xINVOICETYPE, txtCode.Text, xAcctCode)
    End If
    Set rsINVOICE = Nothing
End Sub
