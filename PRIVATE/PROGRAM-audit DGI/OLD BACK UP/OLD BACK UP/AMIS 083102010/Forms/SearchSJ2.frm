...
Private Sub txtInvoiceNo_Change()
    If txtInvoiceNo = "" Then
        ...
        If chkCurrentCust.Value = True Then
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HAI" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HCI" Then
        ...
        Else
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HAI" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HCI" Then
                ...
                Set rsJournal_HD = gconDMIS.Execute("SELECT DISTINCT TOP 22 INVOICETYPEWITHNO,ACCOUNTNAME,JDATE,VOUCHERNO,INVOICETYPE,INVOICENO,INVOICEDATE,INVOICEAMT,BALANCE,JTYPE " & _
                                                    ...
                                                    " INNER JOIN ALL_ENTITY " & _
                                                    " ON HD.CUSTOMERCODE = ALL_ENTITY.CODE " & _
                                                    " WHERE HD.STATUS <> 'C' (((HD.JTYPE IN ('SJ','CSJ','COB')) AND LEFT(DET.ACCT_CODE,5) IN ('11-02','11-03')) OR (HD.JTYPE IN ('CRJ') AND LEFT(DET.ACCT_CODE,5)='21-07')) AND ALL_ENTITY.ENTITYCODE='C' " & _
                                                    " ) T WHERE BALANCE <> 0 " & _
                                                    " ORDER BY INVOICETYPE,INVOICENO ASC ")
            Else
                Set rsJournal_HD = gconDMIS.Execute("select * FROM (SELECT TOP 18 AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.InvoiceNo,AMIS_Journal_Hd.Invoicetype,AMIS_Journal_Hd.InvoiceDate,AMIS_Journal_Hd.INVOICEAMT - ISNULL((SELECT SUM(INVOICEAMOUNT) FROM AMIS_CRJ_DETAIL WHERE (AMIS_JOURNAL_HD.INVOICETYPE=AMIS_CRJ_DETAIL.INVOICETYPE AND AMIS_JOURNAL_HD.INVOICENO=AMIS_CRJ_DETAIL.INVOICENO)),0) AS XBALANCE from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where status <> 'C' and (AMIS_Journal_Hd.jtype IN ('SJ','CSJ','COB'))) T WHERE T.XBalance <> 0 order by InvoiceNo asc")
            End If
        End If
...
Private Sub txtVoucherNo_Change()
    ...
    Else
        ...
        If chkCurrentCust.Value = True Then
            ...
            End If
        Else
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HAI" Or COMPANY_CODE = "HGC" Then
