    ...
    If Not rsJournal_HD.EOF And Not rsJournal_HD.EOF Then
        ...
        If REPORT_RANGETYPE = "GJ" Then
            ...
            If printable = True Then                       ' Printable Journal
                ...
                If chkSummary.Value = 1 Then ShowRangeReport dtpFrom, dtpTo, "GJSummary", "Summary", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", "General Summary", False
