...
Private Sub cmdPrint_Click()
    ...
    If rptLEDGER.Records.Count <= 0 Then Exit Sub
    ...
    If Not rsLOAD_AP.EOF And Not rsLOAD_AP.BOF Then
        Do While Not rsLOAD_AP.EOF
            ...
            If xVOUCHERNO = "VPJ" Or xVOUCHERNO = "APJ" Then
                ...
                Else
                    REC.AddItem (Trim(Null2String(rsLOAD_AP!AMOUNTPAID)))
                    REC.AddItem (Trim("0.00"))

                    TOTAL_DEBIT = ToDoubleNumber(Round((TOTAL_DEBIT + NumericVal(rsLOAD_AP!AMOUNT2PAY)), 2))
                    xBALANCE = ToDoubleNumber(Round((xBALANCE - NumericVal(rsLOAD_AP!AMOUNTPAID)), 2))

                    REC.AddItem (Trim(ToDoubleNumber(xBALANCE)))
                End If

                'If Null2String(rsLOAD_AP!APVoucherno) = "VPJ-000370" Then Stop
                Call REFERENCE_VOUCHER(Null2String(rsLOAD_AP!APVOUCHERNO), txtCode.Text, Null2String(rsLOAD_AP!Acct_code))
                'Call ADJUSTMENT_BYVOUHCHERNO(Null2String(rsLOAD_AP!VOUCHERNO), xVOUCHERNO, txtCode.Text, Null2String(rsLOAD_AP!Acct_code))
                rptLEDGER.Populate
                Set REC = Nothing
                ...
                '
                '                    TOTAL_CREDIT = ToDoubleNumber(Round((TOTAL_CREDIT + NumericVal(rsLOAD_AP!AMOUNTPAID)), 2))
                '                    xBALANCE = ToDoubleNumber(Round((xBALANCE - NumericVal(rsLOAD_AP!AMOUNTPAID)), 2))
                '
                '                    REC.AddItem (Trim(ToDoubleNumber(xBALANCE)))
                '                End If
                ...
                'Call REFERENCE_INVOICE(Right(Null2String(rsLOAD_AP!SJVoucherno), 6), Left(Null2String(rsLOAD_AP!SJVoucherno), 3), txtCode.Text, Null2String(rsLOAD_AP!Account_code))
                'Call ADJUSTMENT_BYVOUHCHERNO(Null2String(rsLOAD_AP!VOUCHERNO), xVOUCHERNO, txtCode.Text, Null2String(rsLOAD_AP!Account_code))
                rptLEDGER.Populate
                Set REC = Nothing
            End If
            rsLOAD_AP.MoveNext
...
Sub LOAD_AP_ENTITY_PRINTING()
    ...
    xlWorksheet.Cells(5, "A") = "Customer Code: " & txtCode.Text
    xlWorksheet.Cells(5, "A").Font.Bold = True
    xlWorksheet.Cells(6, "A") = "Customer Name: " & txtName.Text
...
Sub REFERENCE_VOUCHER(xVOUCHERNO As String, xVENDORCODE As String, xAcctCode As String)
    ...
    rsPAYMENT.Open "SELECT * FROM AMIS_DETAILS WHERE PV_VOUCHERNO = '" & xVOUCHERNO & "' AND VENDORCODE = '" & xVENDORCODE & "' AND ACCT_CODE = '" & xAcctCode & "' " & _
    ...
    If Not rsPAYMENT.EOF And Not rsPAYMENT.BOF Then
        Do While Not rsPAYMENT.EOF
            ...
            REC.AddItem (Trim(Null2String(rsPAYMENT!INVOICENO))) 'Null2String(rsPAYMENT!InvoiceType) & "-" &
...
Sub REFERENCE_VOUCHER_PRINTING(xVOUCHERNO As String, xVENDORCODE As String, xAcctCode As String)
    ...
    If Not rsPAYMENT.EOF And Not rsPAYMENT.BOF Then
        Do While Not rsPAYMENT.EOF
            ...
            xlWorksheet.Cells(xCounter, "C") = (Trim(Null2String(rsPAYMENT!INVOICENO))) 'Null2String(rsPAYMENT!InvoiceType) & "-" &
...
Sub ADJUSTMENT_BYVOUHCHERNO(xVOUCHERNO As String, xJType As String, xVENDORCODE As String, xACCT_CODE As String)
    ...
    rsADJ.Open "SELECT JDATE, VOUCHERNO, INVOICETYPE + '-' + INVOICENO AS REF_INVOICE,AMOUNT2PAY,AMOUNTPAID " & _
               ...
               "Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "' AND  VOUCHERNO = '" & xVOUCHERNO & "' AND ACCT_CODE = '" & xACCT_CODE & "' AND LEFT(VOUCHERNO,2) = 'GJ'", gconDMIS, adOpenKeyset
