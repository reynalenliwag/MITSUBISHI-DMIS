...
Sub StoreMemVars()
    ...
    ElseIf Status = "SLGLCUSTOMER" Then
        ...
        rsSLGLCustomer.Open "SELECT * FROM (SELECT ACCTCODE AS ACCT_CODE,DESCRIPTION AS ACCT_NAME," & _
                            "CAST(ISNULL((SELECT ABS(SUM(H.INVOICEAMT)) FROM AMIS_JOURNAL_HD H  INNER JOIN AMIS_JOURNAL_DET D  ON H.VOUCHERNO=D.VOUCHERNO  AND  H.JTYPE=D.JTYPE WHERE  H.JTYPE='COB' AND H.STATUS='P' AND ACCT_CODE =ACCTCODE ),0) AS DECIMAL(18,2)) [SLBALANCE] , " & _
                            "ISNULL((SELECT ABS(SUM(ISNULL(DEBIT,0))-SUM(ISNULL(CREDIT,0))) FROM AMIS_JOURNAL_DET WHERE JTYPE='OPB' AND STATUS='P' AND ACCT_CODE=ACCTCODE),0) [GLBALANCE] " & _
                            "FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,5) IN ('11-02') " & _
    ...
    ElseIf Status = "SLGLVENDOR" Then
        ...
        rsSLGLVendor.Open "SELECT * FROM (SELECT ACCTCODE AS ACCT_CODE, DESCRIPTION AS ACCT_NAME, " & _
                          "CAST(ISNULL((SELECT ABS(SUM(H.AMOUNTTOPAY)) FROM AMIS_JOURNAL_HD H  INNER JOIN AMIS_JOURNAL_DET D  ON H.VOUCHERNO=D.VOUCHERNO  AND  H.JTYPE=D.JTYPE WHERE  H.JTYPE='VPJ' AND H.STATUS='P' AND ACCT_CODE =ACCTCODE ),0) AS DECIMAL(18,2)) [SLBALANCE] , " & _
                          "ISNULL((SELECT ABS(SUM(ISNULL(DEBIT,0))-SUM(ISNULL(CREDIT,0))) FROM AMIS_JOURNAL_DET WHERE JTYPE='OPB' AND STATUS='P' AND ACCT_CODE=ACCTCODE),0) [GLBALANCE] " & _
