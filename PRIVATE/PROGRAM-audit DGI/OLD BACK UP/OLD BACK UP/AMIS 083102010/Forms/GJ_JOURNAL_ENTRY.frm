...
On Error GoTo ErrorCode
    ...
    If MsgBox("Are you sure you want to Post this transaction?", vbQuestion + vbYesNo, "Message") = vbYes Then
        ...
        'DESCRIPTION: CHECK IF IS AR SCHEDULE ACCOUNT
        '             IF AR SCHEDULE ACCOUNT THEN CHECK IF TOTAL_AR_AMOUNT IS BALANCE WITH
        '             THE TOTALPVAMOUNT

        '        If ISAR(txtVoucherNo.Text) = True Then
...
Sub PASS_INFO_ENRTY(xID As Variant)
    ...
    If Not rsPASS_INFO_ENRTY.EOF And Not rsPASS_INFO_ENRTY.BOF Then
        With frmAMISJournalEntry_GJDetails
            .txtCode = Right(Null2String(rsPASS_INFO_ENRTY!ENTITY), 6)
...
Function GET_NAME(xCUSCODE As String) As String
    ...
    rsGet_Name.Open "Select AccountName from All_Entity  where Code = '" & Right(xCUSCODE, 6) & "' and EntityCode = '" & Left(xCUSCODE, 1) & "' ", gconDMIS, adOpenKeyset
...
Sub GET_AR_GJ()
    ...
    If Not rsGET_AR_GJ.EOF And Not rsGET_AR_GJ.BOF Then
        Do While Not rsGET_AR_GJ.EOF
            ...
            xAMOUNT_TO_PAY = GET_SUM_GJ_AR(Null2String(rsGET_AR_GJ!INVOICENO), Null2String(rsGET_AR_GJ!InvoiceType), rsGET_AR_GJ!IS_OTHERS, Null2String(rsGET_AR_GJ!ADJ_VOUCHERNO), Null2String(rsGET_AR_GJ!ADJ_JTYPE), Null2String(rsGET_AR_GJ!Acct_code), XCustomerCode)
            xAMOUNT_PAID = 0
...
Function GET_SUM_GJ_AR(xINVOICENO As String, xINVOICETYPE As String, xIS_OTHERS As Boolean, xADJ_VOUCHERNO As String, xADJ_JTYPE As String, xACCT_CODE As String, xCUST_CODE As String) As Double
    ...
    If xINVOICENO = "" And xINVOICETYPE = "" And xIS_OTHERS = False Then
        rsGET_SUM_GJ_AR.Open "SELECT ROUND(SUM(DEBIT),2) AS SUM_GJ_AR FROM AMIS_JOURNAL_DET WHERE ADJ_VOUCHERNO = " & xADJ_VOUCHERNO & " AND ADJ_JTYPE = " & N2Str2Null(xADJ_JTYPE) & " AND IS_OTHERS = 0 and RIGHT(ENTITY,6) = " & xCUST_CODE & " and DEBIT <> 0 AND ACCT_CODE = '" & xACCT_CODE & "'", gconDMIS, adOpenKeyset
    ElseIf xINVOICENO <> "" And xINVOICETYPE <> "" And xIS_OTHERS = False Then
        rsGET_SUM_GJ_AR.Open "SELECT ROUND(SUM(DEBIT),2) AS SUM_GJ_AR FROM AMIS_JOURNAL_DET WHERE INVOICENO  = " & xINVOICENO & " AND INVOICETYPE = " & N2Str2Null(xINVOICETYPE) & " AND IS_OTHERS = 0 and RIGHT(ENTITY,6) = " & xCUST_CODE & " AND DEBIT <> 0 AND ACCT_CODE = '" & xACCT_CODE & "'", gconDMIS, adOpenKeyset
    ElseIf xINVOICETYPE = "" And xINVOICETYPE = "" And xIS_OTHERS = True Then
        rsGET_SUM_GJ_AR.Open "SELECT ROUND(SUM(DEBIT),2) AS SUM_GJ_AR FROM AMIS_JOURNAL_DET WHERE ADJ_VOUCHERNO  = " & N2Str2Null(xADJ_VOUCHERNO) & " AND ADJ_JTYPE = " & N2Str2Null(xADJ_JTYPE) & " AND IS_OTHERS = 1 and RIGHT(ENTITY,6) = " & xCUST_CODE & " AND  DEBIT <> 0 AND ACCT_CODE = '" & xACCT_CODE & "'", gconDMIS, adOpenKeyset
    Else
        GET_SUM_GJ_AR = 0
        Exit Function
    End If
...
Sub GET_AP_GJ()
    ...
    rsGJ_VOUCHER.Open "SELECT DISTINCT HD.VOUCHERNO,HD.VENDORCODE,HD.JDATE,DET.ENTITY,HD.JTYPE,HD.CUSTOMERCODE,HD.INVOICENO,HD.INVOICETYPE,HD.INVOICEDATE,HD.DUEDATE,ACCT_CODE,IS_OTHERS " & _
                      ...
                      "WHERE LEFT(DET.ACCT_CODE,5) IN ('21-01','21-02','21-07') AND HD.JTYPE = " & N2Str2Null(xJOURNALTYPE) & " AND HD.VOUCHERNO = " & N2Str2Null(txtVoucherNo.Text) & " AND HD.STATUS = 'P' AND DET.CREDIT <> 0", gconDMIS, adOpenKeyset
    If Not rsGJ_VOUCHER.EOF And Not rsGJ_VOUCHER.BOF Then
        Do While Not rsGJ_VOUCHER.EOF
            ...
            xAMOUNT_TO_PAY = GET_SUM_GJ_AP(Null2String(rsGJ_VOUCHER!IS_OTHERS), Null2String(rsGJ_VOUCHER!VOUCHERNO), Null2String(rsGJ_VOUCHER!jtype), Null2String(rsGJ_VOUCHER!Acct_code), XCustomerCode)
            xAMOUNT_PAID = 0
            xBAL = Round((xAMOUNT_TO_PAY - xAMOUNT_PAID), 2)
...
Function GET_SUM_GJ_AP(xIS_OTHERS As Boolean, xADJ_VOUCHERNO As String, xADJ_JTYPE As String, xACCT_CODE As String, xCUST_CODE As String) As Double
    Dim rsGET_SUM_GJ_AP                           As ADODB.Recordset
    Set rsGET_SUM_GJ_AP = New ADODB.Recordset

    If xIS_OTHERS = False Then
        rsGET_SUM_GJ_AP.Open "SELECT ROUND(SUM(CREDIT),2) AS SUM_GJ_AP FROM AMIS_JOURNAL_DET WHERE ADJ_VOUCHERNO = " & xADJ_VOUCHERNO & " AND ADJ_JTYPE = " & N2Str2Null(xADJ_JTYPE) & " AND IS_OTHERS = 0 and RIGHT(ENTITY,6) = " & xCUST_CODE & " and CREDIT <> 0 AND ACCT_CODE = '" & xACCT_CODE & "'", gconDMIS, adOpenKeyset
