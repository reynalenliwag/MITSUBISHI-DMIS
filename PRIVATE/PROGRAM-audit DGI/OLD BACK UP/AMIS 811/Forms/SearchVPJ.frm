...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 HD.VOUCHERNO,HD.JDATE,AL.ACCOUNTNAME,HD.BANKCODE " & _
...
Private Sub txtVendorPayeeName_Change()
    If txtVendorPayeeName = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 AL.ACCOUNTNAME,HD.JDATE,HD.VOUCHERNO,HD.BANKCODE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' order by All_VENDOR.nameofvendor asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListVendorPayeeName.Sorted = False: Me.ListVendorPayeeName.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("SELECT AL.ACCOUNTNAME,HD.JDATE,HD.VOUCHERNO,HD.BANKCODE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' and ALL_VENDOR.nameofvendor like '" & Trim(Me.txtVendorPayeeName) & "%' order by All_VENDOR.nameofvendor asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVendorPayeeName.ListItems, rsJournal_HD
            ListVendorPayeeName.Enabled = True
        Else
