...
Begin VB.Form frmAMISSearchDRJ 
   ...
   End
End
Attribute VB_Name = "frmAMISSearchDRJ"
...
Private Sub ListCustomer_DblClick()
    Call frmAMISJournalEntry_DRJ.SearchVoucherNo(Trim(Me.ListCustomer.SelectedItem.SubItems(2)))
    Unload Me
End Sub
...
Private Sub ListVoucherNo_DblClick()
    Call frmAMISJournalEntry_DRJ.SearchVoucherNo(Trim(Me.ListVoucherNo.SelectedItem))
    Unload Me
End Sub

Private Sub ListOrNumber_DblClick()
    Call frmAMISJournalEntry_DRJ.SearchVoucherNo(Trim(Me.ListOrNumber.SelectedItem.SubItems(4)))
    Unload Me
End Sub
...
Private Sub txtOrNumber_Change()
    If txtOrNumber = "" Then
        ...
        If chkShowAll3.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.paytype,AMIS_Journal_Hd.InvoiceNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.VoucherNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' order by ALL_CUSTMASTER_AMIS.CustName asc")
        Else
    ...
    Else
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkShowAll3.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.paytype,AMIS_Journal_Hd.InvoiceNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.VoucherNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and AMIS_Journal_Hd.InvoiceNo like '" & Format(Trim(Me.txtOrNumber), "000000") & "%' order by AMIS_Journal_Hd.InvoiceNo asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 AMIS_Journal_Hd.paytype,AMIS_Journal_Hd.InvoiceNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.VoucherNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and AMIS_Journal_Hd.InvoiceNo like '" & Format(Replace(Trim(Me.txtOrNumber), "'", ""), "000000") & "%' order by AMIS_Journal_Hd.InvoiceNo asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListOrNumber.ListItems, rsJournal_HD
            ListOrNumber.Enabled = True
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        If chkShowAll.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO  from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO  from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkShowAll.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Repleys(Me.txtVoucherNo)), "000000") & "%' order by VoucherNo asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Repleys(Me.txtVoucherNo)), "000000") & "%' order by VoucherNo asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_HD
            ListVoucherNo.Enabled = True
...
Private Sub txtCustomer_Change()
    If txtCustomer = "" Then
        ...
        If chkShowAll2.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' order by ALL_CUSTMASTER_AMIS.CustName asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' order by ALL_CUSTMASTER_AMIS.CustName asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkShowAll2.Value = 1 Then
            Set rsJournal_HD = gconDMIS.Execute("select ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and ALL_CUSTMASTER_AMIS.CustName like '" & Trim(Me.txtCustomer) & "%' order by ALL_CUSTMASTER_AMIS.CustName asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.paytype + ' OR# ' + AMIS_Journal_Hd.InvoiceNo as paytypeWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.jtype='DRJ' and status = '" & StatusToSearch & "' and ALL_CUSTMASTER_AMIS.CustName like '" & Replace(Trim(Me.txtCustomer), "'", "") & "%' order by ALL_CUSTMASTER_AMIS.CustName asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListCustomer.ListItems, rsJournal_HD
            ListCustomer.Enabled = True
