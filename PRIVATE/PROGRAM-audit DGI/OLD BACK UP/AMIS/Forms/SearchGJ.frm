...
'frmAMISJournalEntry.SearchVoucherNo (Trim(Me.ListVoucherNo.SelectedItem))
    If xJOURNALTYPE = "GJ" Then
        Call frmAMISJournalEntry_GJ.SearchVoucherNo(Trim(Me.ListVoucherNo.SelectedItem))
    Else
        Call frmAMISJournalEntry_OPB.SearchVoucherNo(Trim(Me.ListVoucherNo.SelectedItem))
    End If
    Unload Me
End Sub
...
Private Sub txtVoucherNo_Change()
    ...
    Else
        ...
        If xJOURNALTYPE = "GJ" Then
            If chkShowAll.Value = 1 Then
                Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.REMARKS,AMIS_Journal_HD.status from AMIS_Journal_HD Where Jtype='GJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Replace(Trim(Me.txtVoucherNo), "'", ""), "000000") & "%' order by VoucherNo asc")
            Else
