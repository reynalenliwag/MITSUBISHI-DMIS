...
Private Sub cmdPrint_Click()
    ...
    Dim rsCodeDesc1                                    As New ADODB.Recordset
...
'-------------------------------------------------------------------
    ...
    Dim Datetostring1                                      As String
    Dim Datetostring2                                      As String
 ...
 '------------------------------------------------------------------
    ...
    Datetostring1 = DTPicker1.Value
    Datetostring2 = DTPicker2.Value
    xlSheet.Cells(4, "A") = "Internal Charges"
    xlSheet.Cells(5, "A") = "For the date of " + Datetostring1 + " to " + Datetostring2
    
    SQL = ("SELECT  CSMS_Repor.invoice,CSMS_Repor.dte_rel,CSMS_Repor.REP_OR,CSMS_Ro_Det.WCODE, " & _
    ...
    If Not (rsIntCharge.BOF And rsIntCharge.EOF) Then
        ...
        Do While Not rsIntCharge.EOF
         ...
         InvoiceDte = Null2String(Trim(rsIntCharge!dte_rel))
...
'--------------------------------------------------------------------------
          ...
          Else
            ...
            Set rsCodeDesc1 = New ADODB.Recordset
            Set rsCodeDesc1 = gconDMIS.Execute("Select DESCNAME,Chartcodes from CMIS_SBOOK where BOOK = 'S' AND code = '" & Null2String(rsIntCharge!Code) & "'")
            If Not rsCodeDesc1.EOF And Not rsCodeDesc1.BOF Then
                xlSheet.Cells(COUNTER, "Q") = Null2String(rsCodeDesc1!DESCNAME)
                xlSheet.Cells(COUNTER, "R") = Null2String(rsCodeDesc1!Chartcodes)
            End If
            
            If Null2String(rsIntCharge!wCode) = "S" Then
                Chargeto = "Service"
            ElseIf Null2String(rsIntCharge!wCode) = "C" Then
                Chargeto = "Company"
            End If
                
                xlSheet.Cells(COUNTER, "P") = Chargeto
