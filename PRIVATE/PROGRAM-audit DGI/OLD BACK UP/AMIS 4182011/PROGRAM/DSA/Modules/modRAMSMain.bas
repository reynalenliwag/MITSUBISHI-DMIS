...
Function CheckLoginSetting() As Boolean
    ...
    If CHANGE_USER = True Then
        Set TEMPRS = cnx.Execute("Select user_name as username from ALL_Rams_Users WHERE USERGROUP='SDM'")
    Else
        Set TEMPRS = cnx.Execute("Select username from ALL_Rams_Users WHERE USERGROUP='SDM'")
    End If
...
On Error GoTo FALSEUSER
    ...
    Set rsUSERNAME = gconDMIS.Execute(SQL)
