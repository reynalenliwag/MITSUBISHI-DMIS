...
Private Sub cboRefPRSNo_GotFocus()
    ...
    Else
        rsPRS.Open "Select tranno,refpisno from PMIS_vw_PRS WHERE [TYPE] = 'P' and (SALES_ORIGIN = 'W' or SALES_ORIGIN = 'O' or SALES_ORIGIN = 'M' or SALES_ORIGIN = 'J')  order by Tranno asc", gconDMIS, adOpenForwardOnly, adLockReadOnly
    End If
...
Private Sub cmdTranSave_Click()
    ...
    'description: To update the discount amount if edited
            SQL_STATEMENT = ""
                SQL_STATEMENT = "UPDATE PMIS_ORD_HD SET" & _
                " ds_amt1 = " & ToDoubleNumber(txtDS_Amt1.Text) & "," & _
                " TOTALQTY = " & ORD_TOTQTY & "," & _
                " TTLINVAMT = " & ORD_TOTUPRICE & "," & _
                " NETINVAMT = " & ORD_TOTINVAMT & _
                " WHERE ID = " & labID.Caption
