...
'Function CheckifDetailsIsSublet(xID As Integer) As Boolean
...
Function checkdup_rr(XTYPE As String, xtrano As String) As Boolean
    ...
    sqlcommand = ""
    sqlcommand = "select rrno from PMIS_RR_Hd where [TYPE] = '" & XTYPE & "' AND rrno = '" & Format(xtrano, "000000") & "' "
    sqlcommand = sqlcommand + "UNION ALL "
    sqlcommand = sqlcommand + "select rrno from PMIS_Rec_hist where [TYPE] = '" & XTYPE & "' AND rrno = '" & Format(xtrano, "000000") & "' "
    Set rsfindDup = New ADODB.Recordset
...
Function checkdup_INVO(XTYPE As String, xinvo As String) As Boolean
    Dim rsINVNO                                        As ADODB.Recordset
    Dim XINVNO                                         As String

    Set rsINVNO = New ADODB.Recordset
    Dim strsqlcommand                                  As String
    strsqlcommand = "Select invno from pmis_rr_hd where invno = '" & Null2String(xinvo) & "' and [TYPE] = '" & XTYPE & "'"
    strsqlcommand = strsqlcommand + " UNION ALL "
    strsqlcommand = strsqlcommand + "Select invno from pmis_rec_hist where invno = '" & Null2String(xinvo) & "' and [TYPE] = '" & XTYPE & "' "
