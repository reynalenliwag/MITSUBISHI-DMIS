...
Sub LOAD_AP_ENTITY_PRINTING()
    ...
    Else
        ...
        rsLOAD_AP.Open "SELECT ap.JDATE,ap.VOUCHERNO AS APVOUCHERNO,ap.INVOICETYPE + '-' + ap.INVOICENO AS INVOICE,ap.AMOUNT2PAY,ap.AMOUNTPAID,ap.INVOICENO,ap.INVOICETYPE,ap.VENDOR_CODE,AP.ACCT_CODE,RIGHT(ap.VOUCHERNO,6) AS VOUCHERNO, hd.remarks FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE inner join amis_journal_hd hd on ap.voucherno=hd.jtype+'-'+hd.voucherno WHERE STATUS='P' AND ap.VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND ap.Jdate >= '" & dtFrom.Value & "'and ap.Jdate <= '" & dtTo.Value & "' AND " & _
...
Sub REFERENCE_VOUCHER(xVOUCHERNO As String, xVENDORCODE As String, xAcctCode As String)
    ...
    rsPAYMENT.Open "SELECT JDATE,JTYPE,VOUCHERNO,ISNULL(INVOICETYPE,'') + CASE WHEN INVOICETYPE IS NULL THEN '' ELSE '-' END + ISNULL(INVOICENO,'') AS INVOICENO,AMOUNTPAID FROM AMIS_DETAILS WHERE STATUS='P' AND PV_VOUCHERNO = '" & xVOUCHERNO & "' AND VENDORCODE = '" & xVENDORCODE & "' AND ACCT_CODE = '" & xAcctCode & "' " & _
