...
Begin VB.Form frmAMISSearchBOB 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Search Bank Opening Balance"
   ClientHeight    =   6435
   ClientLeft      =   2970
   ClientTop       =   3735
   ClientWidth     =   8775
   ...
   EndProperty
   ForeColor       =   &H00FFFFFF&
   Icon            =   "frmAMISSearchBOB.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
      ...
      Begin XtremeSuiteControls.TabControlPage TabControlPage2 
         ...
         Begin MSComctlLib.ListView ListVendorPayeeName 
            ...
            BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
               ...
               Strikethrough   =   0   'False
            EndProperty
            MouseIcon       =   "frmAMISSearchBOB.frx":000C
            NumItems        =   3
      ...
      Begin XtremeSuiteControls.TabControlPage TabControlPage1 
         ...
         Begin MSComctlLib.ListView ListVoucherNo 
            ...
            BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
               ...
               Strikethrough   =   0   'False
            EndProperty
            MouseIcon       =   "frmAMISSearchBOB.frx":016E
            NumItems        =   3
            BeginProperty ColumnHeader(1) {BDD1F052-858B-11D1-B16A-00C0F0283628} 
               Text            =   "VOUCHER NO."
               Object.Width           =   2822
   ...
   End
End
Attribute VB_Name = "frmAMISSearchBOB"
...
Private Sub ListVendorPayeeName_DblClick()
    frmAMISbanksOpening.SearchVoucherNo (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(2)))
    Unload Me
End Sub
...
Private Sub ListVoucherNo_DblClick()
    frmAMISbanksOpening.SearchVoucherNo (Trim(Me.ListVoucherNo.SelectedItem))
    Unload Me
End Sub
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = gconDMIS.Execute("select top 25 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDate,ALL_VENDOR.nameofvendor,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where Jtype='BOB' and status = '" & StatusToSearch & "' order by VoucherNo asc")
    ...
    Else
        Me.ListVoucherNo.Sorted = False: Me.ListVoucherNo.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select top 25 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDate,ALL_VENDOR.nameofvendor,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code Where Jtype='BOB' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_HD
            ListVoucherNo.Enabled = True
        Else
...
Private Sub txtVendorPayeeName_Change()
    If txtVendorPayeeName = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select top 25 All_VENDOR.nameofvendor,AMIS_Journal_Hd.JDate,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where Jtype='BOB' and status = '" & StatusToSearch & "' order by All_VENDOR.nameofvendor asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListVendorPayeeName.Sorted = False: Me.ListVendorPayeeName.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select top 25 All_VENDOR.nameofvendor,AMIS_Journal_Hd.JDate,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where Jtype='BOB' and status = '" & StatusToSearch & "' and ALL_VENDOR.nameofvendor like '" & Trim(Me.txtVendorPayeeName) & "%' order by All_VENDOR.nameofvendor asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVendorPayeeName.ListItems, rsJournal_HD
            ListVendorPayeeName.Enabled = True
        Else
