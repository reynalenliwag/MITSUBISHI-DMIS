...
Private Sub txtCustomer_Change()
    If txtCustomer = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.Jtype='COB' and status = '" & StatusToSearch & "' order by ALL_CUSTMASTER_AMIS.CustName asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
