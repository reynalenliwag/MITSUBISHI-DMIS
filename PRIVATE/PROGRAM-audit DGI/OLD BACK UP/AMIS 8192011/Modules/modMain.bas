...
'            If COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HMH" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "HCI" Or COMPANY_CODE = "DGI" Then
'                GJ_REMARKS_XXX
'                CrystalRpt.Formulas(59) = "REMARK = '" & xEntity & "'"
'            End If
'        End If

        If COMPANY_CODE = "HGC" Or COMPANY_CODE = "DGI" Then
            ' Update By BTT : 07282008
            If ReportName = "AccountsPayable" Or ReportName = "SalesJournal" Or ReportName = "GeneralJournal" Or ReportName = "CashDisbursement" Or ReportName = "CashReceipts" Then
                If REPRINT_CAPTION = "YES" Then
                    CrystalRpt.Formulas(3) = "Reprint= '" & "REPRINTED" & "'"
...
Function CheckARDetails(xSJVOUCHERNO As String, xACCT_CODE As String) As Boolean
    Dim rsAR                                           As ADODB.Recordset
    Set rsAR = New ADODB.Recordset
    rsAR.Open "SELECT SJVOUCHERNO FROM AMIS_AR WHERE SJVOUCHERNO='" & xSJVOUCHERNO & "' AND ACCOUNT_CODE='" & xACCT_CODE & "'", gconDMIS, adOpenForwardOnly
    If Not rsAR.EOF And Not rsAR.BOF Then
        CheckARDetails = True
    Else
...
Function CheckAPDetails(xSJVOUCHERNO As String, xACCT_CODE As String) As Boolean
    Dim rsAR                                           As ADODB.Recordset
    Set rsAR = New ADODB.Recordset
    rsAR.Open "SELECT VOUCHERNO FROM AMIS_AP WHERE VOUCHERNO='" & xSJVOUCHERNO & "' AND ACCT_CODE='" & xACCT_CODE & "'", gconDMIS, adOpenForwardOnly
    If Not rsAR.EOF And Not rsAR.BOF Then
        CheckAPDetails = True
    Else
...
Function CheckARPaymentDetails(xJType As String, xVOUCHERNO As String, xACCT_CODE As String) As Boolean
    Dim rsAR                                           As ADODB.Recordset
    Set rsAR = New ADODB.Recordset
    rsAR.Open "SELECT JTYPE,VOUCHERNO FROM AMIS_DETAIL WHERE JTYPE='" & xJType & "' AND VOUCHERNO = '" & xVOUCHERNO & "' AND ACCT_CODE='" & xACCT_CODE & "'", gconDMIS, adOpenForwardOnly
    If Not rsAR.EOF And Not rsAR.BOF Then
...
Function CheckAPPaymentDetails(xJType As String, xVOUCHERNO As String, xACCT_CODE As String) As Boolean
    Dim rsAR                                           As ADODB.Recordset
    Set rsAR = New ADODB.Recordset
    rsAR.Open "SELECT JTYPE,VOUCHERNO FROM AMIS_DETAILS WHERE JTYPE='" & xJType & "' AND VOUCHERNO = '" & xVOUCHERNO & "' AND ACCT_CODE='" & xACCT_CODE & "'", gconDMIS, adOpenForwardOnly
    If Not rsAR.EOF And Not rsAR.BOF Then
...
Function JOURNALLASTTRANS(XXX As String) As String
    ...
    rsJOURNAL.Open "SELECT MAX(JDATE) AS JDATE FROM AMIS_JOURNAL_HD WHERE JTYPE='" & XXX & "' AND STATUS='N'", gconDMIS, adOpenForwardOnly
...
Function JOURNALFIRSTTRANS(XXX As String) As String
    Dim rsJOURNAL                                          As ADODB.Recordset
    Set rsJOURNAL = New ADODB.Recordset
    rsJOURNAL.Open "SELECT MIN(JDATE) AS JDATE FROM AMIS_JOURNAL_HD WHERE JTYPE='" & XXX & "' AND STATUS='N'", gconDMIS, adOpenForwardOnly
    If Not rsJOURNAL.EOF And Not rsJOURNAL.BOF Then
