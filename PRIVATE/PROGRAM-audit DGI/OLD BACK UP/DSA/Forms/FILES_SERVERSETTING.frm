VERSION 5.00
Begin VB.Form frmFiles_ServerSetting 
   ...
   EndProperty
   Icon            =   "FILES_SERVERSETTING.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.PictureBox picStep1 
      ...
      Begin VB.PictureBox picConnection 
         ...
         Begin VB.Image img4 
            Height          =   720
            Left            =   0
            Picture         =   "FILES_SERVERSETTING.frx":08CA
            Top             =   0
            Visible         =   0   'False
            Width           =   720
         End
         Begin VB.Image img3 
            Height          =   720
            Left            =   0
            Picture         =   "FILES_SERVERSETTING.frx":1057
            Top             =   0
            Width           =   720
         End
         Begin VB.Image img2 
            Height          =   720
            Left            =   0
            Picture         =   "FILES_SERVERSETTING.frx":17C4
            Top             =   0
            Width           =   720
         End
         Begin VB.Image img1 
            Height          =   720
            Left            =   0
            Picture         =   "FILES_SERVERSETTING.frx":1F33
            Top             =   0
            Width           =   720
         End
      End
      Begin VB.PictureBox Picture4 
         ...
         Height          =   4965
         Left            =   0
         Picture         =   "FILES_SERVERSETTING.frx":26E4
         ScaleHeight     =   4965
         ScaleWidth      =   3435
         TabIndex        =   4
         Top             =   -60
   ...
   Begin VB.PictureBox picStep3 
      ...
      Begin VB.Image Image4 
         Height          =   4125
         Left            =   0
         Picture         =   "FILES_SERVERSETTING.frx":54A9
         Top             =   900
         Width           =   2820
      End
      ...
      Begin VB.Image Image1 
         Height          =   885
         Left            =   0
         Picture         =   "FILES_SERVERSETTING.frx":CC4F
         Top             =   0
         Width           =   7665
      End
   End
End
Attribute VB_Name = "frmFiles_ServerSetting"
...
Private Sub cmdNext_Click()
    ...
    ElseIf intSteps = 1 Then
        If LTrim(RTrim(txtRpt_AMIS)) = "" Then
            If MsgBox("Accounting Module (AMIS) Report Path has not been Configured . Are You Sure ?", vbQuestion + vbOKCancel, App.TITLE) = vbCancel Then
                txtRpt_AMIS.SetFocus
                Exit Sub
            End If
        End If
        If LTrim(RTrim(txtRpt_CMIS)) = "" Then
            If MsgBox("Cash Monitoring Module (CMIS) Report Path has not been Configured . Are You Sure ?", vbQuestion + vbOKCancel, App.TITLE) = vbCancel Then
...
Private Sub txtRpt_AMIS_LostFocus()
    'If LTrim(RTrim(txtRpt_CMIS)) = "" Then
    txtRpt_CMIS = UCase(Replace(txtRpt_AMIS, "AMIS", "CMIS"))
    'End If
    'If LTrim(RTrim(txtRpt_CRIS)) = "" Then
    txtRpt_CRIS = UCase(Replace(txtRpt_AMIS, "AMIS", "CRIS"))
    'End If
    'If LTrim(RTrim(txtRpt_CSMS)) = "" Then
    txtRpt_CSMS = UCase(Replace(txtRpt_AMIS, "AMIS", "CSMS"))
    'End If
    'If LTrim(RTrim(txtRpt_HRMS)) = "" Then
    txtRpt_HRMS = UCase(Replace(txtRpt_AMIS, "AMIS", "HRMS"))
    'End If
    'If LTrim(RTrim(txtRpt_OSMS)) = "" Then
    txtRpt_OSMS = UCase(Replace(txtRpt_AMIS, "AMIS", "OSMS"))
    'End If
    'If LTrim(RTrim(txtRpt_SMIS)) = "" Then
    txtRpt_SMIS = UCase(Replace(txtRpt_AMIS, "AMIS", "SMIS"))

    txtRpt_PMIS = UCase(Replace(txtRpt_AMIS, "AMIS", "PMIS"))
    'End If
End Sub
...
Private Sub txtSQLServerName_KeyPress(KeyAscii As Integer)
    If KeyAscii = 32 Then KeyAscii = 0
    KeyAscii = UpperAscii(KeyAscii)
End Sub
...
Sub Savedbsettings()
    ...
    ADD_REGISTRY txtDATABASE, txtSERVERNAME
    ServerName = txtSERVERNAME
    SQLSERVERNAME = txtSQLServerName
    ...
    On Error GoTo CONLINE:
    If Not gconDMIS Is Nothing Then
        If gconDMIS.State = 1 Then
            gconDMIS.Close
        End If
    End If

    Set gconDMIS = Nothing
    Set gconDMIS = New ADODB.Connection
    gconDMIS.Open DMIS_Connection

    On Error Resume Next

    If Not gconAudit Is Nothing Then
        If gconAudit.State = 1 Then
            gconAudit.Close
        End If
    End If


    Set gconAudit = Nothing
...
Function ADD_REGISTRY(XDATABASE, XSERVER)
    ...
    DATASOURCENAME = XDATABASE

    DATABASENAME = XDATABASE
    DRIVERPATH = Environ("windir") & "\SYSTEM32\SQLSRV32.DLL"
    LASTUSER = "SA"
    SERVER = XSERVER

    TRUSTED_CONNECTION = "YES"
    DESCRIPTION = "ODBC DSN FOR THE DMIS TRANSACTION DATABASE"
    DRIVERNAME = "SQL SERVER"
    SCOMPUTER = Environ("COMPUTERNAME")
    SPATH = "SOFTWARE\ODBC\ODBC.INI\" & DATABASENAME
    ...
    DESCRIPTION = "ODBC DSN FOR THE DATABASE AUDIT DATABASE"
    SPATH = "SOFTWARE\ODBC\ODBC.INI\" & DATABASENAME & "_AUDIT"
    If (0 = CreateRegKey(SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH)) Then
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "Database", DATABASENAME & "_AUDIT"
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "Description", DESCRIPTION
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "Driver", DRIVERPATH
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "LastUser", LASTUSER
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "Server", SERVER
        SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, SPATH, "Trusted_Connection", TRUSTED_CONNECTION
    End If
    SetRegKeyStrValue SCOMPUTER, HKEY_LOCAL_MACHINE, "SOFTWARE\ODBC\ODBC.INI\ODBC Data Sources", DATASOURCENAME & "_AUDIT", DRIVERNAME
