...
Private Sub Form_Load()
    ...
    Set rs = gconDMIS.Execute("select description,id from AIS_USER_MODULES order by id")
    ...
    End If
        Set rs = gconDMIS.Execute("SELECT A.[DESCRIPTION], A.[ID] FROM AIS_USER_MODULES A " & _
                            " Inner Join " & _
                            " AIS_USERACCESS U ON A.[ID] = U.MODULE_ID " & _
...
Private Sub Save_Modules()
    ...
    gconDMIS.Execute ("Delete from AIS_USERACCESS where userid = " & mUserID & " ")
    For i = 1 To Me.lvwGranted.ListItems.Count
        gconDMIS.Execute ("Insert into AIS_USERACCESS(module_id, userid) values(" & _
