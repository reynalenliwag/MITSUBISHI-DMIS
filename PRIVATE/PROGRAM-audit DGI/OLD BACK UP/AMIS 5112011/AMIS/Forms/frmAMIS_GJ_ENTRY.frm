...
'DESCRIPTION: DISPLAY THE ACCOUNT DESCRIPTION
    ...
    End If
    '---al for GJ country
    Dim rxAR As New ADODB.Recordset
    Set rxAR = New ADODB.Recordset
    If cboJTYPE.Text = "APJ" Then
    '    rxAR.Open "SELECT * FROM AMIS_PV_DETAIL WHERE JTYPE = 'APJ' AND VOUCHERNO = '" & cboCDJNo & "' AND J_CLASS = '" & cboGJAccountNo.Text & "' AND ENTITYCODE = '" &  & "' ", gconDMIS
    ElseIf cboJTYPE.Text = "CDJ" Then
        rxAR.Open "SELECT * FROM AMIS_PV_DETAIL WHERE JTYPE = 'CDJ' AND VOUCHERNO = '" & cboCDJNo & "'", gconDMIS
    End If
    If Not rxAR.EOF And Not rxAR.BOF Then
