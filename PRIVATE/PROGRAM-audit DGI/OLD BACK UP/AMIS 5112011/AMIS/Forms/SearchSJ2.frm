        ...
        Else
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HCI" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "DGI" Then
                ...
                Set rsJournal_HD = gconDMIS.Execute("SELECT DISTINCT JTYPE,VOUCHERNO,JDATE,ACCOUNTNAME,INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,INVOICEAMT,BALANCE " & _
                                                    " FROM (" & _
                                                    " SELECT CASE WHEN LEN(SJVOUCHERNO)=10 THEN LEFT(SJVOUCHERNO,3) ELSE LEFT(SJVOUCHERNO,2) END AS JTYPE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,JDATE,CUSTOMERCODE,CUSTOMERNAME AS ACCOUNTNAME,INVOICETYPE + ' ' + INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT_TOPAY AS INVOICEAMT,AMOUNT_TOPAY - ISNULL((SELECT SUM(INVOICEAMOUNT) FROM AMIS_DETAIL AD WHERE (AR.INVOICETYPE=AD.INVOICETYPE AND AR.INVOICENO=AD.INVOICENO AND AR.CUSTOMERCODE=AD.CUSTOMERCODE)),0) AS BALANCE " & _
                                                    " FROM AMIS_AR AR ) T WHERE BALANCE <> 0 " & _
                                                    " ORDER BY VOUCHERNO ASC ")
            Else
        ...
        If chkCurrentCust.Value = True Then
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HCI" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "DGI" Then
                ...
                Set rsJournal_HD = gconDMIS.Execute("SELECT DISTINCT JTYPE,VOUCHERNO,JDATE,ACCOUNTNAME,INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,INVOICEAMT,BALANCE " & _
                                                    " FROM (" & _
                                                    " SELECT CASE WHEN LEN(SJVOUCHERNO)=10 THEN LEFT(SJVOUCHERNO,3) ELSE LEFT(SJVOUCHERNO,2) END AS JTYPE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,JDATE,CUSTOMERCODE,CUSTOMERNAME AS ACCOUNTNAME,INVOICETYPE + ' ' + INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT_TOPAY AS INVOICEAMT,AMOUNT_TOPAY - ISNULL((SELECT SUM(INVOICEAMOUNT) FROM AMIS_DETAIL AD WHERE (AR.INVOICETYPE=AD.INVOICETYPE AND AR.INVOICENO=AD.INVOICENO AND AR.CUSTOMERCODE=AD.CUSTOMERCODE)),0) AS BALANCE " & _
                                                    " FROM AMIS_AR AR ) T WHERE BALANCE <> 0  AND CUSTOMERCODE = '" & CURRENT_CUSCODE & "' AND VOUCHERNO LIKE '" & Format(Replace(Trim(Me.txtVoucherNo), "'", ""), "000000") & "%' " & _
                                                    " ORDER BY VOUCHERNO ASC ")
            Else
        ...
        Else
            If COMPANY_CODE = "HMH" Or COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "DGI" Then
                ...
                Set rsJournal_HD = gconDMIS.Execute("SELECT DISTINCT JTYPE,VOUCHERNO,JDATE,ACCOUNTNAME,INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,INVOICEAMT,BALANCE " & _
                                                    " FROM (" & _
                                                    " SELECT CASE WHEN LEN(SJVOUCHERNO)=10 THEN LEFT(SJVOUCHERNO,3) ELSE LEFT(SJVOUCHERNO,2) END AS JTYPE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,JDATE,CUSTOMERCODE,CUSTOMERNAME AS ACCOUNTNAME,INVOICETYPE + ' ' + INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT_TOPAY AS INVOICEAMT,AMOUNT_TOPAY - ISNULL((SELECT SUM(INVOICEAMOUNT) FROM AMIS_DETAIL AD WHERE (AR.INVOICETYPE=AD.INVOICETYPE AND AR.INVOICENO=AD.INVOICENO AND AR.CUSTOMERCODE=AD.CUSTOMERCODE)),0) AS BALANCE " & _
                                                    " FROM AMIS_AR AR ) T WHERE BALANCE <> 0  AND VOUCHERNO LIKE '" & Format(Replace(Trim(Me.txtVoucherNo), "'", ""), "000000") & "%' " & _
                                                    " ORDER BY VOUCHERNO ASC ")
            Else
