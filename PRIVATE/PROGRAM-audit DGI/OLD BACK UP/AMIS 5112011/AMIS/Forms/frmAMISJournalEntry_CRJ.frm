...
Function DetailPosting() As Boolean
    ...
    If xJOURNALTYPE = "CRJ" Then
        If Not (txtMRR_No = "AI" Or txtMRR_No = "VI" Or txtMRR_No = "SI" Or txtMRR_No = "PI" Or txtMRR_No = "MI" Or txtMRR_No = "CI" Or txtMRR_No = "UI" Or txtMRR_No = "OTH") Then
        ...
        If AddorEdit = "ADD" Or AddorEdit = "EDIT" Then
            If getJTYPE(RTrim(LTrim(txtINV_No)), RTrim(LTrim(txtMRR_No)), Setacctcode(cboARTag)) = "SJ" Then
            ...
            ElseIf getJTYPEGJ(RTrim(LTrim(txtINV_No)), RTrim(LTrim(txtMRR_No)), Setacctcode(cboARTag)) = "GJ" Then
    ...
    If xJOURNALTYPE = "CRJ" Then
        If Setacctcode(cboARTag.Text) = "11-02002-00" And (COMPANY_CODE = "HGC" Or COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HGH") Then
            ...
            rsCREDIT_CARD.Open "SELECT * " & _
                               ...
                               "WHERE CRJ.INVOICENO = " & N2Str2Null(txtINV_No.Text) & " AND CRJ.INVOICETYPE = " & N2Str2Null(txtMRR_No.Text) & " AND HD.BANK = " & N2Str2Null(GetBankCode(txtINV_No.Text)) & " AND LEFT(DET.ACCT_CODE,5) IN ('11-02','11-03','11-04') AND DET.DEBIT <> 0 AND HD.STATUS = 'P' AND HD.JTYPE = 'CRJ'", gconDMIS, adOpenKeyset
...
'        End If
        ...
        If xJOURNALTYPE = "CRJ" Then
            ...
            Else
                ...
                Else
                    rsGET_SJVOOUCHERNO.Open "SELECT CUSTOMERCODE,CASE WHEN LEN(SJVOUCHERNO)=10 THEN LEFT(SJVOUCHERNO,3) ELSE LEFT(SJVOUCHERNO,2) END AS VOUCHERNO FROM AMIS_AR WHERE INVOICENO = " & RTrim(LTrim(PV_INVNO)) & " AND INVOICETYPE = " & RTrim(LTrim(PV_MRRNO)) & " AND CUSTOMERCODE = '" & RTrim(LTrim(txtCustCode.Text)) & "'", gconDMIS, adOpenKeyset
                End If
                If Not rsGET_SJVOOUCHERNO.EOF And Not rsGET_SJVOOUCHERNO.BOF Then
        ...
        If xJOURNALTYPE = "CRJ" Then
            ...
            Else
                ...
                If txtMRR_No.Text = "CI" Then
                    rsGET_SJVOOUCHERNO2.Open "SELECT CUSTOMERCODE,VOUCHERNO FROM AMIS_JOURNAL_HD WHERE INVOICENO = " & RTrim(LTrim(PV_INVNO)) & " AND INVOICETYPE = " & RTrim(LTrim(PV_MRRNO)) & " AND CUSTOMERCODE = '" & RTrim(LTrim(txtCustCode.Text)) & "'", gconDMIS, adOpenKeyset
                Else
                    rsGET_SJVOOUCHERNO2.Open "SELECT CUSTOMERCODE,CASE WHEN LEN(SJVOUCHERNO)=10 THEN LEFT(SJVOUCHERNO,3) ELSE LEFT(SJVOUCHERNO,2) END AS VOUCHERNO FROM AMIS_AR WHERE INVOICENO = " & RTrim(LTrim(PV_INVNO)) & " AND INVOICETYPE = " & RTrim(LTrim(PV_MRRNO)) & " AND CUSTOMERCODE = '" & RTrim(LTrim(txtCustCode.Text)) & "'", gconDMIS, adOpenKeyset
                End If
                If Not rsGET_SJVOOUCHERNO2.EOF And Not rsGET_SJVOOUCHERNO2.BOF Then
...
Function JournalPosting() As Boolean
    ...
    If MsgBox("Are you sure you want to Post this transaction?", vbQuestion + vbYesNo, "Message") = vbYes Then
        ...
        If xJOURNALTYPE = "CRJ" Then
            ...
            rsMULTI_ACCT_CODE.Open "SELECT COUNT(DISTINCT ACCT_CODE) AS ACCT_CODE_COUNT FROM AMIS_JOURNAL_DET WHERE VOUCHERNO = '" & txtVoucherNo.Text & "' AND JTYPE = 'CRJ' AND LEFT(ACCT_CODE,5) IN ('11-02','11-03','11-04') AND CREDIT <> 0", gconDMIS, adOpenKeyset
                ...
                Else
                    ...
                    Else
                        ...
                        If Not rsAcctCode.EOF And Not rsAcctCode.BOF Then
                            ...
                            ' AND CUSTOMERCODE='" & txtCustCode.Text & "'
                            If Not rsDETAILS_ACCTCODE.EOF And Not rsDETAILS_ACCTCODE.BOF Then
                                xCI = rsDETAILS_ACCTCODE!InvoiceType
                                ...
                                Else
                                    rsSJACCTCODE.Open "SELECT INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE FROM AMIS_AR WHERE INVOICENO = '" & Null2String(rsDETAILS_ACCTCODE!INVOICENO) & "' AND INVOICETYPE='" & Null2String(rsDETAILS_ACCTCODE!InvoiceType) & "' AND CUSTOMERCODE = '" & Null2String(rsDETAILS_ACCTCODE!CustomerCode) & "' AND ACCOUNT_CODE = '" & rsDETAILS_ACCTCODE!J_CLASS & "'", gconDMIS, adOpenForwardOnly
                                End If
            ...
            If Not rsACCT_CODE.EOF And Not rsACCT_CODE.BOF Then
                Do While Not rsACCT_CODE.EOF
                    ...
                    Else
                        If (xCI = "CI" Or txtMRR_No.Text = "CI") And CheckIfBank(txtCustCode.Text) = False Then
                        ...
                        Else
                            rsINVALID.Open "SELECT * FROM AMIS_AR " & _
                                           "Where INVOICENO = '" & Null2String(rsACCT_CODE!INVOICENO) & "' AND INVOICETYPE = '" & Null2String(rsACCT_CODE!InvoiceType) & "' AND CUSTOMERCODE = '" & rsACCT_CODE!CustomerCode & "' AND ACCOUNT_CODE='" & rsACCT_CODE!J_CLASS & "'", gconDMIS, adOpenKeyset
                        End If
                        'AND HD.STATUS = 'P'
                    End If
...
'UPDATED BY: JUN --- DATE UPDATED: 11/19/2009 --- DESCRIPTION: THIS IS TO GET THE AR IN CRJ MOSTLY ARE A/R CREDIT CARD TRANSACTION
    ...
    Else
        ...
        rsNO_REFERENCE.Open "SELECT DISTINCT DT.ACCT_CODE,HD.JTYPE + '-' + DT.VOUCHERNO AS SJVOUCHERNO,DT.VOUCHERNO,HD.JDATE,HD.JTYPE,HD.CUSTOMERCODE,INVOICEDATE,HD.INVOICENO,HD.INVOICETYPE, " & _
                            ...
                            "ON HD.VOUCHERNO = DT.VOUCHERNO AND HD.JTYPE = DT.JTYPE WHERE LEFT(ACCT_CODE,5) IN ('11-02','11-03','11-04') " & _
