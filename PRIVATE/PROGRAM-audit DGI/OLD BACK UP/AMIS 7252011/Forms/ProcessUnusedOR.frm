...
Private Sub cmdPrint_Click()
    ...
    Set rsINVOICES = New ADODB.Recordset
    If cboORType.Text = "VAT" Then
        Set rsINVOICES = gconDMIS.Execute("Select invoiceno,jno,VOUCHERNO from AMIS_Journal_HD where jtype = 'CRJ' and left(invoiceno,2) <> 'NV' and jdate >= '" & dtpFrom & "' and jdate <= '" & dtpTo & "' order by invoiceno asc")
    Else
        Set rsINVOICES = gconDMIS.Execute("Select invoiceno,jno,VOUCHERNO from AMIS_Journal_HD where status = 'P' and jtype = 'CRJ' and left(invoiceno,2) = 'NV' and jdate >= '" & dtpFrom & "' and jdate <= '" & dtpTo & "' order by invoiceno asc")
    End If
