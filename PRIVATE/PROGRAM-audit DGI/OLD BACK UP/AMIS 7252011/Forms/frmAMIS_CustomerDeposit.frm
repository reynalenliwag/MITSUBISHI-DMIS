...
Sub LOAD_VENDOR()
    ...
    rsLOAD_VENDOR.Open "SELECT DISTINCT TOP 22 VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenKeyset
...
Sub LOAD_AP_ENTITY()
    ...
    If cboAR_ACCT_CODE.Text = "ALL" Then
        rsLOAD_AP.Open "SELECT JDATE,VOUCHERNO AS APVOUCHERNO,ISNULL(INVOICETYPE,'') + CASE WHEN INVOICETYPE IS NULL THEN '' ELSE '-' END + ISNULL(INVOICENO,'') AS INVOICE,AMOUNT2PAY,AMOUNTPAID,INVOICENO,INVOICETYPE,VENDOR_CODE,AP.ACCT_CODE,RIGHT(VOUCHERNO,6) AS VOUCHERNO FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME WHERE AP.VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTO.Value & "' ORDER BY JDATE,VOUCHERNO", gconDMIS, adOpenKeyset
    Else
        rsLOAD_AP.Open "SELECT JDATE,VOUCHERNO AS APVOUCHERNO,ISNULL(INVOICETYPE,'') + '-' + ISNULL(INVOICENO,'') AS INVOICE,AMOUNT2PAY,AMOUNTPAID,INVOICENO,INVOICETYPE,VENDOR_CODE,AP.ACCT_CODE,RIGHT(VOUCHERNO,6) AS VOUCHERNO FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME WHERE AP.VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTO.Value & "' AND " & _
...
Sub rsRefresh()
    ...
    rsREF.Open "SELECT DISTINCT VENDOR_NAME,VENDOR_CODE FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2= 'DEPOSIT' ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenForwardOnly, adLockReadOnly
End Sub
