   ...
   Begin TabDlg.SSTab JournalTAB 
      ...
      Begin VB.PictureBox fraAddJournal 
         ...
         Begin VB.CommandButton cmdJournalDelete 
            ...
            Height          =   795
            Left            =   60
            MouseIcon       =   "frmAMISJournalEntry_APJ.frx":1B41
            MousePointer    =   99  'Custom
            Picture         =   "frmAMISJournalEntry_APJ.frx":1C93
            Style           =   1  'Graphical
            TabIndex        =   127
            Top             =   765
            Width           =   705
         ...
         Begin VB.Frame Frame2 
            ...
            Begin RichTextLib.RichTextBox txtAcct_Name 
               ...
               Enabled         =   -1  'True
               MultiLine       =   0   'False
               TextRTF         =   $"frmAMISJournalEntry_APJ.frx":1FBE
               BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
                  Name            =   "Verdana"
                  Size            =   9
                  Charset         =   0
         ...
         Begin VB.CommandButton cmdJournalSave 
            ...
            Height          =   795
            Left            =   7620
            MouseIcon       =   "frmAMISJournalEntry_APJ.frx":2051
            MousePointer    =   99  'Custom
            Picture         =   "frmAMISJournalEntry_APJ.frx":21A3
            Style           =   1  'Graphical
            TabIndex        =   143
            Top             =   765
            Width           =   705
...
Function SetVendorCode(VVV As Variant)
    Set rsVENDOR = New ADODB.Recordset
    rsVENDOR.Open "Select code,nameofvendor from ALL_VENDOR_TABLE where nameofvendor = " & N2Str2Null(VVV), gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not rsVENDOR.EOF And Not rsVENDOR.BOF Then
    ...
    If Not rsATC.EOF And Not rsATC.BOF Then
        ...
        If NumericVal(txtRATE.Text) > 0 Then
            If COMPANY_CODE = "HCC" Then
                Dim Xinput                             As New ADODB.Recordset
                Set Xinput = New ADODB.Recordset
                Xinput.Open "select * from amis_journal_det where jtype = '" & xJOURNALTYPE & "' and voucherno = '" & txtVoucherNo & "' and acct_code = '11-07000-00'", gconDMIS
                If Xinput.RecordCount = 1 Then
                    txtCredit.Text = Round((NumericVal(txtTotDebit.Text) / 1.12) * (NumericVal(txtRATE.Text) / 100), 2)
                    xTOTAL = (NumericVal(txtTotDebit.Text) - txtCredit.Text)
                    Set RSATCRTE = New ADODB.Recordset
                    RSATCRTE.Open "update amis_journal_det set credit = ' " & xTOTAL & " ' where voucherno = '" & txtVoucherNo & "' and  jtype = 'APJ'  and left(acct_code,5) in ('21-01','21-02') and acct_code in (select acctcode from amis_chartaccount where is_schedule_accnt = 1)", gconDMIS
                Else
                    txtCredit.Text = Round((NumericVal(txtTaxBase.Text) / 1.12) * (NumericVal(txtRATE.Text) / 100), 2)
                    '                xTOTAL = (NumericVal(txtTaxBase.Text) - txtCredit.Text)
                    xTOTAL = (NumericVal(txtAmountToPay.Text))    '- txtCredit.Text)
                    Set RSATCRTE = New ADODB.Recordset
                    RSATCRTE.Open "update amis_journal_det set credit = ' " & xTOTAL & " ' where voucherno = '" & txtVoucherNo & "' and  jtype = 'APJ'  and left(acct_code,5) in ('21-01','21-02') and acct_code in (select acctcode from amis_chartaccount where is_schedule_accnt = 1)", gconDMIS
                End If
            Else
...
Function DetailPosting() As Boolean
    ...
    If AddorEdit = "ADD" Then
        ...
        If xJOURNALTYPE = "APJ" Then
            ...
            NEW_LogAudit "AA", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, labID.Caption, Null2String(PV_MRRNO) & "-" & Null2String(PV_INVNO), txtVoucherNo, xJOURNALTYPE, txtJNo
        End If
    Else
        ...
        If xJOURNALTYPE = "APJ" Then
            ...
            gconDMIS.Execute SQL_STATEMENT
            NEW_LogAudit "AA", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, labID.Caption, "", txtVoucherNo, xJOURNALTYPE, txtJNo
        End If
    End If                                            ' end of ADD
...
Private Sub cmdAddJournal_Click()
    ...
    AddorEdit = "ADD"
    InitJournal
    '    frmAMISJournalEntries.Caption = frmAMISJournalEntry_APJ.Caption
    '    Call frmAMISJournalEntries.LoadJournal("APJ")
...
Private Sub cmdJournalDelete_Click()
    ...
    If MsgBox("Delete This Journal, Are you Sure?", vbQuestion + vbYesNo, "Delete Journal Entry") = vbYes Then
        ...
        NEW_LogAudit "XX", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, labID.Caption, xACCT_CODE, txtVoucherNo, xJOURNALTYPE, txtJNo
    End If
    Dim cnt                                            As Integer
...
Function JournalEntries() As Boolean
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute SQL_STATEMENT
        NEW_LogAudit "AA", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, labID.Caption, Null2String(J_ACCT_CODE), txtVoucherNo, xJOURNALTYPE, txtJNo
        
        If cboAcct_Code.Text = DEALER_ITW_EXPANDED Then
            SQL_STATEMENT = "UPDATE AMIS_JOURNAL_DET SET CREDIT = '" & NumericVal(txtTotDebit.Text) - NumericVal(txtCredit.Text) & "' WHERE VOUCHERNO='" & txtVoucherNo.Text & "' AND JTYPE='" & xJOURNALTYPE & "' AND LEFT(ACCT_CODE,5) IN ('21-01','21-02','21-07') AND ACCT_CODE IN (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1)"
            gconDMIS.Execute SQL_STATEMENT
        End If
    ...
    If MsgBox("Are you sure you want to Post this transaction?", vbQuestion + vbYesNo, "Message") = vbYes Then
        ...
        If xJOURNALTYPE = "APJ" Then
            ...
            If COMPANY_CODE = "HMH" Then
                Set rsCheckDetails = gconDMIS.Execute("Select Acct_Code, Credit from AMIS_Journal_Det DET INNER JOIN AMIS_CHARTACCOUNT AC ON DET.ACCT_CODE=AC.ACCTCODE Where LEFT(AC.ACCTCODE,2)='21' and Jtype = '" & xJOURNALTYPE & "' and VoucherNo = " & N2Str2Null(txtVoucherNo.Text))    'AND IS_SCHEDULE_ACCNT=1
            Else
...
Private Sub cmdSave_Click()
    ...
    If AddorEdit = "ADD" Then
        ...
        NEW_LogAudit "A", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, labID.Caption, "", txtVoucherNo, xJOURNALTYPE, txtJNo
    Else
        SQL_STATEMENT = "update AMIS_Journal_HD set" & _
                        " jdate = " & J_JDATE & "," & _
                        " voucherno = " & J_VOUCHERNO & "," & _
...
Function VendorATC(XXX As String) As String
    ...
    rsVendorATC.Open "SELECT ATC FROM ALL_VENDOR_TABLE WHERE CODE='" & txtCode.Text & "'", gconDMIS, adOpenForwardOnly
