...
Private Sub Form_Load()
    ...
    '== Get Modules list ==
    Set rs = gconDMIS.Execute("select description,id from CMIS_User_Modules order by id")
    ...
    Set rs = gconDMIS.Execute("SELECT A.[DESCRIPTION], A.[ID] FROM CMIS_USER_MODULES A " & _
                            " Inner Join " & _
                            " CMIS_USERACCESS U ON A.[ID] = U.MODULE_ID " & _
...
Private Sub Save_Modules()
    ...
    gconDMIS.Execute ("Delete from CMIS_USERACCESS where userid = " & mUserID & " ")
    For i = 1 To Me.lvwGranted.ListItems.Count
        gconDMIS.Execute ("Insert into CMIS_USERACCESS(module_id, userid) values(" & _
