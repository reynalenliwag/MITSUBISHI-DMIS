...
Begin VB.Form frmPMISReports_Receipts_MAT 
   AutoRedraw      =   -1  'True
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   795
      Left            =   2460
      MouseIcon       =   "MAT_Receipts.frx":0E42
      MousePointer    =   99  'Custom
      Picture         =   "MAT_Receipts.frx":0F94
      Style           =   1  'Graphical
      TabIndex        =   7
   ...
   Begin VB.CommandButton cmdPrint 
      ...
      Height          =   795
      Left            =   1740
      MouseIcon       =   "MAT_Receipts.frx":13DF
      MousePointer    =   99  'Custom
      Picture         =   "MAT_Receipts.frx":1531
      Style           =   1  'Graphical
      TabIndex        =   8
   ...
   End
End
Attribute VB_Name = "frmPMISReports_Receipts_MAT"
...
Private Sub cmdPrint_Click()
    ...
    RSREC_HIST.Open "select RRdate from PMIS_Rec_Hist where TYPE = 'M' AND month(RRDate) = " & What_month(cboMonth) & " AND year(RRDate) =" & cboYear.Text, gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not RSREC_HIST.EOF And Not RSREC_HIST.EOF Then
        ...
        If cboSupplier.Text = "ALL SUPPLIERS" Then
            ...
            PrintSQLReport rptReceipts, PMIS_REPORT_PATH & "receipts.rpt", "{RR_HD.TYPE} = 'M' AND month({RR_HD.RRDate}) = " & What_month(cboMonth.Text) & " AND year({RR_HD.RRDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
            PrintSQLReport rptReceipts, PMIS_REPORT_PATH & "receiptsum.rpt", "{RR_HD.TYPE} = 'M' AND month({RR_HD.RRDate}) = " & What_month(cboMonth.Text) & " AND year({RR_HD.RRDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
        Else
            rptReceipts.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptReceipts.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptReceipts, PMIS_REPORT_PATH & "receipts.rpt", "{RR_HD.TYPE} = 'M' and {RR_HD.RECVD_CODE} = '" & txtSupCode.Text & "' AND month({RR_HD.RRDate}) = " & What_month(cboMonth.Text) & " AND year({RR_HD.RRDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
            PrintSQLReport rptReceipts, PMIS_REPORT_PATH & "receiptsum.rpt", "{RR_HD.TYPE} = 'M' and {RR_HD.RECVD_CODE} = '" & txtSupCode.Text & "' AND month({RR_HD.RRDate}) = " & What_month(cboMonth.Text) & " AND year({RR_HD.RRDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
        End If
        Call NEW_LogAudit("V", "MATERIALS MONTHLY REPORT", "", "", "", cboSupplier & " " & cboMonth & " " & cboYear, "", "")
