   ...
   Begin VB.Frame fraAddTran 
      ...
      Begin VB.Frame fraUpdateMaster 
         ...
         Begin VB.CommandButton cmdOKUpdate 
            ...
            Height          =   555
            Left            =   4260
            MouseIcon       =   "Receiving.frx":61FC
            MousePointer    =   99  'Custom
            Picture         =   "Receiving.frx":634E
            Style           =   1  'Graphical
            TabIndex        =   89
            Top             =   1470
            Width           =   555
...
Private Sub cmdPost_Click()
    ...
    If MsgQuestionBox("Are you sure you want to Post this Transactions?", "Post Transactions") = True Then
        ...
        If Not RSTDAYTRAN.EOF And Not RSTDAYTRAN.BOF Then
            ...
            Do While Not RSTDAYTRAN.EOF
                Set RSPARTMAS = New ADODB.Recordset
                RSPARTMAS.Open "Select ID,STOCKNO,onhand,trecqty,onorder,served,receipts,ACTIVE from PMIS_PARTMAS where TYPE = " & MODULE_STOCK_TYPE & " AND STOCKNO = " & N2Str2Null(RSTDAYTRAN!STOCK_ORD), gconDMIS
                If Not RSPARTMAS.EOF And Not RSPARTMAS.BOF Then
    ...
    If COMPANY_CODE = "HLU" Then
        ...
        If Trim(txtRecvd_Code.Text) = "H00001" Then
            ...
            gconDMIS.Execute ("Update pmis_stockmas set " & _
                            " LOC_NUMBER = " & N2Str2Null(txtloc_num.Text) & ", " & _
                            " srp = " & NumericVal(txtNewSRP.Text) & _
                            " where stockno = '" & LTrim(RTrim(cboTranPartNo)) & _
                              "' and [type] = 'P'")
...
Private Sub cmdUnPost_Click()
    ...
    'DESCRIPTION:
        ...
        Else
            sqlcommand = "select id,itemno,trantype,tranno,STOCK_ORD,tranqty,traninvamt,status from PMIS_TdayTran where [TYPE] = 'P' AND trantype = 'RR' and tranno = " & N2Str2Null(rsRR_HD!RRNO) & " order by itemno asc"
        End If
    '------------------------------------------------------------------------------------------------------------------
...
Private Sub cmdSave_Click()
    ...
    If cboClasscode.ListIndex = 1 Then
        If cboTerms.Text = 0 Then
            ...
            Exit Sub
        End If
    End If    'VALIDATION FOR transaction number
...
'---------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        If AddorEdit = "ADD" Then
            If checkdup_INVO("P", txtINVNo.Text) = True Then
...
'description: To desplay loc number for HLU dealer only
    ...
    If COMPANY_CODE = "HLU" Then
        sqlcommand = "select STOCKNO,tpoqty,onorder,mac,dnp,srp,onhand,NON_HARI,PARTSORIGIN,stocktype,LOC_NUMBER,non_hari,isnull(genuine,'N') as genuine from PMIS_PARTMAS where STOCKNO = " & N2Str2Null(cboTranPartNo.Text)
    Else
        sqlcommand = "select STOCKNO,tpoqty,onorder,mac,dnp,srp,onhand,NON_HARI,PARTSORIGIN,stocktype from PMIS_PARTMAS where STOCKNO = " & N2Str2Null(cboTranPartNo.Text)
    End If
'--------------------------------------------------------------------------------------
       ...
       ' txtNewMAC.Text = Format(ComputeTransactionMac(cboTranPartNo, txtTranQty, txtUnitCost, txtRRDate), MAXIMUM_DIGIT)
        ...
        If Trim(cboTranPartNo.Text) <> "" Then
            ...
            gconDMIS.Execute "insert into PMIS_PARTMAS " & _
                ...
                " ('P' " & _
                ", " & N2Str2Null(cboTranPartNo.Text) & _
                ", " & NewPmasMAC & _
                ", " & NewPmasDNP & _
                ", " & NewPmasSRP & _
