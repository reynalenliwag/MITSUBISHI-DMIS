...
Begin VB.Form frmPMISReports_Issuances_AC 
   ...
   Begin VB.CommandButton cmdCancel 
      ...
      Height          =   795
      Left            =   1530
      MouseIcon       =   "AC_Issuances.frx":0E42
      MousePointer    =   99  'Custom
      Picture         =   "AC_Issuances.frx":0F94
      Style           =   1  'Graphical
      TabIndex        =   5
   ...
   Begin VB.CommandButton cmdPrint 
      ...
      Height          =   795
      Left            =   810
      MouseIcon       =   "AC_Issuances.frx":13DF
      MousePointer    =   99  'Custom
      Picture         =   "AC_Issuances.frx":1531
      Style           =   1  'Graphical
      TabIndex        =   6
   ...
   End
End
Attribute VB_Name = "frmPMISReports_Issuances_AC"
...
Private Sub cmdPrint_Click()
    ...
    If Not RSORD_HIST.EOF And Not RSORD_HIST.EOF Then
        ...
        If ISSREPTYPE = "RIV_INPROCESS" Then
            ...
            RIV_Filter1 = "{ORD_HD.TYPE} = 'A' AND (({ORD_HD.TRANDATE} <= DATESERIAL(" & cboYear.Text & "," & What_month(cboMonth.Text) & "," & Day(LastDate) & "))"
            RO_Filter1 = "({repor.DTE_REL} > DATESERIAL(" & cboYear.Text & "," & What_month(cboMonth.Text) & "," & Day(LastDate) & ")))"

            RIV_Filter2 = "{ORD_HD.TYPE} = 'A' AND (({ORD_HD.TRANDATE} <= DATESERIAL(" & cboYear.Text & "," & What_month(cboMonth.Text) & "," & Day(LastDate) & "))"
        ...
        Else
            ...
            rptIssuances.Formulas(0) = "CompanyName = '" & COMPANY_NAME & "'"
            rptIssuances.Formulas(1) = "CompanyAddress = '" & COMPANY_ADDRESS & "'"
            PrintSQLReport rptIssuances, PMIS_REPORT_PATH & "Issuances.rpt", "{Ord_hd.TYPE} = 'A' AND month({Ord_hd.TranDate}) = " & What_month(cboMonth.Text) & " AND year({Ord_hd.TranDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
            PrintSQLReport rptIssuances, PMIS_REPORT_PATH & "Issuancesum.rpt", "{Ord_hd.TYPE} = 'A' AND month({Ord_hd.TranDate}) = " & What_month(cboMonth.Text) & " AND year({Ord_hd.TranDate}) = " & cboYear.Text, DMIS_REPORT_Connection, 1
