...
Sub FillGrid()
    ...
    Set RSPO_HD = New ADODB.Recordset
    Set RSPO_HD = gconDMIS.Execute("select pono,supname from PMIS_PO_Hist WHERE [TYPE] = 'A' order by pono asc")
...
Sub FillSearchGrid(XXX As String)
    ...
    Set RSPO_HD = gconDMIS.Execute("select pono from PMIS_PO_Hist where [TYPE] = 'A' AND pono like'" & XXX & "%'")
    If Not (RSPO_HD.EOF And RSPO_HD.BOF) Then
...
Private Sub lstPO_HD_GotFocus()
    If optPONo.Value = True Then
        RSPO_HD.Find = "pono=" & lstPO_HD.SelectedItem
    Else
