...
Private Sub cboRefPRSNo_GotFocus()
    ...
    ElseIf COUNTERTYPE = "CSH" Or COUNTERTYPE = "CHG" Or COUNTERTYPE = "DR" Then
        rsPRS.Open "Select tranno,refpisno from PMIS_vw_PRS WHERE [TYPE] = 'M' and (SALES_ORIGIN ='W' or SALES_ORIGIN ='O' OR SALES_ORIGIN ='M' or SALES_ORIGIN = 'J') order by Tranno asc", gconDMIS, adOpenForwardOnly, adLockReadOnly
    End If
...
Private Sub cmdTranSave_Click()
    ...
    If txtTranType.Text <> "ADB" Then
        Set RSPARTMAS = New ADODB.Recordset
        RSPARTMAS.Open "Select isnull(srp,0) as srp,STOCKNO,onhand,sstock,resservice,TISSQTY,Mac,issuances from CSMS_MATMAS where STOCKNO = '" & cboTranPartNo.Text & "' AND ACTIVE = 'Y'", gconDMIS
        If Not RSPARTMAS.EOF And Not RSPARTMAS.BOF Then
            CURONHAND = N2Str2IntZero(RSPARTMAS!ONHAND)
            CURSAFESTOCK = N2Str2IntZero(RSPARTMAS!SSTOCK)
            CURTISSQTY = N2Str2IntZero(RSPARTMAS!TISSQTY)
