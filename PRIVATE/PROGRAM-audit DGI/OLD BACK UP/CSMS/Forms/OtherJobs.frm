...
Private Sub txtKeyword_Change()
    ...
    Set RSUPLOAD = New ADODB.Recordset
    Set RSUPLOAD = gconDMIS.Execute("Select TOP 50 OPERATING_CODE,DESCRIPTION,OPERATING_TIME,FLATRATE,OPERATING_CODE from FLATRATE_OTHJOBS WHERE MAIN_CAT = " & JOB_GROUP & " AND DESCRIPTION LIKE '" & Replace(txtKeyword.Text, "'", "''") & "%' Order by OPERATING_CODE Asc")
    lstJObs.Sorted = False: lstJObs.ListItems.Clear
    If Not RSUPLOAD.EOF And Not RSUPLOAD.BOF Then
        Listview_Loadval Me.lstJObs.ListItems, RSUPLOAD
        lstJObs.Enabled = True
