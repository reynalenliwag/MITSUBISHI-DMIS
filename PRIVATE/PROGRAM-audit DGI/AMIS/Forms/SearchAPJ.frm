...
Private Sub txtVoucherNo_Change()
    ...
    Else
        ...
        If chkShowAll.Value = 1 Then
            ...
            Set rsJournal_HD = gconDMIS.Execute("select HD.VoucherNo,HD.JDate,AE.ACCOUNTNAME,PV.inv_no from AMIS_Journal_HD HD inner join All_ENTITY AE on HD.vendorcode = AE.code AND HD.ENTITY_CLASS=AE.ENTITYCODE left outer join amis_pv_detail PV on HD.VoucherNo= PV.voucherno AND HD.JTYPE=PV.JTYPE where HD.Jtype='APJ' and HD.status = '" & StatusToSearch & "' and HD.VoucherNo like '" & Format(Trim(Replace(Me.txtVoucherNo, "'", "")), "000000") & "%' order by HD.VoucherNo asc")
        Else
            'Set rsJournal_HD = gconDMIS.Execute("select TOP 18 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDate,All_VENDOR.nameofvendor,amis_pv_detail.inv_no from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code left outer join amis_pv_detail on AMIS_Journal_Hd.VoucherNo= amis_pv_detail.voucherno AND AMIS_Journal_Hd.JTYPE=amis_pv_detail.JTYPE where AMIS_Journal_Hd.Jtype='APJ' and AMIS_Journal_Hd.status = '" & StatusToSearch & "' and AMIS_Journal_Hd.VoucherNo like '" & Format(Trim(Replace(Me.txtVoucherNo, "'", "")), "000000") & "%' order by AMIS_Journal_Hd.VoucherNo asc")
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 HD.VoucherNo,HD.JDate,AE.ACCOUNTNAME,PV.inv_no from AMIS_Journal_HD HD inner join All_ENTITY AE on HD.vendorcode = AE.code AND HD.ENTITY_CLASS = AE.ENTITYCODE left outer join amis_pv_detail PV on HD.VoucherNo= PV.voucherno AND HD.JTYPE=PV.JTYPE where HD.Jtype='APJ' and HD.status = '" & StatusToSearch & "' and HD.VoucherNo like '" & Format(Trim(Replace(Me.txtVoucherNo, "'", "")), "000000") & "%' order by HD.VoucherNo asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_HD
            ListVoucherNo.Enabled = True
...
Private Sub txtVendorPayeeName_Change()
    ...
    Else
        ...
        If chkShowAll2.Value = 1 Then
            ...
            Set rsJournal_HD = gconDMIS.Execute("SELECT AE.ACCOUNTNAME,HD.JDATE,HD.VOUCHERNO,PV.INV_NO FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AE ON HD.VENDORCODE = AE.CODE AND HD.ENTITY_CLASS = AE.ENTITYCODE LEFT OUTER JOIN AMIS_PV_DETAIL PV ON HD.VOUCHERNO= PV.VOUCHERNO AND HD.JTYPE=PV.JTYPE WHERE HD.JTYPE='APJ' AND HD.STATUS = '" & StatusToSearch & "' and AE.ACCOUNTNAME like '" & Trim(Me.txtVendorPayeeName) & "%' order by AE.ACCOUNTNAME asc")
        Else
            ...
            Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 18 AE.ACCOUNTNAME,HD.JDATE,HD.VOUCHERNO,PV.INV_NO FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AE ON HD.VENDORCODE = AE.CODE AND HD.ENTITY_CLASS = AE.ENTITYCODE LEFT OUTER JOIN AMIS_PV_DETAIL PV ON HD.VOUCHERNO= PV.VOUCHERNO AND HD.JTYPE=PV.JTYPE WHERE HD.JTYPE='APJ' AND HD.STATUS = '" & StatusToSearch & "' and AE.ACCOUNTNAME like '" & Replace(Trim(Me.txtVendorPayeeName), "'", "") & "%' order by AE.ACCOUNTNAME asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVendorPayeeName.ListItems, rsJournal_HD
            ListVendorPayeeName.Enabled = True
...
Private Sub txtInvoiceNo_Change()
    ...
    Else
        ...
        If chkShowAll3.Value = 1 Then
            ...
            Set rsJournal_HD = gconDMIS.Execute("select PV.inv_no,PV.PO_NO,AE.ACCOUNTNAME,HD.JDate,HD.VoucherNo from AMIS_Journal_HD HD inner join ALL_ENTITY AE on HD.vendorcode = AE.code AND HD.ENTITY_CLASS = AE.ENTITYCODE left outer join amis_pv_detail PV on HD.VoucherNo= PV.voucherno AND HD.JTYPE=PV.JTYPE where HD.Jtype='APJ' and HD.status = '" & StatusToSearch & "' and PV.INV_No like '" & Trim(Me.txtInvoiceNo) & "%' order by PV.INV_No asc")
        Else
            Set rsJournal_HD = gconDMIS.Execute("select TOP 18 PV.inv_no,PV.PO_NO,AE.ACCOUNTNAME,HD.JDate,HD.VoucherNo from AMIS_Journal_HD HD inner join ALL_ENTITY AE on HD.vendorcode = AE.code AND HD.ENTITY_CLASS = AE.ENTITYCODE left outer join amis_pv_detail PV on HD.VoucherNo= PV.voucherno AND HD.JTYPE=PV.JTYPE where HD.Jtype='APJ' and HD.status = '" & StatusToSearch & "' and PV.INV_No like '" & Replace(Trim(Me.txtInvoiceNo), "'", "") & "%' order by PV.INV_No asc")
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListInvoiceNo.ListItems, rsJournal_HD
            ListInvoiceNo.Enabled = True
