...
'UPDATED BY: JUN --- DATE UPDATED: 11/19/2009 --- DESCRIPTION: THIS IS TO GET THE AR IN CRJ MOSTLY ARE A/R CREDIT CARD TRANSACTION
    ...
    If Not rsGET_AR_CRJ.EOF And Not rsGET_AR_CRJ.BOF Then
        ...
        If N2Str2Null(rsGET_AR_CRJ!ACCT_CODE) <> xCARD_CODE Then
            ...
            '            AMIS_AR.Open "SELECT * FROM AMIS_AR WHERE INVOICENO=" & xInvoiceNo & " AND INVOICETYPE=" & xInvoiceType & " AND ACCOUNT_CODE = " & xACCT_CODE & " AND CUSTOMERCODE = " & XCustomerCode & " AND STATUS='N'", gconDMIS, adOpenForwardOnly
            '            If Not AMIS_AR.EOF And Not AMIS_AR.BOF Then
            '                gconDMIS.Execute "DELETE FROM AMIS_AR WHERE INVOICENO=" & xInvoiceNo & " AND INVOICETYPE=" & xInvoiceType & " AND ACCOUNT_CODE = " & xACCT_CODE & " AND CUSTOMERCODE = " & XCustomerCode & " AND STATUS='N'"
            '            End If
            '            Set AMIS_AR = Nothing
...
Function CheckIFinSJ(JTYPE As String, VOUCHERNO As String) As Boolean
    ...
    rsJournalDTCRJ.Open "SELECT ACCT_CODE FROM AMIS_JOURNAL_DET DT INNER JOIN AMIS_CHARTACCOUNT AC ON DT.ACCT_CODE=AC.ACCTCODE WHERE DT.JTYPE = '" & JTYPE & "' AND DT.VOUCHERNO = '" & VOUCHERNO & "' AND IS_SCHEDULE_ACCNT=1 AND LEFT(ACCT_CODE,5) IN ('11-02','11-03','11-04') AND CREDIT > 0", gconDMIS, adOpenForwardOnly
