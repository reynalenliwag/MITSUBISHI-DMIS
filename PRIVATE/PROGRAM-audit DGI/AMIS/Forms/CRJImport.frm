...
Begin VB.Form frmCRJImport 
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin VB.ComboBox cboMonth 
         ...
         EndProperty
         Height          =   330
         ItemData        =   "CRJImport.frx":26A1
         Left            =   780
         List            =   "CRJImport.frx":26A3
         Style           =   2  'Dropdown List
         TabIndex        =   18
         Top             =   30
         Width           =   1455
   ...
   Begin wizProgBar.Prg progCPB 
      ...
      _ExtentX        =   8017
      _ExtentY        =   556
      Picture         =   "CRJImport.frx":3879
      ForeColor       =   255
      Appearance      =   2
      BorderStyle     =   2
      BarPicture      =   "CRJImport.frx":3895
      ShowText        =   -1  'True
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "MS Sans Serif"
         Size            =   8.25
...
Attribute DEF_INVOICETYPE.VB_VarUserMemId = 1073938554
Dim DEF_INVOICENO                                           As String
Attribute DEF_INVOICENO.VB_VarUserMemId = 1073938555
Dim xCUT_OFF_DATE                                           As String
Attribute xCUT_OFF_DATE.VB_VarUserMemId = 1073938556
    ...
    For GridImport = 1 To Grid1.Rows - 1
        If N2Str2Zero(Grid1.Cell(GridImport, 1).Text) = 0 Then
            ...
            If Not rsOFF_HD.EOF And Not rsOFF_HD.BOF Then
                'If rsOFF_HD!OR_NUM = "056448" Then Stop
                ...
                If Not rsOFF_DT.EOF And Not rsOFF_DT.BOF Then
                    ...
                    Do While Not rsOFF_DT.EOF
                        ...
                        If Null2String(rsOFF_DT!TranType) = "OTH" Then
                            'SetTransaction(Null2String(rsOFF_DT!TranType))
                ...
                If CMIS_TYPE_PAYMENT = "1" Or CMIS_TYPE_PAYMENT = "2" Then
                    ...
                    If J_INVOICENUM <> "" Then
                        ...
                        'rsCheckDeposit.Open "Select * from CMIS_Deposits where InvoiceNo = '" & J_INVOICENUM & "' and cuscde = " & J_CUSTOMERCODE & " And InvoiceType = '" & J_INVOICETYPE2 & "'", gconDMIS, adOpenForwardOnly
                        rsCheckDeposit.Open "Select DP.PAIDFOR,DT.AMOUNT from CMIS_DepositDT DT INNER JOIN CMIS_DEPOSITS DP ON DP.ID=DT.DEPOSIT_ID where DT.InvoiceNo = '" & J_INVOICENUM & "' and DP.cuscde = " & J_CUSTOMERCODE & " And DT.InvoiceType = '" & J_INVOICETYPE2 & "'", gconDMIS, adOpenForwardOnly
                ...
                If CMIS_TYPE_PAYMENT = "3" Then
                    ...
                    ElseIf COMPANY_CODE = "DGI" Or COMPANY_CODE = "HCA" Then
                        J_ACCT_CODE = N2Str2Null(ReturnAccountCode("CARD ON HAND"))
                        J_ACCT_NAME = N2Str2Null(Setacctname(ReturnAccountCode("CARD ON HAND")))
                    Else
                ...
                If Not rsOFF_DT.EOF And Not rsOFF_DT.BOF Then
                    ...
                    Do While Not rsOFF_DT.EOF
                        ...
                        If UCase(CMIS_DT_PAIDFOR) = "412S" Or UCase(CMIS_DT_PAIDFOR) = "412P" Or UCase(CMIS_DT_PAIDFOR) = "412V" Or UCase(CMIS_DT_PAIDFOR) = "415" Or UCase(CMIS_DT_PAIDFOR) = "416" Then
                        ...
                        If Not rsSJ_DATA.EOF And Not rsSJ_DATA.BOF Then
                            ...
                            'PV_ACCTCODE = N2Str2Null(rsSJ_DATA!ACCT_CODE)
                            J_JVOUCHERNO = J_VOUCHERNO
                            PV_ITEMNO = N2Str2Null(Format(SJ_PV_ITEMNO, "0000"))
                            PV_INVNO = N2Str2Null(CMIS_DT_REFERENCE)    ' NO
                        ...
                        If CMIS_DT_TRANTYPE = "SI" Then
                            If COMPANY_CODE = "DGI" Then
                                ...
                                rsDEFERRED.Open "SELECT DT.CREDIT FROM AMIS_JOURNAL_HD HD " & _
                                                ...
                                                "WHERE HD.INVOICENO=" & DEF_INVOICENO & " AND HD.INVOICETYPE=" & DEF_INVOICETYPE & " AND DT.ACCT_CODE='21-05001-00'", gconDMIS, adOpenForwardOnly
                                If Not rsDEFERRED.EOF And Not rsDEFERRED.BOF Then
                                    DEFERRED_OUTPUT = NumericVal(rsDEFERRED!Credit)
                                End If
                                Set rsDEFERRED = Nothing
                                ...
                                SQL_STATEMENT = "insert into AMIS_Journal_Det " & _
                                                "(jdate,voucherno,jtype,jno,jitemno,acct_code,acct_name,debit,credit,tax,grossamt,netamt,status,Entity)" & _
                                                " values (" & J_JDATE & ", " & J_VOUCHERNO & ", " & J_JTYPE & _
                                                ", " & J_JNO & ", " & J_JITEMNO & ", " & J_ACCT_CODE & ", " & J_ACCT_NAME & ", " & Round((J_DEBIT / 1.12) * 0.12, 2) & _
                                                ", " & J_CREDIT & ", " & J_TAX & "," & J_GROSS & "," & J_NET & ", " & J_STATUS & "," & J_ALLENTITY & ")"
                                gconDMIS.Execute SQL_STATEMENT
                            End If
                            ...
                            If COMPANY_CODE = "DGI" Then
                                ...
                                'J_DEBIT = 0 '= Round(NumericVal(rsCheckDeposit!amount), 2)
                                'J_CREDIT = 0
                                J_TAX = 0
                                J_GROSS = 0
                                J_NET = 0
                                ...
                                SQL_STATEMENT = "insert into AMIS_Journal_Det " & _
                                                "(jdate,voucherno,jtype,jno,jitemno,acct_code,acct_name,debit,credit,tax,grossamt,netamt,status,Entity)" & _
                                                " values (" & J_JDATE & ", " & J_VOUCHERNO & ", " & J_JTYPE & _
                                                ", " & J_JNO & ", " & J_JITEMNO & ", " & J_ACCT_CODE & ", " & J_ACCT_NAME & ", " & J_DEBIT * 0 & _
                                                ", " & Round((J_DEBIT / 1.12) * 0.12, 2) & ", " & J_TAX & "," & J_GROSS & "," & J_NET & ", " & J_STATUS & "," & J_ALLENTITY & ")"
                                gconDMIS.Execute SQL_STATEMENT
                            End If
                        End If
                        ...
                        If J_ACCT_CODE = N2Str2Null(ReturnAccountCode("CARD")) Or J_ACCT_CODE = N2Str2Null(ReturnAccountCode("CARD ON HAND")) Then
                            ...
                            If CheckIfBank(CMIS_CUSCDE) = True And CMIS_DT_PAIDFOR = "427" Then
                                ...
                                PV_MRRNO = "'CI'"
                                'CREDITCARD
                                If COMPANY_CODE = "HGC" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "DGI" Or COMPANY_CODE = "HCA" Then
                                    PV_INVDATE = GetCustomerORDate(CMIS_DT_REFERENCE)
                        ...
                        Else
                            ...
                            Else
                                ...
                                If (CMIS_DT_TRANTYPE = "OTH" And CMIS_DT_PAIDFOR = "415") Or (CMIS_DT_TRANTYPE = "OTH" And CMIS_DT_PAIDFOR = "421") Then
                                    SQL_STATEMENT = "INSERT INTO AMIS_AP(VOUCHERNO,INVOICETYPE,INVOICENO,VENDOR_CODE,VENDOR_NAME,AMOUNT2PAY,AMOUNTPAID,BALANCE,ACCT_CODE,INVOICEDATE,LASTUPDATED,JDATE,ENTITYCODE,REFCODE,DUEDATE,JOURNAL_DET_ID) " & _
                                                    "VALUES(" & N2Str2Null(J_JTYPE & "-" & J_VOUCHERNO) & "," & J_INVOICETYPE & "," & J_INVOICENO & ",'" & Right(CMIS_DT_REFCODE, 6) & "','" & SetVendorName(Right(CMIS_DT_REFCODE, 6)) & "'," & J_CREDIT & ",'0'," & J_CREDIT & "," & J_ACCT_CODE & "," & J_JDATE & "," & LOGDATE & "," & J_JDATE & ",'V'," & N2Str2Null("C" + J_CUSTOMERCODE) & "," & J_JDATE & "," & TransactionID & ")"
                                    gconDMIS.Execute SQL_STATEMENT
                                End If
                            End If
                        End If
                        ...
                        If CheckIfBank(CMIS_CUSCDE) = False And COMPANY_CODE <> "DGI" Then
                            If COMPANY_CODE <> "HCA" Then
                                If CMIS_DT_TAX <> 0 Then
                                    J_ITEMCOUNT = J_ITEMCOUNT + 1
                                    J_JITEMNO = "'" & Format(J_ITEMCOUNT, "0000") & "'"
...
Function ImportPurelyInternal() As Boolean
    ...
    For GridImport = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImport, 1).Text) = 0 Then
            ...
            Set rsCSMIOS_REPOR = gconDMIS.Execute("Select * from CSMS_REPOR where REP_OR = '" & Grid3.Cell(GridImport, 2).Text & "' and DTE_REL > '" & GetCUTOFF_DATE & "' AND DTE_REL <= '" & CDate(dtpTranDate) & "' order by invoice ASC")
            If Not rsCSMIOS_REPOR.EOF And Not rsCSMIOS_REPOR.BOF Then
                ItemCnt = 0
...
Function ImportCSMSSales() As Boolean
    ...
    For GridImport = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImport, 1).Text) = 0 Then
            ...
            'Set rsCSMIOS_REPOR = gconDMIS.Execute("Select * from CSMS_REPOR where RO_AMOUNT > 0 AND invoice = '" & Grid2.Cell(GridImport, 3).Text & "' and dte_comp = '" & CDate(dtpTranDate) & "' order by invoice ASC")
            'ORIGINAL CODE:
            SQL_STATEMENT = "Select * from CSMS_REPOR where REP_OR = '" & Grid3.Cell(GridImport, 2).Text & "' and DTE_REL > '" & GetCUTOFF_DATE & "' AND DTE_REL <= '" & CDate(dtpTranDate) & "' order by invoice ASC"
    ...
    For GridImport = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImport, 1).Text) = 0 Then
            If Grid3.Cell(GridImport, 6).Text = "PARTS" Or Grid3.Cell(GridImport, 6).Text = "ACCESSORIES" Or Grid3.Cell(GridImport, 6).Text = "MATERIALS" Then
                Call ImportSales2("SALES", Grid3.Cell(GridImport, 2).Text, Grid3.Cell(GridImport, 3).Text)
                Grid3.Cell(GridImport, 1).Text = 1
            End If
        End If
    ...
    For GridImport = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImport, 1).Text) = 0 Then
            If Grid3.Cell(GridImport, 6).Text = "VEHICLE" Then
                Call ImportSales2("SALES", "VEHICLES", Right(Grid3.Cell(GridImport, 3).Text, 6))
                Grid3.Cell(GridImport, 1).Text = 1
            End If
        End If
        i = i + 1
...
Sub ImportSales2(xTRANTYPE As String, XTYPE As String, XTRANNO As String)
    Dim CMD                                                 As New ADODB.Command
