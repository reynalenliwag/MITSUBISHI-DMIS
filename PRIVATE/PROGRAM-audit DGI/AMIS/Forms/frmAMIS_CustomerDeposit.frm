...
Begin VB.Form frmAMIS_CustomerDepositLedger 
   ...
   EndProperty
   Icon            =   "frmAMIS_CustomerDeposit.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Image Image1 
         Height          =   360
         Left            =   150
         Picture         =   "frmAMIS_CustomerDeposit.frx":1082
         Top             =   180
         Width           =   360
      End
   End
   Begin VB.CommandButton cmdExit 
      ...
      Height          =   795
      Left            =   11070
      MouseIcon       =   "frmAMIS_CustomerDeposit.frx":2104
      MousePointer    =   99  'Custom
      Picture         =   "frmAMIS_CustomerDeposit.frx":2256
      Style           =   1  'Graphical
      TabIndex        =   11
   ...
   Begin VB.CommandButton cmdPrint 
      ...
      Height          =   795
      Left            =   10380
      MouseIcon       =   "frmAMIS_CustomerDeposit.frx":25BC
      MousePointer    =   99  'Custom
      Picture         =   "frmAMIS_CustomerDeposit.frx":270E
      Style           =   1  'Graphical
      TabIndex        =   12
   ...
   Begin VB.CommandButton cmdFind 
      ...
      Height          =   795
      Left            =   9690
      MouseIcon       =   "frmAMIS_CustomerDeposit.frx":2A74
      MousePointer    =   99  'Custom
      Picture         =   "frmAMIS_CustomerDeposit.frx":2BC6
      Style           =   1  'Graphical
      TabIndex        =   13
   ...
   Begin VB.CommandButton cmdNext 
      ...
      Height          =   795
      Left            =   9000
      MouseIcon       =   "frmAMIS_CustomerDeposit.frx":2EC0
      MousePointer    =   99  'Custom
      Picture         =   "frmAMIS_CustomerDeposit.frx":3012
      Style           =   1  'Graphical
      TabIndex        =   14
   ...
   Begin VB.CommandButton cmdPrevious 
      ...
      Height          =   795
      Left            =   8310
      MouseIcon       =   "frmAMIS_CustomerDeposit.frx":336A
      MousePointer    =   99  'Custom
      Picture         =   "frmAMIS_CustomerDeposit.frx":34BC
      Style           =   1  'Graphical
      TabIndex        =   15
   ...
   Begin VB.CommandButton cmdOK 
      ...
      Height          =   435
      Left            =   11280
      Picture         =   "frmAMIS_CustomerDeposit.frx":381B
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   60
      Width           =   495
   ...
   End
End
Attribute VB_Name = "frmAMIS_CustomerDepositLedger"
...
Sub LOAD_ACCT_CODE()
    ...
    rsLOAD_ACCT_CODE.Open "SELECT DESCRIPTION FROM AMIS_CHARTACCOUNT WHERE LEFT(ACCTCODE,5) IN ('21-01','21-06','21-02','21-07') AND TRANTYPE2='DEPOSIT'", gconDMIS, adOpenKeyset
...
Sub LOAD_VENDOR()
    ...
    SQL_STATEMENT = "SELECT * FROM (SELECT DISTINCT  VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2='DEPOSIT')A ORDER BY VENDOR_NAME ASC" & vbCr
    'SQL_STATEMENT = SQL_STATEMENT & "UNION" & vbCrLf
    'SQL_STATEMENT = SQL_STATEMENT & "SELECT * FROM (SELECT DISTINCT TOP 22 CUSTOMERCODE as VENDOR_CODE,CUSTOMERNAME as VENDOR_NAME FROM AMIS_AR AR INNER JOIN ALL_CUSTOMER_TABLE ACT ON AR.CUSTOMERCODE=ACT.CUSCDE WHERE CUSTOMERCODE IS NOT NULL AND LEFT(ACCOUNT_CODE,5)='21-01' ORDER BY CUSTOMERNAME ASC)B"
    Set rsLOAD_VENDOR = gconDMIS.Execute(SQL_STATEMENT)
...
Private Sub textSearch_Change()
    ...
    If TextSearch.Text <> "" Then
        'rssearch.Open "SELECT DISTINCT VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2='DEPOSIT' AND VENDOR_NAME LIKE '" & Replace(TextSearch.Text, "'", "") & "%' ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenKeyset
        rssearch.Open "SELECT DISTINCT VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2='DEPOSIT' AND VENDOR_NAME LIKE '" & Replace(TextSearch.Text, "'", "") & "%' ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenKeyset
    Else
        'rssearch.Open "SELECT DISTINCT TOP 22 VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE AND AP.VENDOR_NAME=AVT.ACCTNAME INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2='DEPOSIT' ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenKeyset
        rssearch.Open "SELECT DISTINCT TOP 22 VENDOR_CODE,VENDOR_NAME FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2='DEPOSIT' ORDER BY VENDOR_NAME ASC", gconDMIS, adOpenKeyset
    End If
...
Sub LOAD_AP_ENTITY()
    ...
    If cboAR_ACCT_CODE.Text = "ALL" Then
        rsLOAD_AP.Open "SELECT JDATE,VOUCHERNO AS APVOUCHERNO,ISNULL(INVOICETYPE,'') AS INVOICETYPE,ISNULL(INVOICENO,'') AS INVOICE,AMOUNT2PAY,AMOUNTPAID,INVOICENO,INVOICETYPE,VENDOR_CODE,AP.ACCT_CODE,RIGHT(VOUCHERNO,6) AS VOUCHERNO FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE INNER JOIN ALL_CUSTOMER_TABLE AVT ON AP.VENDOR_CODE=AVT.CUSCDE WHERE AC.TRANTYPE2='DEPOSIT' AND AP.VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "' ORDER BY JDATE,VOUCHERNO", gconDMIS, adOpenKeyset
    Else
    ...
    If Not rsLOAD_AP.EOF And Not rsLOAD_AP.BOF Then
        Do While Not rsLOAD_AP.EOF
            ...
            End If
            If xVOUCHERNO = "COB" Or xVOUCHERNO = "VPJ" Or xVOUCHERNO = "APJ" Or xVOUCHERNO = "CDJ" Or xVOUCHERNO = "SJ" Or xVOUCHERNO = "CRJ" Or xVOUCHERNO = "GJ" Then
                Set REC = rptLEDGER.Records.Add
...
Sub LOAD_AP_ENTITY_PRINTING()
    ...
    Set xlWorkbook = xlApplication.Workbooks.Open(AMIS_REPORT_PATH & "\Ledgers\CustomersDepositLedger.xlt")
    ...
    xlWorksheet.Cells(4, "A") = "CUSTOMER'S DEPOSIT LEDGER"
    ...
    Else
        ...
        rsLOAD_AP.Open "SELECT ap.JDATE,ap.VOUCHERNO AS APVOUCHERNO,ISNULL(AP.INVOICETYPE,'') + CASE WHEN AP.INVOICETYPE IS NULL THEN '' ELSE '-' END + ISNULL(INVOICENO,'') AS INVOICE,ap.AMOUNT2PAY,ap.AMOUNTPAID,ap.INVOICENO,ap.INVOICETYPE,ap.VENDOR_CODE,AP.ACCT_CODE,RIGHT(ap.VOUCHERNO,6) AS VOUCHERNO, hd.remarks FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE inner join amis_journal_hd hd on ap.voucherno=hd.jtype+'-'+hd.voucherno WHERE ap.VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND ap.Jdate >= '" & dtFrom.Value & "'and ap.Jdate <= '" & dtTo.Value & "' AND " & _
                       "ap.ACCT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "' AND TRANTYPE2='DEPOSIT')", gconDMIS, adOpenKeyset
    End If
    '       Set rsLOAD_AP = New ADODB.Recordset
    '        If cboAR_ACCT_CODE.Text = "ALL" Then
    '        rsLOAD_AP.Open "SELECT JDATE,VOUCHERNO AS APVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT2PAY,AMOUNTPAID,INVOICENO,INVOICETYPE,VENDOR_CODE,AP.ACCT_CODE,RIGHT(VOUCHERNO,6) AS VOUCHERNO FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE VENDOR_CODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "'", gconDMIS, adOpenKeyset
...
Sub FORWARDED_BALANCE()
    ...
    Else
        rsLOAD_AP.Open "SELECT JDATE,VOUCHERNO AS APVOUCHERNO,INVOICETYPE + '-' + INVOICENO,AMOUNT2PAY,AMOUNTPAID,INVOICENO,INVOICETYPE,VENDOR_CODE,ACCT_CODE,RIGHT(VOUCHERNO,6) AS VOUCHERNO " & _
                       ...
                       "ACCT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "' AND TRANTYPE2='DEPOSIT')", gconDMIS, adOpenKeyset
    End If
...
Sub rsRefresh()
    ...
    SQL_STATEMENT = "SELECT * FROM (SELECT DISTINCT VENDOR_NAME,VENDOR_CODE FROM AMIS_AP AP INNER JOIN AMIS_CHARTACCOUNT AC ON AP.ACCT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2= 'DEPOSIT')A" & vbCrLf
    'SQL_STATEMENT = SQL_STATEMENT & "UNION" & vbCrLf
    'SQL_STATEMENT = SQL_STATEMENT & "SELECT * FROM (SELECT DISTINCT CUSTOMERNAME AS VENDOR_NAME,CUSTOMERCODE AS VENDOR_CODE FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC ON AR.ACCOUNT_CODE=AC.ACCTCODE WHERE AC.TRANTYPE2= 'DEPOSIT')B"
    Set rsREF = gconDMIS.Execute(SQL_STATEMENT)
End Sub
