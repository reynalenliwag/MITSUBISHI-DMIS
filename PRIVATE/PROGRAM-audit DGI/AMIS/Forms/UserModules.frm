...
Private Sub Form_Load()
    ...
    '== Get Modules list ==
    'Set rs = gconDMIS.Execute("select description,id from AMIS_USER_MODULES order by description")
    Set rs = gconDMIS.Execute("select description,id from AMIS_USER_MODULES order by id")
    ...
    Set rs = gconDMIS.Execute("SELECT A.[DESCRIPTION], A.[ID] FROM AMIS_USER_MODULES A " & _
                            " Inner Join " & _
                            " AMIS_USERACCESS U ON A.[ID] = U.MODULE_ID " & _
...
Private Sub Save_Modules()
    ...
    gconDMIS.Execute ("Delete from AMIS_USERACCESS where userid = " & mUserID & " ")
    For i = 1 To Me.lvwGranted.ListItems.Count
        gconDMIS.Execute ("Insert into AMIS_USERACCESS(module_id, userid) values(" & _
                          Me.lvwGranted.ListItems(i).SubItems(1) & ", " & _
                          mUserID & ") ")
    Next i
    MessagePop RecSave, "Sucessfull Update", "Selected modules successfully added!", 1000
    Me.cmdSave.Enabled = False
    Exit Sub
errZone:
