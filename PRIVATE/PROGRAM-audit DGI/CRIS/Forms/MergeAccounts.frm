...
'            On Error GoTo ErrorCode:
            ...
            Call mergeRecorder("CMIS_OFF_HD", old_code, txtCuscde)
            Call mergeRecorder("CMIS_OFF_DT", old_code, txtCuscde)
            Call mergeRecorder("CSMS_CUSVEH", old_code, txtCuscde)
            Call mergeRecorder("CSMS_APPOINTMENT", old_code, txtCuscde)
            Call mergeRecorder("pmis_ord_hist", old_code, txtCuscde)
            Call mergeRecorder("pmis_ord_hd", old_code, txtCuscde)
            Call mergeRecorder("amis_openinvoice", old_code, txtCuscde)
            Call mergeRecorder("smis_salesorder", old_code, txtCuscde)
            Call mergeRecorder("CSMS_repairorder", old_code, txtCuscde)
            Call mergeRecorder("CSMS_REPOR", old_code, txtCuscde)
            Call mergeRecorder("CSMS_ESTDETAILS", old_code, txtCuscde)
            Call mergeRecorder("CSMS_ESTHD", old_code, txtCuscde)
            Call mergeRecorder("ALL_CUSTOMER_CHILD", old_code, txtCuscde)
            Call mergeRecorder("CRIS_LGM", old_code, txtCuscde)
            Call mergeRecorder("SMIS_PO", old_code, txtCuscde)
            Call mergeRecorder("SMIS_MRRINV", old_code, txtCuscde)
            Call mergeRecorder("AMIS_JOURNAL_HD", old_code, txtCuscde)
            Call mergeRecorder("AMIS_AR", old_code, txtCuscde)
            Call mergeRecorder("AMIS_DETAIL", old_code, txtCuscde)
            Call mergeRecorder("ALL_CUSTOMER_TERMS", old_code, txtCuscde)
            Call mergeRecorder("CRIS_PROSPECT_CALLS", old_code, txtCuscde)
            Call mergeRecorder("CRIS_PROSPECT_EMAIL", old_code, txtCuscde)
            Call mergeRecorder("CRIS_PROSPECT_LETTER", old_code, txtCuscde)
            Call mergeRecorder("CRIS_PROSPECT_VISITS", old_code, txtCuscde)
            Call mergeRecorder("CRIS_REMINDERS", old_code, txtCuscde)
            Call mergeRecorder("CRIS_PROSPECTS", old_code, txtCuscde)
            Call mergeRecorder("SMIS_LOANINDIV", old_code, txtCuscde)
            Call mergeRecorder("SMIS_LOANCORP", old_code, txtCuscde)
           ...
           'GOT RECEIPTS
            ...
            gconDMIS.Execute ("UPDATE AMIS_AR SET CUSTOMERCODE=" & N2Str2Null(txtCuscde) & "  WHERE CUSTOMERCODE=" & N2Str2Null(Xcode))
            gconDMIS.Execute ("UPDATE AMIS_DETAIL SET CUSTOMERCODE=" & N2Str2Null(txtCuscde) & "  WHERE CUSTOMERCODE=" & N2Str2Null(Xcode))
            gconDMIS.Execute ("UPDATE ALL_CUSTOMER_TERMS SET CUSTOMERCODE=" & N2Str2Null(txtCuscde) & "  WHERE CUSTOMERCODE=" & N2Str2Null(Xcode))
            LogAudit "E", "MERGE ACCOUNT-ACCOUNTING", " FROM " & Xcode & "  TO " & txtCuscde
            ...
            gconDMIS.Execute ("INSERT INTO Customer_DELETED select CUSCDE,APOD,ACCOUNTNO,CUSCOMP,COMPANYADD,ACCTNAME,LASTNAME,FIRSTNAME,MIDDLEINITIAL,SEX,CUSTOMERADD,PROVINCIALADD,ZIPCODE,HOMEPHONE,TELEPHONENO,CUSTYPE,LEADSOURCE,EMAIL,MOBILE,FAX,ASSISTANT,ASSTPHONE,CITY,BIRTHDATE,SPOUSE,DESCRIPTION,DELIVERYADDRESS,CREDITTERM,CREDITDAYS,CREDITLIMIT,USERCODE,TIN from ALL_CUSTOMER_TABLE WHERE CUSCDE = " & N2Str2Null(Xcode) & "")
    ...
    Call NEW_LogAudit("E", "CUSTOMER", TEMPSQL & " FROM " & Xcode & "  TO " & txtCuscde, FindTransactionID(N2Str2Null(labCustCode), "CUSCDE", "ALL_CUSTOMER"), "", "CUST CODE: " & Null2String(txtCuscde), "", "")
...
Public Sub FillCustomerListView(RS As Recordset, grd As ListView, Optional WithSN As Boolean = False, Optional WITHCOLUMNHEADER As Boolean = False)
    ...
    Set LST = Nothing
End Sub
Sub mergeRecorder(tbl As String, Xcode As String, Xcodeto As String)
    Dim merger_hist                                              As ADODB.Recordset
    Dim table_identity As String
    Dim xnew_code As String
    Set merger_hist = gconDMIS.Execute("Select acctname From all_customer_table Where cuscde='" & Xcode & "'")
        If Not merger_hist.EOF And Not merger_hist.EOF Then
            xnew_code = merger_hist!AcctName
        End If '
    Set merger_hist = Nothing
    If tbl = "pmis_ord_hist" Or tbl = "pmis_ord_hd" Or tbl = "amis_openinvoice" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where custcode='" & Xcode & "'")
    ElseIf tbl = "smis_salesorder" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where code='" & Xcode & "'")
    ElseIf tbl = "CMIS_OFF_HD" Or tbl = "CMIS_OFF_DT" Or tbl = "CSMS_CUSVEH" Or tbl = "CSMS_APPOINTMENT" Or tbl = "ALL_CUSTOMER_CHILD" Or tbl = "CRIS_LGM" Or tbl = "SMIS_PO" Or tbl = "CRIS_PROSPECTS" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where cuscde='" & Xcode & "'")
    ElseIf tbl = "SMIS_MRRINV" Or tbl = "AMIS_JOURNAL_HD" Or tbl = "AMIS_AR" Or tbl = "AMIS_DETAIL" Or tbl = "ALL_CUSTOMER_TERMS" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where customercode='" & Xcode & "'")
    ElseIf tbl = "CRIS_PROSPECT_CALLS" Or tbl = "CRIS_PROSPECT_EMAIL" Or tbl = "CRIS_PROSPECT_LETTER" Or tbl = "CRIS_PROSPECT_VISITS" Or tbl = "CRIS_REMINDERS" Or tbl = "SMIS_PO" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where cscde='" & Xcode & "'")
    ElseIf tbl = "SMIS_LOANINDIV" Or tbl = "SMIS_LOANCORP" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where APLCODE='" & Xcode & "'")
    ElseIf tbl = "CSMS_repairorder" Or tbl = "CSMS_REPOR" Or tbl = "CSMS_ESTDETAILS" Or tbl = "CSMS_ESTHD" Then
        Set merger_hist = gconDMIS.Execute("Select * From " & tbl & " Where ACCT_NO='" & Xcode & "'")
    End If

        Do While Not merger_hist.EOF
            If tbl = "CRIS_PROSPECTS" Then
                table_identity = Null2String(merger_hist!PROSPECTID)
            Else
                table_identity = Null2String(merger_hist!ID)
            End If
            gconDMIS.Execute ("insert into MERGE_TRANSACTION_TABLE VALUES('" & tbl & "'," & N2Str2Null(Xcode) & ",'" & xnew_code & "'," & table_identity & ", '" & Xcodeto & "')")
'            MsgBox "TEST"
            merger_hist.MoveNext
        Loop '
'        End If
    Set merger_hist = Nothing
End Sub
