...
Begin VB.Form frmCRIS_Inquiry_CustomerTransHistory 
   ...
   Begin VB.PictureBox Picture3 
      ...
      Begin VB.CommandButton cmdPrint 
         ...
         Height          =   645
         Left            =   10920
         MouseIcon       =   "CustomerTransHistory.frx":C71E
         MousePointer    =   99  'Custom
         Picture         =   "CustomerTransHistory.frx":C870
         Style           =   1  'Graphical
         TabIndex        =   25
         TabStop         =   0   'False
         ToolTipText     =   "Print Report"
      ...
      Begin VB.CommandButton cmdClose 
         ...
         Height          =   645
         Left            =   11580
         MouseIcon       =   "CustomerTransHistory.frx":CCD7
         MousePointer    =   99  'Custom
         Picture         =   "CustomerTransHistory.frx":CE29
         Style           =   1  'Graphical
         TabIndex        =   24
         TabStop         =   0   'False
   ...
   Begin VB.PictureBox fraSearch 
      ...
      Begin VB.OptionButton Option1 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D2BD
         MousePointer    =   99  'Custom
         TabIndex        =   23
         Top             =   1140
         Width           =   2325
      ...
      Begin VB.OptionButton optSearchKeyLast 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D40F
         MousePointer    =   99  'Custom
         TabIndex        =   6
         Top             =   0
         Width           =   2295
      ...
      Begin VB.OptionButton optSearchKeyCompany 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D561
         MousePointer    =   99  'Custom
         TabIndex        =   5
         Top             =   225
         Width           =   2295
      ...
      Begin VB.OptionButton optSearchKeyAcctName 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D6B3
         MousePointer    =   99  'Custom
         TabIndex        =   4
         Top             =   450
         Value           =   -1  'True
      ...
      Begin VB.OptionButton optSearchKeyAddress 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D805
         MousePointer    =   99  'Custom
         TabIndex        =   3
         Top             =   675
         Width           =   2295
      ...
      Begin VB.OptionButton optSearchKeyEmail 
         ...
         Height          =   255
         Left            =   30
         MouseIcon       =   "CustomerTransHistory.frx":D957
         MousePointer    =   99  'Custom
         TabIndex        =   2
         Top             =   900
         Width           =   2295
      ...
      Begin MSComctlLib.ListView lstCustomer 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         MouseIcon       =   "CustomerTransHistory.frx":DAA9
         NumItems        =   2
...
'    Else
        ...
        Dim rsOllEx                                             As New ADODB.Recordset
        Dim rsif_merged                                         As New ADODB.Recordset
        Dim XTYPE                                               As String
        Dim TTYPE                                               As String
    ...
    If cmdHistbutt = "H" Then
        ...
        rsif_merged.Open "SELECT * FROM MERGE_TRANSACTION_TABLE WHERE MERGE_TO = '" & labCusCode & "'", gconDMIS, adOpenForwardOnly, adLockReadOnly
        If Not rsif_merged.EOF Or Not rsif_merged.BOF Then

        rsOllEx.Open "SELECT distinct MERGE_FROM_CUSTOMERCODE,MERGE_FROM_CUSTOMERNAME FROM MERGE_TRANSACTION_TABLE WHERE MERGE_TO = '" & labCusCode & "'", gconDMIS, adOpenForwardOnly, adLockReadOnly
            While Not rsOllEx.EOF
                Print #1, "<div>*************************** " & Null2String(rsOllEx!MERGE_FROM_CUSTOMERNAME) & " TRANSCATIONS DETAILS ***************************</div>"

              ''''''''''''''''''''''''''''''''''''''''''''UNMERGED SALES DELETED CUST'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                Set rsSales = Nothing
                rsSales.Open "SELECT * FROM SMIS_SALESORDER WHERE CODE= '" & labCusCode & "' AND ID IN(SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE='SMIS_SALESORDER' AND MERGE_FROM_CUSTOMERCODE = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "' )", gconDMIS, adOpenForwardOnly, adLockReadOnly
                Print #1, "<html>"
                Print #1, "<style>"
                ...
                End If
                ''''''''''''''''''''''''''''''''''''''''''''UNMERGED PARTS'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                rsParts.Open "SELECT *,(SELECT MERGE_FROM_CUSTOMERNAME FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN('PMIS_OR_HD','PMIS_ORD_HIST') AND ORIG_id = ID ) FROM PMIS_vw_ISS_HISTORY WHERE ISNULL(TRANTYPE,'') <>'RIV'  and CUSTCODE='" & labCusCode & "' AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('PMIS_ORD_HD','PMIS_ORD_HIST') and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "') ORDER BY trandate,TRANTYPE", gconDMIS, adOpenForwardOnly, adLockReadOnly
                If Not rsParts.EOF Or Not rsParts.BOF Then
                    ...
                    While Not rsParts.EOF
                        ...
                        If Not rsInvoicePayment.EOF Or Not rsInvoicePayment.BOF Then
                            ...
                            Wend
                        End If
                        Set rsInvoicePayment = Nothing
                        rsParts.MoveNext
                        Print #1, "</table>"
                    Wend
                    Set rsParts = Nothing
                End If
            '    '''''''''''''''''''''''''''''''''''''''''''''UNMERGED CUSTOMER VEHICLES DELETED CUST''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                rsCusVeh.Open "SELECT MAKE, YER, MODEL, DESCRIPTION, (SELECT TOP 1 COLOR_DESC FROM ALL_COLOR  WHERE COLOR_CODE=CLRCDE) AS COLOR , VIN, PLATE_NO, VCOND_NO, ENGINE, KMREADING, PRODNO, SERIAL,  D_SOLD, WAR_CERT, DEL_DATE, SELLING_DEALER, ENDUSER FROM CSMS_CUSVEH WHERE CUSCDE='" & labCusCode & "' AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_CUSVEH')and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')", gconDMIS, adOpenForwardOnly, adLockReadOnly
                ...
                While Not rsCusVeh.EOF
                    ...
                    '''''''''''''''''''''''''''''''''''''''''''''UNMERGED SERVICE''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
SERV:
                    Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "'AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')")
                    'Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "' AND PLATE_NO='" & rsCusVeh!PLATE_NO & "'AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')")
                    ...
                    Print #1, "</td></tr></table>"
                Wend
            '''''''''''''''''''''''''''''''''''''''''''''UNMERGED PAYMENT DELETED CUST'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            Set rsCash = gconDMIS.Execute("SELECT * FROM CMIS_OFF_DT WHERE OR_NUM IN (SELECT OR_NUM FROM CMIS_OFF_HD WHERE CUSCDE='" & labCusCode & "') and id in (select orig_id from MERGE_TRANSACTION_TABLE where module_table = 'CMIS_OFF_DT'and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')ORDER BY ORDATE, OR_NUM, TRANTYPE")
                If Not rsCash.EOF Or Not rsCash.BOF Then
                    DoEvents
     ...
     '=================================================RETAINED CUSTOMER==================================================
               ...
               rsOllEx.Open "SELECT ACCTNAME FROM ALL_CUSTOMER_TABLE WHERE CUSCDE = '" & labCusCode & "'", gconDMIS, adOpenForwardOnly, adLockReadOnly
               If Not rsOllEx.EOF And Not rsOllEx!BOF Then
               RET_NAME = Null2String(rsOllEx!AcctName)
               Print #1, "<div>*************************** " & Null2String(rsOllEx!AcctName) & " TRANSCATIONS DETAILS ***************************</div>"
               rsSales.Open "SELECT *,(SELECT ACCTNAME FROM ALL_CUSTOMER_TABLE WHERE CUSCDE = '" & labCusCode & "') AS TO_NAME FROM SMIS_SALESORDER WHERE CODE= '" & labCusCode & "' AND ID NOT IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE='SMIS_SALESORDER' )", gconDMIS, adOpenForwardOnly, adLockReadOnly
                Print #1, "<html>"
                Print #1, "<style>"
                Print #1, "Div {BORDER:1PX SOLID ORANGE;color:navy;text-align:CENTER;font:700 11px Arial;BACKGROUND-COLOR:#ffffa6;BORDER-BOTTOM:3PX DOUBLE ORANGE;}"
                Print #1, "Body {font:.7em Arial;background-color:buttonface;border:0px; margin:0 5 0 0;}"
                ...
                End If
                ''''''''''''''''''''''''''''''''''''''''''''UNMERGED PARTS'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                rsParts.Open "SELECT *,(SELECT ACCTNAME FROM ALL_CUSTOMER_TABLE WHERE CUSCDE = '" & labCusCode & "') AS TO_NAME FROM PMIS_vw_ISS_HISTORY WHERE ISNULL(TRANTYPE,'') <>'RIV'  and CUSTCODE='" & labCusCode & "' AND ID NOT IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('PMIS_ORD_HD','PMIS_ORD_HIST')) ORDER BY trandate,TRANTYPE", gconDMIS, adOpenForwardOnly, adLockReadOnly
                If Not rsParts.EOF Or Not rsParts.BOF Then
                    Print #1, "<DIV>PART ISSUANCES (PARTS DEPARTMENT)</DIV>"
                    While Not rsParts.EOF
                        DoEvents
            ...
            '    '''''''''''''''''''''''''''''''''''''''''''''UNMERGED CUSTOMER VEHICLES''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                rsCusVeh.Open "SELECT MAKE, YER, MODEL, DESCRIPTION, (SELECT TOP 1 COLOR_DESC FROM ALL_COLOR  WHERE COLOR_CODE=CLRCDE) AS COLOR , VIN, PLATE_NO, VCOND_NO, ENGINE, KMREADING, PRODNO, SERIAL,  D_SOLD, WAR_CERT, DEL_DATE, SELLING_DEALER, ENDUSER FROM CSMS_CUSVEH WHERE CUSCDE='" & labCusCode & "' AND ID not IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_CUSVEH')and merge_from_customername = '" & RET_NAME & "')", gconDMIS, adOpenForwardOnly, adLockReadOnly
                If Not rsCusVeh.EOF Then
                    Print #1, "<DIV>CUSTOMER VEHICLE(SERVICE DEPARTMENT)</DIV>"
                Else
                    GoTo SERV1
...
SERV1:
                    Set rsService = Nothing
                    Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "'AND ID not IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customername = '" & RET_NAME & "')")
                    'Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "' AND PLATE_NO='" & rsCusVeh!PLATE_NO & "'AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')")
                    If Not rsService.EOF Or Not rsService.BOF Then
                        Print #1, "<tr><td colspan=4 STYLE='BACKGROUND-COLOR:BUTTONFACE;PADDING:0PX;MARGIN:0PX;'>"
                        Print #1, "<DIV>REPAIRS ORDERS(SERVICE DEPARTMENT)</DIV>"
                        While Not rsService.EOF
                            ...
                            Set rsInvoicePayment = gconDMIS.Execute("SELECT ORDATE, OR_NUM, TRANTYPE, INVOICENO, DESCRIPT, PAYMENT ,AMOUNT, BALANCE,  DISCOUNT FROM CMIS_OFF_DT where invoiceno='" & Null2String(rsService!invoice) & "' and trantype='SI' AND ID NOT IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CMIS_OFF_DT'))ORDER BY ORDATE, OR_NUM, TRANTYPE")
                            If Not rsInvoicePayment.EOF Or Not rsInvoicePayment.BOF Then
                                Print #1, "<tr><th>Date Paid</th><th>RefNo</th><th>ORNo</th><th colspan=2>Application</th><th style='text-align:right;'>Amount Paid</th></tr>"
                                While Not rsInvoicePayment.EOF
                                    Print #1, "<tr><td>" & Format(Null2String(rsInvoicePayment!ORDATE), "MM/DD/YYYY") & "</td><td>" & Null2String(rsInvoicePayment!INVOICENO) & "</td><td>" & Null2String(rsInvoicePayment!OR_NUM) & "</td><td colspan=2 style=""font:8px arial;"">" & Null2String(rsInvoicePayment!DESCRIPT) & "</td><td class='CUR'>" & FormatNumber(NumericVal(rsInvoicePayment!PAYMENT)) & "</td></tr>"
            ...
            '''''''''''''''''''''''''''''''''''''''''''''UNMERGED PAYMENT'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            Set rsCash = gconDMIS.Execute("SELECT *,(SELECT ACCTNAME FROM ALL_CUSTOMER_TABLE WHERE CUSCDE = '" & labCusCode & "') AS TO_NAME FROM CMIS_OFF_DT WHERE OR_NUM IN (SELECT OR_NUM FROM CMIS_OFF_HD WHERE CUSCDE='" & labCusCode & "') and id NOT in (select orig_id from MERGE_TRANSACTION_TABLE where module_table = 'CMIS_OFF_DT')ORDER BY ORDATE, OR_NUM, TRANTYPE")
                If Not rsCash.EOF Or Not rsCash.BOF Then
                    DoEvents
                    Print #1, "<DIV> " & Null2String(rsCash!FROM_NAME) & " PAYMENT DETAILS</DIV>"
                    Print #1, "<table  width=""100%"" CELLSPACING=1 CELLPADDING=1 >"
                    Print #1, "<col width=""60""/><col width =30/><col width =40/><col width =200/><col width =40 style='text-align:center;' /><col width =20 align='right'/>"
                    Print #1, "<tr><th>Date</th><th>ORNo</th><th>Type</th><th>Application</th><th style='text-align:center;'>Ref No</th><th style='text-align:right;'>Amount Paid</th></tr>"
                    While Not rsCash.EOF
        ...
        Print #1, "<div>########################### MERGED TRANSCATIONS DETAILS TO THIS CUSTOMER " & RET_NAME & " ###########################</div>"
        End If
...
''HEADER
    Set rsif_merged = New ADODB.Recordset
    rsif_merged.Open "SELECT * FROM MERGE_TRANSACTION_TABLE WHERE MERGE_TO = '" & labCusCode & "'", gconDMIS, adOpenForwardOnly, adLockReadOnly
    If Not rsif_merged.EOF Or Not rsif_merged.BOF Then
    
    Set rsOllEx = New ADODB.Recordset
    rsOllEx.Open "SELECT distinct MERGE_FROM_CUSTOMERNAME,MERGE_FROM_CUSTOMERCODE FROM MERGE_TRANSACTION_TABLE WHERE MERGE_TO = '" & labCusCode & "'", gconDMIS, adOpenForwardOnly, adLockReadOnly
        While Not rsOllEx.EOF
            Print #1, "<div>*************************** " & Null2String(rsOllEx!MERGE_FROM_CUSTOMERNAME) & " TRANSCATIONS DETAILS ***************************</div>"
...
SERV:
 ...
 '=================================================RETAINED CUSTOMER==================================================
        ...
        '    '''''''''''''''''''''''''''''''''''''''''''''UNMERGED CUSTOMER VEHICLES''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            rsCusVeh.Open "SELECT MAKE, YER, MODEL, DESCRIPTION, (SELECT TOP 1 COLOR_DESC FROM ALL_COLOR  WHERE COLOR_CODE=CLRCDE) AS COLOR , VIN, PLATE_NO, VCOND_NO, ENGINE, KMREADING, PRODNO, SERIAL,  D_SOLD, WAR_CERT, DEL_DATE, SELLING_DEALER, ENDUSER FROM CSMS_CUSVEH WHERE CUSCDE='" & labCusCode & "' AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_CUSVEH')and merge_from_customername = '" & RET_NAME & "')", gconDMIS, adOpenForwardOnly, adLockReadOnly
            If Not rsCusVeh.EOF Then
                Print #1, "<DIV>CUSTOMER VEHICLE(SERVICE DEPARTMENT)</DIV>"
            Else
                GoTo SERV1
...
SERV1:
                Set rsService = Nothing
                Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "'AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customername = '" & RET_NAME & "')")
                'Set rsService = gconDMIS.Execute("SELECT  AMOUNT, DTE_COMP, REP_OR,invoice , isnull(L_DISC,0) + isnull(P_DISC,0) + isnull(M_DISC,0) + isnull(A_DISC,0) discount, AMOUNT , ISNULL(INSAMT,0)  AS INSAMT FROM  CSMS_REPOR WHERE ACCT_NO='" & labCusCode & "' AND PLATE_NO='" & rsCusVeh!PLATE_NO & "'AND ID IN (SELECT ORIG_ID FROM MERGE_TRANSACTION_TABLE WHERE MODULE_TABLE IN ('CSMS_REPOR')and merge_from_customercode = '" & rsOllEx!MERGE_FROM_CUSTOMERCODE & "')")
                If Not rsService.EOF Or Not rsService.BOF Then
                    Print #1, "<tr><td colspan=4 STYLE='BACKGROUND-COLOR:BUTTONFACE;PADDING:0PX;MARGIN:0PX;'>"
                    Print #1, "<DIV>REPAIRS ORDERS(SERVICE DEPARTMENT)</DIV>"
