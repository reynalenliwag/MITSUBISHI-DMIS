...
Function FindTransactionID(XTRANNO As Variant, xKEYFIELD As Variant, xTABLE As Variant, Optional XDETAILS As String, Optional XDETTYPE As String, Optional XDETFIELD As String) As String
    ...
    If XDETAILS = "" Then
        Set RSTMP = gconDMIS.Execute("SELECT MAX(ID) ID FROM " & xTABLE & " WHERE " & xKEYFIELD & " = " & N2Str2Null(XTRANNO) & "")
        ...
        End If
    Else
        Set RSTMP = gconDMIS.Execute("SELECT MAX(ID) ID FROM " & xTABLE & " WHERE " & xKEYFIELD & " = " & N2Str2Null(XTRANNO) & " AND " & XDETFIELD & " = " & N2Str2Null(XDETTYPE) & "")
        If Not (RSTMP.EOF And RSTMP.EOF) Then
            FindTransactionID = (Null2String(RSTMP!ID))
        Else
            FindTransactionID = ""
        End If
    End If
...
Function FindNewID(xVOUCHERNO As Variant, xKEYFIELD1 As Variant, xTABLE As Variant, Optional xJType As String, Optional xKEYFIELD2 As String) As String
    Dim RSTMP                                               As New ADODB.Recordset
    Set RSTMP = gconDMIS.Execute("SELECT MAX(ID) AS ID FROM " & xTABLE & " WHERE " & xKEYFIELD1 & " = " & xVOUCHERNO & " AND " & xKEYFIELD2 & " = " & xJType & "")
    If Not (RSTMP.EOF And RSTMP.EOF) Then
        FindNewID = RSTMP!ID
    Else
        FindNewID = ""
