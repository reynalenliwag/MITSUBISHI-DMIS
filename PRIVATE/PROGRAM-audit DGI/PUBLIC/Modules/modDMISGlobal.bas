...
Public VARIABLE_EXPENSE                                     As String
Public FIXED_EXPENSE                                        As String
...
Public Const RANK_SUPER_FAST_MOVING = "A"
Public Const RANK_FAST_MOVING = "B"
Public Const RANK_MEDIUM_MOVING = "C"
Public Const RANK_SLOW_MOVING = "D"
Public Const RANK_NON_MOVING = "E"
Public Const RANK_NEW_PARTS = "F"
...
Sub DMIS_VERSION()
    ...
    Dim rsALLPROFILEApp                                     As ADODB.Recordset
...
FALSEUSERS:
            ...
            Set rsALLPROFILEApp = New ADODB.Recordset
            rsALLPROFILEApp.Open "SELECT MODULENAME FROM ALL_PROFILE WHERE MODULENAME='" & App.EXEName & "'", gconACCESS, adOpenForwardOnly
            If Not rsALLPROFILEApp.EOF And Not rsALLPROFILEApp.BOF Then
            ...
            Else
                MsgBox "Please check the application you are using " & App.EXEName
                End
            End If
            Set rsALLPROFILEApp = Nothing
        End If
    End If
End Sub
...
'    End If
    ...
    Set rsxterms = New ADODB.Recordset
    rsxterms.Open "Select isnull(creditterm,0) as creditdays from all_customer_terms where customercode = '" & XXX & "'", gconDMIS
    If Not (rsxterms.EOF And rsxterms.BOF) Then
        TERMS = N2Str2Zero(rsxterms!CREDITDAYS)
    Else
        TERMS = 0
...
Public Function Null2Bit(XXX As Variant) As Integer
    ...
    End If
End Function
Function getlastmac(Code As String, XTYPE As String, petsa As String) As String
    Dim RSRR                                                As ADODB.Recordset
    Set RSRR = New ADODB.Recordset

    Set RSRR = gconDMIS.Execute("Select top 1 mac from pmis_alldaytran where trantype in('RIV','DR','CSH','CHG','BEG') and [type] = '" & XTYPE & "' and status = 'P' and stock_ord = " & Code & " and trandate <= '" & petsa & "' order by id desc")
    If Not (RSRR.EOF And RSRR.BOF) Then
        getlastmac = N2Str2Zero(RSRR!MAC)
    Else
        getlastmac = "0.00"
    End If
    Set RSRR = Nothing
...
Function checkifro_void(XRONO As String) As Boolean
    Dim rsvoid                                              As ADODB.Recordset
    Set rsvoid = New ADODB.Recordset

    Set rsvoid = gconDMIS.Execute("Select status from csms_repor where rep_or = '" & XRONO & "'")
    If Not (rsvoid.EOF And rsvoid.EOF) Then
        If Null2String(rsvoid!Status) = "V" Then
            checkifro_void = True
        Else
            checkifro_void = False
        End If
    End If
    Set rsvoid = Nothing
End Function
...
Sub FORINVGEN()
    ...
    sqlcommand = "IF EXISTS( SELECT 1 FROM SYS.TABLES WHERE [NAME] = 'CSMS_INVOICECTR' AND [TYPE] = 'U') " & vbCrLf
    sqlcommand = sqlcommand & " print 'Meron na'" & vbCrLf
    sqlcommand = sqlcommand & " ELSE " & vbCrLf
    sqlcommand = sqlcommand & " USE " & DATABASE & "" & vbCrLf
    sqlcommand = sqlcommand & " CREATE TABLE [dbo].[csms_invoicectr](" & vbCrLf
    sqlcommand = sqlcommand & " [Id] [int] IDENTITY(1,1) NOT NULL," & vbCrLf
    sqlcommand = sqlcommand & " [invoicenumber] [nvarchar](6) COLLATE SQL_Latin1_General_CP1_CI_AS NULL," & vbCrLf
    sqlcommand = sqlcommand & " [User] [nvarchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NULL," & vbCrLf
    sqlcommand = sqlcommand & " CONSTRAINT [PK_csms_invoicectr] PRIMARY KEY CLUSTERED" & vbCrLf
    sqlcommand = sqlcommand & " (" & vbCrLf
    sqlcommand = sqlcommand & " [ID] Asc" & vbCrLf
    sqlcommand = sqlcommand & " )WITH (IGNORE_DUP_KEY = OFF) ON [PRIMARY]" & vbCrLf
    sqlcommand = sqlcommand & " ) ON [PRIMARY]"
...
Function GetCustomerBalance(XCUSTOMERCODE As String) As Double
    ...
    rsBALANCE.Open "SELECT ISNULL(SUM(ISNULL(AMOUNT_TOPAY,0)),0)-ISNULL((SELECT SUM(ISNULL(INVOICEAMOUNT,0)) FROM AMIS_DETAIL WHERE CUSTOMERCODE=AR.CUSTOMERCODE AND INVOICENO=AR.INVOICENO AND INVOICETYPE=AR.INVOICETYPE),0) AS BALANCE FROM AMIS_AR AR WHERE INVOICETYPE IN ('PI','SI') AND CUSTOMERCODE='" & XCUSTOMERCODE & "' GROUP BY CUSTOMERCODE,INVOICETYPE,INVOICENO", gconDMIS, adOpenForwardOnly
    If Not rsBALANCE.EOF And Not rsBALANCE.BOF Then
        GetCustomerBalance = rsBALANCE!BALANCE
    End If
...
Function CreditLimitLine(xCustCode As String, xbaltofinanced As Double) As Boolean
    ...
    If Not RsCreditLimit1.EOF And RsCreditLimit1.BOF Then
        If RsCreditLimit1!CREDITLIMIT < xbaltofinanced Then
            CreditLimitLine = True
        End If
    Else
        If MsgBox("Credit Terms for charge transaction is required! Do you want to update your Masterfile?", vbYesNo, "Terms is missing") = vbYes Then
            frmAllCustomer.Show vbModal
            frmAllCustomer.Option1.Value = 1
            frmAllCustomer.txtSearch.Text = xCustCode
        Else
            CreditLimitLine = True
        End If
...
'    'credit balanse ni itay
    ...
    End If
'    'past due
    Set RsCreditLimit1 = gconDMIS.Execute("select * from (select ar.customercode,ar.invoicetype,ar.invoiceno,ar.amount_topay,dt.invoiceamount from amis_ar ar inner join amis_detail dt on ar.InvoiceType = dt.InvoiceType " & _
    ...
    " and invoiceno not in (select invoiceno from amis_detail where amis_detail.invoicetype = 'VI' and status = 'P'and customercode = '" & xCustCode & "' ) and ar.customercode = '" & xCustCode & "' and DateDiff(Day, ar.invoicedate, getdate()) > tr.creditterm)T")
...
Sub Override_rec(xCustCode As String, xbaltofin As Double, XINVNO As String, xinvtype As String, xMODULENAME As String, xTranDate As String)
    Dim SQL                                                           As String
    ...
    Else
        ...
        SQL = SQL & " values ('" & frmCMISCreditLimitInfo.lblUsercode & "','" & xinvtype & "', '" & XINVNO & "', '" & xTranDate & "','" & ToDoubleNumber(xlimit) & "','" & ToDoubleNumber(xavailable) & "','" & ToDoubleNumber(xbaltofin) & "','" & xpastdue & "','" & xXpired & "','" & xcodeuser & "','" & xMODULENAME & "')"
        gconDMIS.Execute SQL
    End If
End Sub
