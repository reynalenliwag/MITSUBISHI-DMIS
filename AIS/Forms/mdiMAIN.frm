...
Begin VB.MDIForm frmMain 
   BackColor       =   &H8000000C&
   Caption         =   "Human Resource Management System"
   ClientHeight    =   8250
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   12690
   ...
   Begin XtremeCommandBars.CommandBars CommandBars1 
      ...
      _StockProps     =   0
      VisualTheme     =   6
      DesignerControls=   "mdiMAIN.frx":3A64E
   End
End
...
Select Case Control.ID
    Case AIS_INQUIRY_SEARCH
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS INQUIRY SEARCH") = False Then Exit Sub
        End If
    
    Case AIS_MASTER_APPLICATION
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS MASTER APPLICATION") = False Then Exit Sub
        End If
            frmAISApplications.Show
            frmAISApplications.tbcApplication.SelectedItem = 0
    
    Case AIS_MASTER_OPENPOSITION
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS MASTER OPENPOSITION") = False Then Exit Sub
        End If
            frmAISPOSITION.tcPOSITION.SelectedItem = 0
            frmAISPOSITION.Show
    
    Case AIS_MASTER_TYPESOFEXAM
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS MASTER TYPESOFEXAM") = False Then Exit Sub
        End If
            frmAISAdd_TYPEofEXAM.Show
    
    Case AIS_PROCESS_SCHEDULEEXAM
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS PROCESS SCHEDULEEXAM") = False Then Exit Sub
        End If
    
    Case AIS_PROCESS_SCHEDULEINTERVIEW
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS PROCESS SCHEDULEINTERVIEW") = False Then Exit Sub
        End If
    
    Case AIS_PROCESS_UPLOAD
        If ApplySecurityValidation = True Then
            If Module_Access(LOGID, "AIS PROCESS UPLOAD") = False Then Exit Sub
        End If
    
    Case AIS_SYSTEM_EXIT
        Unload Me
...
Private Sub MDIForm_Unload(Cancel As Integer)
    If MsgBox("Exit AIS, Are You Sure?", vbExclamation + vbOKCancel, "Exit System") = vbOK Then
        Dim frm          As Form
        ...
        If gconDMIS.State = adStateOpen Then
        gconDMIS.Close
...
Private Sub ApplyThemes()
    ...
    With CommandBars1
        .LoadDesignerBars
        .LoadCommandBars "AIS", App.TITLE, "Layout"
