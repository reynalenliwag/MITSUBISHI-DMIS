...
Public OTHER_EXPENSE, DIRECT_EXPENSE, INDIRECT_EXPENSE              As String
...
Function IS_IN_AMIS(DMISProcessType As String, DMISTranType As String, DMISTranno As String, Optional IF_VAT As Boolean) As Boolean
    ...
    If DMISProcessType = "RECEIVED" Then
        ...
        If Not rsAMISJournal.EOF And Not rsAMISJournal.BOF Then
            IS_IN_AMIS = True
        End If
    ElseIf DMISProcessType = "PAYMENT" Then
        Set rsAMISJournal = New ADODB.Recordset
        If IF_VAT = True Then
        ...
        If Not rsAMISJournal.EOF And Not rsAMISJournal.BOF Then
            IS_IN_AMIS = True
        End If
    ElseIf DMISProcessType = "INVOICE" Then
...
End Function
'updated BY: IEBV
'description: to  avoid wrong mac issue
Function getlastmac(Code As String, XTYPE As String, petsa As String, xID As Long) As Double
    Dim RSRR                                                        As ADODB.Recordset
    Set RSRR = New ADODB.Recordset
    Set RSRR = gconDMIS.Execute("Select top 1 mac from pmis_alldaytran where trantype in('BEG','RR') and [type] = '" & XTYPE & "' and status in ('P','B') and stock_ord = " & Code & " and trandate <= '" & petsa & "' and id < '" & xID & "'order by id desc")
...
'updated BY: IEBV
'description: to check if R.O. is voided
...
Function getnextISSPORR(XTYPE As String, xTRANTYPE As String) As String
...
searchagain:
    ...
    If xTRANTYPE = "RR" Then
        SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
              " Select right(rrno,6) as tranno from pmis_rr_hd where [TYPE] = '" & XTYPE & "' " & _
              " Union All " & _
              " Select right(rrno,6) as tranno from pmis_rec_hist where [TYPE] = '" & XTYPE & "' ) T "
        Set RS = gconDMIS.Execute(SQL)
        If Not (RS.EOF And RS.BOF) Then
            tmpnext = Format(NumericVal((N2Str2Zero(RS!nextcounter))), "000000")
        Else
            tmpnext = "000001"
        End If
    ...
    '        If XTYPE = "P" Then
    '            getnextISSPORR = "PR" + tmpnext
    '        ElseIf XTYPE = "A" Then
    '            getnextISSPORR = "AR" + tmpnext
    '        ElseIf XTYPE = "M" Then
    '            getnextISSPORR = "MR" + tmpnext
    '        Else
    '            getnextISSPORR = tmpnext
    '        End If
    '    Else
            getnextISSPORR = tmpnext
    '    End If
    ...
    ElseIf xTRANTYPE = "PO" Then
        SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
              " Select right(PONO,6) as tranno from pmis_po_HD where [TYPE] = '" & XTYPE & "' " & _
              " Union All " & _
              " Select right(PONO,6) as tranno from pmis_PO_HISt where [TYPE] = '" & XTYPE & "' ) T "
        Set RS = gconDMIS.Execute(SQL)
        If Not (RS.EOF And RS.BOF) Then
            tmpnext = Format(NumericVal((N2Str2Zero(RS!nextcounter))), "000000")
        Else
    ...
    '    If COMPANY_CODE = "DJM" Then       ** FOR APPROVAL **
    '        If XTYPE = "P" Then
    '            getnextISSPORR = "PP" + tmpnext
    '        ElseIf XTYPE = "A" Then
    '            getnextISSPORR = "AP" + tmpnext
    '        ElseIf XTYPE = "M" Then
    '            getnextISSPORR = "MP" + tmpnext
    '        Else
    '            getnextISSPORR = tmpnext
    '        End If
    '    Else
    ...
    Else
        ...
        If COMPANY_CODE = "DSSC" Then
            If xTRANTYPE = "CSH" Then
                SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
                      " Select replicate ( '0', 6 - len(right(tranno,6))) + right(tranno,6) as tranno from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and trantype in ('CSH')" & _
                      " Union All " & _
                      " Select replicate ( '0', 6 - len(right(tranno,6))) + right(tranno,6) as tranno from pmis_ord_hist where [TYPE] = '" & XTYPE & "' and trantype in ('CSH')) T "
            Else
                SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
                      " Select replicate ( '0', 6 - len(right(tranno,6))) + right(tranno,6) as tranno from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "'" & _
                      " Union All " & _
                      " Select replicate ( '0', 6 - len(right(tranno,6))) + right(tranno,6) as tranno from pmis_ord_hist where [TYPE] = '" & XTYPE & "'  and trantype = '" & xTRANTYPE & "') T "
            End If
        'JJE
        ElseIf COMPANY_CODE = "DGI" Then
            If xTRANTYPE = "CSH" Or xTRANTYPE = "CHG" Then
                SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
                      " Select right(tranno,6) as tranno from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and trantype  in ('CSH','CHG') " & _
                      " Union All " & _
                      " Select right(tranno,6) as tranno from pmis_ord_hist where [TYPE] = '" & XTYPE & "'  and trantype in ('CSH','CHG')) T "
            Else
                SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
                      " Select right(tranno,6) as tranno from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "' " & _
                      " Union All " & _
                      " Select right(tranno,6) as tranno from pmis_ord_hist where [TYPE] = '" & XTYPE & "'  and trantype = '" & xTRANTYPE & "') T "
            End If
        Else
            SQL = " Select max(tranno) + 1 as nextcounter from ( " & _
                  " Select right(tranno,6) as tranno from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and trantype = '" & xTRANTYPE & "'" & _
        ...
        If xTRANTYPE = "ARS" Or xTRANTYPE = "PRS" Or xTRANTYPE = "MRS" Then
            getnextISSPORR = tmpnext
        Else
            If FOR_CAS = True Then
                If XTYPE = "P" Then
                    getnextISSPORR = "P" & tmpnext
                ElseIf XTYPE = "A" Then
                    getnextISSPORR = "A" & tmpnext
                Else
                    getnextISSPORR = "M" & tmpnext
                End If
            Else
                If COMPANY_CODE = "DSSC" Then
                    If XTYPE = "P" Then
    ...
    '                If XTYPE = "P" Then
    '                    If xTRANTYPE = "CSH" Then
    '                        getnextISSPORR = "PI" & tmpnext
    '                    Else
    '                        getnextISSPORR = tmpnext
    '                    End If
    '                ElseIf XTYPE = "A" Then
    '                    If xTRANTYPE = "CSH" Or xTRANTYPE = "CHG" Then
    '                        getnextISSPORR = "AI" & tmpnext
    '                    Else
    '                        getnextISSPORR = tmpnext
    '                    End If
    '                ElseIf XTYPE = "M" Then
    '                    If xTRANTYPE = "CSH" Or xTRANTYPE = "CHG" Then
    '                        getnextISSPORR = "MI" & tmpnext
    '                    Else
    '                        getnextISSPORR = tmpnext
    '                    End If
    '                End If
    ...
    If xTRANTYPE = "RR" Then
        SQL = " Select * from (" & _
              " Select * from pmis_rr_hd where [TYPE] = '" & XTYPE & "' and rrno = '" & getnextISSPORR & "'" & _
              " Union All " & _
              " Select * from pmis_rec_hist where [TYPE] = '" & XTYPE & "' and rrno = '" & getnextISSPORR & "') T "
        Set rsCheck = gconDMIS.Execute(SQL)
        If Not (rsCheck.EOF And rsCheck.BOF) Then
            GoTo searchagain
        End If
    ElseIf xTRANTYPE = "PO" Then
        SQL = " Select * from (" & _
              " Select * from pmis_po_hd where [TYPE] = '" & XTYPE & "' and pono = '" & getnextISSPORR & "'" & _
              " Union All " & _
              " Select * from pmis_po_hist where [TYPE] = '" & XTYPE & "' and pono = '" & getnextISSPORR & "') T "
        Set rsCheck = gconDMIS.Execute(SQL)
        If Not (rsCheck.EOF And rsCheck.BOF) Then
            GoTo searchagain
        End If
    Else
        If COMPANY_CODE = "DGI" Then
            If xTRANTYPE = "CSH" Or xTRANTYPE = "CHG" Then
                SQL = " Select * from ( " & _
                      " Select * from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and tranno = '" & getnextISSPORR & "' and trantype in('CSH','CHG')" & _
                      " Union All " & _
                      " Select * from pmis_ord_hist where [TYPE] = '" & XTYPE & "' and tranno = '" & getnextISSPORR & "'  and trantype in('CSH','CHG') ) T "
            Else
                SQL = " Select * from ( " & _
                      " Select * from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and tranno = '" & getnextISSPORR & "' and trantype = '" & xTRANTYPE & "'" & _
                      " Union All " & _
                      " Select * from pmis_ord_hist where [TYPE] = '" & XTYPE & "' and tranno = '" & getnextISSPORR & "'  and trantype = '" & xTRANTYPE & "' ) T "
            End If
        Else
            SQL = " Select * from ( " & _
                  " Select * from pmis_ord_hd where [TYPE] = '" & XTYPE & "' and tranno = '" & getnextISSPORR & "' and trantype = '" & xTRANTYPE & "'" & _
...
End Function
'JJE
Function OR_OPTION() As Boolean

    Dim X                                                           As Long
    Dim xCounter                                                    As Long
    
    xCounter = 3
    ReDim Xstr(xCounter) As String
...
'JJE with Prefix CR as Collection Receipt
Function COLLECTION_RECEIPTS_CR() As Boolean
    ...
    For X = 0 To xCounter
        If Xstr(X) = COMPANY_CODE Then
            COLLECTION_RECEIPTS_CR = True
        End If
    Next
    
End Function

'JJE with Prefix OR as Official Receipt
Function OFFICIAL_RECEIPT_OR() As Boolean
...
'JJE
Function NEXTNUMBER_CR_OR() As Boolean
...
'JJE
Function CR_OR_PRINTING() As Boolean
...
Function Getdetailsinfo()
    ...
    Dim RSdetailsinfo                                           As ADODB.Recordset
    Set RSdetailsinfo = New ADODB.Recordset
    
    Set RSdetailsinfo = gconDMIS.Execute("Select * from CMIS_OFF_DT where or_num = '" & OR_NUMBER_GLOBAL & "'")
    If Not RSdetailsinfo.EOF And Not RSdetailsinfo.BOF Then
        vOR_NUM2 = (RSdetailsinfo!INVOICENO)
    End If

End Function

Function FOR_J() As Boolean
...
Function DEPOSITAPPLIED(XXX As String) As Boolean

    Dim DEPO                                                        As ADODB.Recordset
    Set DEPO = New ADODB.Recordset
    
    Set DEPO = gconDMIS.Execute("Select descript from cmis_off_dt where or_num = '" & XXX & "' and descript = 'DEPOSIT APPLIED'")
    If Not DEPO.EOF And Not DEPO.BOF Then
        DEPOSITAPPLIED = True
    End If

End Function

'JJE Dealers with gatepass print report feature
Function FORGATEPASS() As Boolean
    Dim i                                                           As Long
    Dim COUNTER                                                     As Long
