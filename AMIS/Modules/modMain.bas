...
Public Sub Main()
    ...
    DMIS_Audit_Connection = "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=DMIS_AUDIT ;Data Source=" & SQLSERVERNAME ':Shell "\\samsung-pc\Paste\AMIS\DLL.exe", vbNormalFocus
    frmMain.Show
    frmMain.ZOrder 1
    Load frmSecurity
...
Public Sub ShowReport(ReportName As Variant, ReportFolder As Variant, filter As Variant, ReportHeading As String, REPORT_DATE As String, WithDate As Boolean)
    ...
    If Not (rsProfile.EOF And rsProfile.BOF) Then
        ...
        If COMPANY_CODE = "HMH" Then
              CrystalRpt.Formulas(100) = "TIN = '" & "VAT Reg TIN: " & Null2String(rsProfile!companytinno) & "'"
        End If
               
        'UPDATED BY: JUN/ARNOLD | DATE UPDATED: 06112009
        ...
        If COMPANY_CODE = "HGC" Or COMPANY_CODE = "DGI" Or COMPANY_CODE = "HCA" Or COMPANY_CODE = "HCC" Or COMPANY_CODE = "HQA" Or COMPANY_CODE = "HNE" Or COMPANY_CODE = "HSM" Then
            'UPDATED BY: BTT | DATE UPDATED: 07282008
            If ReportName = "AccountsPayable" Or ReportName = "SalesJournal" Or ReportName = "GeneralJournal" Or ReportName = "CashDisbursement" Or ReportName = "CashReceipts" Or ReportName = "CAJE" Then
        ...
        If WithDate = True Then
            CrystalRpt.Formulas(2) = "ReportDate = '" & REPORT_DATE & "'"
        Else
            CrystalRpt.Formulas(11) = "PRINTEDBY = '" & LOGNAME & "'"
        End If
        ...
        Else
            'UPDATED BY: JUN | DATE UPDATED: 03-20-2009
            ...
            Else
                ...
                End If
                If COMPANY_CODE = "HMR" And frmAMISJournalEntry.xJOURNALTYPE = "CLO" Then
                    CrystalRpt.ParameterFields(0) = "@VOUCHERNO;" & frmAMISJournalEntry.txtVoucherNo.Text & ";true"
                    CrystalRpt.Formulas(3) = "JDATE='" & frmAMISJournalEntry.txtJDate.Text & "'"
                    PrintSQLReport CrystalRpt, AMIS_REPORT_PATH & ReportFolder & "\" & ReportName & ".rpt", "", DMIS_REPORT_Connection, 1
                Else
                    PrintSQLReport CrystalRpt, AMIS_REPORT_PATH & ReportFolder & "\" & ReportName & ".rpt", filter, DMIS_REPORT_Connection, 1
                End If
...
Public Sub WriteFileError()
    Dim ifileno As Integer
    'Write the error into a file
    'Log.txt
    ifileno = FreeFile
    Open App.path & "\" & "LOG.txt" For Output As #ifileno
    Print #ifileno, err.DESCRIPTION
    Close #ifileno
...
Public Sub ShowRangeReport(V_From As String, V_To As String, ReportName As Variant, ReportFolder As Variant, filter As Variant, ReportHeading As String, WithDate As Boolean)
    ...
    If Not (rsProfile.EOF And rsProfile.BOF) Then
        ...
        If COMPANY_CODE = "HMH" Then
            CrystalRpt.Formulas(6) = "TIN = '" & "VAT Reg TIN: " & Null2String(rsProfile!companytinno) & "'"
        End If
...
Function CheckIfARAccount(xACCT_CODE As String) As Boolean
    ...
    rsCheck.Open "SELECT * FROM AMIS_CHARTACCOUNT WHERE LEFT(ACCTCODE,2) IN ('11') AND IS_SCHEDULE_ACCNT=1 AND ACCTCODE = " & xACCT_CODE & "", gconDMIS, adOpenForwardOnly
...
Function CheckIfARDebitNotZero(ACCT_CODE As String, ar As Boolean, Debit As Double) As Boolean
    ...
    rsChartAccount.Open "SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,2) IN ('11') AND ACCTCODE = '" & ACCT_CODE & "'", gconDMIS, adOpenForwardOnly
...
Function CheckIfAPDebitNotZero(ACCT_CODE As String, ar As Boolean, Debit As Double) As Boolean
    Dim rsChartAccount                                      As ADODB.Recordset
    Set rsChartAccount = New ADODB.Recordset
    rsChartAccount.Open "SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,2) IN ('21') AND ACCTCODE = '" & ACCT_CODE & "'", gconDMIS, adOpenForwardOnly
    If Not rsChartAccount.EOF And Not rsChartAccount.BOF Then
...
Function CheckGLSLAPDebit(xJType As String, xVOUCHERNO) As Boolean
    ...
    Set rsJournalDT = New ADODB.Recordset
    rsJournalDT.Open "SELECT * FROM (SELECT ACCT_CODE,SUM(ISNULL(DEBIT,0)) AS DEBIT FROM AMIS_JOURNAL_DET WHERE LEFT(ACCT_CODE,2) IN ('21') AND JTYPE='" & xJType & "' AND VOUCHERNO = '" & xVOUCHERNO & "' AND DEBIT > 0 GROUP BY ACCT_CODE) DT INNER JOIN AMIS_CHARTACCOUNT AC ON DT.ACCT_CODE=AC.ACCTCODE WHERE IS_SCHEDULE_ACCNT=1", gconDMIS, adOpenForwardOnly
    If Not rsJournalDT.EOF And Not rsJournalDT.BOF Then
        Do While Not rsJournalDT.EOF
...
Function CheckGLSLAPCredit(xJType As String, xVOUCHERNO) As Boolean
    ...
    Dim rsAP                                                As ADODB.Recordset
    Set rsJournalDT = New ADODB.Recordset
    rsJournalDT.Open "SELECT * FROM (SELECT ACCT_CODE,SUM(ISNULL(CREDIT,0)) AS CREDIT FROM AMIS_JOURNAL_DET WHERE LEFT(ACCT_CODE,2) IN ('21') AND JTYPE='" & xJType & "' AND VOUCHERNO = '" & xVOUCHERNO & "' AND CREDIT > 0 GROUP BY ACCT_CODE) DT INNER JOIN AMIS_CHARTACCOUNT AC ON DT.ACCT_CODE=AC.ACCTCODE WHERE IS_SCHEDULE_ACCNT=1", gconDMIS, adOpenForwardOnly
    If Not rsJournalDT.EOF And Not rsJournalDT.BOF Then
        Do While Not rsJournalDT.EOF
            Set rsAP = New ADODB.Recordset
...
Function CheckUnposted(xFROM As Date, xTO As Date) As Boolean
    ...
    rsCHECKUNPOSTED.Open "SELECT * FROM AMIS_JOURNAL_HD WHERE JDATE BETWEEN '" & xFROM & "' AND '" & xTO & "' AND STATUS='N' AND JTYPE <> 'CCM'", gconDMIS, adOpenForwardOnly
...
Function CheckUnpostedAsOf(xTO As Date) As Boolean
    Set rsCHECKUNPOSTED = New ADODB.Recordset
    rsCHECKUNPOSTED.Open "SELECT * FROM AMIS_JOURNAL_HD WHERE JDATE <= '" & xTO & "' AND STATUS='N' AND JTYPE <> 'CCM'", gconDMIS, adOpenForwardOnly
    If Not rsCHECKUNPOSTED.EOF And Not rsCHECKUNPOSTED.BOF Then
        CheckUnpostedAsOf = True
        'SJR 6/16/14
        If COMPANY_CODE = "HCE" Or COMPANY_CODE = "HNE" Then
        Else
            UnpostedReportPrinting
        End If
    Else
...
Function CheckYearEndClosing(YearEnd As Date) As Boolean
    Dim rsCheckYearEnd As ADODB.Recordset
    Set rsCheckYearEnd = New ADODB.Recordset
    rsCheckYearEnd.Open "SELECT * FROM AMIS_JOURNAL_HD WHERE JTYPE='CLO' AND STATUS='P' AND JDATE = '" & YearEnd & "'", gconDMIS, adOpenForwardOnly
    If Not rsCheckYearEnd.EOF And Not rsCheckYearEnd.BOF Then
        CheckYearEndClosing = True
    Else
        CheckYearEndClosing = False
    End If
    Set rsCheckYearEnd = Nothing
End Function
...
Function CIP(xJType As String, xVOUCHERNO) As String
''COUNTER CHECKING BY NRE
    Dim rsCIF                                         As ADODB.Recordset
    Set rsCIF = New ADODB.Recordset
    Set rsCIF = gconDMIS.Execute("SELECT STATUS FROM AMIS_JOURNAL_HD WHERE JTYPE = '" & xJType & "' and voucherno = '" & xVOUCHERNO & "'")
    If Not rsCIF.EOF And Not rsCIF.BOF Then
        CIP = rsCIF!Status
    End If
End Function

Function CIB(xJType As String, xVOUCHERNO) As Double
''CHECKING FOR DEBIT-CREDIT
    Dim rsCIB                                         As ADODB.Recordset
    Set rsCIB = New ADODB.Recordset
    Set rsCIB = gconDMIS.Execute("SELECT (SUM(ISNULL(DEBIT,0)) - SUM(ISNULL(CREDIT,0)))  AS BALANCE  FROM AMIS_JOURNAL_DET WHERE JTYPE = '" & xJType & "' and voucherno = '" & xVOUCHERNO & "'")
    If Not rsCIB.EOF And Not rsCIB.BOF Then
        CIB = NumericVal(rsCIB!BALANCE)
    End If
End Function

Function CHKACCTCDE(xJType As String, xVOUCHERNO, lblJ_CLASS) As String
''ACCOUNT CHECKING BY NRE
    Dim rsCACCTCDE                                         As ADODB.Recordset
    Set rsCACCTCDE = New ADODB.Recordset
    Set rsCACCTCDE = gconDMIS.Execute("SELECT ACCT_CODE FROM AMIS_JOURNAL_DET WHERE JTYPE = '" & xJType & "' and voucherno = '" & xVOUCHERNO & "' AND ACCT_CODE = " & N2Str2Null(lblJ_CLASS.Caption) & " AND CREDIT > 0")
    If Not rsCACCTCDE.EOF And Not rsCACCTCDE.BOF Then
        CHKACCTCDE = rsCACCTCDE!ACCT_CODE
    End If
End Function

Function XDETAIL(xJType As String, xVOUCHERNO) As String
Dim rsGJ_Detail                                    As ADODB.Recordset
''***AR ACCOUNT
    Set rsGJ_Detail = New ADODB.Recordset
    Set rsGJ_Detail = gconDMIS.Execute("Select * from AMIS_AR where SJVOUCHERNO = '" & xJType + "-" + xVOUCHERNO & "'")
    If Not rsGJ_Detail.EOF And Not rsGJ_Detail.BOF Then
        XDETAIL = Null2String(rsGJ_Detail!ACCOUNT_CODE)
        Exit Function
    End If

    Set rsGJ_Detail = New ADODB.Recordset
    Set rsGJ_Detail = gconDMIS.Execute("Select * from AMIS_Detail where JTYPE ='" & xJType & "'  and voucherno = '" & xVOUCHERNO & "'")
    If Not rsGJ_Detail.EOF And Not rsGJ_Detail.BOF Then
        XDETAIL = Null2String(rsGJ_Detail!ACCT_CODE)
        Exit Function
    End If

''***AP ACCOUNT
    Set rsGJ_Detail = New ADODB.Recordset
    Set rsGJ_Detail = gconDMIS.Execute("Select * from AMIS_AP where VOUCHERNO = '" & xJType + "-" + xVOUCHERNO & "'")
    If Not rsGJ_Detail.EOF And Not rsGJ_Detail.BOF Then
        XDETAIL = Null2String(rsGJ_Detail!ACCT_CODE)
        Exit Function
    End If
    ...
    Set rsGJ_Detail = New ADODB.Recordset
    Set rsGJ_Detail = gconDMIS.Execute("Select * from AMIS_DetailS where JTYPE ='" & xJType & "'  and voucherno = '" & xVOUCHERNO & "'")
    If Not rsGJ_Detail.EOF And Not rsGJ_Detail.BOF Then
        XDETAIL = Null2String(rsGJ_Detail!ACCT_CODE)
        Exit Function
    End If
...
'Call ChangeListVwColor(lstDetails, lstDetails.ListItems.Count, lstDetails.ColumnHeaders.Count)
'Sub ChangeListVwForeColor(lv As ListView, rowcount As Integer, columncount As Integer)
'    Dim c As Integer
'    Dim r As Integer
'
'    For r = 1 To rowcount
...
'                lv.ListItems(r).ListSubItems(c).ForeColor = vbGreen
'            Next
'        End If
'    Next
'End Sub

Public Sub AlternateBackground(lv As ListView, xForm As Form)
    Dim lH      As Long
    Dim lSM     As Byte
    ...
    With lv
        If .View = lvwReport And .ListItems.Count Then
            Set picAlt = xForm.ControlS.Add("VB.PictureBox", "picAlt")
            ...
            With picAlt
                ...
                picAlt.Line (0, lH)-(.ScaleWidth, lH * 2), &H80000004, BF
                Set lv.Picture = .Image
            End With
            Set picAlt = Nothing
            xForm.ControlS.Remove "picAlt"
