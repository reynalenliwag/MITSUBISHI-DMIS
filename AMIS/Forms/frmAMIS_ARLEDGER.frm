...
Sub LOAD_ACCT_CODE()
    ...
    rsLOAD_ACCT_CODE.Open "SELECT DESCRIPTION FROM AMIS_CHARTACCOUNT WHERE LEFT(ACCTCODE,2) = '11' AND  IS_SCHEDULE_ACCNT = '1'", gconDMIS, adOpenKeyset
...
'        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC ON AR.ACCOUNT_CODE=AC.ACCTCODE WHERE STATUS='P' AND IS_SCHEDULE_ACCNT=1 AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND CUSTOMERNAME = '" & txtName.Text & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTO.Value & "' ORDER BY JDATE,SJVOUCHERNO", gconDMIS, adOpenKeyset
    
     ''updated by RRL 07/07/2015
     rsLOAD_AR.Open "SELECT AR.JDATE,AR.SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO AS INVOICE,AR.AMOUNT_TOPAY,AR.AMOUNT_PAID,AR.INVOICENO,AR.INVOICETYPE,AR.CUSTOMERCODE,AR.ACCOUNT_CODE,RIGHT(AR.SJVOUCHERNO,6) AS VOUCHERNO FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC ON AR.ACCOUNT_CODE=AC.ACCTCODE " & _
                       "INNER JOIN AMIS_JOURNAL_DET DT ON CASE WHEN (LEFT(SJVOUCHERNO,2) = 'GJ' OR LEFT(SJVOUCHERNO,2) = 'SJ') THEN LEFT(SJVOUCHERNO,2) ELSE LEFT(SJVOUCHERNO,3) END = DT.JTYPE AND RIGHT(SJVOUCHERNO,6) = DT.VOUCHERNO AND AR.ACCOUNT_CODE = DT.ACCT_CODE " & _
                       "WHERE AR.STATUS='P' AND IS_SCHEDULE_ACCNT=1 AND AR.CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND AR.CUSTOMERNAME = '" & txtName.Text & "' AND AR.Jdate >= '" & dtFrom.Value & "'and AR.Jdate <= '" & dtTo.Value & "' " & _
                       "GROUP BY AR.JDATE,SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO,AR.AMOUNT_TOPAY,AR.AMOUNT_PAID,AR.INVOICENO , AR.INVOICETYPE, CUSTOMERCODE, ACCOUNT_CODE, Right(AR.SJVOUCHERNO, 6) ORDER BY AR.JDATE,AR.SJVOUCHERNO", gconDMIS, adOpenKeyset
    Else
...
''               "ACCOUNT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "') ORDER BY JDATE,SJVOUCHERNO", gconDMIS, adOpenKeyset
        ...
        '    '    UPDATED BY RRL 07/29/2015
            If COMPANY_CODE = "HSM" Then
                rsLOAD_AR.Open "SELECT AR.JDATE,SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,AR.INVOICENO,AR.INVOICETYPE," & _
                               " CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO  FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC" & _
                               " ON AR.ACCOUNT_CODE=AC.ACCTCODE inner join AMIS_JOURNAL_DET dt  on dt.Acct_Code= ar.ACCOUNT_CODE WHERE AR.STATUS='P' AND IS_SCHEDULE_ACCNT=1" & _
                               " AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND AR.Jdate >= '" & dtFrom.Value & "'and AR.Jdate <= '" & dtTo.Value & "' AND " & _
                               "ACCOUNT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT  WHERE DESCRIPTION =  '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "')" & _
                               "GROUP BY AR.JDATE,SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO ,AMOUNT_TOPAY,AMOUNT_PAID,AR.INVOICENO,AR.INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) order by ar.jdate asc ", gconDMIS, adOpenKeyset
            Else
                rsLOAD_AR.Open "SELECT AR.JDATE,SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,AR.INVOICENO,AR.INVOICETYPE," & _
                               " CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO  FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC" & _
                               " ON AR.ACCOUNT_CODE=AC.ACCTCODE inner join AMIS_JOURNAL_DET dt  on dt.Acct_Code= ar.ACCOUNT_CODE WHERE AR.STATUS='P' AND IS_SCHEDULE_ACCNT=1" & _
                               " AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND AR.ENTITYCODE = 'C' AND AR.Jdate >= '" & dtFrom.Value & "'and AR.Jdate <= '" & dtTo.Value & "' AND " & _
                               "ACCOUNT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT  WHERE DESCRIPTION =  '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "')" & _
                               "GROUP BY AR.JDATE,SJVOUCHERNO,AR.INVOICETYPE + '-' + AR.INVOICENO ,AMOUNT_TOPAY,AMOUNT_PAID,AR.INVOICENO,AR.INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) ", gconDMIS, adOpenKeyset
            End If
        End If
    ...
    If Not rsLOAD_AR.EOF And Not rsLOAD_AR.BOF Then
        Do While Not rsLOAD_AR.EOF
            ...
            End If
            If xSJVOUCHERNO = "COB" Or xSJVOUCHERNO = "SJ" Or xSJVOUCHERNO = "CRJ" Or xSJVOUCHERNO = "GJ" Or xSJVOUCHERNO = "CDM" Or xSJVOUCHERNO = "CCM" Or xSJVOUCHERNO = "VDM" Or xSJVOUCHERNO = "VCM" Then
                Set REC = rptLEDGER.Records.Add
...
Sub LOAD_AR_ENTITY_PRINTING()
   Dim xSJVOUCHERNO                                        As String
    ...
    Set rsLOAD_AR = New ADODB.Recordset
    If cboAR_ACCT_CODE.Text = "ALL" Then
        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC ON AC.ACCTCODE=AR.ACCOUNT_CODE WHERE STATUS = 'P' AND  IS_SCHEDULE_ACCNT=1 AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND CUSTOMERNAME = '" & txtName.Text & "' AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTo.Value & "' ORDER BY JDATE,SJVOUCHERNO", gconDMIS, adOpenKeyset
    Else
        If COMPANY_CODE = "DGI" Then
            rsLOAD_AR.Open "select * from " & _
            "(SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS FROM AMIS_AR AR INNER JOIN AMIS_CHARTACCOUNT AC ON AR.ACCOUNT_CODE=AC.ACCTCODE WHERE STATUS='P' AND IS_SCHEDULE_ACCNT=1 AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND Jdate >= '" & dtFrom.Value & "' and Jdate <= '" & dtTo.Value & "' AND ACCOUNT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "' )) as a " & _
            "Union All" & _
            "(SELECT JDATE,JTYPE + '-' + VOUCHERNO AS SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE, AMOUNT_TOPAY ='0.00',INVOICEAMOUNT AS AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCT_CODE as ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS FROM AMIS_DETAIL WHERE STATUS='P' AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND ACCT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "' ) AND Jdate >= '" & dtFrom.Value & "' and Jdate <= '" & dtTo.Value & "') " & _
            "ORDER BY INVOICENO", gconDMIS, adOpenKeyset
        Else
    ...
    If Not rsLOAD_AR.EOF And Not rsLOAD_AR.BOF Then
        Do While Not rsLOAD_AR.EOF
            ...
            Else
                xSJVOUCHERNO = Left(Null2String(rsLOAD_AR!SJVoucherno), 2)
            End If
            If xSJVOUCHERNO = "COB" Or xSJVOUCHERNO = "SJ" Or xSJVOUCHERNO = "GJ" Or xSJVOUCHERNO = "CRJ" Or xSJVOUCHERNO = "CDM" Or xSJVOUCHERNO = "CCM" Or xSJVOUCHERNO = "VDM" Or xSJVOUCHERNO = "VCM" Then
                xCounter = xCounter + 1
                ...
                If NumericVal(rsLOAD_AR!AMOUNT_TOPAY) <> 0 Then
                    ...
                    xlWorksheet.Cells(xCounter, "G") = (Trim(Null2String(rsLOAD_AR!remarks)))
                Else
            ...
            ElseIf xSJVOUCHERNO = "APJ" Or xSJVOUCHERNO = "CDJ" Then
               ...
               'End If
                ...
                'SJR 7/29/15 | 11/03/2015
                If COMPANY_CODE = "DGI" Then
                    'ELDAN 10-20-14
                    'Call REFERENCE_INVOICE_PRINTING(Right(Null2String(rsLOAD_AR!SJVoucherno), 6), Left(Null2String(rsLOAD_AR!SJVoucherno), 3), txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                    Call REFERENCE_INVOICE_PRINTING(Mid(Null2String(rsLOAD_AR!INVOICE), 4), Left(Null2String(rsLOAD_AR!INVOICE), 2), txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                    Call ADJUSTMENT_BYVOUHCHERNO_PRINTING(Null2String(rsLOAD_AR!VOUCHERNO), xSJVOUCHERNO, txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                ElseIf COMPANY_CODE = "DJM" Or COMPANY_CODE = "HSM" Or COMPANY_CODE = "HSB" Or COMPANY_CODE = "HLB" Or COMPANY_CODE = "HBC" Or COMPANY_CODE = "HCR" Then
                    Call REFERENCE_INVOICE_PRINTING(Right(Null2String(rsLOAD_AR!INVOICENO), 15), Left(Null2String(rsLOAD_AR!INVOICETYPE), 3), txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                    Call ADJUSTMENT_BYVOUHCHERNO_PRINTING(Null2String(rsLOAD_AR!VOUCHERNO), xSJVOUCHERNO, txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                Else
                    Call REFERENCE_INVOICE_PRINTING(Right(Null2String(rsLOAD_AR!SJVoucherno), 6), Left(Null2String(rsLOAD_AR!SJVoucherno), 3), txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
                    Call ADJUSTMENT_BYVOUHCHERNO_PRINTING(Null2String(rsLOAD_AR!VOUCHERNO), xSJVOUCHERNO, txtCode.Text, Null2String(rsLOAD_AR!ACCOUNT_CODE))
...
'                   "AND Jdate >= '" & dtFrom.Value & "'and Jdate <= '" & dtTO.Value & "'", gconDMIS, adOpenKeyset
    ...
    ''updated by RRL 07/07/2015
        If COMPANY_CODE = "HSM" Then
            rsINVOICE.Open "select DET.INVOICETYPE , DET.INVOICEAMOUNT ,DET.INVOICENO,det.CUSTOMERCODE,det.ACCT_CODE , " & _
            "Det.JDATE , Det.REMARKS, Det.VOUCHERNO, Det.JTYPE, Det.SJVoucherno, Det.invoicedate, " & _
            "Det.MODIFIED_DATE , Det.ENTITYCODE, Det.REFCODE, Det.Status " & _
            "from AMIS_DETAIL det inner join AMIS_JOURNAL_DET dt on det.ACCT_CODE=DT.ACCT_CODE " & _
            "where DET.STATUS='P' AND DET.INVOICETYPE='" & xInvoiceType & "' AND DET.ACCT_CODE='" & xAcctCode & "' and customercode='" & txtCode.Text & "' " & _
            "AND det.Jdate >= '" & dtFrom.Value & "' and " & _
            "det.JDATE <= '" & dtTo.Value & "' AND DET.INVOICENO='" & xInvoiceNo & "'" & _
            "GROUP BY  DET.INVOICETYPE , DET.INVOICEAMOUNT , DET.VOUCHERNO ,DET.INVOICENO, " & _
            "det.CUSTOMERCODE,det.ACCT_CODE ,DET.JDATE,DET.REMARKS,DET.VOUCHERNO,DET.JTYPE, " & _
            "Det.SJVoucherno , Det.invoicedate, Det.MODIFIED_DATE, Det.ENTITYCODE, Det.REFCODE, Det.Status order by det.jdate asc", gconDMIS, adOpenKeyset
        Else
             rsINVOICE.Open "select DET.INVOICETYPE , DET.INVOICEAMOUNT ,DET.INVOICENO,det.CUSTOMERCODE,det.ACCT_CODE , " & _
            "Det.JDATE , Det.REMARKS, Det.VOUCHERNO, Det.JTYPE, Det.SJVoucherno, Det.invoicedate, " & _
            "Det.MODIFIED_DATE , Det.ENTITYCODE, Det.REFCODE, Det.Status " & _
            ...
            "GROUP BY  DET.INVOICETYPE , DET.INVOICEAMOUNT , DET.VOUCHERNO ,DET.INVOICENO, " & _
            "det.CUSTOMERCODE,det.ACCT_CODE ,DET.JDATE,DET.REMARKS,DET.VOUCHERNO,DET.JTYPE, " & _
            "Det.SJVoucherno , Det.invoicedate, Det.MODIFIED_DATE, Det.ENTITYCODE, Det.REFCODE, Det.Status", gconDMIS, adOpenKeyset
        End If
...
Sub ADJUSTMENT_BYVOUHCHERNO_PRINTING(xVOUCHERNO As String, xJType As String, xCUSCDE As String, xACCT_CODE As String)
    Dim rsADJ                                               As ADODB.Recordset
    Set rsADJ = New ADODB.Recordset
    rsADJ.Open "SELECT JDATE, SJVOUCHERNO, INVOICETYPE + '-' + INVOICENO AS REF_INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,REMARKS " & _
...
Sub FORWARDED_BALANCE()
    ...
    If cboAR_ACCT_CODE.Text = "ALL" Then
        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS " & _
...
'                        "Union All" & _
'                        "(SELECT JDATE,JTYPE + '-' + VOUCHERNO AS SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO AS INVOICE, AMOUNT_TOPAY ='0.00',INVOICEAMOUNT AS AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCT_CODE as ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS FROM AMIS_DETAIL WHERE STATUS='P' AND CUSTOMERCODE = '" & RTrim(LTrim(txtCode.Text)) & "' AND ACCT_CODE = (SELECT ACCTCODE FROM AMIS_CHARTACCOUNT WHERE DESCRIPTION = '" & (RTrim(LTrim(cboAR_ACCT_CODE.Text))) & "' ) AND Jdate >= '" & dtFrom.Value & "' and Jdate <= '" & dtTO.Value & "') " & _
'                        "ORDER BY JDATE, SJVOUCHERNO", gconDMIS, adOpenKeyset
'        Else
        rsLOAD_AR.Open "SELECT JDATE,SJVOUCHERNO,INVOICETYPE + '-' + INVOICENO,AMOUNT_TOPAY,AMOUNT_PAID,INVOICENO,INVOICETYPE,CUSTOMERCODE,ACCOUNT_CODE,RIGHT(SJVOUCHERNO,6) AS VOUCHERNO,REMARKS " & _
