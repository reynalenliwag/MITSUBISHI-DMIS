...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If Shift = 2 Then
        ...
        Case vbKeyI: SearchTab.SelectedItem = 1: txtInvoiceNo_Change
        End Select
        On Error Resume Next
...
Private Sub ListVendorPayeeName_DblClick()
    frmAMISVendorAPOpening.SearchVoucherNo (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(1)))
    Unload Me
End Sub
...
Private Sub ListInvoiceNo_DblClick()
    frmAMISVendorAPOpening.SearchVoucherNo (Trim(Me.ListInvoiceNo.SelectedItem.SubItems(1)))
    Unload Me
End Sub
...
Private Sub SearchTab_SelectedChanged(ByVal Item As XtremeSuiteControls.ITabControlItem)
    ...
    Case 2
        ...
        Me.Caption = "Search Item by InvoiceNo"
        On Error Resume Next
        txtInvoiceNo.SetFocus
    End Select
End Sub
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 HD.VOUCHERNO,AL.ACCOUNTNAME,HD.INVOICENO,HD.JDATE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' order by VoucherNo asc")
    ...
    Else
        ...
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 HD.VOUCHERNO,AL.ACCOUNTNAME,HD.INVOICENO,HD.JDATE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "Where Jtype='VPJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_HD
            ListVoucherNo.Enabled = True
        Else
...
Private Sub txtVendorPayeeName_Change()
    If txtVendorPayeeName = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 AL.ACCOUNTNAME,HD.VOUCHERNO,HD.INVOICENO,HD.JDATE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' order by AL.ACCOUNTNAME asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        ...
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 AL.ACCOUNTNAME,HD.VOUCHERNO,HD.INVOICENO,HD.JDATE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' and AL.ACCOUNTNAME like '" & Trim(Me.txtVendorPayeeName) & "%' order by AL.ACCOUNTNAME asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVendorPayeeName.ListItems, rsJournal_HD
            ListVendorPayeeName.Enabled = True
        Else
...
Private Sub txtInvoiceNo_Change()
    If txtInvoiceNo = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("SELECT TOP 22 HD.INVOICENO,HD.VOUCHERNO,AL.ACCOUNTNAME,HD.JDATE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "where Jtype='VPJ' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListInvoiceNo.Sorted = False: Me.ListInvoiceNo.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("SELECT HD.VOUCHERNO,HD.JDATE,AL.ACCOUNTNAME,HD.BANKCODE " & _
                                            "FROM AMIS_JOURNAL_HD HD INNER JOIN ALL_ENTITY AL " & _
                                            "ON HD.VENDORCODE = AL.CODE  AND HD.ENTITY_CLASS = AL.ENTITYCODE " & _
                                            "Where Jtype='VPJ' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
