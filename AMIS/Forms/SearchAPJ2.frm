...
Private Sub Form_Load()
    ...
    txtVendorPayeeName.Enabled = True
...
Private Sub ListInvoiceNo_DblClick()
    ...
    If xJOURNALTYPE = "CDJ" Then
        ...
        On Error Resume Next
        frmAMISJournalEntry_CDJ.cmdPVSave.SetFocus
    End If
    Unload Me
End Sub
...
'    xOPTION1 = "VOUCHER"
    If txtInvoiceNo = "" Then
        ...
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            ...
                            "AMOUNT2PAY - ISNULL((SELECT SUM(AMOUNTPAID) FROM AMIS_DETAILS AD WHERE (AP.INVOICENO=AD.INVOICENO AND AP.VENDOR_CODE=AD.VENDORCODE AND AP.ACCT_CODE=AD.ACCT_CODE AND AP.VOUCHERNO = AD.PV_VOUCHERNO)),0) AS BALANCE,ACCT_CODE,DUEDATE " & _
                            ...
                            "WHERE T.BALANCE <> 0 AND T.VENDORCODE = '" & CURRENT_VENDORCODE & "' " & _
                            "GROUP BY T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
    ...
    Else
        ...
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            ...
                            "FROM AMIS_AP AP WHERE STATUS='P')T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.VENDORCODE = '" & CURRENT_VENDORCODE & "' AND T.BALANCE <> 0 AND T.INVOICENO LIKE '" & Trim(Repleys(Me.txtInvoiceNo)) & "%' " & _
                            "GROUP BY T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.JTYPE,T.VOUCHERNO ASC"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        Else
            SQL_STATEMENT = "SELECT T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            ...
                            "FROM AMIS_AP AP WHERE STATUS='P')T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE  T.BALANCE <> 0 AND T.INVOICENO LIKE '" & Trim(Repleys(Me.txtInvoiceNo)) & "%' " & _
                            "GROUP BY T.INVOICETYPEWITHNO,T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.VOUCHERNO,T.JTYPE ASC"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListInvoiceNo.ListItems, rsJournal_HD
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            "SELECT CASE WHEN LEN(VOUCHERNO)=10 THEN LEFT(VOUCHERNO,3) ELSE LEFT(VOUCHERNO,2) END AS JTYPE,RIGHT(VOUCHERNO,6) AS VOUCHERNO,JDATE,VENDOR_CODE AS VENDORCODE,VENDOR_NAME AS ACCOUNTNAME,INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT2PAY AS INVOICEAMT,ENTITYCODE, " & _
                            "AMOUNT2PAY - ISNULL((SELECT SUM(AMOUNTPAID) FROM AMIS_DETAILS AD WHERE (AP.INVOICENO=AD.INVOICENO AND AP.VENDOR_CODE=AD.VENDORCODE AND AP.ACCT_CODE=AD.ACCT_CODE AND AP.VOUCHERNO = AD.PV_VOUCHERNO)),0) AS BALANCE,ACCT_CODE,DUEDATE " & _
                            "FROM AMIS_AP AP WHERE STATUS='P') T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE  T.BALANCE <> 0 AND T.VENDORCODE = '" & CURRENT_VENDORCODE & "' " & _
                            "GROUP BY T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.JTYPE,T.VOUCHERNO ASC"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        Else
            SQL_STATEMENT = "SELECT T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            " FROM (" & _
                            " SELECT CASE WHEN LEN(VOUCHERNO)=10 THEN LEFT(VOUCHERNO,3) ELSE LEFT(VOUCHERNO,2) END AS JTYPE,RIGHT(VOUCHERNO,6) AS VOUCHERNO,JDATE,VENDOR_CODE AS VENDORCODE,VENDOR_NAME AS ACCOUNTNAME,INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT2PAY AS INVOICEAMT,ENTITYCODE, " & _
                            "AMOUNT2PAY - ISNULL((SELECT SUM(AMOUNTPAID) FROM AMIS_DETAILS AD WHERE (AP.INVOICENO=AD.INVOICENO AND AP.INVOICETYPE=AD.INVOICETYPE AND AP.VENDOR_CODE=AD.VENDORCODE AND AP.ACCT_CODE=AD.ACCT_CODE AND AP.VOUCHERNO = AD.PV_VOUCHERNO)),0) AS BALANCE,ACCT_CODE,DUEDATE " & _
                            "FROM AMIS_AP AP WHERE STATUS='P')T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.BALANCE <> 0 " & _
                            "GROUP BY T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
    ...
    Else
        ...
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            "SELECT CASE WHEN LEN(VOUCHERNO)=10 THEN LEFT(VOUCHERNO,3) ELSE LEFT(VOUCHERNO,2) END AS JTYPE,RIGHT(VOUCHERNO,6) AS VOUCHERNO,JDATE,VENDOR_CODE AS VENDORCODE,VENDOR_NAME AS ACCOUNTNAME,INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT2PAY AS INVOICEAMT,ENTITYCODE, " & _
                            "AMOUNT2PAY - ISNULL((SELECT SUM(AMOUNTPAID) FROM AMIS_DETAILS AD WHERE (AP.INVOICENO=AD.INVOICENO AND AP.VENDOR_CODE=AD.VENDORCODE AND AP.ACCT_CODE=AD.ACCT_CODE AND AP.VOUCHERNO=AD.PV_VOUCHERNO AND AP.VOUCHERNO = AD.PV_VOUCHERNO)),0) AS BALANCE,ACCT_CODE,DUEDATE " & _
                            "FROM AMIS_AP AP WHERE STATUS='P')T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE  T.VENDORCODE = '" & CURRENT_VENDORCODE & "' AND T.BALANCE <> 0 AND T.VOUCHERNO LIKE '" & Format(Trim(Repleys(Me.txtVoucherNo)), "000000") & "%' " & _
                            "GROUP BY T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.JTYPE,T.VOUCHERNO ASC"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        Else
            SQL_STATEMENT = "SELECT T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            ...
                            "FROM AMIS_AP AP WHERE STATUS='P')T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE  T.BALANCE <> 0 AND T.VOUCHERNO LIKE '" & Format(Trim(Repleys(Me.txtVoucherNo)), "000000") & "%' " & _
                            "GROUP BY T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.VOUCHERNO,T.JTYPE ASC"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        End If
...
'DESCRIPTION: ONLY TAG DETAILS WITH SCHEDULE
    If txtVendorPayeeName = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            "SELECT CASE WHEN LEN(VOUCHERNO)=10 THEN LEFT(VOUCHERNO,3) ELSE LEFT(VOUCHERNO,2) END AS JTYPE,RIGHT(VOUCHERNO,6) AS VOUCHERNO,JDATE,VENDOR_CODE AS VENDORCODE,VENDOR_NAME AS ACCOUNTNAME,INVOICENO AS INVOICETYPEWITHNO,INVOICETYPE,INVOICENO,INVOICEDATE,AMOUNT2PAY AS INVOICEAMT,ENTITYCODE, " & _
                            "AMOUNT2PAY - ISNULL((SELECT SUM(AMOUNTPAID) FROM AMIS_DETAILS AD WHERE (AP.INVOICENO=AD.INVOICENO  AND AP.VENDOR_CODE=AD.VENDORCODE AND AP.ACCT_CODE=AD.ACCT_CODE AND AP.VOUCHERNO = AD.PV_VOUCHERNO)),0) AS BALANCE,ACCT_CODE,DUEDATE " & _
                            "FROM AMIS_AP AP WHERE STATUS='P') T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.VENDORCODE = '" & CURRENT_VENDORCODE & "' AND T.BALANCE <> 0 " & _
                            "GROUP BY T.JTYPE,T.VOUCHERNO,T.JDATE,T.ACCOUNTNAME,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.ACCOUNTNAME,T.JTYPE"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        Else
            SQL_STATEMENT = "SELECT T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            ...
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.BALANCE <> 0 " & _
                            "GROUP BY T.ACCOUNTNAME, T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.ACCOUNTNAME,T.JTYPE"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        End If
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        ...
        If chkCurrentVend.Value = True Then
            SQL_STATEMENT = "SELECT T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            ...
                            "FROM AMIS_AP AP WHERE STATUS='P') T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.VENDORCODE = '" & CURRENT_VENDORCODE & "' AND T.BALANCE <> 0 AND T.ACCOUNTNAME like '" & Format(Trim(Repleys(Me.txtVendorPayeeName)), "000000") & "%' " & _
                            "GROUP BY T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        Else
            SQL_STATEMENT = "SELECT T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "FROM (" & _
                            ...
                            "FROM AMIS_AP AP WHERE STATUS='P') T " & _
                            "INNER JOIN AMIS_JOURNAL_DET DET ON T.ACCT_CODE = DET.ACCT_CODE AND T.JTYPE = DET.JTYPE AND T.VOUCHERNO = DET.VOUCHERNO " & _
                            "WHERE T.BALANCE <> 0 AND T.ACCOUNTNAME like '" & Format(Trim(Repleys(Me.txtVendorPayeeName)), "000000") & "%' " & _
                            "GROUP BY T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO,T.JDATE,T.INVOICETYPEWITHNO,T.INVOICETYPE,T.INVOICENO,T.INVOICEDATE,T.INVOICEAMT,T.BALANCE,T.VENDORCODE,T.ENTITYCODE,T.ACCT_CODE,T.DUEDATE " & _
                            "ORDER BY T.ACCOUNTNAME,T.JTYPE,T.VOUCHERNO"
            Set rsJournal_HD = gconDMIS.Execute(SQL_STATEMENT)
        End If
