...
Begin VB.Form frmAMISSearchVCJ 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Search Vendor Credit Memo"
   ClientHeight    =   6450
   ClientLeft      =   2970
   ClientTop       =   3495
   ClientWidth     =   8745
   ...
   End
End
Attribute VB_Name = "frmAMISSearchVCJ"
...
Private Sub ListBankName_DblClick()
    frmAMISJournalEntry_VCM.SearchVoucherNo (Trim(Me.ListBankName.SelectedItem.SubItems(3)))
    Unload Me
End Sub
...
Private Sub ListVendorPayeeName_DblClick()
    frmAMISJournalEntry_VCM.SearchVoucherNo (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(2)))
    Unload Me
End Sub
...
Private Sub ListVoucherNo_DblClick()
    frmAMISJournalEntry_VCM.SearchVoucherNo (Trim(Me.ListVoucherNo.SelectedItem))
    Unload Me
End Sub
...
Private Sub ListVendorPayeeName_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        frmAMISJournalEntry_VCM.SearchVoucherNo (Trim(Me.ListVendorPayeeName.SelectedItem.SubItems(1)))
        Unload Me
    End If
End Sub
...
Private Sub ListBankName_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        frmAMISJournalEntry_VCM.SearchVoucherNo (Trim(Me.ListBankName.SelectedItem.SubItems(2)))
        Unload Me
    End If
End Sub
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = gconDMIS.Execute("select AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,All_VENDOR.nameofvendor,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where Jtype IN('CDM','VCM') and status = '" & StatusToSearch & "' order by VoucherNo asc")
...
Private Sub txtVendorPayeeName_Change()
    ...
    Else
        Me.ListVendorPayeeName.Sorted = False: Me.ListVendorPayeeName.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select All_VENDOR.nameofvendor,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.BankCode from AMIS_Journal_HD inner join All_VENDOR on AMIS_Journal_Hd.vendorcode = All_VENDOR.code where Jtype IN('CDM','VCM')  and status = '" & StatusToSearch & "' AND All_VENDOR.nameofvendor like '" & Trim(Me.txtVendorPayeeName) & "%' order by All_VENDOR.nameofvendor asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListVendorPayeeName.ListItems, rsJournal_HD
            ListVendorPayeeName.Enabled = True
        Else
...
Private Sub txtBankName_Change()
    If txtBankName = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select VoucherNo,Jdate,VendorCode,BankCode from AMIS_Journal_HD where Jtype IN('APJ','VCM')  and status = '" & StatusToSearch & "' order by BankCode asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListBankName.Sorted = False: Me.ListBankName.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select VoucherNo,Jdate,VendorCode,BankCode from AMIS_Journal_HD where Jtype IN('APJ','VCM')  and status = '" & StatusToSearch & "' AND BankCode like '" & Trim(Me.txtBankName) & "%' order by BankCode asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            Listview_Loadval Me.ListBankName.ListItems, rsJournal_HD
            ListBankName.Enabled = True
        Else
