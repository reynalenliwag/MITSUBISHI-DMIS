...
Private Sub cmdSave_Click()
    ...
    'SJR 7/10/14
    'DESCRIPTION: USER WON'T BE ABLE TO EDIT BANK CODE WHEN BANK IS ALREADY USED FOR BANK DEPOSIT.
    ElseIf AddorEdit = "EDIT" Then
        Dim countBankCOde                                 As Long
        countBankCOde = gconDMIS.Execute("SELECT COUNT(*) FROM CMIS_BANKDEPO WHERE DEPOSIT_TO=" & N2Str2Null(lblBankCode.Caption)).Fields(0).Value
        If countBankCOde > 0 Then
            MsgBox "Bank record already used in bank deposits." & vbCrLf & "Cannot edit record.", vbExclamation
            Exit Sub
        End If
