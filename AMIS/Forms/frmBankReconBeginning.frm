...
Begin VB.Form frmBankReconBeginning 
   ...
   Begin VB.PictureBox picPayables 
      ...
      Begin RichTextLib.RichTextBox txtParticulars 
         ...
         ScrollBars      =   2
         Appearance      =   0
         TextRTF         =   $"frmBankReconBeginning.frx":2AE5
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Courier New"
            Size            =   9.75
            Charset         =   0
...
Private Sub Form_Load()
    ...
    If Not rsJDate.EOF And Not rsJDate.BOF Then
        dtCurrent.Value = Null2Date(rsJDate!Cut_Off_Date)
    End If

    Dim rsOpeningBalance                                    As ADODB.Recordset
    Set rsOpeningBalance = New ADODB.Recordset
    rsOpeningBalance.Open "SELECT DT.DEBIT FROM ALL_BANKS AB " & _
                          "INNER JOIN AMIS_JOURNAL_DET DT " & _
                          "ON AB.ACCTCODE=DT.ACCT_CODE " & _
                          "INNER JOIN AMIS_JOURNAL_HD HD " & _
                          "ON HD.VOUCHERNO=DT.VOUCHERNO AND HD.JTYPE=DT.JTYPE " & _
                          "WHERE AB.BANKACCTNO='" & frmReconcileAccount.lblBank.Caption & "' AND HD.JTYPE='OPB'", gconDMIS, adOpenForwardOnly
    If Not rsOpeningBalance.EOF And Not rsOpeningBalance.BOF Then
        txtBook.Text = ToDoubleNumber(N2Str2Zero(rsOpeningBalance!Debit))
    End If
End Sub
...
Private Sub optDeposits_Click()
    ...
    InitCombo
    cboBankName2.Text = lblBankName.Caption
    txtBankCode.Text = SetBankCode(cboBankName2)
    xSELECTED = "Customer"
...
Private Sub optOutstanding_Click()
    ...
    picList.Enabled = True
    InitCombo
    cboBankName.Text = lblBankName.Caption
    txtBankCode.Text = SetBankCode(cboBankName)
    xSELECTED = "Vendor"
    cmdVendor.SetFocus
End Sub
...
Private Sub txtORAmount_GotFocus()
    If NumericVal(txtORAmount.Text) = 0 Then
        txtORAmount.Text = ""
    Else
...
Sub ADD_COLUMN()
    Dim SQL                                                 As String
    SQL = ""
    SQL = "IF NOT EXISTS(SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='ALL_BANKS' AND COLUMN_NAME='BEGINNING_BOOK')" & vbCrLf
    SQL = SQL + "ALTER TABLE ALL_BANKS" & vbCrLf
    SQL = SQL + "ADD     BEGINNING_BOOK      DECIMAL(18,2)"
    gconDMIS.Execute (SQL)

    SQL = ""
    SQL = "IF NOT EXISTS(SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='ALL_BANKS' AND COLUMN_NAME='BEGINNING_BANK')" & vbCrLf
    SQL = SQL + "ALTER TABLE ALL_BANKS" & vbCrLf
    SQL = SQL + "ADD     BEGINNING_BANK  DECIMAL(18,2)"
    gconDMIS.Execute (SQL)
End Sub
