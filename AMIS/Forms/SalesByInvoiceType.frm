...
Private Sub cmdPrint_Click()
    ...
    If Not rsJournal_HD.EOF And Not rsJournal_HD.EOF Then
        ...
        Set rsProfile = gconDMIS.Execute("Select top 1 * from ALL_PROFILE")
        If Not (rsProfile.EOF And rsProfile.BOF) Then
            ...
            If INVOICE_Type = "MATERIALS-CASH" Then
                rptAMISIncomeStatement.ReportTitle = "MATERIALS CASH INVOICE"
                rptAMISIncomeStatement.WindowTitle = "MATERIALS CASH INVOICE"
            End If
            
            If INVOICE_Type = "MATERIALS-CHARGE" Then
                rptAMISIncomeStatement.ReportTitle = "MATERIALS CHARGE INVOICE"
                rptAMISIncomeStatement.WindowTitle = "MATERIALS CHARGE INVOICE"
            End If
            
            If INVOICE_Type = "ACCESSORIES-CASH" Then
                rptAMISIncomeStatement.ReportTitle = "ACCESSORIES CASH INVOICE"
                rptAMISIncomeStatement.WindowTitle = "ACCESSORIES CASH INVOICE"
            End If
            
            If INVOICE_Type = "ACCESSORIES-CHARGE" Then
                rptAMISIncomeStatement.ReportTitle = "ACCESSORIES CHARGE INVOICE"
                rptAMISIncomeStatement.WindowTitle = "ACCESSORIES CHARGE INVOICE"
            End If
            ...
            If INVOICE_Type = "SERVICE-CASH" Then
                If COMPANY_CODE = "HCA" Or COMPANY_CODE = "DJM" Then
                    rptAMISIncomeStatement.ReportTitle = "SALES SERVICE INVOICE"
                    rptAMISIncomeStatement.WindowTitle = "SALES SERVICE INVOICE"
                Else
                    rptAMISIncomeStatement.Reset
        ...
        If INVOICE_Type = "VEHICLE" Then
            If COMPANY_CODE = "HCA" Then
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\VehicleInvoices.rpt", "{OUTPUT_TAX.JDate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {OUTPUT_TAX.JDate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            ElseIf COMPANY_CODE = "DJM" Then
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\VehicleInvoices.rpt", "{AMIS_VW_VEHICLE_INVOICE_REGISTRY.JDate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {AMIS_VW_VEHICLE_INVOICE_REGISTRY.JDate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            Else
        ...
        If INVOICE_Type = "PARTS-CASH" Then
            If COMPANY_CODE = "HCA" Then
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\PartsInvoice.rpt", "{OUTPUT_TAX.JDate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {OUTPUT_TAX.JDate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            ElseIf COMPANY_CODE = "DJM" Then
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\PartsInvoice.rpt", "{Invoice_Registry_Parts.JDate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Invoice_Registry_Parts.JDate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            Else
        ...
        If INVOICE_Type = "MATERIALS-CASH" Then
            PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\MaterialsCashInvoice.rpt", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            LogAudit "V", "INCOME STATEMENT - MATERIALS CASH INVOICE", " & dtpFrom & " - " & dtpTo"
        End If
        
        If INVOICE_Type = "MATERIALS-CHARGE" Then
            PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\MaterialsChargeInvoice.rpt", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            LogAudit "V", "INCOME STATEMENT - MATERIALS CHARGE INVOICE", " & dtpFrom & " - " & dtpTo"
        End If
        
        If INVOICE_Type = "ACCESSORIES-CASH" Then
            PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\AccessoriesCashInvoice.rpt", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            LogAudit "V", "INCOME STATEMENT - ACCESSORIES CASH INVOICE", " & dtpFrom & " - " & dtpTo"
        End If
        
        If INVOICE_Type = "ACCESSORIES-CHARGE" Then
            PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\AccessoriesChargeInvoice.rpt", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            LogAudit "V", "INCOME STATEMENT - ACCESSORIES CHARGE INVOICE", " & dtpFrom & " - " & dtpTo"
        End If
        ...
        If INVOICE_Type = "SERVICE-CASH" Then
            If COMPANY_CODE = "HCA" Then
                rptAMISIncomeStatement.Formulas(32) = "JDATETO =  '" & dtpTo.Value & "'"
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\Service_Invoices.rpt", "{OUTPUT_TAX.JDATE} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {OUTPUT_TAX.JDATE} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            ElseIf COMPANY_CODE = "DJM" Then
                rptAMISIncomeStatement.Formulas(32) = "JDATETO =  '" & dtpTo.Value & "'"
                PrintSQLReport rptAMISIncomeStatement, AMIS_REPORT_PATH & "InvoicesReport\Service_Invoices.rpt", "{AMIS_VW_SERVICE_JOURNAL_HD_REGISTRY.JDATE} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {AMIS_VW_SERVICE_JOURNAL_HD_REGISTRY.JDATE} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", DMIS_REPORT_Connection, 1
            Else
                rptAMISIncomeStatement.Formulas(2) = "CompanyName = '" & Null2String(rsProfile!CompanyName) & "'"
                rptAMISIncomeStatement.Formulas(1) = "CompanyAddress = '" & Null2String(rsProfile!Companyaddress) & "'"
