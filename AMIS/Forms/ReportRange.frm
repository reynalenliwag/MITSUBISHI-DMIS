...
Private Sub cmdPrint_Click()
    ...
    End If
    If REPORT_RANGETYPE = "CREDITMEMO_REGISTER" Then
        ' If Function_Access(LOGID, "Acess_Print", "") = False Then Exit Sub
        Set rsJournal_HD = New ADODB.Recordset
        rsJournal_HD.Open "select * from AMIS_Journal_HD where jtype = 'CCM' AND INVOICETYPE='VI' and (jdate >= '" & dtpFrom & "' AND jdate <= '" & dtpTo & "') order by voucherno asc", gconDMIS, adOpenForwardOnly, adLockReadOnly
        If Not rsJournal_HD.EOF And Not rsJournal_HD.EOF Then
            ShowRangeReport dtpFrom, dtpTo, "CreditMemoRegisters", "Registers", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", "CREDIT MEMO REGISTERS", False
            Unload Me
        Else
            ShowNoRecord
        End If
        Call NEW_LogAudit("V", "CREDIT MEMO REGISTER REPORT", "", "", "", dtpFrom & " " & dtpTo, "", "")
    End If
    If REPORT_RANGETYPE = "INV_REGISTER" Then
        ...
        If Not rsJournal_HD.EOF And Not rsJournal_HD.EOF Then
            ...
            ElseIf COMPANY_CODE = "DGI" Then
                ShowRangeReport dtpFrom, dtpTo, "SalesInvoiceRegisters", "Registers", "{AMIS_VW_INVOICE_REGISTRY_NEW.JDATE} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {AMIS_VW_INVOICE_REGISTRY_NEW.JDATE} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", "INVOICES REGISTERS", False
            Else
                ShowRangeReport dtpFrom, dtpTo, "SalesInvoiceRegisters", "Registers", "{Journal_HD.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_HD.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ")", "INVOICES REGISTERS", False
            End If
            Unload Me
    ...
    If REPORT_RANGETYPE = "EX_TAX" Then
        ...
        If COMPANY_CODE = "CMC" Then
            rsJournal_HD.Open "select * from AMIS_Journal_Det where jtype in ('APJ','CDJ','GJ') and (jdate >= '" & dtpFrom & "' AND jdate <= '" & dtpTo & "')", gconDMIS, adOpenForwardOnly, adLockReadOnly
        Else
