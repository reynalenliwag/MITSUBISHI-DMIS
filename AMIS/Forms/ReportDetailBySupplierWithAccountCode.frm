    ...
    If Not rsJournal_HD.EOF And Not rsJournal_HD.EOF Then
        ...
        ElseIf REPORT_RANGETYPE = "SJ" Then
            ...
            If COMPANY_CODE = "HSM" Then
                ShowRangeReport dtpFrom, dtpTo, "SJDetailByCustomer", "AccountDetail", "{Journal_Hd.jdate} >= date(" & Year(dtpFrom) & "," & Month(dtpFrom) & "," & Day(dtpFrom) & ") AND {Journal_Hd.jdate} <= date(" & Year(dtpTo) & "," & Month(dtpTo) & "," & Day(dtpTo) & ") AND {Journal_Det.Acct_Code} = '" & txtDescription.Text & "' AND {AMIS_AR.CustomerCode} = '" & SetVendorCode(cboVendor.Text) & "'", "Account Detail Report By Customer", False
            Else
