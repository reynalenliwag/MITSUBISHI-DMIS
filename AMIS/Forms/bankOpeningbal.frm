...
Sub FillCboAcctName()
    Dim rsCOBAcctName                                       As ADODB.Recordset
    Set rsCOBAcctName = New ADODB.Recordset
    Set rsCOBAcctName = gconDMIS.Execute("Select * from AMIS_ChartAccount Where (Titles = '1101') AND (DESCRIPTION IS NOT NULL AND DESCRIPTION <> '-') Order by AcctCode asc")
    If Not rsCOBAcctName.EOF And Not rsCOBAcctName.BOF Then
...
'If Function_Access(LOGID, "Acess_CancelEntry", LocalAcess) = False Then Exit Sub
    If MsgBox("Are you sure you want to Cancel this Transaction?", vbQuestion + vbYesNo, "Cancel Journal") = vbYes Then
        ...
        If COMPANY_CODE = "HGC" Or COMPANY_CODE = "HSB" Or COMPANY_CODE = "HLB" Or COMPANY_CODE = "HCR" Then
...
Private Sub cmdSave_Click()
    ...
    If LTrim(RTrim(txtCustCode)) = "" Then
        MsgBox "Vendor Code must not be empty", vbInformation, "Incompelete Information"
        On Error Resume Next
    ...
    If AddorEdit = "ADD" Then
        ...
        gconDMIS.Execute "Insert into AMIS_Journal_HD" & _
                         ...
                         ", " & J_JNO & ", " & J_DEBIT & ", " & J_CREDIT & ", " & J_OUTBALANCE & "," & J_STATUS & ", " & J_CHECKNO & "," & N2Date2Null(txtCheckDate) & "," & J_BANKCODE & "," & J_REMARKS & "," & J_PAIDSTATUS & "," & J_RECEIVESTATUS & ",'" & LOGCODE & "','" & LOGPCNAME & "','" & LOGDATE & "')"
