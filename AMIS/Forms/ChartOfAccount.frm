...
Sub FillSearchGrid(XXX As String)
    ...
    If Option1.Value = True Then
        Set rsChartOfAccount = gconDMIS.Execute("select acctcode,Description,Accttype,ID,Is_Schedule_Accnt from AMIS_ChartAccount where description like'" & XXX & "%'")
    Else
        Set rsChartOfAccount = gconDMIS.Execute("select acctcode,Description,Accttype,ID,Is_Schedule_Accnt from AMIS_ChartAccount where acctcode like'" & XXX & "%'")
    End If
...
Private Sub cmdDelete_Click()
    ...
    If MsgBox("Delete Current Record", vbQuestion + vbYesNo, "Delete") = vbYes Then
        ...
        If Not rsCheckUsedAcctCode.EOF And Not rsCheckUsedAcctCode.BOF Then
            MessagePop RecLocekd, "Record already used", "Account is in use and cannot be deleted. Please check this account in General Ledger."
            Exit Sub
        End If
...
'SJR 091114
'Description: Cannot schedule an account if it is already used in amis_journal_det
Public Function hasGL(XXX As String) As Boolean
    Dim rsHasGL                                             As ADODB.Recordset
    Dim lvHasGL                                             As Integer
    
    Set rsHasGL = New ADODB.Recordset
    Set rsHasGL = gconDMIS.Execute("Select distinct acct_code from amis_journal_det where acct_code='" & XXX & "'")
        If Not (rsHasGL.EOF And rsHasGL.BOF) Then
            hasGL = True
        End If
    Set rsHasGL = Nothing
End Function
...
Private Sub cmdEdit_Click()
    ...
    Picture2.Visible = True
    chkSchedule.Enabled = True
    PREV_ACCT_CODE = Null2String(txtCode.Text)
    ...
    Else
        'SJR 091114
        If hasGL(txtCode.Text) = False Then
        Else
        chkSchedule.Enabled = False
        End If
    End If
...
Private Sub cmdSave_Click()
    ...
    If cboHeader.Text = "" Or cboSubHeader.Text = "" Or cboTitleCode.Text = "" Or cboDepartment.Text = "" Or cboAccountType.Text = "" Then
        MessagePop InfoVoid, "Invalid Entry", "All fields are required!"
        Exit Sub
    End If
    
    '01132016
    'DESCRIPTION ALREADY EXISTS VALIDATION
    Set rsChartAccountDup = New ADODB.Recordset
    rsChartAccountDup.Open "select description from AMIS_ChartAccount where description = " & vtxtDescription, gconDMIS
    If Not rsChartAccountDup.EOF And Not rsChartAccountDup.BOF Then
        MessagePop RecSaveError, "Duplicate Entry", "Account Description Already Exists!"
        Exit Sub
    End If
...
'        For lvHasGL = 1 To lstAccounts.ListItems.Count
'            Set rsHasGL = gconDMIS.Execute("Select distinct acct_code from amis_journal_det where acct_code='" & lstAccounts.ListItems(lvHasGL).Text & "'")
'            If Not (rsHasGL.EOF And rsHasGL.BOF) Then
''            lstAccounts.ListItems(lvHasGL).ListSubItems.Item(1).ForeColor = &H404040
''            lstAccounts.ListItems(lvHasGL).ListSubItems.Item(2).ForeColor = &H404040
'            Else
'            End If
'        Next
