...
Sub StoreMemVars()
    ...
    If Status = "TRIAL BALANCE" Then
        ...
        rsTrialBalance.Open "SELECT * FROM (SELECT VOUCHERNO,JTYPE,SUM(ISNULL(DEBIT,0)) AS DEBIT, SUM(ISNULL(CREDIT,0)) AS CREDIT FROM AMIS_JOURNAL_DET " & _
                            " WHERE STATUS = 'P' AND JTYPE NOT IN ('VPJ','COB') GROUP BY VOUCHERNO,JTYPE) A WHERE DEBIT <> CREDIT", gconDMIS
    ...
    ElseIf Status = "SLGLCUSTOMER" Then
        ...
        If COMPANY_CODE = "HCA2" Then
            rsSLGLCustomer.Open "SELECT * FROM (SELECT ACCTCODE AS ACCT_CODE,DESCRIPTION AS ACCT_NAME," & _
                                ...
                                "FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,5) IN ('11-02','11-03','11-06') " & _
                                ") T WHERE (SLBALANCE<>0 OR GLBALANCE<>0) ORDER BY ACCT_CODE", gconDMIS, adOpenKeyset
        Else
            rsSLGLCustomer.Open "SELECT * FROM (SELECT ACCTCODE AS ACCT_CODE,DESCRIPTION AS ACCT_NAME," & _
                                "CAST(ISNULL((SELECT  CASE WHEN H.JTYPE='COB' THEN (SUM(H.INVOICEAMT)) ELSE SUM(H.AMOUNTTOPAY) END  FROM AMIS_JOURNAL_HD H  INNER JOIN AMIS_JOURNAL_DET D  ON H.VOUCHERNO=D.VOUCHERNO  AND  H.JTYPE=D.JTYPE WHERE  H.JTYPE IN ('COB','VPJ') AND H.STATUS='P' AND ACCT_CODE =ACCTCODE GROUP BY H.JTYPE),0) AS DECIMAL(18,2)) [SLBALANCE] , " & _
                                "ISNULL((SELECT SUM(ISNULL(DEBIT,0))-SUM(ISNULL(CREDIT,0)) FROM AMIS_JOURNAL_DET WHERE JTYPE='OPB' AND STATUS='P' AND ACCT_CODE=ACCTCODE),0) [GLBALANCE] " & _
                                "FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,2) IN ('11') " & _
                                ") T  WHERE (SLBALANCE<>0 OR GLBALANCE<>0) ORDER BY ACCT_CODE", gconDMIS, adOpenKeyset
        End If
    ...
    ElseIf Status = "SLGLVENDOR" Then
        ...
        rsSLGLVendor.Open "SELECT * FROM (SELECT ACCTCODE AS ACCT_CODE, DESCRIPTION AS ACCT_NAME, " & _
                          "CAST(ISNULL((SELECT CASE WHEN H.JTYPE = 'VPJ' THEN (SUM(H.AMOUNTTOPAY)) ELSE SUM(H.INVOICEAMT) END FROM AMIS_JOURNAL_HD H  INNER JOIN AMIS_JOURNAL_DET D  ON H.VOUCHERNO=D.VOUCHERNO  AND  H.JTYPE=D.JTYPE WHERE  H.JTYPE IN ('VPJ','COB') AND H.STATUS='P' AND ACCT_CODE =ACCTCODE GROUP BY H.JTYPE),0) AS DECIMAL(18,2)) [SLBALANCE] , " & _
                          ...
                          "FROM AMIS_CHARTACCOUNT WHERE IS_SCHEDULE_ACCNT=1 AND LEFT(ACCTCODE,2) = '21' " & _
                          ") T WHERE (SLBALANCE<>0 OR GLBALANCE<>0) ORDER BY ACCT_CODE", gconDMIS, adOpenKeyset
