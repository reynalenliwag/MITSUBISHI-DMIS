...
Begin VB.Form frmAMISSearchCSJ 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Search Customer AR Opening"
   ClientHeight    =   6435
   ClientLeft      =   2970
   ClientTop       =   3495
   ClientWidth     =   8685
   ...
   Begin XtremeSuiteControls.TabControl SearchTab 
      ...
      Item(0).Tooltip =   "Search Customer AR Opening by Voucher Number"
      Item(0).ControlCount=   1
      Item(0).Control(0)=   "TabControlPage1"
      Item(1).Caption =   "By Customer Name"
      Item(1).Tooltip =   "Search Customer AR Opening by Customer Name"
      Item(1).ControlCount=   1
      Item(1).Control(0)=   "TabControlPage2"
      Item(2).Caption =   "By Invoice No"
      Item(2).Tooltip =   "Search Customer AR Opening by Invoice Number"
   ...
   End
End
Attribute VB_Name = "frmAMISSearchCSJ"
...
Private Sub ListCustomer_DblClick()
    On Error Resume Next
    frmAMISCustomerAROpening.SearchVoucherNo (Trim(Me.ListCustomer.SelectedItem.SubItems(2)))
    Unload Me
End Sub
...
Private Sub ListInvoiceNo_DblClick()
    On Error Resume Next
    frmAMISCustomerAROpening.SearchVoucherNo (Trim(Me.ListInvoiceNo.SelectedItem.SubItems(3)))
    Unload Me
End Sub
...
Private Sub ListVoucherNo_DblClick()
    On Error Resume Next
    frmAMISCustomerAROpening.SearchVoucherNo (Trim(Me.ListVoucherNo.SelectedItem))
    Unload Me
End Sub
...
Private Sub txtInvoiceNo_Change()
    If txtInvoiceNo = "" Then
        ...
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.Jtype='COB' and status = '" & StatusToSearch & "' order by AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo asc")
    ...
    Else
        Me.ListInvoiceNo.Sorted = False: Me.ListInvoiceNo.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO,ALL_ENTITY.ACCOUNTNAME,AMIS_Journal_Hd.JDATE,AMIS_Journal_Hd.VoucherNo from AMIS_Journal_HD inner join ALL_ENTITY on AMIS_Journal_Hd.customercode = ALL_ENTITY.CODE and AMIS_Journal_Hd.ENTITY_CLASS = ALL_ENTITY.ENTITYCODE   where AMIS_Journal_Hd.Jtype='COB' and status = '" & StatusToSearch & "' and AMIS_Journal_Hd.InvoiceNo like '" & Trim(Me.txtInvoiceNo) & "%' order by AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            ListInvoiceNo.Enabled = True
            Listview_Loadval Me.ListInvoiceNo.ListItems, rsJournal_HD
        Else
...
Private Sub txtVoucherNo_Change()
    If txtVoucherNo = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_CUSTMASTER_AMIS.CustName,AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO  from AMIS_Journal_HD inner join ALL_CUSTMASTER_AMIS on AMIS_Journal_Hd.customercode = ALL_CUSTMASTER_AMIS.CustCode where AMIS_Journal_Hd.Jtype='COB' and status = '" & StatusToSearch & "' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListVoucherNo.Sorted = False: Me.ListVoucherNo.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AMIS_Journal_Hd.VoucherNo,AMIS_Journal_Hd.JDATE,ALL_ENTITY.ACCOUNTNAME,AMIS_Journal_Hd.Invoicetype + ' ' + AMIS_Journal_Hd.InvoiceNo as INVOICETYPEWITHNO from AMIS_Journal_HD inner join ALL_ENTITY on AMIS_Journal_Hd.customercode = ALL_ENTITY.CODE and AMIS_Journal_Hd.ENTITY_CLASS = ALL_ENTITY.ENTITYCODE  where AMIS_Journal_Hd.Jtype='COB' and status = '" & StatusToSearch & "' and VoucherNo like '" & Format(Trim(Me.txtVoucherNo), "000000") & "%' order by VoucherNo asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            ListVoucherNo.Enabled = True
            Listview_Loadval Me.ListVoucherNo.ListItems, rsJournal_HD
        Else
...
Private Sub txtCustomer_Change()
    If txtCustomer = "" Then
        ...
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AE.ACCOUNTNAME,HD.JDATE,HD.VoucherNo,HD.Invoicetype + ' ' + HD.InvoiceNo as INVOICETYPEWITHNO from AMIS_Journal_HD  HD inner join ALL_ENTITY AE on HD.customercode = AE.CODE AND HD.ENTITY_CLASS = AE.ENTITYCODE where HD.Jtype='COB' and status = '" & StatusToSearch & "' order by AE.ACCOUNTNAME asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
    ...
    Else
        Me.ListCustomer.Sorted = False: Me.ListCustomer.ListItems.Clear
        Set rsJournal_HD = New ADODB.Recordset
        Set rsJournal_HD = gconDMIS.Execute("select TOP 22 AE.ACCOUNTNAME,HD.JDATE,HD.VoucherNo,HD.Invoicetype + ' ' + HD.InvoiceNo as INVOICETYPEWITHNO from AMIS_Journal_HD  HD inner join ALL_ENTITY AE on HD.customercode = AE.CODE AND HD.ENTITY_CLASS = AE.ENTITYCODE where HD.Jtype='COB' and status = '" & StatusToSearch & "' and AE.ACCOUNTNAME like '" & Trim(Me.txtCustomer) & "%' order by AE.ACCOUNTNAME asc")
        If Not (rsJournal_HD.EOF And rsJournal_HD.BOF) Then
            ListCustomer.Enabled = True
            Listview_Loadval Me.ListCustomer.ListItems, rsJournal_HD
        Else
