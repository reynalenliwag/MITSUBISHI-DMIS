    ...
    If chkClear.Value = 1 Then
        With grdRecon
            ...
            For GridNo = 1 To .Rows - 1
                If NumericVal(.Cell(GridNo, 7).Text) < 1 Then
                    .Cell(GridNo, 7).Text = 1
                    xSkip = False
                Else
                    xSkip = True
                End If
                If NumericVal(.Cell(GridNo, 7).Text) >= 1 Then
                    If xSkip = False Then
                        xdebit = xdebit - NumericVal(.Cell(GridNo, 5).Text)
                        xcredit = xcredit - NumericVal(.Cell(GridNo, 6).Text)
                    End If
                End If
                START_DEBIT = xdebit
...
Private Sub cmdReload_Click()
    ...
    lblDifference = ToDoubleNumber(Round((NumericVal(txtEndingBal.Text) + lblDeposit.Caption - lblOutstanding.Caption), 2))
    ...
    Else
        ...
        If grdRecon.ActiveCell.Col = 7 Then
            If NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 7).Text) >= 1 Then
                xdebit = xdebit - NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 5).Text)
                xcredit = xcredit - NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 6).Text)
                lblClearedDeposits = ToDoubleNumber(NumericVal(lblClearedDeposits.Caption) + NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 5).Text))
                lblClearedWithdrawals = ToDoubleNumber(NumericVal(lblClearedWithdrawals.Caption) + NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 6).Text))
            Else
                xdebit = xdebit + NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 5).Text)
                xcredit = xcredit + NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 6).Text)
                lblClearedDeposits = ToDoubleNumber(NumericVal(lblClearedDeposits.Caption) - NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 5).Text))
        ...
        If grdRecon.ActiveCell.Col = 8 Then
            If NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 8).Text) >= 1 Then
                xdebit = xdebit - NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 5).Text)
                xcredit = xcredit - NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 6).Text)
                lblBalance = ToDoubleNumber(NumericVal(lblBalance) + NumericVal(grdRecon.Cell(grdRecon.ActiveCell.Row, 6).Text))
        ...
        With grdRecon
            xVOUCHERNO = .Cell(.ActiveCell.Row, 9).Text
            xJType = .Cell(.ActiveCell.Row, 10).Text
            ...
            End If
            If NumericVal(.Cell(.ActiveCell.Row, 8).Text) > 0 Then
                gconDMIS.Execute "update AMIS_Journal_HD set" & _
                                 " ReconStatus = 'S' " & "" & _
                                 " where VoucherNo = '" & xVOUCHERNO & "' AND JType = '" & xJType & "'"
            End If
