...
Sub FillCboAcctName()
    ...
    '    Else
    If COMPANY_CODE = "DJM" Or COMPANY_CODE = "HCA" Or COMPANY_CODE = "HSM" Or COMPANY_CODE = "DSSC" Or COMPANY_CODE = "HGS" Then
        Set rsCOBAcctName = gconDMIS.Execute("Select * from AMIS_ChartAccount Where (IS_SCHEDULE_ACCNT=1 and left(Titles,2) ='11') Order by AcctCode asc")
    Else
...
Sub initMemvars()
    ...
    If COMPANY_CODE = "CMC" Or COMPANY_CODE = "HSM" Or COMPANY_CODE = "DSSC" Or COMPANY_CODE = "HMH" Or COMPANY_CODE = "DGI" Or COMPANY_CODE = "HCA" Or COMPANY_CODE = "HQA" Or COMPANY_CODE = "HNE" Or COMPANY_CODE = "DJM" Or COMPANY_CODE = "HCE" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "HMR" Or COMPANY_CODE = "HGS" Or COMPANY_CODE = "HSB" Or COMPANY_CODE = "HLB" Or COMPANY_CODE = "HCR" Or COMPANY_CODE = "DPI" Then
...
SaveInvoice:
            ...
            gconDMIS.Execute "Insert into AMIS_Journal_HD" & _
                             ...
                             ", " & J_JNO & ", " & J_DEBIT & ", " & J_CREDIT & ", " & J_OUTBALANCE & ", " & J_STATUS & ", " & J_CHECKNO & ", " & J_BANKCODE & "," & J_REMARKS & "," & J_PAIDSTATUS & "," & J_RECEIVESTATUS & ",'" & LOGCODE & "','" & LOGPCNAME & "','" & LOGDATE & "'," & J_ENTITYCLASS & ")"
...
Private Sub cmdUnPost_Click()
    ...
    If MsgBox("Are you sure you want to Unpost this transaction?", vbQuestion + vbYesNo, "Message") = vbYes Then
        Dim rsCRJ_Detail                                    As ADODB.Recordset
        Set rsCRJ_Detail = New ADODB.Recordset
        Set rsCRJ_Detail = gconDMIS.Execute("Select * from AMIS_Detail where CUSTOMERCODE = '" & txtCustCode.Text & "'  and  INVOICETYPE = '" & SetInvCode(cboInvoiceType.Text) & "' AND INVOICENO = '" & txtInvoiceNo.Text & "' and  ACCT_CODE = '" & txtCOBAcctNo.Text & "' and status <> 'C'")
        If Not rsCRJ_Detail.EOF And Not rsCRJ_Detail.BOF Then
            MsgBox "Warning: This Customre Opening is already link to   " & Null2String(rsCRJ_Detail!JTYPE) + "-" + Null2String(rsCRJ_Detail!VOUCHERNO) & vbCrLf & _
...
Sub GET_AR_VOUCHERNO()
    ...
    rsCOUNT_CODE.Open "SELECT COUNT(DISTINCT ACCT_CODE) AS COUNT_CODE FROM AMIS_JOURNAL_DET " & _
                      "WHERE VOUCHERNO = " & N2Str2Null(txtVoucherNo.Text) & " AND JTYPE = " & N2Str2Null(xJOURNALTYPE) & " AND LEFT(ACCT_CODE,2) IN('11')", gconDMIS, adOpenKeyset
    If Not rsCOUNT_CODE.EOF And Not rsCOUNT_CODE.BOF Then
        If NumericVal(rsCOUNT_CODE!COUNT_CODE) > 1 Then
            ...
            rsAR_VOUCHER.Open "SELECT DISTINCT HD.VOUCHERNO,HD.VENDORCODE,HD.HD.JDATE,HD.JTYPE,HD.CUSTOMERCODE,HD.INVOICENO,HD.INVOICETYPE,HD.INVOICEDATE,ACCT_CODE,INVOICEAMT " & _
                              ...
                              "WHERE LEFT(DET.ACCT_CODE,2) IN ('11') AND HD.JTYPE = " & N2Str2Null(xJOURNALTYPE) & " AND HD.VOUCHERNO = " & N2Str2Null(txtVoucherNo.Text) & "", gconDMIS, adOpenKeyset
            If Not rsAR_VOUCHER.EOF And Not rsAR_VOUCHER.BOF Then
                Do While Not rsAR_VOUCHER.EOF
                    ...
                    xLAST_UPDATED = N2Str2Null(LOGDATE)
                     ...
                     Dim rsCHKDETAILS As New ADODB.Recordset
                    Set rsCHKDETAILS = New ADODB.Recordset
                    Set rsCHKDETAILS = gconDMIS.Execute("select * from amis_ar where sjvouchenro = " & xVOUCHERNO & " ")
                    If Not rsCHKDETAILS.EOF And Not rsCHKDETAILS.BOF Then
                         gconDMIS.Execute "Update amis_ar set  INVOICETYPE = " & xInvoiceType & ",  INVOICENO = " & xInvoiceNo & ", CUSTOMERCODE = " & XCustomerCode & ",CUSTOMERNAME = " & xCUST_NAME & ", AMOUNT_TOPAY = " & xAMOUNT_TO_PAY & ",AMOUNT_PAID = 0,BALANCE = " & xAMOUNT_TO_PAY & ",INVOICEDATE = " & xInvoicedate & ",LASTUPDATED = " & xLAST_UPDATED & ",JDATE = " & xJdate & " where SJVOUCHERNO = " & xVOUCHERNO & " "
                    Else
        ...
        Else
            Set rsAR_VOUCHER = New ADODB.Recordset
                If COMPANY_CODE = "DPI" Then
                    rsAR_VOUCHER.Open "SELECT DISTINCT HD.VOUCHERNO,HD.VENDORCODE,HD.JDATE,HD.JTYPE,HD.CUSTOMERCODE,HD.INVOICENO,HD.INVOICETYPE,HD.INVOICEDATE,ACCT_CODE,INVOICEAMT,HD.ENTITY_CLASS " & _
                              "FROM AMIS_JOURNAL_HD HD INNER JOIN AMIS_JOURNAL_DET DET ON HD.VOUCHERNO = DET.VOUCHERNO AND HD.JTYPE = DET.JTYPE " & _
                              "WHERE LEFT(DET.ACCT_CODE,2) IN ('11') AND HD.JTYPE = " & N2Str2Null(xJOURNALTYPE) & " AND HD.VOUCHERNO = " & N2Str2Null(txtVoucherNo.Text) & "", gconDMIS, adOpenKeyset
            ...
            If Not rsAR_VOUCHER.EOF And Not rsAR_VOUCHER.BOF Then
                ...
                Set rsCHKDETAILS = New ADODB.Recordset
                Set rsCHKDETAILS = gconDMIS.Execute("select * from amis_ar where SJVOUCHERNO = " & xVOUCHERNO & " ")
                If Not rsCHKDETAILS.EOF And Not rsCHKDETAILS.BOF Then
                     gconDMIS.Execute "Update amis_ar set  INVOICETYPE = " & xInvoiceType & ",  INVOICENO = " & xInvoiceNo & ", CUSTOMERCODE = " & XCustomerCode & ",CUSTOMERNAME = " & xCUST_NAME & ", AMOUNT_TOPAY = " & xAMOUNT_TO_PAY & ",AMOUNT_PAID = 0,BALANCE = " & xAMOUNT_TO_PAY & ",INVOICEDATE = " & xInvoicedate & ",LASTUPDATED = " & xLAST_UPDATED & ",JDATE = " & xJdate & " where SJVOUCHERNO = " & xVOUCHERNO & " "
                Else
                    gconDMIS.Execute "INSERT INTO AMIS_AR(SJVOUCHERNO,INVOICETYPE,INVOICENO,CUSTOMERCODE,CUSTOMERNAME,AMOUNT_TOPAY,AMOUNT_PAID,BALANCE,ACCOUNT_CODE,INVOICEDATE,LASTUPDATED,JDATE) " & _
...
Function VALIDATE_UNPOSTING() As Boolean
    ...
    If Not rsVALIDATE_UNPOSTING.EOF And Not rsVALIDATE_UNPOSTING.BOF Then
        ...
        If Not rsGET_CRJ_VOUCHERNO.EOF And Not rsGET_CRJ_VOUCHERNO.BOF Then
            MessagePop InfoFriend, "INFORMATION", "You can't un-post this voucher it has a payment please see  Journal " & "" & Null2String(rsGET_CRJ_VOUCHERNO!JTYPE) & "" & " - " & "" & Null2String(rsGET_CRJ_VOUCHERNO!VOUCHERNO) & ""
