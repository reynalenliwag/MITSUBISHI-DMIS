...
Begin VB.Form frmAPJImport 
   ...
   Begin VB.CommandButton cmdCheck 
      ...
      Height          =   795
      Left            =   12510
      MouseIcon       =   "APJImport.frx":07C2
      MousePointer    =   99  'Custom
      Picture         =   "APJImport.frx":0914
      Style           =   1  'Graphical
      TabIndex        =   1
   ...
   Begin VB.CommandButton cmdBatchImport 
      ...
      Height          =   795
      Left            =   11790
      MouseIcon       =   "APJImport.frx":0BAF
      MousePointer    =   99  'Custom
      Picture         =   "APJImport.frx":0D01
      Style           =   1  'Graphical
      TabIndex        =   61
      ToolTipText     =   "Process Importing of Purchases"
      Top             =   6840
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.ComboBox cboMonth 
         ...
         EndProperty
         Height          =   345
         ItemData        =   "APJImport.frx":2E05
         Left            =   780
         List            =   "APJImport.frx":2E07
   ...
   Begin VB.CommandButton cmdShowTrans 
      ...
      Height          =   375
      Left            =   3630
      MouseIcon       =   "APJImport.frx":3727
      MousePointer    =   99  'Custom
      Style           =   1  'Graphical
      TabIndex        =   2
      ToolTipText     =   "Process Import of Purchases"
      Top             =   90
      Width           =   2010
   End
   ...
   Begin wizProgBar.Prg progCPB 
      ...
      _ExtentX        =   8123
      _ExtentY        =   556
      Picture         =   "APJImport.frx":96AF
      ForeColor       =   255
      Appearance      =   0
      BorderStyle     =   2
      BarPicture      =   "APJImport.frx":96CB
      ShowText        =   -1  'True
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Segoe UI"
         Size            =   8.25
...
Dim xRSRR_HD                                             As ADODB.Recordset
Dim xRSPURCH_AGREE                                       As ADODB.Recordset
Dim xRSTransferIn                                        As ADODB.Recordset
Dim xRSSublet                                            As ADODB.Recordset

Dim xKIM                                                 As Integer
Dim xRRTYPE                                              As String
Dim xIS_EXIST                                            As Byte
Dim xCOSTPRICE                                           As String
Dim Xcode                                                As String
    ...
    For GridImports = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImports, 1).Text) = 0 Then
            ...
            If Not rsSublet.EOF And Not rsSublet.BOF Then
                ...
                Set RsSublet_Det = gconDMIS.Execute("SELECT WCODE,DETCDE,(SELECT SUM(CONTRACTAMOUNT) FROM CSMS_PO_RC_DT WHERE PO_NO=DT.PO_NO AND STATUS='P') AS CONTRACTAMOUNT FROM CSMS_PO_RC_DT DT where STATUS='P' AND PO_no='" & ThePO & "'")
                ...
                If COMPANY_CODE = "HGC" Or COMPANY_CODE = "HMH" Or COMPANY_CODE = "HGH" Then
                ...
                J_ITEMCOUNT = J_ITEMCOUNT + 1
                J_JITEMNO = "'" & Format(J_ITEMCOUNT, "0000") & "'"
                If COMPANY_CODE = "HGC" Or COMPANY_CODE = "HMH" Or COMPANY_CODE = "HCI" Or COMPANY_CODE = "HGH" Then
...
Private Sub chkService_Click()
    If chkService.Value = False Then
        'If MsgBox("Are you sure that Sublet will not be included during importing?", vbQuestion + vbYesNo, "Import Sublet") = vbYes Then
            chkService.Value = 0
            Grid3.Rows = 1
        'Else
        '    chkService.Value = 1
...
Private Sub cmdBatchImport_Click()
    ...
    dtTo.Value = Format(APJLASTTRANS, "mm/dd/yyyy")
...
InitGrids:     DoEvents:
    ...
    If BATCHIMPORT = True Then
        Call frmListofUnposted.LOADJOURNAL("APJ")
        If UNPOSTEDAPJ = True Then
...
Private Sub cmdCheck_Click()
    ...
    If COMPANY_CODE = "CMC" Or COMPANY_CODE = "MGS" Or COMPANY_CODE = "DPI" Or COMPANY_CODE = "DMI" Or COMPANY_CODE = "HSB" Or COMPANY_CODE = "HLB" Or COMPANY_CODE = "HBC" Or COMPANY_CODE = "HCR" Or COMPANY_CODE = "DAI" Or COMPANY_CODE = "HGS" Or COMPANY_CODE = "HSM" Or COMPANY_CODE = "HCA" Or COMPANY_CODE = "DGI" Or COMPANY_CODE = "HQA" Or COMPANY_CODE = "HNE" Or COMPANY_CODE = "DJM" Or COMPANY_CODE = "HMH" Or COMPANY_CODE = "DSSC" Or COMPANY_CODE = "HCE" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "HMR" Then
        ...
        Else
            ...
            NEW_LogAudit "R", "ACCOUNTS PAYABLE IMPORT", dtpTranDate, "", "", "", "APJ", ""
        End If
    Else
        gconDMIS.BeginTrans
        
        If ImportPurchases = False Then
            str_MSG = Replace(str_MSG, "@ACL09182716350", "Purchases")
        ...
        If ImportSublet = False Then
            str_MSG = Replace(str_MSG, "@ACL09182716350", "Sublet")
            MsgBox str_MSG, vbCritical, "Import Error "
            cmdExit.Enabled = True
            gconDMIS.RollbackTrans
            Screen.MousePointer = 0
...
Sub ImportPurchasesNew()
    ...
    For GridImport = 1 To Grid2.Rows - 1
        If N2Str2Zero(Grid2.Cell(GridImport, 1).Text) = 0 Then
            If CHKDUPLICATEINV(Grid2.Cell(GridImport, 2).Text, Grid2.Cell(GridImport, 3).Text, Grid2.Cell(GridImport, 3).Text) = "" Then
    ...
    For GridImport = 1 To Grid3.Rows - 1
        If N2Str2Zero(Grid3.Cell(GridImport, 1).Text) = 0 Then
            If CHKDUPLICATEINV(Grid3.Cell(GridImport, 2).Text, Grid3.Cell(GridImport, 3).Text, Grid3.Cell(GridImport, 3).Text) = "" Then
...
Sub ImportPurchase2(xTRANTYPE As String, XTYPE As String, XTRANNO As String, xTranDate As Date)
    ...
    If xTRANTYPE = "PURCHASES" And XTYPE = "VEHICLES" Then
        CMD.CommandText = "USP_PURCHASES_VEHICLES"
    ElseIf xTRANTYPE = "PURCHASES" And XTYPE = "PARTS" Then
         CMD.CommandText = "USP_PURCHASES_PARTS"
    ElseIf xTRANTYPE = "PURCHASES" And XTYPE = "ACCESSORIES" Then
         CMD.CommandText = "USP_PURCHASES_ACCESSORIES"
    ElseIf xTRANTYPE = "PURCHASES" And XTYPE = "MATERIALS" Then
         CMD.CommandText = "USP_PURCHASES_MATERIALS"
    ElseIf xTRANTYPE = "PURCHASES" And XTYPE = "SUBLET" Then
         CMD.CommandText = "USP_PURCHASES_SUBLET"
    End If
    ...
    With CMD.Parameters
        ...
        If xTRANTYPE = "PURCHASES" And XTYPE = "VEHICLES" Then
            .Append CMD.CreateParameter("@MESSAGE", adVarChar, adParamOutput, 1000)
        End If
    End With
    
    CMD.Execute
    If IsNull(CMD("@VOUCHERNO")) = True And IsNull(CMD("@JTYPE")) And IsNull(CMD("@JNO")) = True Then
        MsgBox CMD("@MESSAGE"), vbExclamation, "Information"
    Else
        ...
        NEW_LogAudit "R", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, N2Str2Zero(TransactionID), "", N2Str2Null(J_VOUCHERNO), J_JTYPE, N2Str2Null(J_JNO)
    End If
    ...
    For GridImport = 1 To Grid1.Rows - 1
        If N2Str2Zero(Grid1.Cell(GridImport, 1).Text) = 0 Then
            ...
            If Not rsRR_HD.EOF And Not rsRR_HD.BOF Then
                ...
                If PMIOS_NETRRAMT > 0 Then
                    ...
                    If PMIOS_TYPE = "M" Then
                        If COMPANY_CODE = "HOT" Then
                            ...
                            Else
                                ...
                                J_ACCT_NAME = N2Str2Null(Setacctname(ReturnInventory("PARTS", "INVN")))
                            End If
                        ElseIf COMPANY_CODE = "HSB" Or COMPANY_CODE = "HBC" Or COMPANY_CODE = "HLB" Or COMPANY_CODE = "HCR" Then
                    ...
                    If COMPANY_CODE = "HGC" Then
                        ...
                        Else
                            ...
                            J_ACCT_NAME = N2Str2Null(Setacctname(ReturnAP_AccountCode("GENERAL")))
                        End If
                    ElseIf COMPANY_CODE = "HGH" Then
                        If PMIOS_RECVD_CODE = "H00259" Then
                            J_ACCT_CODE = N2Str2Null(ReturnAP_AccountCode("HYUNDAI"))
                            J_ACCT_NAME = N2Str2Null(Setacctname(ReturnAP_AccountCode("HYUNDAI")))
                        ElseIf PMIOS_CLASSCODE = "IBT" Then
                            J_ACCT_CODE = N2Str2Null(ReturnAP_AccountCode("BRANCH"))
                ...
                If COMPANY_CODE = "HCI" Or COMPANY_CODE = "HOT" Or COMPANY_CODE <> "DJM" Or COMPANY_CODE <> "CMC" Then
                    Set rsATC = New ADODB.Recordset
                    ATCRate = 0
                    rsATC.Open "SELECT ATC FROM AMIS_ATC", gconDMIS, adOpenKeyset
                    If Not rsATC.EOF And Not rsATC.BOF Then
                    ...
                    Set rsATC = Nothing
                    '''
                    If COMPANY_CODE = "HOT" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HGH" Then
                ...
                If COMPANY_CODE = "HGC" Or COMPANY_CODE = "HOT" Or COMPANY_CODE = "HGH" Then
                    If PMIOS_CLASSCODE = "IBT" Or PMIOS_RECVD_CODE = "M00002" Or PMIOS_RECVD_CODE = "M00003" Or PMIOS_RECVD_CODE = "S00002" Then
                        J_CREDIT = Round(NumericVal(PMIOS_NETRRAMT), 2)
                    End If
                End If
                ...
                If COMPANY_CODE = "CMC" Or COMPANY_CODE = "HCI" Or COMPANY_CODE = "DJM" Then
                    Set rsATC = New ADODB.Recordset
                    ATCRate = 0
                    rsATC.Open "SELECT ATC FROM AMIS_ATC", gconDMIS, adOpenKeyset
                    If Not rsATC.EOF And Not rsATC.BOF Then
    ...
    For GridImport = 1 To Grid2.Rows - 1
        If N2Str2Zero(Grid2.Cell(GridImport, 1).Text) = 0 Then
            ...
            If Not rsRR_HD.EOF And Not rsRR_HD.BOF Then
                ...
                If COMPANY_CODE = "HBK" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HAS" Or COMPANY_CODE = "HGH" Then
                ...
                If PMIOS_NETRRAMT > 0 Then
                    ...
                    ElseIf COMPANY_CODE = "HGC" And PMIOS_RECVD_CODE = "HGC" Then
                        ...
                        J_STATUS = "'N'"
                        TOTAL_DEBIT = TOTAL_DEBIT + J_DEBIT
                    ElseIf COMPANY_CODE = "HLP" Or COMPANY_CODE = "HAM" Or COMPANY_CODE = "HSP" Or COMPANY_CODE = "HOT" Then
                ...
                TransactionID = (FindTransactionID(N2Str2Null(J_VOUCHERNO), "voucherno", "AMIS_Journal_HD", "BERNARD", J_JTYPE, "Jtype"))
                NEW_LogAudit "M", "ACCOUNTS PAYABLE JOURNAL", SQL_STATEMENT, N2Str2Zero(TransactionID), "", N2Str2Null(J_VOUCHERNO), J_JTYPE, N2Str2Null(J_JNO)
...
'Updated by: ACL
'Description: Delete transactions in the list only, excluding manual entry
'02102011
    Screen.MousePointer = 0
    ...
    Else
        ...
        For i = 1 To Grid1.Rows - 1
            ...
            rsJournalHD.Open ("SELECT VOUCHERNO,JTYPE FROM AMIS_JOURNAL_HD WHERE STATUS='N' AND INVOICETYPE ='" & xInvoiceType & "' AND INVOICENO='" & xInvoiceNo & "' AND JTYPE='APJ'"), gconDMIS, adOpenForwardOnly
            If Not rsJournalHD.EOF And Not rsJournalHD.BOF Then
            
                '02052016
                If COMPANY_CODE = "DPI" Or COMPANY_CODE = "DAI" Or COMPANY_CODE = "DMI" Then
                    gconDMIS.Execute ("Delete from AMIS_Detail Where STATUS = 'N' AND JTYPE ='APJ' AND VOUCHERNO='" & rsJournalHD!VOUCHERNO & "'")
                End If
                
                gconDMIS.Execute ("Delete from AMIS_PV_Detail Where STATUS = 'N' AND JTYPE ='APJ' AND VOUCHERNO='" & rsJournalHD!VOUCHERNO & "'")
                gconDMIS.Execute ("Delete from AMIS_Journal_Det Where STATUS = 'N' AND Jtype = 'APJ' AND VOUCHERNO='" & rsJournalHD!VOUCHERNO & "'")
                gconDMIS.Execute ("Delete from AMIS_Journal_HD Where STATUS = 'N' AND Jtype = 'APJ' and VOUCHERNO='" & rsJournalHD!VOUCHERNO & "'")
            End If
        Next i

        i = 0
        For i = 1 To Grid2.Rows - 1
            xInvoiceType = Grid2.Cell(i, 2).Text
            xInvoiceNo = Grid2.Cell(i, 3).Text
            Set rsJournalHD = New ADODB.Recordset
            rsJournalHD.Open ("SELECT VOUCHERNO,JTYPE FROM AMIS_JOURNAL_HD WHERE STATUS='N' AND INVOICETYPE ='" & xInvoiceType & "' AND INVOICENO='" & xInvoiceNo & "' AND JTYPE='APJ'"), gconDMIS, adOpenForwardOnly
            If Not rsJournalHD.EOF And Not rsJournalHD.BOF Then
                gconDMIS.Execute ("Delete from AMIS_PV_Detail Where STATUS = 'N' AND JTYPE ='APJ' AND VOUCHERNO='" & rsJournalHD!VOUCHERNO & "'")
        ...
        For i = 1 To Grid3.Rows - 1
            xInvoiceType = Grid3.Cell(i, 2).Text
...
Private Sub cmdShowTrans_Click()
    ...
    If COMPANY_CODE = "CMC" Or COMPANY_CODE = "MGS" Or COMPANY_CODE = "DPI" Or COMPANY_CODE = "DMI" Or COMPANY_CODE = "DAI" Or COMPANY_CODE = "HGC" Or COMPANY_CODE = "HPI" Or COMPANY_CODE = "HCC" Or COMPANY_CODE = "HGH" Or COMPANY_CODE = "HCA" Or COMPANY_CODE = "DJM" Or COMPANY_CODE = "HCE" Or COMPANY_CODE = "FMC" Or COMPANY_CODE = "HSM" Or COMPANY_CODE = "HMR" Or COMPANY_CODE = "DSSC" Then
...
Sub ShowPMISTransactions()
    If COMPANY_CODE = "CMC" Then Exit Sub
    
    If chkParts.Value = 1 Then
        Set xRSRR_HD = New ADODB.Recordset
        If COMPANY_CODE = "HCE" Then
            Set xRSRR_HD = gconDMIS.Execute("Select * from PMIS_vw_RR_TRANS Where STATUS = 'P' AND (CLASSCODE = 'PCG' or CLASSCODE = 'PCS' or CLASSCODE = 'IBT' or CLASSCODE = 'RCS' ) AND RRDATE = '" & CDate(dtpTranDate) & "' Order by RRNO ASC")
        ElseIf COMPANY_CODE = "DPI" Or COMPANY_CODE = "DMI" Or COMPANY_CODE = "DAI" Then
            Set xRSRR_HD = gconDMIS.Execute("Select * from PMIS_vw_RR_TRANS Where STATUS = 'P' AND (CLASSCODE = 'PCG' or CLASSCODE = 'PCS' or CLASSCODE = 'IBT' or CLASSCODE = 'CUR') AND RRDATE = '" & CDate(dtpTranDate) & "' Order by RRNO ASC")
        ElseIf COMPANY_CODE = "CMC" Or COMPANY_CODE = "DJM" Then
            Set xRSRR_HD = gconDMIS.Execute("Select * from PMIS_vw_RR_TRANS Where STATUS = 'P' AND (CLASSCODE = 'PCG' or CLASSCODE = 'PCS') AND RRDATE = '" & CDate(dtpTranDate) & "' Order by RRNO ASC")
        Else
            Set xRSRR_HD = gconDMIS.Execute("Select * from PMIS_vw_RR_TRANS Where STATUS = 'P' AND (CLASSCODE = 'PCG' or CLASSCODE = 'PCS' or CLASSCODE = 'IBT' ) AND RRDATE = '" & CDate(dtpTranDate) & "' Order by RRNO ASC")
        End If
        
        If Not xRSRR_HD.EOF And Not xRSRR_HD.BOF Then
            xRSRR_HD.MoveFirst: xKIM = 0
            Grid1.AutoRedraw = False
            Do While Not xRSRR_HD.EOF
                xKIM = xKIM + 1
                If Null2String(xRSRR_HD!Type) = "P" Then
                    xRRTYPE = "PARTS"
                ElseIf Null2String(xRSRR_HD!Type) = "A" Then
                    xRRTYPE = "ACCESSORIES"
                ElseIf Null2String(xRSRR_HD!Type) = "M" Then
                    xRRTYPE = "MATERIALS"
                Else
                    xRRTYPE = ""
                End If
                If CheckAPJExisting(Null2String(xRSRR_HD!RRNO), xRRTYPE) = True Then
                    xIS_EXIST = 1
                Else
                    xIS_EXIST = 0
                End If
                ...
                If COMPANY_CODE = "HCE" Then
                    Grid1.AddItem xIS_EXIST & Chr(9) & xRRTYPE & Chr(9) & Null2String(xRSRR_HD!RRNO) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSRR_HD!NETRRAMT)) & Chr(9) & Null2String(xRSRR_HD!RECVD_FROM)
                'UPDATED BY: JULIE 04192013
                ElseIf COMPANY_CODE = "HNE" Then
                        If Null2String(xRSRR_HD!RECVD_CODE) = "H00001" Then
                            Grid1.AddItem xIS_EXIST & Chr(9) & xRRTYPE & Chr(9) & Null2String(xRSRR_HD!RRNO) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSRR_HD!NETRRAMT)) & Chr(9) & Null2String(xRSRR_HD!RECVD_FROM)
                        Else
                            Grid1.AddItem xIS_EXIST & Chr(9) & xRRTYPE & Chr(9) & Null2String(xRSRR_HD!RRNO) & Chr(9) & ToDoubleNumber((N2Str2Zero(xRSRR_HD!NETRRAMT)) - (N2Str2Zero(xRSRR_HD!TTLRRAMT) * 0.01)) & Chr(9) & Null2String(xRSRR_HD!RECVD_FROM)
                        End If
                Else
                    Grid1.AddItem xIS_EXIST & Chr(9) & xRRTYPE & Chr(9) & Null2String(xRSRR_HD!RRNO) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSRR_HD!TTLRRAMT)) & Chr(9) & Null2String(xRSRR_HD!RECVD_FROM)
                End If
                xRSRR_HD.MoveNext
            Loop
...
Sub ShowSMISTransactions()
    If chkVehicles.Value = 1 Then
        Set xRSPURCH_AGREE = New ADODB.Recordset
        Set xRSPURCH_AGREE = gconDMIS.Execute("Select SMIS_MRRINV.*,CSMS_SELLINGDEALER.DEALERNAME from SMIS_MRRINV left outer JOIN CSMS_SELLINGDEALER ON SMIS_MRRINV.SOURCE = CSMS_SELLINGDEALER.DEALERCODE Where STATUS = 'P' AND DateReceived = '" & Format(CDate(dtpTranDate), "MM/DD/YYYY") & "' Order by DateReceived ASC")
        If Not xRSPURCH_AGREE.EOF And Not xRSPURCH_AGREE.BOF Then
            xRSPURCH_AGREE.MoveFirst: xKIM = 0
            Grid2.AutoRedraw = False
            Do While Not xRSPURCH_AGREE.EOF
                xKIM = xKIM + 1
                If CheckAPJExisting(Null2String(xRSPURCH_AGREE!Code), "VEHICLES") = True Then
                    xIS_EXIST = 1
                Else
                    xIS_EXIST = 0
                End If
                ...
                If COMPANY_CODE = "HNE" Then
                    Grid2.AddItem xIS_EXIST & Chr(9) & "VEHICLES" & Chr(9) & Null2String(xRSPURCH_AGREE!Code) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSPURCH_AGREE!PURCHPRICE) / 1.12) & Chr(9) & Null2String(xRSPURCH_AGREE!DealerName)
                Else
                    Grid2.AddItem xIS_EXIST & Chr(9) & "VEHICLES" & Chr(9) & Null2String(xRSPURCH_AGREE!Code) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSPURCH_AGREE!PURCHPRICE)) & Chr(9) & Null2String(xRSPURCH_AGREE!DealerName)
                End If
                xRSPURCH_AGREE.MoveNext
            Loop
...
Sub ShowCSMSTransactions()
    ...
    If chkService.Value = 1 Then
        Set xRSSublet = New ADODB.Recordset
        
        If COMPANY_CODE = "HCE" Then
            Set xRSSublet = gconDMIS.Execute("Select A.RC_NO,SUM(B.CONTRACTAMOUNT) AS CONTRACTAMOUNT, A.CONTRACTOR_NAME from CSMS_PO_RC_HD A INNER JOIN CSMS_PO_RC_DT B ON A.RC_NO = B.RC_NO Where A.STATUS = 'P' AND A.Rc_DATE = '" & Format(CDate(dtpTranDate), "MM/DD/YYYY") & "' GROUP BY A.RC_NO,A.CONTRACTOR_NAME ")
        Else
            Set xRSSublet = gconDMIS.Execute("Select * from CSMS_PO_RC_HD Where STATUS = 'P' AND Rc_DATE = '" & Format(CDate(dtpTranDate), "MM/DD/YYYY") & "' Order by RC_date ASC")
        End If
        
        Grid3.Rows = 1
        If Not xRSSublet.EOF And Not xRSSublet.BOF Then
            xRSSublet.MoveFirst: xKIM = 0
            Grid3.AutoRedraw = False
            Do While Not xRSSublet.EOF
                xKIM = xKIM + 1
                If CheckAPJExisting(Null2String(xRSSublet!RC_NO), "SUBLET") = True Then
                    xIS_EXIST = 1
                Else
                    xIS_EXIST = 0
                End If
                If COMPANY_CODE = "HCE" Then
                    Grid3.AddItem xIS_EXIST & Chr(9) & "SUBLET" & Chr(9) & Null2String(xRSSublet!RC_NO) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSSublet!CONTRACTAMOUNT)) & Chr(9) & Null2String(xRSSublet!contractor_name)
                Else
                    Grid3.AddItem xIS_EXIST & Chr(9) & "SUBLET" & Chr(9) & Null2String(xRSSublet!RC_NO) & Chr(9) & ToDoubleNumber(N2Str2Zero(xRSSublet!sublet_Total_net_AMT)) & Chr(9) & Null2String(xRSSublet!contractor_name)
                End If
                xRSSublet.MoveNext
            Loop
        End If
...
'Update: ACL 11162009
    ...
    rsCheckAP.Open "Select TOP 1 * from AMIS_Journal_HD where JTYPE='APJ' AND JDATE < '" & CDate(xRRDate) & "' ORDER BY JDATE DESC", gconDMIS, adOpenKeyset
...
Function APJLASTTRANS() As String
'Update: ACL 05092011
    Dim xNoDays                                             As Integer
    Dim rsCheckTrans                                        As ADODB.Recordset
    Set rsCheckTrans = New ADODB.Recordset
    rsCheckTrans.Open "Select MAX(TRANDATE) AS TRANDATE from (SELECT MAX(RRDATE) AS TRANDATE FROM PMIS_VW_RR_TRANS WHERE STATUS = 'P' AND (CLASSCODE = 'PCG' OR CLASSCODE = 'PCS' ) " & _
                      "Union SELECT MAX(DATERECEIVED) AS TRANDATE FROM SMIS_MRRINV LEFT OUTER JOIN CSMS_SELLINGDEALER ON SMIS_MRRINV.SOURCE = CSMS_SELLINGDEALER.DEALERCODE WHERE STATUS = 'P' " & _
                      "UNION SELECT MAX(RC_DATE) AS TRANDATE FROM CSMS_PO_RC_HD WHERE STATUS='P') X", gconDMIS, adOpenKeyset
    If Not rsCheckTrans.EOF And Not rsCheckTrans.BOF Then
        APJLASTTRANS = Null2Date(rsCheckTrans!trandate)
    End If
...
Function SetInvCode(INV As String)
    If INV = "ACCESSORIES" Then
        SetInvCode = "ACCESSORIES"
    ElseIf INV = "PARTS" Then
        SetInvCode = "PARTS"
    ElseIf INV = "MATERIALS" Then
        SetInvCode = "MATERIALS"
    ElseIf INV = "VEHICLES" Then
        SetInvCode = "VEHICLES"
    ElseIf INV = "SUBLET" Then
        SetInvCode = "SUBLET"
    Else
        SetInvCode = "Null"
    End If
...
Function CHKDUPLICATEINV(xInvoiceType As String, xInvoiceNo, xPAYTYPE) As String
    ...
    Set rsCHECKINVOICE = gconDMIS.Execute("SELECT * FROM AMIS_JOURNAL_HD WHERE INVOICETYPE = '" & xIMPORTINGINVTYPE & "' AND INVOICENO = '" & xInvoiceNo & "'  AND JTYPE = 'APJ'")
...
Private Sub Grid1_DblClick()
    ...
    Call frmAMISJournalEntry_APJ.StoreSearch3(HDInvNo, "")
End Sub

Private Sub Grid2_DblClick()
    If Grid2.Rows = 0 Then Exit Sub
    If Grid2.ActiveCell.Col <> 3 Then Exit Sub
    
    Dim HDInvNo As String
    HDInvNo = Grid2.ActiveCell.Text
